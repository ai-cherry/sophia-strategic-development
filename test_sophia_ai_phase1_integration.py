...
async def test_sales_coach_integration():
    ...
    try:
        ...
        if coaching_response.success:
            ...
            print(f"     Insights generated: {coaching_response.coaching_insights_generated}")
        else:
        ...
        if orchestrator.sales_coach:
            ...
            slack_response = await orchestrator.sales_coach.slack_coaching_interface(slack_event)
...
async def test_system_health():
    ...
    try:
        ...
        if analytics_response.success:
            ...
            orchestration_analytics = analytics_response.primary_response.get("orchestration_analytics", {})
            total_requests = orchestration_analytics.get("total_requests", 0)
            success_rate = orchestration_analytics.get("successful_requests", 0) / max(total_requests, 1)
            
            print(f"     Total requests processed: {total_requests}")
            print(f"     Success rate: {success_rate:.1%}")
            print(f"     Average response time: {orchestration_analytics.get('average_response_time', 0):.2f}ms")
        else:
