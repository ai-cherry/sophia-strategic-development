...
repo_description = config.get(
    "repo_description", "The official repository for the Sophia AI project."
)
...
github_provider = github.Provider("github-provider", token=github_token)
...
sophia_repo = github.get_repository(
    name=repo_name, opts=pulumi.ResourceOptions(provider=github_provider)
)

# 2. Define a branch protection rule for the 'main' branch
main_branch_protection = github.BranchProtection(
...
example_secret = github.ActionsSecret(
    "example-repo-secret",
