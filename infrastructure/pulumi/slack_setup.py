...
class SlackChannelProvider(pulumi.dynamic.ResourceProvider):
    ...
    def create(self, props):
        ...
        return pulumi.dynamic.CreateResult(id_=channel_id, outs={"id": channel_id, **props})

    def delete(self, id, props):
        """Archives a Slack channel."""
        token = props["token"]
        requests.post("https://slack.com/api/conversations.archive",
            headers={"Authorization": f"Bearer {token}"},
            json={"channel": id}
        )
...
deployment_channel = SlackChannel("deployments-channel",
