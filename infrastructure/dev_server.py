...
class LambdaLabsProvider(pulumi.dynamic.ResourceProvider):
    ...
    def create(self, props):
        ...
        return pulumi.dynamic.CreateResult(
            id_=instance_id,
            outs={"id": instance_id, "ip": instance_data.get("ip"), **props},
        )
...
class LambdaLabsServer(pulumi.dynamic.Resource):
    ...
    def __init__(
        self, name, api_key, region, instance_type, ssh_key_name, server_name, opts=None
    ):
        super().__init__(
            LambdaLabsProvider(),
            name,
...
pulumi.export(
    "dev_server_ssh_command", pulumi.Output.concat("ssh ubuntu@", dev_server.ip)
)
