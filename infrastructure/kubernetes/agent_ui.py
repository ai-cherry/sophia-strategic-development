...
ui_namespace = k8s.core.v1.Namespace("agno-ui-ns",
    metadata={"name": "agno-ui"},
    opts=pulumi.ResourceOptions(provider=k8s_provider))

# --- 2. Deployment for the Agno Agent UI ---
app_labels = {"app": "agno-agent-ui"}
deployment = k8s.apps.v1.Deployment("agno-ui-deployment",
...
service = k8s.core.v1.Service("agno-ui-service",
    metadata=k8s.meta.v1.ObjectMetaArgs(
        namespace=ui_namespace.metadata["name"],
    ),
...
pulumi.export("agno_ui_url", service.status.apply(lambda status: status.load_balancer.ingress[0].hostname if status.load_balancer.ingress else "pending")) 