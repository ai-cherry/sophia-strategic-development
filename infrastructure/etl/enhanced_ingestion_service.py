...
from infrastructure.etl.estuary.estuary_configuration_manager import (
...
class EnhancedIngestionService:
    ...
    async def _setup_enhanced_connection(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = await self.estuary_manager.setup_source_pipeline(
                config["source"].value
            )

            if result.status.value != "success":
                raise Exception(
                    f"Failed to create estuary materialization for {config['source'].value}: {result.error_message}"
                )
            ...
            logger.info(
                f"Enhanced connection setup complete for {config['source'].value}"
            )
            return True
