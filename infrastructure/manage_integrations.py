...
from backend.core.integration_registry import discover_integrations, registry
...
class IntegrationManager:
    ...
    async def get_integration_status(self, service_name: str) -> Dict[str, Any]:
        ...
        missing_secrets = [
            key for key in required_secret_keys if key not in secret_keys
        ]
    ...
    async def test_integration(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"service_name": service_name, "status": "ok", "error": None}
        except Exception as e:
            return {
                "service_name": service_name,
                "status": "test_failed",
...
async def main():
    ...
    register_parser = subparsers.add_parser(
        "register", help="Register a new integration"
    )
    register_parser.add_argument("--service", required=True, help="Service name")
    register_parser.add_argument(
        "--metadata", required=True, help="Metadata file (JSON)"
    )
