...
class BaseChatService(ABC):
    ...
    def validate_request(self, request: ChatRequest) -> None:
        ...
        if request.configuration:
            if (
                ...
                or request.configuration.temperature > 2
            ):
                raise ValueError("Temperature must be between 0 and 2")

            if (
                request.configuration.max_tokens < 1
                or request.configuration.max_tokens > 8000
            ):
    ...
    async def process_with_timing(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            # Log request
            self.logger.info(
                f"Processing {self.mode.value} chat: {request.message[:50]}..."
            )
