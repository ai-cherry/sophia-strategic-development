...
class EnhancedSemanticLayerService:
    ...
    async def _get_connection(self):
        """Get Snowflake connection with entity resolution schema"""
    ...
    async def build_semantic_query(self, entity_config: Dict[str, Any]) -> str:
        ...
        for i, source in enumerate(entity_config.get("enrichment_sources", [])):
            ...
            if join_type == "FUZZY_TEXT_MATCH":
                ...
                join_sql = join_sql.replace(f") resolved_entities", f") {source_alias}")
            else:
                join_sql = f"""
            ...
            for metric in source.get("metrics", []):
                metric_sql = metric["agg"].replace("DISTINCT ", f"DISTINCT {source_alias}.")
        ...
        try:
            ...
            if needs_clarification:
                return {
                    ...
                    "clarification_message": self._generate_clarification_message(entity_matches)
                }
            
            # Execute query with resolved entities
            resolved_query = await self._substitute_resolved_entities(query_text, entity_matches)
    ...
    def _generate_clarification_message(self, entity_matches: Dict[str, Any]) -> str:
        ...
        for entity_text, match_info in entity_matches.items():
            if match_info.get("needs_clarification"):
                ...
                if len(candidates) <= 3:
                    # Show all candidates
                    options = ", ".join([f"'{c['canonical_name']}'" for c in candidates])
                    ...
                    options = ", ".join([f"'{c['canonical_name']}'" for c in top_candidates])
                    clarification_parts.append(
    ...
    async def _substitute_resolved_entities(
        ...
        for entity_text, match_info in entity_matches.items():
            if not match_info.get("needs_clarification") and "resolved_entity" in match_info:
    ...
    async def _log_resolution_event(
        ...
        try:
            ...
            """, [
                event_id,
                original_query,
                candidates,  # Snowflake will handle the array conversion
                len(candidates),
                sum(c.get("similarity_score", 0) for c in candidates) / max(len(candidates), 1),
                resolution_method,
                user_id
            ])
    ...
    async def get_entity_resolution_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("""
                ...
                LIMIT 20
            """)
            ambiguous = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()]
            
            # Get usage analytics
            cursor.execute("""
                SELECT * FROM SOPHIA_ENTITY_RESOLUTION.ENTITY_USAGE_ANALYTICS
            """)
            usage = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()]
