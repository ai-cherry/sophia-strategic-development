...
sophia_db = snowflake.Database(db_name,
    name=db_name,
    comment="Main database for the Sophia AI application."
)

# 2. Create the primary data warehouse for compute
sophia_warehouse = snowflake.Warehouse(warehouse_name,
    name=warehouse_name,
...
raw_schema = snowflake.Schema("schema-raw",
    database=sophia_db.name,
    name="SOPHIA_RAW",
    comment="Schema for raw, unprocessed data ingested from sources."
)

analytics_schema = snowflake.Schema("schema-analytics",
    database=sophia_db.name,
...
sophia_admin_role = snowflake.Role(admin_role_name,
...
snowflake.RoleGrants(f"grant-db-usage-to-{admin_role_name}",
...
snowflake.RoleGrants(f"grant-warehouse-usage-to-{admin_role_name}",
    role_name=sophia_admin_role.name,
...
for schema in [raw_schema, analytics_schema]:
    snowflake.RoleGrants(f"grant-schema-all-on-{schema._name}-to-{admin_role_name}",
        role_name=sophia_admin_role.name,
        privileges=["ALL"],
        on="SCHEMA",
