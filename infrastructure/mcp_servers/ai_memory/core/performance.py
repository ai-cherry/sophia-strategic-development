...
from .exceptions import MemoryTimeoutError, MemoryCapacityError, handle_async_exception
...
class AsyncSemaphorePool:
    ...
    @asynccontextmanager
    async def acquire(self, operation_name: str) -> AsyncGenerator[PerformanceMetrics, None]:
    ...
    async def compute_similarity_batch(
        ...
        async with self._computation_pool.acquire("similarity_batch") as metrics:
            try:
                ...
                candidate_norms = candidate_arrays / np.linalg.norm(candidate_arrays, axis=1, keepdims=True)
                ...
                try:
                    del query_array, candidate_arrays, query_norm, candidate_norms, similarities
                except:
                    pass
        ...
        async with self._lock:
            if self._active_operations >= self.max_concurrent_ops:
                raise MemoryCapacityError(
                    message=f"Maximum concurrent operations exceeded",
            ...
            try:
                ...
                if memory_usage > self.max_memory_mb:
                    raise MemoryCapacityError(
                        message=f"Memory usage limit exceeded",
