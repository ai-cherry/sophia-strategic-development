...
class MemoryMetadata(BaseModel):
    ...
    confidence_score: float = Field(default=1.0, ge=0.0, le=1.0, description="Confidence in memory accuracy")
    ...
    custom_attributes: Dict[str, Any] = Field(default_factory=dict, description="Custom metadata attributes")
...
class MemoryEmbedding(BaseModel):
    ...
    def validate_dimensions_match(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(vector) != dimensions:
            raise ValueError(f"Vector length {len(vector)} does not match dimensions {dimensions}")
...
class MemoryRecord(BaseModel):
    ...
    parent_memory_id: Optional[str] = Field(None, description="Parent memory ID for hierarchical memories")
    child_memory_ids: List[str] = Field(default_factory=list, description="Child memory IDs")
...
class SearchQuery(BaseModel):
    ...
    categories: List[MemoryCategory] = Field(default_factory=list, description="Filter by categories")
    priorities: List[MemoryPriority] = Field(default_factory=list, description="Filter by priorities")
    ...
    updated_before: Optional[datetime] = Field(None, description="Filter by update time")
...
class SearchResult(BaseModel):
    ...
    match_type: str = Field(..., description="Type of match (exact, semantic, metadata)")
    ...
    def validate_match_type(cls, v: str) -> str:
        """Validate match type"""
        valid_types = {"exact", "semantic", "metadata", "tag", "category", "type", "hybrid"}
