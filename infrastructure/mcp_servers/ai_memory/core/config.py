...
class AIMemoryConfig(BaseSettings):
    ...
    server_port: int = Field(default=9001, env="AI_MEMORY_SERVER_PORT", ge=1024, le=65535)
    ...
    pinecone_index_name: str = Field(default="sophia-ai-memory", env="PINECONE_INDEX_NAME")
    ...
    embedding_dimensions: int = Field(default=1536, env="EMBEDDING_DIMENSIONS", ge=512, le=3072)
    
    # Performance Configuration
    embedding_cache_ttl: int = Field(default=3600, env="EMBEDDING_CACHE_TTL", ge=60, le=86400)
    max_concurrent_operations: int = Field(default=10, env="MAX_CONCURRENT_OPS", ge=1, le=100)
    vector_similarity_threshold: float = Field(default=0.7, env="SIMILARITY_THRESHOLD", ge=0.0, le=1.0)
    memory_cleanup_interval: int = Field(default=3600, env="CLEANUP_INTERVAL", ge=300, le=86400)
    
    # Search Configuration
    default_search_limit: int = Field(default=10, env="DEFAULT_SEARCH_LIMIT", ge=1, le=100)
    ...
    enable_embedding_optimization: bool = Field(default=True, env="ENABLE_EMBEDDING_OPT")
    ...
    health_check_interval: int = Field(default=60, env="HEALTH_CHECK_INTERVAL", ge=10, le=300)
    metrics_export_interval: int = Field(default=60, env="METRICS_EXPORT_INTERVAL", ge=10, le=300)
