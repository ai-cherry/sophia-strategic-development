...
from .docker_build import sophia_api_image  # and others
...
subnet = aws.ec2.Subnet("sophia-subnet", vpc_id=vpc.id, cidr_block="10.0.1.0/24")
...
api_task_definition = aws.ecs.TaskDefinition(
    ...
    execution_role_arn=aws.iam.Role(
        "ecs-task-execution-role",  # A real setup would use a predefined role
        assume_role_policy="""{
            "Version": "2012-10-17",
    ...
    container_definitions=pulumi.Output.all(
        sophia_api_image.image_name, db_instance.address, db_password
    ).apply(
            ...
            [
                {
                    ...
                    "environment": [
                        {
                            "name": "DATABASE_URL",
                            "value": f"postgresql://sophia_admin:{args[2]}@{args[1]}/sophia",
                        },
...
pulumi.export(
    "deployment_status",
    "Production deployment orchestrated via Pulumi. Further services (MCP Gateway, etc.) would be added similarly.",
)
