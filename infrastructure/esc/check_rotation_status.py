...
class RotationStatusChecker:
    ...
    def _load_rotation_history(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(history_path):
                ...
                logger.info("Loaded rotation history")
                return history
            else:
                logger.warning(
                    f"Rotation history not found at {history_path}, creating empty history"
                )
                return {"services": {}}
        except Exception as e:
    ...
    def check_rotation_status(self) -> Dict[str, Any]:
        ...
        for service, config in self.service_registry.items():
            ...
            rotation_interval = self._parse_rotation_schedule(
                service_status["rotation_schedule"]
            )
...
def main():
    ...
    parser.add_argument(
        ...
        "-u",
        action="store_true",
        help="Update rotation history for a secret",
    )
    ...
    parser.add_argument(
        "--timestamp", "-t", help="Timestamp (ISO format, defaults to now)"
    )
