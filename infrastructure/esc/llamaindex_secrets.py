...
def setup_llamaindex_secrets():
    ...
    if missing_secrets:
        ...
        if still_missing:
            ...
            logger.info("Please add these secrets manually or ensure they exist in GitHub")
            return False
...
def update_env_file():
    ...
    for secret_name, secret_value in secrets.items():
        if f"{secret_name}=" in env_content:
            ...
            env_content = "\n".join([
                line if not line.startswith(f"{secret_name}=") else f"{secret_name}={secret_value}"
...
def main():
    ...
    parser.add_argument("--sync-secrets", action="store_true", help="Sync secrets between GitHub and Pulumi ESC")
    parser.add_argument("--update-env", action="store_true", help="Update .env file with LlamaIndex secrets")
    parser.add_argument("--list-secrets", action="store_true", help="List available LlamaIndex secrets")
