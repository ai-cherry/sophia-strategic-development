...
class OpenRouterSecretManager(BaseSecretManager):
                ...
                async def test_api():
                    async with aiohttp.ClientSession() as session:
                        headers = {
                            ...
                            "HTTP-Referer": self.get_secret("OPENROUTER_SITE_URL") or "https://payready.com",
                            "X-Title": self.get_secret("OPENROUTER_APP_NAME") or "Sophia AI"
                        }
    ...
    def setup_local_development(self):
        ...
        if not site_url:
            site_url = input("\nEnter your site URL (default: https://payready.com): ").strip()
            if not site_url:
            ...
            if models:
                ...
                for provider, provider_models in sorted(providers.items()):
                    ...
                    for model in provider_models[:3]:  # Show top 3 per provider
                        print(f"  - {model.get('id')}: ${model.get('pricing', {}).get('prompt', 0):.6f}/token")
