...
async def get_service_secrets(
    ...
    try:
        ...
        registry_path = os.path.join(
            project_root, "infrastructure", "integration_registry.json"
        )
...
async def main():
    ...
    parser.add_argument(
        "--key",
        "-k",
        help="Secret key (if not provided, all secrets for the service will be returned)",
    )
    parser.add_argument(
        "--organization", "-o", default="ai-cherry", help="Pulumi organization"
    )
    parser.add_argument("--project", "-p", default="sophia", help="Pulumi project")
    parser.add_argument(
        "--environment", "-e", default="production", help="Environment (stack)"
    )
    parser.add_argument(
        "--config",
        "-c",
        action="store_true",
        help="Get configuration instead of secret",
    )
    ...
    try:
        if args.key:
            ...
            if value is None:
                logger.error(
                    f"Secret or configuration not found: {args.service}_{args.key}"
                )
                return 1
            ...
            secrets = await get_service_secrets(
                args.service, args.organization, args.project, args.environment
            )
