...
esc_environment = pulumiservice.Environment(
    f"sophia-{environment}",
...
for group_name, description in secret_groups.items():
    pulumiservice.SecretGroup(
        f"sophia-{environment}-{group_name}",
        organization=pulumi_organization,
...
service_secret_mappings = {
    ...
    "lambda_labs": {
        ...
        "jupyter_password": {
            ...
            "description": "Lambda Labs Jupyter password",
        },
        "ssh_public_key": {
            "group": "security-tokens",
            "description": "Lambda Labs SSH public key",
        },
        "ssh_private_key": {
            "group": "security-tokens",
            "description": "Lambda Labs SSH private key",
        },
    },
    "airbyte": {
    ...
    "openai": {"api_key": {"group": "api-keys", "description": "OpenAI API key"}},
    "anthropic": {"api_key": {"group": "api-keys", "description": "Anthropic API key"}},
    "github": {
        "token": {
            "group": "security-tokens",
    ...
    for identity in policy["identities"]:
        if identity["type"] == "user":
            policy_identities.append(
                pulumiservice.AccessPolicyIdentityArgs(
                    type="user", name=identity["name"]
                )
            )
        elif identity["type"] == "github":
            policy_identities.append(
                pulumiservice.AccessPolicyIdentityArgs(
                    type="github", name=identity["name"]
                )
            )

    pulumiservice.AccessPolicy(
        f"sophia-{environment}-{policy['name']}",
        organization=pulumi_organization,
        name=policy["name"],
...
stack = pulumiservice.Stack(
    f"sophia-{environment}-stack",
    organization=pulumi_organization,
    project=pulumi_project,
    stack=environment,
    tags={"environment": environment, "application": "sophia"},
)

# Associate the stack with the environment
stack_environment_binding = pulumiservice.StackEnvironmentBinding(
    f"sophia-{environment}-stack-binding",
    organization=pulumi_organization,
    project=pulumi_project,
    stack=stack.stack,
    environment=esc_environment.name,
...
pulumi_esc_config = pulumi.asset.AssetArchive(
    {
        "pulumi_esc_config.json": pulumi.asset.StringAsset(
            json.dumps(
                {
                    "organization": pulumi_organization,
                    "project": pulumi_project,
