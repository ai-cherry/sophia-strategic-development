...
sophia_esc_environment = esc.Environment(
    ...
    definition={
        "values": {
            ...
            "ai_intelligence": {
                ...
                "openrouter": {"api_key": "${github.OPENROUTER_API_KEY}"},
                "portkey": {
                    "api_key": "${github.PORTKEY_API_KEY}",
                    "config": "${github.PORTKEY_CONFIG}",
                },
                "huggingface": {"api_token": "${github.HUGGINGFACE_API_TOKEN}"},
                "together_ai": {"api_key": "${github.TOGETHER_AI_API_KEY}"},
            },
            "data_intelligence": {
                "apify": {"api_token": "${github.APIFY_API_TOKEN}"},
                "phantombuster": {"api_key": "${github.PHANTOM_BUSTER_API_KEY}"},
                "twingly": {"api_key": "${github.TWINGLY_API_KEY}"},
                "tavily": {"api_key": "${github.TAVILY_API_KEY}"},
                "zenrows": {"api_key": "${github.ZENROWS_API_KEY}"},
            },
            "infrastructure": {
                "lambda_labs": {
