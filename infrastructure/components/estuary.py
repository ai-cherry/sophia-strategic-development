...
class EstuaryComponent(BaseComponent):
    def __init__(self, name: str, opts: ResourceOptions = None):
        ...
        self.estuary_api_url = config.get("estuary_api_url") or "https://api.estuary.tech"
        ...
        self.collection_name = collection_names.get(env, collection_names["development"])

        # This is a custom resource that uses the Pulumi Command provider to call the Estuary API
        self.collection = command.local.Command("estuary_collection",
            create=Output.concat(
                "curl -X POST '", self.estuary_api_url, "/collections' ",
