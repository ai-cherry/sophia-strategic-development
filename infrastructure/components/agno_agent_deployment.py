...
class AgnoAgentDeployment(pulumi.ComponentResource):
    ...
    def __init__(self,
        ...
        # --- 6. Auto Scaling ---
        scaling_target = aws.appautoscaling.Target(f"{name}-scaling-target",
            max_capacity=5,
            min_capacity=1,
            resource_id=self.service.id.apply(lambda id: f"service/{cluster_arn.split('/')[-1]}/{id.split('/')[-1]}"),
            scalable_dimension="ecs:service:DesiredCount",
            service_namespace="ecs",
            opts=pulumi.ResourceOptions(parent=self))

        aws.appautoscaling.Policy(f"{name}-cpu-scaling-policy",
