...
class AirbyteComponent(BaseComponent):
    def __init__(self, name: str, snowflake_config: dict, opts: ResourceOptions = None):
        ...
        self.workspace = airbyte.Workspace(f"sophia-{env}-workspace",
            name=f"sophia-{env}",
        ...
        for conn_config in connection_configs:
            ...
            source_conf = next((s for s in source_configs if s['name'] == source_name), None)
            dest_conf = next((d for d in destination_configs if d['name'] == dest_name), None)
        ...
        self.register_outputs({
            ...
            "connection_ids": [c.connection.connection_id for c in self.connections]
        }) 