...
class SnowflakeComponent(BaseComponent):
    def __init__(self, name: str, opts: ResourceOptions = None):
        ...
        self.database = snowflake.Database(db_name,
            name=db_name,
            comment="Database for the Sophia AI platform"
        )

        self.warehouse = snowflake.Warehouse(wh_name,
            name=wh_name,
        ...
        self.schema = snowflake.Schema(schema_name,
            name=schema_name,
            database=self.database.name,
            comment="Schema for raw data ingested from various sources"
        )

        self.role = snowflake.Role(role_name,
            name=role_name,
