...
from lambda_labs_provisioner import LambdaLabsProvisioner, LambdaLabsConfig
...
class SophiaInfrastructureManager:
    ...
    async def deploy_complete_infrastructure(self) -> Dict:
        ...
        try:
            ...
            async with LambdaLabsProvisioner(self.lambda_config) as provisioner:
                private_key_path = get_config_value("lambda_labs_ssh_private_key_path", "/tmp/lambda_labs_key")
                ...
                infrastructure_result = await provisioner.deploy_complete_infrastructure(private_key_path)
    ...
    async def _update_mcp_configurations(self, infrastructure_result: Dict):
        ...
        postgresql_conn = infrastructure_result.get("connection_strings", {}).get("postgresql")
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        ...
                        if response.status in [200, 201]:
                            ...
                            logger.warning(f"⚠️ Estuary Flow update failed: {response.status}")
            else:
    ...
    async def _deploy_data_pipeline(self, infrastructure_result: Dict):
        ...
        try:
            instance_ip = infrastructure_result.get("ip_address")
            if not instance_ip:
                logger.warning("⚠️ Instance IP not available for data pipeline deployment")
                return
            
            # Deploy Gong API extractor
            gong_extractor_path = "/home/ubuntu/sophia-project/backend/etl/gong_api_extractor_clean.py"
            if os.path.exists(gong_extractor_path):
                ...
                logger.info(f"📋 Gong API extractor ready for deployment to {instance_ip}")
    ...
    async def _update_repository_configs(self, infrastructure_result: Dict):
        ...
        try:
            ...
            env_updates = {
                ...
                "POSTGRESQL_CONNECTION_STRING": infrastructure_result.get("connection_strings", {}).get("postgresql"),
                "REDIS_CONNECTION_STRING": infrastructure_result.get("connection_strings", {}).get("redis"),
