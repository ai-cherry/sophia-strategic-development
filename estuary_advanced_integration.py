...
class EstuaryAdvancedIntegration:
    ...
    def deploy_advanced_estuary_integration(self):
        ...
        self.create_advanced_flow_configuration()

        # Step 4: Create new draft for advanced integration
        if not self.execute_flowctl_command(
            "flowctl draft create", "Creating new draft for advanced integration"
        ):
            return False

        # Step 5: Initialize development
        if not self.execute_flowctl_command(
            "flowctl draft develop", "Initializing draft development"
        ):
            return False
        ...
        test_result = self.execute_flowctl_command(
            "flowctl draft test", "Testing advanced Estuary-Snowflake integration"
        )

        if test_result:
            # Step 8: Publish if test passes
            if self.execute_flowctl_command(
                "flowctl draft publish", "Publishing advanced integration to production"
            ):
                logger.info(
                    "✅ Advanced Estuary-Snowflake integration deployed successfully!"
                )
                return True
        else:
            logger.warning(
                "⚠️ Test failed, but configuration is saved for manual review"
            )
