...
class GeminiCLIProvider:
    ...
    async def process_large_document(
        ...
        if token_count <= chunk_size:
            ...
            # Combine results
            combined = "\n\n".join(results)
            final_prompt = f"Combine these {operation} results into a coherent whole: {combined}"
    ...
    async def route_request(
        ...
        **kwargs
        ...
        if self.should_use_gemini(context_size, task_type, api_limits):
            logger.info(f"Routing to Gemini CLI (context: {context_size}, task: {task_type})")
            ...
            if result["success"]:
                ...
                estimated_cost = result["usage"]["total_tokens"] * 0.00003  # Rough estimate
