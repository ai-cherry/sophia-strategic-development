...
class GeminiMCPIntegration:
        ...
        try:
            ...
            if server_info.pid:
                ...
                try:
                    ...
                    logger.info(f"Server {server_name} stopped (PID: {server_info.pid})")
                    
                except psutil.NoSuchProcess:
                    logger.info(f"Server {server_name} process not found (already stopped)")
    ...
    async def _is_server_running(self, server_name: str) -> bool:
        ...
        for conn in psutil.net_connections():
            if conn.laddr.port == server_info.port and conn.status == psutil.CONN_LISTEN:
                return True
                ...
                for server_name, server_info in self.servers.items():
                    if server_info.status == MCPServerStatus.RUNNING:
                        ...
                        if not is_healthy and server_info.auto_start:
                            logger.warning(f"Server {server_name} failed health check, attempting restart")
    ...
    async def route_request(self, query: str) -> str:
        ...
        fallback_server = self.config.get("routing", {}).get("fallback", "sophia-ai-intelligence")
        ...
        for rule in routing_rules:
            ...
            if any(keyword in query.lower() for keyword in pattern.split("|")):
                if server in self.servers:
                    logger.info(f"Routing query to {server} based on pattern: {pattern}")
                    return server
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Sophia AI - Gemini CLI MCP Integration")
    parser.add_argument("--config", default=".gemini/settings.json", help="Configuration file path")
    parser.add_argument("--action", choices=["start", "stop", "status", "monitor"], required=True, help="Action to perform")
