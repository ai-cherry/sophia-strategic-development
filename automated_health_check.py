...
class HealthCheckRunner:
    ...
    async def run_all_checks(self) -> Dict[str, Any]:
        ...
        overall_health = "healthy" if healthy_checks == total_checks else "degraded" if healthy_checks > total_checks * 0.7 else "unhealthy"
    ...
    async def check_pulumi_esc(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check stack access
            stack_result = subprocess.run(["pulumi", "stack", "ls"], capture_output=True, text=True)
            stacks = stack_result.stdout.strip().split('\n')[1:] if stack_result.returncode == 0 else []
    ...
    async def check_secret_access(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await pulumi_esc_client.get_secret("test_secret", default="test_value")
        ...
        try:
            ...
            return {
                ...
                "config_files": {
                    ...
                    "docker-compose.mcp.yml": (project_root / "docker-compose.mcp.yml").exists()
                }
            }
        except Exception as e:
            return {"success": False, "error": str(e)}
...
async def main():
    ...
    print(f"{'='*50}")
    print(f"Overall Status: {'✅ HEALTHY' if results['overall_status'] == 'healthy' else '⚠️ DEGRADED' if results['overall_status'] == 'degraded' else '❌ UNHEALTHY'}")
    print(f"Health Score: {results['health_percentage']:.1f}% ({results['healthy_checks']}/{results['total_checks']} checks passed)")
    ...
    results_file = project_root / f"health_check_results_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
