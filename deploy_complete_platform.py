...
class CompletePlatformDeployment:
    ...
    async def _deploy_snowflake_infrastructure(self):
        ...
        try:
            ...
            result = subprocess.run([
                'python3', '/home/ubuntu/snowflake_advanced_features_implementation.py'
            ], capture_output=True, text=True, cwd='/home/ubuntu')
            
            if result.returncode == 0:
                self.deployment_status['snowflake']['details'].append("✅ Advanced features deployed")
            else:
                self.deployment_status['snowflake']['details'].append(f"⚠️ Advanced features: {result.stderr}")
            ...
            if result.returncode == 0:
                self.deployment_status['snowflake']['details'].append("✅ Cortex Agents deployed")
            else:
                self.deployment_status['snowflake']['details'].append(f"⚠️ Cortex Agents: {result.stderr}")
    ...
    async def _deploy_estuary_configuration(self):
        ...
        try:
            ...
            config_path = '/home/ubuntu/sophia-main/estuary_comprehensive_flow_config.yaml'
            if os.path.exists(config_path):
                self.deployment_status['estuary']['details'].append("✅ Flow configuration created")
                
                # Validate configuration
                try:
                    with open(config_path, 'r') as f:
                        config_content = f.read()
                        if 'collections:' in config_content and 'materializations:' in config_content:
                            self.deployment_status['estuary']['details'].append("✅ Configuration validated")
                        else:
                            self.deployment_status['estuary']['details'].append("⚠️ Configuration incomplete")
                except Exception as e:
                    self.deployment_status['estuary']['details'].append(f"⚠️ Validation error: {str(e)}")
                
                # Note: Actual deployment would require valid Estuary token
                self.deployment_status['estuary']['details'].append("⚠️ Deployment pending valid Estuary token")
            else:
                self.deployment_status['estuary']['details'].append("❌ Configuration file not found")
        ...
        try:
            ...
            for service_file in service_files:
                if os.path.exists(service_file):
                    ...
                    self.deployment_status['application']['details'].append(f"✅ {service_name} deployed")
                else:
                    service_name = os.path.basename(service_file)
                    self.deployment_status['application']['details'].append(f"❌ {service_name} missing")
            ...
            for config_file in config_files:
                if os.path.exists(config_file):
                    ...
                    self.deployment_status['application']['details'].append(f"✅ {config_name} configured")
                else:
                    config_name = os.path.basename(config_file)
                    self.deployment_status['application']['details'].append(f"❌ {config_name} missing")
            ...
            self.deployment_status['application']['details'].append(f"❌ Error: {str(e)}")
    ...
    async def _verify_deployment(self):
        ...
        try:
            ...
            try:
                ...
                self.deployment_status['verification']['details'].append(f"✅ Snowflake connectivity verified (v{version})")
                
            except Exception as e:
                self.deployment_status['verification']['details'].append(f"❌ Snowflake connectivity failed: {str(e)}")
            ...
            try:
                ...
                if databases:
                    self.deployment_status['verification']['details'].append("✅ SOPHIA_AI_ADVANCED database exists")
                    ...
                    self.deployment_status['verification']['details'].append(f"✅ {schema_count} schemas deployed")
                else:
                    self.deployment_status['verification']['details'].append("❌ SOPHIA_AI_ADVANCED database not found")
                
                cursor.close()
                conn.close()
                
            except Exception as e:
                self.deployment_status['verification']['details'].append(f"❌ Database verification failed: {str(e)}")
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute("SELECT SNOWFLAKE.CORTEX.SENTIMENT('This is a great platform!')")
                sentiment_result = cursor.fetchone()[0]
                
                if sentiment_result:
                    self.deployment_status['verification']['details'].append("✅ Cortex AI functions working")
                else:
                    self.deployment_status['verification']['details'].append("❌ Cortex AI functions not responding")
                
                cursor.close()
                conn.close()
                
            except Exception as e:
                self.deployment_status['verification']['details'].append(f"⚠️ AI functions test: {str(e)}")
            ...
            self.deployment_status['verification']['details'].append(f"❌ Error: {str(e)}")
    ...
    def _get_overall_status(self) -> str:
        ...
        statuses = [component['status'] for component in self.deployment_status.values()]
    ...
    def _print_deployment_summary(self, report: Dict[str, Any]):
        ...
        print(f"   Overall Status: {report['deployment_summary']['overall_status'].upper()}")
        
        print(f"\n🏗️ Component Status:")
        for component, status in self.deployment_status.items():
            status_icon = "✅" if status['status'] == 'completed' else "⚠️" if status['status'] == 'partial' else "❌"
