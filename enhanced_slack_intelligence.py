...
class SlackIntelligenceEngine:
    ...
    def _analyze_sentiment(self, text: str) -> tuple[SentimentLevel, float]:
        ...
        try:
            ...
            negative_count = sum(
                1 for word in business_negative_words if word in text_lower
            )
            positive_count = sum(
                1 for word in business_positive_words if word in text_lower
            )
    ...
    def _requires_follow_up(self, text: str, sentiment: SentimentLevel) -> bool:
        ...
        if sentiment in [SentimentLevel.NEGATIVE, SentimentLevel.VERY_NEGATIVE]:
            customer_mentions = any(
                keyword in text_lower for keyword in self.business_keywords["customer"]
            )
    ...
    async def generate_conversation_insights(
        self, messages: List[MessageIntelligence]
        ...
        try:
            ...
            if customer_messages:
                avg_sentiment = sum(
                    msg.sentiment_score for msg in customer_messages
                ...
                if avg_sentiment < -0.3:
                    insights.append(
                        ConversationInsight(
                            ...
                            related_messages=[
                                msg.message_id for msg in customer_messages[-5:]
                            ],
                            trend_data={
            ...
            if deal_messages:
                positive_deals = sum(
                    1 for msg in deal_messages if msg.sentiment_score > 0.2
                )
                ...
                insights.append(
                    ConversationInsight(
                        ...
                        trend_data={
                            "success_rate": deal_success_rate,
                            "total_deals": len(deal_messages),
                        },
                    )
                )
            ...
            if support_messages:
                urgent_issues = sum(
                    1 for msg in support_messages if msg.urgency_score > 0.7
                )
...
class EnhancedSlackIntegration:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Enhanced Slack integration initialized for {auth_response.get('team')}"
            )
            return True
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Channel:* #{intelligence.channel}",
                        },
                        {"type": "mrkdwn", "text": f"*User:* <@{intelligence.user}>"},
                        {
                            "type": "mrkdwn",
                            "text": f"*Urgency:* {intelligence.urgency_score:.1%}",
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Sentiment:* {intelligence.sentiment.value}",
                        },
                    ],
                },
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"*Message:* {intelligence.text[:200]}...",
                    },
                },
            ],
        }
    ...
    async def _process_insights_batch(self):
        ...
        try:
            insights = await self.intelligence_engine.generate_conversation_insights(
                self.message_buffer
            )
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Confidence:* {insight.confidence_score:.1%}",
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Impact:* {insight.business_impact}",
                        },
                    ],
                },
                {
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if await integration.initialize():
            ...
            intelligence = await integration.process_message_with_intelligence(
                sample_message
            )
            ...
            if intelligence:
                ...
                print(
                    f"Sentiment: {intelligence.sentiment.value} ({intelligence.sentiment_score:.2f})"
                )
