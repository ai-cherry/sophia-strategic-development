...
class EstuaryAdapter(PlatformAdapter):
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "webhook_url": os.getenv("ESTUARY_WEBHOOK_URL", "https://app.sophia-intel.ai/webhook/estuary")
        }
    
    async def configure(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """Configure Estuary with given settings."""
        try:
            self.logger.info(f"Configuring Estuary with operations: {list(config.keys())}")
    ...
    async def handle_webhook(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            self.logger.info(f"Received Estuary webhook: {payload.get('event_type', 'unknown')}")
