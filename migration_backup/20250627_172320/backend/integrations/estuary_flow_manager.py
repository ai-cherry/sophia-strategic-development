...
Comprehensive integration replacing Airbyte with Estuary Flow
"""
    ...
    def from_pulumi_esc(cls) -> 'EstuaryCredentials':
        ...
        try:
            ...
            result = subprocess.run([
                'pulumi', 'config', 'get', 'estuary:access_token', '--stack', 'sophia-ai-prod'
            ...
            if result.returncode != 0:
                ...
                if not access_token or not refresh_token:
                    raise ValueError("Estuary credentials not found in Pulumi ESC or environment")
            ...
            # Get refresh token
            result = subprocess.run([
                'pulumi', 'config', 'get', 'estuary:refresh_token', '--stack', 'sophia-ai-prod'
            ], capture_output=True, text=True)
            
            refresh_token = result.stdout.strip() if result.returncode == 0 else os.getenv('ESTUARY_REFRESH_TOKEN')
...
class EstuaryFlowManager:
    ...
    def _ensure_flowctl_installed(self) -> str:
        ...
        try:
            ...
            result = subprocess.run(['which', 'flowctl'], capture_output=True, text=True)
            if result.returncode == 0:
    ...
    def run_flowctl_command(self, command: List[str], description: str = "") -> Optional[Dict]:
    ...
    def create_github_capture(self, repository: str, access_token: str) -> EstuaryCapture:
    ...
    def create_hubspot_capture(self, client_id: str, client_secret: str, refresh_token: str) -> EstuaryCapture:
    ...
    def create_slack_capture(self, api_token: str, channels: List[str]) -> EstuaryCapture:
    ...
    def create_snowflake_materialization(self, snowflake_config: Dict[str, str]) -> EstuaryMaterialization:
    ...
    def create_custom_connector(self, name: str, api_config: Dict[str, Any]) -> EstuaryCapture:
    ...
    def deploy_materialization(self, materialization: EstuaryMaterialization) -> bool:
        ...
        try:
            ...
            if publish_result:
                logger.info(f"✅ Successfully deployed materialization: {materialization.name}")
                return True
            else:
                logger.error(f"❌ Failed to publish materialization: {materialization.name}")
                return False
                
        except Exception as e:
            logger.error(f"❌ Failed to deploy materialization {materialization.name}: {e}")
            return False
    
    def list_captures(self) -> List[Dict[str, Any]]:
        """List all captures in the tenant"""
        result = self.run_flowctl_command(
            ['catalog', 'list', '--captures=true', '--collections=false', '--materializations=false', '--tests=false'],
            "List all captures"
        )
    ...
    def list_materializations(self) -> List[Dict[str, Any]]:
        ...
        result = self.run_flowctl_command(
            ['catalog', 'list', '--captures=false', '--collections=false', '--materializations=true', '--tests=false'],
    ...
    def get_capture_status(self, capture_name: str) -> Optional[Dict[str, Any]]:
        ...
        result = self.run_flowctl_command(
            ['catalog', 'stats', '--name', full_name],
            f"Get status for {capture_name}"
        )
    ...
    def pause_capture(self, capture_name: str) -> bool:
        ...
        full_name = f"{self.credentials.tenant}/{capture_name}"
        result = self.run_flowctl_command(
            ['catalog', 'disable', '--name', full_name],
            f"Pause {capture_name}"
        )
    ...
    def resume_capture(self, capture_name: str) -> bool:
        ...
        full_name = f"{self.credentials.tenant}/{capture_name}"
        result = self.run_flowctl_command(
            ['catalog', 'enable', '--name', full_name],
            f"Resume {capture_name}"
        )
    ...
    def delete_capture(self, capture_name: str) -> bool:
        ...
        full_name = f"{self.credentials.tenant}/{capture_name}"
        result = self.run_flowctl_command(
            ['catalog', 'delete', '--name', full_name],
            f"Delete {capture_name}"
        )
    ...
    def create_sophia_ai_foundation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            deployment_results['materializations_deployed'].append('snowflake-sophia-ai')
            ...
            for name, config in capture_configs.items():
                if name == 'github':
                    capture = self.create_github_capture(config['repository'], config['access_token'])
                elif name == 'hubspot':
                    capture = self.create_hubspot_capture(
                        config['client_id'], config['client_secret'], config['refresh_token']
                    )
                elif name == 'slack':
                    capture = self.create_slack_capture(config['api_token'], config['channels'])
