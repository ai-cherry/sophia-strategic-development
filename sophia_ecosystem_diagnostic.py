...
def check_pulumi_auth() -> Dict[str, Any]:
    ...
    if token.startswith("your-"):
        return {"status": "error", "message": "PULUMI_ACCESS_TOKEN is placeholder", "token_preview": token[:20]}
...
def check_github_secrets() -> Dict[str, Any]:
    ...
    expected_secrets = [
        ...
        "GONG_ACCESS_KEY", "HUBSPOT_ACCESS_TOKEN", "LINEAR_API_KEY",
        "LAMBDA_API_KEY", "SNOWFLAKE_PASSWORD", "PINECONE_API_KEY"
    ]
    
    available = [s for s in expected_secrets if os.getenv(s) and not os.getenv(s).startswith("your-")]
...
def check_mcp_servers() -> Dict[str, Any]:
    ...
    for name, port in servers.items():
        try:
            ...
            sock.close()
            status[name] = {"status": "online" if result == 0 else "offline", "port": port}
        except Exception as e:
            status[name] = {"status": "error", "port": port, "error": str(e)}
    
    online_count = sum(1 for s in status.values() if s["status"] == "online")
    return {"status": "success" if online_count == len(servers) else "partial", "online_count": online_count, "servers": status}
...
def main():
    ...
    print(f"{status_emoji} GITHUB SECRETS: {github_status['available_count']}/{github_status['total_expected']} available")
    ...
    if codacy_status["status"] != "online":
        ...
        print(f"   🚀 Start: {codacy_status.get('start_command', 'See mcp-servers/codacy/')}")
