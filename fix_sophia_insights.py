...
async def fix_insights_generation():
    ...
    try:
        ...
        for user in users:
            insights = {
                ...
                "apartment_expertise": round(
                    float(user["avg_apartment_relevance"] or 0) * 100, 1
                ),
                "avg_call_duration": round(
                    float(user["avg_call_duration"] or 0) / 60, 1
                ),
                "activity_level": "high"
        ...
        for call in calls:
            insights = {
                "apartment_relevance_score": round(
                    float(call["apartment_relevance"] or 0) * 100, 1
                ),
        ...
        print("✅ SOPHIA INSIGHTS GENERATION COMPLETE!")
        ...
        print(
            f"   Average Apartment Relevance: {round(float(stats['avg_relevance']) * 100, 1)}%"
        )
        ...
        for user in sample_users:
            print(
                f"   {user['first_name']} {user['last_name']} ({user['title']}) - Score: {user['performance_score']}, Apartment Expertise: {user['apartment_expertise']}%"
            )

        print("\n🎯 HIGH-VALUE APARTMENT CALLS:")
        for call in sample_calls:
            print(
                f"   {call['title'][:60]}... - Relevance: {call['relevance_score']}% - Value: ${call['business_value']:,}"
            )
