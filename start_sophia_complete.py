    ...
    def start_mcp_servers(self):
        ...
        for port, name in mcp_ports.items():
            if self.check_port_open(port):
                print(f"✅ {name} MCP server already running on port {port}")
                running_count += 1
            else:
                print(f"⚠️  {name} MCP server not running on port {port}")
        
        if running_count == len(mcp_ports):
            print("✅ All MCP servers are running")
            self.services_status['mcp_servers'] = 'healthy'
        else:
            print(f"⚠️  Only {running_count}/{len(mcp_ports)} MCP servers running")
            print("💡 Run 'python start_mcp_servers.py' to start MCP servers")
            self.services_status['mcp_servers'] = 'partial'
    
    def start_backend_simplified(self):
        """Start simplified Sophia backend"""
        print("🚀 Starting Sophia backend...")
        
        # Check if backend is already running
        if self.check_port_open(8000):
            print("✅ Sophia backend already running on port 8000")
...
'''
        ...
        try:
            ...
            if proc.poll() is None and self.check_port_open(8000):
                print("✅ Sophia backend started successfully on port 8000")
                self.services_status['backend'] = 'healthy'
                return True
            else:
    ...
    async def run_system_health_check(self):
        ...
        if healthy_count == len(services):
            print("🎉 SOPHIA AI SYSTEM FULLY OPERATIONAL!")
    ...
    def print_next_steps(self):
        ...
        print("=" * 50)
        print("1. 🔗 **Cursor IDE Integration:**")
        print("   - MCP servers are ready for @ai_memory, @codacy, @asana, @notion commands")
        ...
        print("   - Scale MCP servers: docker-compose -f docker-compose.mcp-gateway.yml up")
...
async def main():
    ...
    try:
        print("🚀 Sophia AI Complete System Startup")
        print("=" * 50)
        ...
        print("\\n🎉 Sophia AI system is ready for development!")
