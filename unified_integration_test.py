...
class GongTest(IntegrationTest):
    ...
    async def _run_test(self) -> bool:
        ...
        try:
            ...
            # Check if credentials are provided
            if not all([os.getenv("GONG_API_KEY"), os.getenv("GONG_API_SECRET")]):
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Gong API connection successful: {self.details['users_count']} users found"
                )
                return True
            else:
                logger.error(
    ...
    async def _run_test(self) -> bool:
        ...
        try:
            ...
            response = session.get(
                f"{api_url}/v1/collections", headers=headers, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Estuary API connection successful: {self.details['total_files']} files"
                )
                return True
            else:
                logger.error(
...
async def run_tests(
    ...
    for test in results:
        ...
        console.print(
            f"  {test['name']}: [{status_color}]{test['status'].upper()}[/{status_color}]"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--output", type=str, help="Output file for test results (JSON format)"
    )
