...
class StrategicRepositoryExpander:
    ...
    def add_repository(self, repo_info: dict) -> dict:
        ...
        if (self.repo_root / target_path).exists():
            ...
            success = self.run_command(
                ["git", "pull"], cwd=self.repo_root / target_path
            )
    ...
    def generate_expansion_report(self, results: list[dict]):
        ...
        if failed:
            ...
            for repo in failed:
                report.append(
                    f"- **{repo['name']}**: {repo.get('error', 'Unknown error')}"
                )
                report.append("")
    ...
    def run_full_expansion(self):
        ...
        try:
            ...
            self.log(
                "Your AI coding assistant now has access to a comprehensive reference library."
            )
