...
def fix_snowflake_cortex_indentation():
    ...
    for i, line in enumerate(lines):
        ...
        if 'try:' in line and i < len(lines) - 1:
            ...
            if next_line and not next_line.startswith(' ' * (len(line) - len(line.lstrip()) + 4)):
                in_try_block = True
                try_line = i
        
        # Fix lines after try that aren't properly indented
        if in_try_block and try_line is not None and i == try_line + 1:
            if line.strip() and not line.startswith(' ' * (len(lines[try_line]) - len(lines[try_line].lstrip()) + 4)):
...
def fix_missing_server_import():
    ...
    if "from mcp.server import Server" not in content and "from mcp import Server" not in content:
