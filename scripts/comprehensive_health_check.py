...
class ConfigurationHealthCheck(ComponentHealthCheck):
    ...
    async def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.status = HealthCheckStatus.HEALTHY if all_critical_passed else HealthCheckStatus.WARNING
            self.details = {
    ...
    async def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            functionality_checks = {
                ...
                "store_functionality": "id" in test_store and test_store.get("status") == "stored"
            }
            
            all_checks_passed = all(functionality_checks.values())
            
            self.status = HealthCheckStatus.HEALTHY if all_checks_passed else HealthCheckStatus.WARNING
            self.details = {
...
class DependencyHealthCheck(ComponentHealthCheck):
    ...
    async def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            problematic_count = sum(1 for dep in dependency_status.values() 
...
class SophiaHealthChecker:
    ...
    def print_summary(self, report: Dict[str, Any]) -> None:
        ...
        print(f"Components: {summary['healthy_count']}/{summary['total_components']} healthy")
            ...
            if result["status"] != HealthCheckStatus.HEALTHY:
                ...
                if "error" in details:
                    ...
                    for key, value in details.items():
                        if isinstance(value, dict) and not all(v for v in value.values()):
