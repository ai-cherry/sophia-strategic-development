...
class DevelopmentMetricsGenerator:
    ...
    def get_commit_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        if metrics["commit_times"]:
            ...
            metrics["most_active_hours"] = self._get_most_frequent(metrics["commit_times"])
        
        metrics["most_active_author"] = max(metrics["authors"].items(), key=lambda x: x[1])[0] if metrics["authors"] else None
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        # Repository size
        try:
            result = subprocess.run(["du", "-sh", str(self.project_root)], 
        ...
        metrics["file_count_by_type"] = dict(sorted(file_types.items(), key=lambda x: x[1], reverse=True))
    ...
    def generate_comprehensive_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "git_branch": self.repo.active_branch.name if self.repo.active_branch else "unknown",
    ...
    def _calculate_overall_scores(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if commit_metrics["commits_per_day"] >= 1:
            scores["development_velocity"] = min(100, commit_metrics["commits_per_day"] * 20)
        ...
        all_scores = [scores["development_velocity"], scores["code_quality"], 
                     scores["testing_maturity"], scores["integration_completeness"]]
    ...
    def generate_metrics_report(self, metrics: Dict[str, Any]) -> str:
        ...
        report.append(f"- **Development Velocity**: {scores['development_velocity']:.1f}/100")
        report.append(f"- **Code Quality**: {scores['code_quality']:.1f}/100")
        report.append(f"- **Testing Maturity**: {scores['testing_maturity']:.1f}/100")
        report.append(f"- **Integration Completeness**: {scores['integration_completeness']:.1f}/100")
        report.append("")
        
        # Commit Metrics
        commit_metrics = metrics["commit_metrics"]
        report.append("## 📝 Commit Activity")
        report.append(f"- **Total Commits (30 days)**: {commit_metrics['total_commits']}")
        report.append(f"- **Commits per Day**: {commit_metrics['commits_per_day']:.1f}")
        report.append(f"- **Most Active Author**: {commit_metrics['most_active_author']}")
        ...
        report.append(f"- **Docstring Coverage**: {quality_metrics['docstring_coverage']:.1f}%")
        report.append(f"- **Type Hint Coverage**: {quality_metrics['type_hint_coverage']:.1f}%")
        report.append(f"- **Average File Size**: {quality_metrics.get('avg_file_size', 0):.0f} lines")
        report.append("")
        ...
        report.append(f"- **Test Frameworks**: {', '.join(testing_metrics['test_frameworks'])}")
        report.append("")
        
        # Dependencies
        dep_metrics = metrics["dependency_metrics"]
        report.append("## 📦 Dependencies")
        report.append(f"- **Python Dependencies**: {dep_metrics['python_dependencies']}")
        report.append(f"- **NPM Dependencies**: {dep_metrics['npm_dependencies']}")
        report.append(f"- **Outdated Packages**: {len(dep_metrics['outdated_packages'])}")
        report.append(f"- **Security Vulnerabilities**: {dep_metrics['security_vulnerabilities']}")
        report.append("")
        
        # MCP Integration
        mcp_metrics = metrics["mcp_integration_metrics"]
        report.append("## 🔌 MCP Integration")
        report.append(f"- **Servers Configured**: {mcp_metrics['mcp_servers_configured']}")
        report.append(f"- **Server Types**: {', '.join(mcp_metrics['server_types'])}")
        report.append(f"- **Auto-triggers**: {'✅' if mcp_metrics['auto_triggers_enabled'] else '❌'}")
        report.append(f"- **Workflow Automation**: {'✅' if mcp_metrics['workflow_automation'] else '❌'}")
        report.append("")
        ...
        report.append(f"- **GitHub Actions**: {'✅' if github_metrics['github_actions_enabled'] else '❌'}")
        report.append(f"- **PR Templates**: {'✅' if github_metrics['pr_templates'] else '❌'}")
        report.append(f"- **Issue Templates**: {'✅' if github_metrics['issue_templates'] else '❌'}")
        report.append("")
        ...
        if scores["code_quality"] < 70:
            report.append("- 📝 Improve code quality by adding docstrings and type hints")
        ...
        if scores["integration_completeness"] < 70:
            report.append("- 🔌 Complete MCP server setup and GitHub workflow configuration")
...
def main():
    ...
    parser.add_argument("--days", type=int, default=30, help="Days to analyze for commit metrics")
