...
class MCPServerDeployer:
    ...
    def health_check(self, server: MCPServer) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"http://localhost:{server.port}{server.health_endpoint}", timeout=5
            )
    ...
    def start_server(self, server_key: str, server: MCPServer) -> tuple[bool, str]:
        ...
        if not self.check_port_availability(server.port):
            if self.health_check(server):
                ...
                logger.warning(
                    f"⚠️  {server.name} port {server.port} in use but not healthy"
                )
    ...
    def deploy_by_priority(self, priority: int) -> dict[str, tuple[bool, str]]:
        ...
        servers = {
            k: v for k, v in self.server_configs.items() if v.priority == priority
        }
        results = {}

        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {
                executor.submit(self.start_server, k, v): k for k, v in servers.items()
            }
    ...
    def generate_status_report(self, all_results: dict[str, tuple[bool, str]]):
        ...
        report = {
            ...
            "deployment_results": {},
            "summary": {"successful": 0, "failed": 0, "already_running": 0},
        }
    ...
    def run(self):
        ...
        for priority in [1, 2, 3]:
            ...
            if priority == 1:
                critical_failures = [
                    k for k, (success, _) in results.items() if not success
                ]
