...
class UVConflictResolver:
    ...
    def backup_file(self, file_path: Path) -> None:
        ...
        if file_path.exists():
            backup_path = self.backup_dir / f"{file_path.name}.{datetime.now().strftime('%Y%m%d_%H%M%S')}.backup"
    ...
    def resolve_pip_commands(self) -> None:
        ...
        pip_replacements = [
            ...
            (r'pip install --no-cache-dir -r requirements\.txt', 'uv sync --frozen --no-cache'),
            (r'pip install -r backend/requirements\.txt', 'uv sync'),
            (r'pip install -r infrastructure/requirements\.txt', 'uv sync'),
            (r'# UV handles package management automatically', '# UV handles package management automatically'),
            (r'# UV handles package management automatically', '# UV handles package management automatically'),
            
            # Subprocess pip commands
            (r'subprocess\.run\(\["pip", "install", "-r", "requirements\.txt"\]', 'subprocess.run(["uv", "sync"]'),
    ...
    def resolve_dockerfile_conflicts(self) -> None:
        ...
        dockerfile_replacements = [
            ...
            (r'FROM python:3\.11-slim(?!\s+AS)', 'FROM python:3.12-slim AS builder\n\n# Install UV\nCOPY --from=ghcr.io/astral-sh/uv:latest /uv /bin/uv'),
            (r'FROM python:3\.12-slim(?!\s+AS)', 'FROM python:3.12-slim AS builder\n\n# Install UV\nCOPY --from=ghcr.io/astral-sh/uv:latest /uv /bin/uv'),
            
            # Fix incorrect UV installation lines
            (r'COPY --from=ghcr\.io/astral-sh/uv:latest /uv /bin/uv AS builder', 'COPY --from=ghcr.io/astral-sh/uv:latest /uv /bin/uv'),
            (r'COPY --from=ghcr\.io/astral-sh/uv:latest /uv /bin/uv AS base', 'COPY --from=ghcr.io/astral-sh/uv:latest /uv /bin/uv'),
        ]
        ...
        for dockerfile in dockerfiles:
            if dockerfile.is_file() and not str(dockerfile).startswith(str(self.backup_dir)):
    ...
    def resolve_github_workflows(self) -> None:
        ...
        workflow_replacements = [
            # Install UV
            (r'python -m # UV handles package management automatically', 'curl -LsSf https://astral.sh/uv/install.sh | sh\n        echo "$HOME/.local/bin" >> $GITHUB_PATH'),
            (r'# UV handles package management automatically', 'curl -LsSf https://astral.sh/uv/install.sh | sh\n        echo "$HOME/.local/bin" >> $GITHUB_PATH'),
            ...
            (r'key: \$\{\{ runner\.os \}\}-pip-.*', 'key: uv-${{ runner.os }}-${{ hashFiles("uv.lock") }}'),
    ...
    def remove_conflicting_requirements_files(self) -> None:
        ...
        for req_file in requirements_files:
            if req_file.is_file() and not str(req_file).startswith(str(self.backup_dir)):
                ...
                if req_file == main_requirements:
                    ...
                    if not backup_name.exists():
                        ...
                        logger.info(f"📦 Backed up main requirements.txt to {backup_name}")
    ...
    def run_complete_resolution(self) -> bool:
        ...
        try:
            ...
            if uv_valid:
                ...
                logger.info(f"📊 Summary: {self.conflicts_resolved} conflicts resolved, {self.files_updated} files updated, {self.files_removed} files removed")
                return True
            else:
                logger.warning("⚠️ UV conflict resolution completed with validation warnings")
                return False
