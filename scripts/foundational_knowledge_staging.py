...
st.set_page_config(
    page_title="Foundational Knowledge Staging", page_icon="📊", layout="wide"
)
...
if page == "Upload Data":
    ...
    with col1:
        ...
        employee_file = st.file_uploader(
            "Upload employee roster (CSV/JSON)", type=["csv", "json"], key="employees"
        )
    ...
    with col2:
        ...
        customer_file = st.file_uploader(
            "Upload customer list (CSV/JSON)", type=["csv", "json"], key="customers"
        )
    ...
    with col3:
        gong_file = st.file_uploader(
            "Upload Gong user export (JSON)", type=["json"], key="gong"
        )
    ...
    with col4:
        slack_file = st.file_uploader(
            "Upload Slack user export (JSON)", type=["json"], key="slack"
        )
...
# Page 3: Correlate
    ...
    if "employees" not in st.session_state.staged_data:
        ...
        email_field = st.session_state.correlations.get("employees", {}).get(
            "email", None
        )
        ...
        if not email_field or email_field == "(none)":
            ...
            if "gong" in st.session_state.staged_data:
                ...
                if gong_email_field:
                    # Count matches
                    employee_emails = set(
                        employees_df[email_field].dropna().str.lower()
                    )
            ...
            if correlations:
                ...
                system = st.selectbox(
                    "Select system to review", [c["System"] for c in correlations]
                )
    ...
    if not st.session_state.staged_data:
            ...
            summary_data.append(
                {
                    ...
                    "Status": (
                        "✅ Ready"
                        if source in st.session_state.correlations
                        else "⚠️ Not Mapped"
                    ),
                }
            )
        ...
        with col1:
            import_mode = st.radio(
                "Import Mode", ["Test Mode (Dry Run)", "Production Import"]
            )
...
WHERE LOWER(e.EMAIL) = LOWER(g.email);
        ...
        if st.button(
            "🚀 Start Import",
            type="primary",
            disabled=(import_mode == "Production Import"),
        ):
...
st.markdown("---")
st.markdown(
    "💡 **Tip**: This staging app helps you review and clean data before importing to production"
)
