...
class EnhancedBatchEmbeddingProcessor:
    ...
    async def _check_table_exists(self, schema_name: str, table_name: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Could not check if table {schema_name}.{table_name} exists: {e}"
            )
            return False
    ...
    async def _get_records_needing_embeddings(
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to get records needing embeddings for {config.table_name}: {e}"
            )
            return []
        ...
        try:
            ...
            for record in records:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to generate embedding for record {record['id']}: {e}"
                    )
                    continue
            ...
            logger.error(
                f"❌ Failed to generate embeddings batch for {config.table_name}: {e}"
            )
            return 0
    ...
    async def _apply_cortex_sentiment_analysis(self, config: EmbeddingConfig) -> int:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Could not apply sentiment analysis to {config.table_name}: {e}"
            )
            return 0
    ...
    async def _apply_cortex_summarization(self, config: EmbeddingConfig) -> int:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Could not apply summarization to {config.table_name}: {e}"
            )
            return 0
        ...
        try:
            ...
            for config in schema_configs:
                ...
                if not await self._check_table_exists(
                    ...
                    logger.warning(
                        f"⚠️ Table {config.schema_name}.{config.table_name} does not exist, skipping"
                    )
                    continue
                ...
                logger.info(
                    f"✅ Processed {config.table_name}: {embedding_count} embeddings, {sentiment_count} sentiment, {summary_count} summaries"
                )
    ...
    async def process_all_embeddings(self) -> Dict[str, Dict[str, int]]:
        ...
        try:
            ...
            logger.info(
                f"✅ Comprehensive embedding processing completed: {all_results}"
            )
            return all_results
