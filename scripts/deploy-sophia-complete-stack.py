...
class SophiaCompleteStackDeployment:
    ...
    async def deploy_infrastructure_services(self):
        ...
        compose_file = (
            self.project_root / "infrastructure" / "sophia-ai-complete-stack.yml"
        )

        if not compose_file.exists():
            logger.error(f"Docker Compose file not found: {compose_file}")
            raise FileNotFoundError(
                f"Missing Docker Compose configuration: {compose_file}"
            )
    ...
    async def configure_estuary_flow(self):
        ...
        try:
            ...
            flow_config_file = (
                self.project_root / "estuary-config" / "sophia-ai-flows.yaml"
            )
            ...
            if flow_config_file.exists():
                ...
                logger.warning(
                    f"⚠️ Estuary Flow configuration not found: {flow_config_file}"
                )
    ...
    async def create_docker_network(self):
        ...
        try:
            self.docker_client.networks.get("sophia-network")
    ...
    async def create_environment_file(self):
        ...
        env_vars = {
            ...
            "DATABASE_PASSWORD": os.getenv(
                "DATABASE_PASSWORD", "sophia_dev_password_123"
            ),
            ...
            "GRAFANA_ADMIN_PASSWORD": os.getenv(
                "GRAFANA_ADMIN_PASSWORD", "sophia_admin_123"
            ),
            # Snowflake
    ...
    async def wait_for_port(
        self, service_name: str, host: str, port: int, timeout: int = 120
    ):
    ...
    async def print_deployment_summary(self):
        ...
        logger.info("\n🔐 Default Credentials:")
        logger.info(
            f"📊 Grafana: admin / {os.getenv('GRAFANA_ADMIN_PASSWORD', 'sophia_admin_123')}"
        )
        logger.info(
            f"🐘 PostgreSQL: sophia_user / {os.getenv('DATABASE_PASSWORD', 'sophia_dev_password_123')}"
        )
