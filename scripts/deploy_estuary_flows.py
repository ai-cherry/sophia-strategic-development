...
class EstuaryFlowDeployer:
    ...
    def __init__(self):
        ...
        self.estuary_endpoint = get_config_value("estuary_endpoint", "https://api.estuary.dev")
    ...
    async def _verify_deployments(self):
        ...
        if result.returncode == 0:
            ...
            for mat in materializations:
                print(f"  - {mat['name']} → {mat['endpoint']['connector']['config']['database']}.{mat['endpoint']['connector']['config']['schema']}")
...
if __name__ == "__main__":
    ...
    if subprocess.run(["which", "flowctl"], capture_output=True).returncode != 0:
        print("❌ Error: flowctl CLI not found")
        print("Install with: curl -L https://github.com/estuary/flow/releases/latest/download/flowctl -o /usr/local/bin/flowctl")
