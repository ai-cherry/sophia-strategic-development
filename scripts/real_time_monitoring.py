...
class RealTimeMonitor:
    ...
    async def check_service(self, session: aiohttp.ClientSession, service_id: str, service_info: Dict) -> Dict[str, Any]:
    ...
    def display_dashboard(self, results: Dict[str, Any]):
        ...
        health_icon = "🟢" if summary["health_percentage"] == 100 else "🟡" if summary["health_percentage"] >= 80 else "🔴"
        
        print("📈 SYSTEM OVERVIEW")
        print("-" * 30)
        print(f"{health_icon} Health: {summary['health_percentage']}% ({summary['healthy_services']}/{summary['total_services']})")
        ...
        if len(self.metrics_history) > 1:
            ...
            recent_response = [m["avg_response_time"] for m in self.metrics_history[-10:]]
            
            avg_health = statistics.mean(recent_health)
            avg_response = statistics.mean(recent_response)
            
            health_trend = "📈" if recent_health[-1] > recent_health[0] else "📉" if recent_health[-1] < recent_health[0] else "➡️"
            response_trend = "📈" if recent_response[-1] > recent_response[0] else "📉" if recent_response[-1] < recent_response[0] else "➡️"
