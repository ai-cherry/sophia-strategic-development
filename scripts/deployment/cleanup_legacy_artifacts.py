...
class LegacyDeploymentCleaner:
    ...
    def find_artifacts(self):
        ...
        for category, config in self.artifacts_to_archive.items():
            patterns = config["pattern"] if isinstance(config["pattern"], list) else [config["pattern"]]
        ...
        for category, config in self.artifacts_to_archive.items():
            ...
            for file_path in config["files"]:
                ...
                try:
                    ...
                    logger.info(f"📦 Archived: {file_path.name} → {target_path.relative_to(self.root_dir)}")
                    total_moved += 1
