...
cat > frontend/src/utils/migrate-jsx-to-tsx.mjs << 'EOF'
import fs from 'fs';
import path from 'path';

export function convertJsxToTsx(jsxContent, componentName) {
...
fs.writeFileSync('temp-navigation.jsx', navigationComponent);
fs.writeFileSync('temp-documentcard.jsx', documentCardComponent);
fs.writeFileSync('temp-documenteditor.jsx', documentEditorComponent);
Object.entries(extractedPages).forEach(([name, content]) => {
    fs.writeFileSync(`temp-${name.toLowerCase()}.jsx`, content);
});
EOF
...
cat > frontend/merge-dependencies.mjs << 'EOF'
import fs from 'fs';
...
cd frontend && node merge-dependencies.mjs && cd ..

# Step 11: Clean up temporary files
print_status "Cleaning up temporary files..."
rm -f frontend/temp-*.jsx
rm -f frontend/extract-components.mjs
rm -f frontend/merge-dependencies.mjs
