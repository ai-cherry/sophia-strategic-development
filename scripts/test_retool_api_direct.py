...
class RetoolAPITester:
    ...
    async def test_api_connection(self) -> bool:
        ...
        if not self.api_token:
            ...
            print(f"Please set it with: export RETOOL_API_TOKEN='your-token-here'")
            return False
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        print(f"{RED}✗ Authentication failed - check your API token{RESET}")
                        return False
                    else:
                        error = await response.text()
                        print(f"{RED}✗ API returned status {response.status}: {error}{RESET}")
                        return False
