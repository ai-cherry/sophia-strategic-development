...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('dockcloud_validation.log'),
...
class DockcloudDeploymentValidator:
    ...
    async def _validate_docker_build(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "build", "--target", "production", "-t", "sophia-ai:validation", ".", "--quiet"],
            ...
            result = subprocess.run(
                ["docker", "build", "-f", "docker/Dockerfile.mcp-server", 
                 "--build-arg", "MCP_SERVER_PATH=backend/mcp_servers/ai_memory",
                 "--build-arg", "MCP_SERVER_MODULE=enhanced_ai_memory_mcp_server",
                 "--build-arg", "MCP_SERVER_PORT=8080",
                 "-t", "sophia-ai:mcp-validation", ".", "--quiet"],
                cwd=self.project_root,
                capture_output=True,
                text=True,
                timeout=300
    ...
    async def _validate_mcp_configuration(self) -> None:
        ...
        cursor_config_path = self.project_root / "config/cursor_enhanced_mcp_config.json"
        ...
        if mcp_servers_path.exists():
            server_dirs = [d for d in mcp_servers_path.iterdir() if d.is_dir() and not d.name.startswith('_')]
            self.results.append(ValidationResult(
    ...
    async def _validate_infrastructure_config(self) -> None:
        ...
        deploy_script_path = self.project_root / "scripts/deploy_to_lambda_labs_cloud.py"
    ...
    async def _validate_service_endpoints(self) -> None:
        ...
        try:
            with open(cloud_compose_path, 'r') as f:
                compose_config = yaml.safe_load(f)
            ...
            for service_name, expected_service_ports in expected_ports.items():
                ...
                for port_config in service_config['ports']:
                    if isinstance(port_config, dict):
                        service_ports.append(port_config.get('published', port_config.get('target')))
    ...
    def _save_report(self, report: ValidationReport) -> None:
        ...
        report_dict = {
            ...
            "dockcloud_readiness": {
                "docker_build_ready": any(r.check_name.startswith("Docker Build") and r.passed for r in report.results),
                "compose_config_ready": any(r.check_name.startswith("Compose File") and r.passed for r in report.results),
                "secret_management_ready": any(r.check_name.startswith("Pulumi ESC") and r.passed for r in report.results),
                "deployment_script_ready": any(r.check_name == "Lambda Labs Deployment Script" and r.passed for r in report.results)
            }
        }
