...
'''
    ...
    def _optimize_database_operations(self):
        ...
        for root, dirs, files in os.walk(self.base_path / "backend"):
            for file in files:
                if file.endswith(".py"):
                    ...
                    try:
                        ...
                        if any(keyword in content for keyword in [
                            "execute(", "executemany(", "INSERT INTO",
                            "UPDATE ", "DELETE FROM", "cursor."
                        ]):
    ...
    def _optimize_db_file(self, file_path: Path):
        ...
        try:
            ...
            if loops_with_db:
                logger.info(f"Found {len(loops_with_db)} loops with DB operations in {file_path.name}")
                self.optimizations_applied.append({
...
'''
    ...
    def _generate_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write("    ) for record in records\n")
            f.write("]\n")
            f.write("await db_manager.execute_batch(ConnectionType.SNOWFLAKE, operations)\n")
            f.write("```\n")
