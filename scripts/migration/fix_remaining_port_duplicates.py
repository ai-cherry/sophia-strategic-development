...
def fix_remaining_duplicates():
    ...
    if "mcp_servers" in data:
        ...
        fixes = {
            ...
            ("core_intelligence", "ui_ux_agent"): 9042,  # Was conflicting with slack at 9008
            ("infrastructure", "lambda_labs_cli"): 9043,  # Was conflicting with snowflake at 9200
            ("infrastructure", "snowflake_cortex"): 9044,  # Was conflicting at 9201
            ("infrastructure", "snowflake_admin"): 9045,  # Was conflicting with estuary at 9202
        }
        
        for (category, server), new_port in fixes.items():
            if category in data["mcp_servers"] and server in data["mcp_servers"][category]:
