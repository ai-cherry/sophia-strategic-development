...
class ComprehensiveIntegrationTest:
    ...
    async def test_arize_integration(self):
        ...
        try:
            ...
            for template in evaluation_templates:
                self.test_results["arize_integration"][f"{template}_template"] = (
                    "configured"
                )
    ...
    async def run_comprehensive_test(self):
        ...
        if not failed_tests:
            ...
            self.test_results["overall_status"] = (
                f"partial_success_with_failures: {failed_tests}"
            )
        ...
        # Save results
        results_file = (
            f"logs/integration_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def print_test_summary(self):
        ...
        print(
            f"⏱️ Test Duration: {self.test_results.get('test_duration_seconds', 0):.2f} seconds"
        )
        ...
        available_secrets = sum(
            1
            for status in self.test_results.get("secret_validation", {}).values()
            if "Available" in str(status) or "Placeholder" in str(status)
        )
        print(
            f"🔐 Secret Access: {available_secrets}/{secret_count} secrets accessible"
        )
