...
class DependencyAuditor:
    ...
    def _find_python_files(self) -> List[Path]:
        ...
        python_files = []
        exclude_dirs = {'.venv', 'venv', '__pycache__', '.git', 'node_modules', 'docs_backup'}
    ...
    def _categorize_imports(self) -> None:
        ...
        for import_name in self.all_imports:
            is_internal = any(re.match(pattern, import_name) for pattern in internal_patterns)
            ...
            if is_internal:
                ...
                if len(parts) > 0:
                    potential_path = self.project_root / Path(*parts[:-1]) / f"{parts[-1]}.py"
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "missing_imports": {
                "count": len(self.missing_imports),
                "details": {}
            },
        ...
        sorted_imports = sorted(import_counts.items(), key=lambda x: x[1], reverse=True)[:20]
