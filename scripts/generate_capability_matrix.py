...
def generate_capability_matrix() -> Dict[str, Any]:
    ...
    if availability.get("OPENAI_API_KEY"):
        matrix["ai_agent_guidance"]["primary_recommendations"]["text_generation"] = "OPENAI_API_KEY"
    elif availability.get("ANTHROPIC_API_KEY"):
        matrix["ai_agent_guidance"]["primary_recommendations"]["text_generation"] = "ANTHROPIC_API_KEY"
    
    if availability.get("PINECONE_API_KEY"):
        matrix["ai_agent_guidance"]["primary_recommendations"]["vector_search"] = "PINECONE_API_KEY"
    elif availability.get("WEAVIATE_API_KEY"):
        matrix["ai_agent_guidance"]["primary_recommendations"]["vector_search"] = "WEAVIATE_API_KEY"
    
    if availability.get("SLACK_BOT_TOKEN"):
        matrix["ai_agent_guidance"]["primary_recommendations"]["team_communication"] = "SLACK_BOT_TOKEN"
    
    if availability.get("GONG_ACCESS_KEY"):
        matrix["ai_agent_guidance"]["primary_recommendations"]["business_intelligence"] = "GONG_ACCESS_KEY"
...
"""
    ...
    for capability, services in matrix["capabilities"].items():
        if services:
            content += f"- **{capability.replace('_', ' ').title()}**: {', '.join(services)}\n"
...
def main():
    ...
    print(f"  - docs/CURRENT_CAPABILITIES.json (machine-readable)")
    print(f"  - docs/CAPABILITY_SUMMARY.md (human-readable)")
