...
class CodebaseCleanup:
    ...
    def remove_vendored_dependencies(self):
        ...
        new_ignores = [ignore for ignore in gitignore_additions if ignore not in existing_ignores]
...
"""
            ...
            if import_end > 0:
                new_content = content[:import_end] + feature_flags + content[import_end:]
                
                if not self.dry_run:
                    main_path.write_text(new_content)
                    self.changes_made.append("Updated backend/main.py with feature flags")
    ...
    def fix_duplicate_integrations(self):
        ...
        duplicates = [
            ("backend/integrations/gong_integration.py", "backend/integrations/gong/enhanced_gong_integration.py"),
            ("backend/integrations/gong_integration.py", "backend/analytics/gong_analytics.py"),
            ("backend/vector/vector_integration.py", "backend/vector/vector_integration_updated.py")
        ]
    ...
    def standardize_secret_management(self):
        ...
        legacy_files = [
            ...
            "backend/config/secure_config.py"
        ]
                ...
                for file_path in source_dir.rglob("*.py"):
                    if file_path.name != "__init__.py":
                        ...
                        if not self.dry_run:
                            ...
                            self.changes_made.append(f"Moved {file_path} to {target_path}")
