...
class PropertyAssetsIngestor:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.config = PropertyIngestionConfig(
                source_system=await get_config_value("property_management_source_system", "CSV_IMPORT"),
                connection_details={
                    "api_endpoint": await get_config_value("property_management_api_endpoint", ""),
                    "api_key": await get_config_value("property_management_api_key", ""),
                    "csv_path": await get_config_value("property_management_csv_path", "")
                }
            )
    ...
    async def extract_properties_data(self) -> pd.DataFrame:
        ...
        try:
            if self.config.source_system == "CSV_IMPORT":
                ...
                logger.info("🔄 API integration not yet implemented - using sample data")
    ...
    async def load_properties(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                """, (
                    row['property_id'], row['property_name'], row['address'],
                    row['property_type'], row['total_units'], row['occupied_units'],
                    row['occupancy_rate'], row['monthly_rent_potential'], row['actual_monthly_rent'],
                    row['property_manager_id'], row['acquisition_date'], row['property_value'],
                    row['property_status']
                ))
    ...
    async def load_property_units(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                """, (
                    row['unit_id'], row['property_id'], row['unit_number'],
                    row['unit_type'], row['square_footage'], row['bedrooms'],
                    row['bathrooms'], row['monthly_rent'], row['occupancy_status'],
                    row['lease_start_date'], row['lease_end_date'], row['tenant_id']
                ))
    ...
    async def load_property_contacts(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                """, (
                    row['contact_id'], row['property_id'], row['first_name'],
                    row['last_name'], row['email'], row['phone'],
                    row['company_name'], row['role'], row['is_primary'],
    ...
    async def run_full_property_sync(self) -> Dict[str, int]:
        ...
        try:
            ...
            results['property_contacts'] = await self.load_property_contacts(contacts_df)
