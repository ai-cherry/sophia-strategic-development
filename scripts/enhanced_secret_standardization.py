...
class EnhancedSecretStandardizer:
            ...
            for py_file in backend_dir.rglob("*.py"):
                try:
                    with open(py_file, 'r') as f:
                        content = f.read()
                        secrets.update(re.findall(r'os\.getenv\(["\']([A-Z_]+)["\']', content))
                        secrets.update(re.findall(r'config\.get\(["\']([a-z_]+)["\']', content))
            ...
            for env in Environment:
                ...
                if service_info:
                    ...
                    mapping = SecretMapping(
                        ...
                        required=env == Environment.PRODUCTION  # Only PROD is required initially
                    )
    ...
    def _extract_service_info(self, secret_name: str) -> Optional[Tuple[str, ServiceCategory, str]]:
        ...
        service_patterns = {
            ...
            # Infrastructure  
            ("VERCEL", "NAMECHEAP", "LAMBDA"): (ServiceCategory.INFRASTRUCTURE, "TOKEN"),
            
            # Data
            ("SNOWFLAKE", "PINECONE", "WEAVIATE"): (ServiceCategory.DATA, "PASSWORD"),
            
            # Integration
            ("LINEAR", "ASANA", "GONG", "APOLLO", "HUBSPOT"): (ServiceCategory.INTEGRATION, "API_KEY"),
    ...
    def generate_esc_environments(self) -> List[ESCEnvironmentConfig]:
        ...
        for env in Environment:
            ...
            for mapping in env_mappings:
                ...
                values_structure["sophia"][category_key][service_key][cred_key] = f"${{{mapping.new_name}}}"
    ...
    def create_migration_scripts(self) -> Dict[str, str]:
        ...
        critical_mappings = [
            m for m in self.secret_mappings 
            if m.environment == Environment.PRODUCTION and m.service in critical_services
        ]
        ...
        prod_mappings = [m for m in self.secret_mappings if m.environment == Environment.PRODUCTION]
        ...
        non_prod_mappings = [m for m in self.secret_mappings if m.environment != Environment.PRODUCTION]
    ...
    def _generate_migration_script(self, mappings: List[SecretMapping], description: str) -> str:
