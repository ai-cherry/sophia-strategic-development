...
class SophiaAICompleteDeployment:
    ...
    def __init__(self, dry_run: bool = False):
        ...
        self.instances = {
            "platform": LambdaLabsInstance(
                ...
                services=["backend-api", "frontend", "postgres", "redis", "grafana", "prometheus"]
            ),
            "mcp": LambdaLabsInstance(
                ...
                region="us-west-1",
                purpose="MCP Servers",
                services=["codacy-mcp", "ai-memory-mcp", "github-mcp", "linear-mcp", "asana-mcp", "hubspot-mcp"]
            ),
            "ai": LambdaLabsInstance(
                ...
                services=["snowflake-cortex", "ai-processing", "ml-workloads", "vector-search"]
            )
        }
    ...
    def print_banner(self):
        ...
        print(f"🌐 Target: All 3 Lambda Labs instances")
        print("="*80)
    ...
    async def deploy_platform_services(self) -> List[DeploymentResult]:
        ...
        try:
            result = subprocess.run([
                "python", "scripts/deploy_to_lambda_labs_cloud.py",
                "--environment", "prod",
                "--target", "platform"
        ...
        for service in mcp_services:
            ...
            try:
                if service == "codacy-mcp":
                    ...
                    result = subprocess.run([
                        "python", "scripts/deploy_mcp_service.py",
                        "--service", service,
                        "--target", self.instances['mcp'].ip
    ...
    def generate_deployment_report(self):
        ...
        for status, count in status_counts.items():
            emoji = {"success": "✅", "failed": "❌", "error": "🚨", "in_progress": "🔄"}.get(status, "❓")
            print(f"{emoji} {status.title()}: {count}")
        
        print("\n📋 Detailed Results:")
        for result in self.deployment_results:
            emoji = {"success": "✅", "failed": "❌", "error": "🚨", "in_progress": "🔄"}.get(result.status, "❓")
            print(f"   {emoji} {result.instance}/{result.service}: {result.status} ({result.duration:.1f}s)")
            if result.error:
