            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.backend_url}/") as response:
                    if response.status == 200:
                        print(f"{GREEN}✓ Backend is running at {self.backend_url}{RESET}")
                        return True
                    else:
                        print(f"{RED}✗ Backend returned status {response.status}{RESET}")
                        return False
                    ...
                    if endpoint['method'] == 'GET':
                        async with session.get(url, headers=headers) as response:
                            if response.status == 200:
                                ...
                                print(f"{RED}✗ {endpoint['name']}: {response.status} - {error}{RESET}")
            ...
            for query in test_queries:
                try:
                    ...
                    async with session.post(url, headers=headers, json=query) as response:
                        if response.status == 200:
                            data = await response.json()
                            print(f"{GREEN}✓ Chat ({query['mode']} mode): Response received{RESET}")
                            return True
                        else:
                            error = await response.text()
                            print(f"{YELLOW}⚠ Chat ({query['mode']} mode): {error}{RESET}")
                except Exception as e:
    ...
    async def test_system_endpoints(self) -> Dict[str, bool]:
        ...
        endpoints = [
            ...
            },
            {
                ...
                "path": "/api/system/infrastructure"
            },
            {
                "name": "API Catalog",
                "path": "/api/system/api-catalog"
            }
        ]
                ...
                try:
                    ...
                    async with session.get(url, headers=headers) as response:
                        if response.status == 200:
                            ...
                            print(f"{YELLOW}⚠ {endpoint['name']}: {response.status}{RESET}")
                            results[endpoint['name']] = False
                except Exception as e:
...
"""
        ...
        if test_results.get('backend_healthy'):
            ...
            guide += f"  cd backend && python main.py\n"
        ...
        guide += f"a) Log into your Retool account\n"
        guide += f"b) Create a new app called 'Sophia CEO Dashboard'\n"
        guide += f"c) Add a REST API resource:\n"
        guide += f"   - Name: SophiaAPI\n"
        guide += f"   - Base URL: {self.backend_url}\n"
        guide += f"   - Headers: X-Admin-Key = {self.admin_key}\n"
        guide += f"\n"
        guide += f"d) Import the configuration from: retool_ceo_dashboard_config.json\n"
        
        guide += f"\n{BLUE}4. Quick Test Commands:{RESET}\n"
        guide += f"# Test dashboard summary\n"
        guide += f'curl -H "X-Admin-Key: {self.admin_key}" {self.backend_url}/api/retool/executive/dashboard-summary\n\n'
        guide += f"# Test strategic chat\n"
     ...
     {self.backend_url}/api/retool/executive/strategic-chat\n'''
        ...
        if OPENROUTER_API_KEY:
            guide += f"OPENROUTER_API_KEY=****** (configured)\n"
        else:
    ...
    async def deploy(self):
        ...
        # Summary
        total_tests = len(executive_results) + len(system_results) + (1 if chat_working else 0)
        passed_tests = sum(executive_results.values()) + sum(system_results.values()) + (1 if chat_working else 0)
        ...
        if passed_tests == total_tests:
            ...
            print(f"\nNext steps:")
            print(f"1. Open Retool and create new app")
            print(f"2. Import configuration from retool_ceo_dashboard_config.json")
            print(f"3. Start using your executive dashboard!")
        else:
