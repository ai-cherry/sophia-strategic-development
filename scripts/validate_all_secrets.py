...
from comprehensive_secret_mapping import COMPLETE_SECRET_MAPPING, APP_KEY_MAPPINGS
        ...
        try:
            ...
            if 'docker_username' in config:
                if str(config['docker_username']).startswith("PLACEHOLDER"):
                    print(f"   ⚠️  docker_username is still a placeholder!")
            if 'docker_token' in config:
                if str(config['docker_token']).startswith("PLACEHOLDER"):
                    print(f"   ⚠️  docker_token is still a placeholder!")
                    
        except json.JSONDecodeError:
            print("   ❌ Could not parse ESC config")
    
    print(f"\n3️⃣ Application Mapping (auto_esc_config.py):")
    print(f"   All variations map to docker_token and docker_username ✅")
