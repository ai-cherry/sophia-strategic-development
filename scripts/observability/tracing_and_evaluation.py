...
class MockGongTool:
    def get_transcript(self, call_id: str):
        with tracer.start_as_current_span("Tool: get_gong_transcript", attributes={"tool.call_id": call_id}):
...
def evaluate_response_with_llm_judge(query: str, response: str, prediction_id: str):
    ...
    with tracer.start_as_current_span("LLM-as-a-Judge Evaluation") as judge_span:
        ...
        judge_response = portkey_client.call_llm(evaluation_prompt, model="anthropic/claude-3-sonnet")
        ...
        logging.info(f"Evaluation complete. Score: {judge_score}, Rationale: '{judge_rationale}'")
        ...
        logging.info(f"[Arize LOG] prediction_id: {prediction_id}, judge_score: {judge_score}")
...
def process_sales_inquiry_workflow(query: str, call_id: str):
    ...
    with tracer.start_as_current_span("Workflow: Sales Inquiry", attributes={"user.query": query}) as root_span:
        ...
        with tracer.start_as_current_span("Agent: AnalystAgent - Fetch Transcript") as agent1_span:
        ...
        with tracer.start_as_current_span("Agent: AnalystAgent - Summarize") as agent1_summarize_span:
            agent1_summarize_span.set_attribute("agent.name", "AnalystAgent")
            summary_prompt = f"Summarize the key points of the following transcript: {transcript}"
        ...
        with tracer.start_as_current_span("Agent: SalesCoachAgent - Provide Feedback") as agent2_span:
            agent2_span.set_attribute("agent.name", "SalesCoachAgent")
            feedback_prompt = f"Based on this summary, provide three coaching tips: {summary}"
            final_response = portkey_client.call_llm(feedback_prompt, model="anthropic/claude-3-opus")
...
if __name__ == "__main__":
    ...
    logging.info("Demonstration finished. Check the console output for the trace structure.")