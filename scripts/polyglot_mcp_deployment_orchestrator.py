...
class MCPServerManager:
    ...
    async def start(self) -> bool:
        ...
        try:
            logger.info(f"🚀 Starting {self.config.language.value} MCP server: {self.config.name}")
            ...
            if self.process.poll() is None:
                ...
                logger.info(f"✅ {self.config.name} started successfully on port {self.config.port}")
                return True
            else:
    ...
    async def restart(self) -> bool:
        ...
        if self.restart_count >= self.config.max_restarts:
            logger.error(f"❌ {self.config.name} exceeded max restarts ({self.config.max_restarts})")
            return False
    ...
    def _build_command(self) -> List[str]:
        ...
        if self.config.language == MCPLanguage.PYTHON:
            return [
                "python", "-m", self.config.python_module,
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                (self.metrics["health_checks_passed"] + self.metrics["health_checks_failed"]) * 100
                if (self.metrics["health_checks_passed"] + self.metrics["health_checks_failed"]) > 0 else 0
            )
        }
...
class PolyglotMCPOrchestrator:
    ...
    async def start_all(self) -> Dict[str, bool]:
        ...
        for priority in priority_order:
            priority_servers = [
                (name, manager) for name, manager in self.servers.items()
            ...
            if priority_servers:
                ...
                for i, (name, _) in enumerate(priority_servers):
                    results[name] = not isinstance(priority_results[i], Exception) and priority_results[i]
        ...
        total = len(results)
        logger.info(f"✅ Polyglot MCP deployment completed: {successful}/{total} servers started")
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        running_servers = sum(1 for m in self.servers.values() if m.status == MCPServerStatus.RUNNING)
        
        # Performance improvements calculation
        go_servers = sum(1 for m in self.servers.values() if m.config.language == MCPLanguage.GO)
        ts_servers = sum(1 for m in self.servers.values() if m.config.language == MCPLanguage.TYPESCRIPT)
        ...
        return {
            "overall_status": "healthy" if running_servers == total_servers else "degraded",
    ...
    async def _start_server_with_timeout(self, name: str, manager: MCPServerManager) -> bool:
                ...
                for name, manager in self.servers.items():
                    if manager.status == MCPServerStatus.RUNNING:
                        ...
                        if not healthy and self.config.auto_restart:
                            logger.warning(f"⚠️ {name} health check failed, attempting restart...")
...
async def main():
    ...
    try:
        ...
        print(f"Performance Boost: {status['performance_improvements']['estimated_boost']}")
        print(f"Community Validation: {status['deployment_advantages']['community_validation']}")
