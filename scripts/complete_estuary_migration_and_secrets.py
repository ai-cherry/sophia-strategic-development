    ...
    def replace_in_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            for old, new in self.replacements.items():
                ...
                content = re.sub(pattern, new, content, flags=re.IGNORECASE if old.islower() else 0)
        ...
        try:
            ...
            if '"ESTUARY_CLIENT_ID"' not in content:
                ...
                if mapping_start != -1:
                    ...
                    ai_services_end = content.find('# Business Intelligence', mapping_start)
...
'''
                        content = content[:ai_services_end] + estuary_mappings + content[ai_services_end:]
    ...
    def create_comprehensive_secret_mapping(self):
        ...
        logger.info(f"✅ Created comprehensive secret mapping with {len(comprehensive_mapping)} secrets")
    ...
    def generate_pulumi_esc_commands(self, mapping: Dict[str, str]):
        ...
        for github_secret, esc_path in mapping.items():
...
"""
        ...
        logger.info("✅ Generated Pulumi ESC commands script: set_all_pulumi_secrets.sh")
    ...
    def run_migration(self):
        ...
        for config_pattern in ['*.yml', '*.yaml', '*.json', '*.md']:
            for config_file in Path('.').rglob(config_pattern):
                if '.git' not in str(config_file) and 'node_modules' not in str(config_file):
