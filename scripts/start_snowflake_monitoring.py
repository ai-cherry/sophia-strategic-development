#!/usr/bin/env python3
"""
Start the ModernStack monitoring service with Prometheus metrics export.
This service monitors ModernStack usage, costs, and performance.
"""
...
from infrastructure.monitoring.modern_stack_monitoring import (
    modern_stack_monitor,
...
async def run_initial_check():
    ...
    logger.info("🔍 Running initial ModernStack monitoring check...")

    try:
        result = await modern_stack_monitor.run_monitoring_cycle()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Start ModernStack monitoring service")
    parser.add_argument(
    ...
    if args.check_only:
        ...
        logger.info(f"🚀 Starting ModernStack monitoring service on port {args.port}")
        logger.info(
            f"📊 Prometheus metrics will be available at http://localhost:{args.port}/metrics"
        )
        logger.info(
            "📈 Monitoring dashboard data will be saved to reports/modern_stack_monitoring_latest.json"
        )
