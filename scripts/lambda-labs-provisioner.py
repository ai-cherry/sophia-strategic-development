...
class LambdaLabsProvisioner:
    ...
    def _create_ssh_client(
        self, ip_address: str, private_key_path: str
    ) -> paramiko.SSHClient:
    ...
    async def execute_ssh_command(
        self, ssh: paramiko.SSHClient, command: str
    ) -> tuple[str, str, int]:
    ...
    async def setup_base_system(self, ssh: paramiko.SSHClient):
        ...
        for command in commands:
            ...
            if (
                exit_code != 0 and "ufw" not in command
            ):  # UFW commands may have non-zero exit codes
    ...
    async def install_postgresql(self, ssh: paramiko.SSHClient, config: DatabaseConfig):
        ...
        config_commands = [
            ...
            'sudo -u postgres psql -c "ALTER USER sophia CREATEDB;"',
        ]
...
"""
        ...
        await self.execute_ssh_command(
            ssh,
            f"echo '{pg_config}' | sudo tee -a /etc/postgresql/15/main/postgresql.conf",
        )
...
"""
        ...
        await self.execute_ssh_command(
            ssh, f"echo '{redis_config}' | sudo tee /etc/redis/redis.conf"
        )
...
"""
        ...
        await self.execute_ssh_command(
            ssh, f"echo '{monitoring_script}' | sudo tee /opt/health_monitor.py"
        )
...
"""
        ...
        await self.execute_ssh_command(
            ssh,
            f"echo '{service_config}' | sudo tee /etc/systemd/system/sophia-health.service",
        )
    ...
    async def deploy_complete_infrastructure(
        ...
        try:
            ...
            key_exists = any(
                key["name"] == self.config.ssh_key_name for key in ssh_keys
            )
            ...
            await self.launch_instance(instance_spec)
