...
class MCPDiagnostic:
    ...
    async def _check_docker_infrastructure(self):
        ...
        try:
            if self.docker_client:
                ...
                self.results["infrastructure"]["docker_daemon"] = {"status": "unavailable"}
    ...
    async def _check_container_status(self):
        ...
        try:
            ...
            mcp_containers = [c for c in containers if 'sophia' in c.name or 'mcp' in c.name]
    ...
    async def _check_network_connectivity(self):
        ...
        if not aiohttp:
            self.results["network"]["error"] = "aiohttp not available for network checks"
            return
        ...
        try:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(f"{gateway_url}/health", timeout=5) as response:
    ...
    def _validate_server_config(self, server_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for var_name, var_value in env_vars.items():
            if isinstance(var_value, str) and var_value.startswith("${") and var_value.endswith("}"):
                ...
                if not os.getenv(actual_var):
                    validation["issues"].append(f"Missing environment variable: {actual_var}")
        ...
        if config.get("command") == "python":
            ...
            if len(args) >= 2 and args[0] == "-m":
                ...
                if not (self.project_root / module_path).exists():
                    validation["issues"].append(f"Server module not found: {module_path}")
    ...
    async def _check_mcp_gateway_health(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.get(f"{gateway_url}/servers", timeout=5) as response:
    ...
    def print_summary(self):
        ...
        print(f"  Docker Daemon: {'✅' if infra.get('docker_daemon', {}).get('status') == 'healthy' else '❌'}")
        print(f"  Docker Compose: {'✅' if infra.get('docker_compose', {}).get('status') == 'available' else '❌'}")
        ...
        for service, info in network.items():
            if isinstance(info, dict):
                status_icon = "✅" if info.get("status") in ["healthy", "reachable"] else "❌"
