...
async def test_ai_memory_server():
    ...
    try:
        ...
        if store_result.get("success"):
            print(f"✅ Conversation stored successfully: {store_result.get('memory_id')}")
        ...
        recall_result = await server._recall_memory({
            "query": "MCP server infrastructure testing",
            "top_k": 3
        })
        ...
        if recall_result.get("success"):
            ...
            for i, memory in enumerate(memories[:2], 1):
                print(f"  {i}. Score: {memory.get('score', 0):.3f} - {memory.get('text', '')[:100]}...")
        else:
        ...
        if memory_id:
            ...
            delete_result = await server._delete_memory({
                "memory_id": memory_id
            })
...
async def test_cursor_ai_integration():
    ...
    try:
        ...
        for scenario in recall_scenarios:
            ...
            result = await server._recall_memory({
                "query": scenario,
                "top_k": 2
            })
            ...
            if result.get("success"):
                ...
                for memory in memories:
                    print(f"      • Score: {memory.get('score', 0):.3f} - {memory.get('context', 'No context')}")
            else:
