    ...
    async def test_server_health(self, server_name: str, port: int) -> dict:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                return {"status": "unreachable", "port": port, "error": "No valid endpoints found"}
...
'''
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        return {
                            ...
                            "security_issues": len([i for i in result.get("issues", []) if i.get("category") == "security"]),
                            "complexity_score": result.get("metrics", {}).get("complexity_score", 0),
                            "sample_issues": result.get("issues", [])[:3]  # First 3 issues
                        }
                    else:
                        ...
                        async with session.post(
                            ...
                            if recall_response.status == 200:
                                ...
                                return {
                                    ...
                                    "memories_recalled": len(recall_result.get("memories", [])),
                                    "sample_memory": recall_result.get("memories", [{}])[0] if recall_result.get("memories") else None
                                }
                    
                    return {"success": False, "error": f"Store failed with HTTP {response.status}"}
    ...
    async def run_comprehensive_test(self):
        ...
        if memory_result.get("success"):
            print(f"✅ Stored & recalled {memory_result.get('memories_recalled', 0)} memories")
        else:
            print("❌ Failed")
    ...
    async def generate_report(self):
        ...
        healthy_servers = sum(1 for k, v in self.test_results.items() 
                             if k.endswith("_health") and v.get("status") == "healthy")
        working_functions = sum(1 for k, v in self.test_results.items() 
                               if not k.endswith("_health") and v.get("success"))
        
        print(f"📈 Overall Status:")
        print(f"   Servers Running: {healthy_servers}/{total_servers} ({healthy_servers/total_servers*100:.0f}%)")
        print(f"   Functions Working: {working_functions}/{len(self.servers)} ({working_functions/len(self.servers)*100:.0f}%)")
        print()
        ...
        for server_name, config in self.servers.items():
            ...
            if func_result.get("success"):
                ...
                if server_name == "codacy":
                    print(f"         Issues detected: {func_result.get('issues_found', 0)}")
                    print(f"         Security issues: {func_result.get('security_issues', 0)}")
                elif server_name == "ai_memory":
                    print(f"         Memories recalled: {func_result.get('memories_recalled', 0)}")
                elif server_name == "ui_ux":
                    print(f"         Accessibility score: {func_result.get('accessibility_score', 0)}/100")
                elif server_name == "huggingface":
                    print(f"         Sentiment confidence: {func_result.get('confidence', 0):.2f}")
        ...
        if healthy_servers == total_servers:
            ...
            for server_name, config in self.servers.items():
                health = self.test_results.get(f"{server_name}_health", {})
                if health.get("status") != "healthy":
                    print(f"      cd mcp-servers/{server_name} && python {server_name}_mcp_server.py")
