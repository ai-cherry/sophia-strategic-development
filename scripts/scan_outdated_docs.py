...
class DocumentationScanner:
    ...
    def _categorize_file(self, result: dict) -> str:
        ...
        has_structure_conflict = (
            len(result["structure_refs"]["old"]) > 0
            and len(result["structure_refs"]["new"]) > 0
        )
    ...
    def generate_report(self) -> dict:
        ...
        report["high_priority_files"] = [
            {"file": r["file"], "category": r["category"], "issues": r["issues"]}
            for r in sorted(
                high_priority_issues, key=lambda x: len(x["issues"]), reverse=True
            )[:10]
        ]
    ...
    def _generate_markdown_report(self, report: dict, output_file: Path):
        ...
        with open(output_file, "w") as f:
            ...
            f.write(
                f"- **Files Needing Updates**: {report['categories'].get('partially_outdated', 0) + report['categories'].get('fully_outdated', 0)}\n"
            )
            f.write(
                f"- **High Priority Files**: {report['priorities'].get('high', 0)}\n"
            )
