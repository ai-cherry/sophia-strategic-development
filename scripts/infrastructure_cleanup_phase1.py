...
class InfrastructureCleanup:
    def __init__(self):
        ...
        self.report = {"timestamp": self.timestamp, "actions": [], "errors": []}
            ...
            if Path(server_path).exists():
                try:
                    ...
                    self.report["errors"].append(
                        f"Failed to archive {server_path}: {e}"
                    )

        self.report["actions"].append(
            {"task": "archive_mcp_servers", "count": archived}
        )
    ...
    def cleanup_duplicate_docs(self):
        ...
        self.report["actions"].append(
            {"task": "cleanup_duplicate_docs", "count": removed}
        )
    ...
    def consolidate_esc_configs(self):
        ...
        self.report["actions"].append(
            {"task": "consolidate_esc", "created": str(config_file)}
        )
    ...
    def cleanup_workflows(self):
        ...
        self.report["actions"].append({"task": "archive_workflows", "count": archived})
    ...
    def standardize_python_infrastructure(self):
        ...
        if (
            ts_dns_path.exists()
            and (ts_dns_path / "sophia-dns-infrastructure.ts").exists()
        ):
...
'''
            ...
            wrapper_file.write_text(
                wrapper_content.replace(
                    f"infrastructure_{self.timestamp}",
                    f"infrastructure_{self.timestamp}",
                )
            )
        ...
        self.report["actions"].append(
            {"task": "standardize_python", "status": "completed"}
        )
    ...
    def update_integration_registry(self):
        ...
        self.report["actions"].append(
            {"task": "update_registry", "path": str(registry_path)}
        )
    ...
    def generate_report(self):
        ...
        # Summary
        total_actions = sum(action.get("count", 1) for action in self.report["actions"])
        print("\n📈 Summary:")
