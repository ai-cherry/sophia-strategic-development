...
class MCPServerValidator:
    ...
    async def _test_capability(self, capability: str) -> Dict[str, Any]:
        ...
        if not endpoint:
            return {
                "status": "unknown",
                "message": f"No test endpoint for {capability}",
            }
...
async def main():
    ...
    parser.add_argument(
        "--config", default="cursor_mcp_config.json", help="MCP configuration file"
    )
    ...
    parser.add_argument(
        "--performance-test", action="store_true", help="Run performance test"
    )
    parser.add_argument(
        "--duration", type=int, default=60, help="Performance test duration"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.health_check or args.integration_test or args.performance_test:
            ...
            results = await validator.comprehensive_validation(
                include_performance=args.performance_test
            )
