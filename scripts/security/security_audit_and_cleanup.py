...
class SecurityAuditor:
    ...
    SECRET_PATTERNS = {
        ...
        "anthropic_api_key": [r"sk-ant-api[0-9]{2}-[a-zA-Z0-9-_]{95}"],
        "gong_api_key": [
            r"[A-Z0-9]{26}",  # Gong access keys are typically 26 characters
        ],
        "pinecone_api_key": [r"pcsk_[a-zA-Z0-9-_]{32,}"],
        "snowflake_password": [
    ...
    def _get_files_to_scan(self) -> List[Path]:
        ...
        for root, dirs, filenames in os.walk(self.project_root):
            ...
            for filename in filenames:
                ...
                # Skip excluded files
                if any(
                    re.search(pattern, str(relative_path))
                    for pattern in self.EXCLUDE_PATTERNS
                ):
                    continue
    ...
    def _check_configuration_files(self) -> None:
        ...
        for config_file in config_files:
            for config_path in self.project_root.rglob(config_file):
                if not any(
                    exclude in str(config_path)
                    for exclude in [".git", "node_modules", "__pycache__"]
                ):
                    self.issues.append(
                        SecurityIssue(
    ...
    def fix_issues_automatically(self) -> int:
        ...
        for issue in self.issues:
            if issue.issue_type == "exposed_secret" and issue.suggested_fix:
                ...
                logger.warning(
                    f"MANUAL FIX REQUIRED: {issue.file_path}:{issue.line_number}"
                )
