...
class VulnerabilityComparator:
    ...
    def __init__(self, current_file: Path, baseline_file: Path):
        ...
        self.baseline_vulns = self._load_vulnerabilities(baseline_file) if baseline_file.exists() else {}
    ...
    def compare(self) -> Dict[str, Any]:
        ...
        def categorize_by_severity(vulns: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
            by_severity = {
                'critical': [],
                'high': [],
                'medium': [],
        ...
        return {
            ...
            'high_priority_new': [v for v in new_vulns if v.get('severity', '').lower() in ['critical', 'high']],
            'high_priority_new_count': len([v for v in new_vulns if v.get('severity', '').lower() in ['critical', 'high']])
        }
    
    def generate_summary(self, comparison: Dict[str, Any]) -> str:
        """Generate a human-readable summary of the comparison."""
        summary = []
        summary.append(f"Vulnerability Scan Comparison - {comparison['scan_timestamp']}")
        summary.append("=" * 60)
        summary.append(f"Total vulnerabilities in current scan: {comparison['total_current_vulnerabilities']}")
        summary.append(f"Total vulnerabilities in baseline: {comparison['total_baseline_vulnerabilities']}")
        summary.append("")
        
        # New vulnerabilities
        if comparison['new_vulnerabilities_count'] > 0:
            summary.append(f"🚨 NEW VULNERABILITIES: {comparison['new_vulnerabilities_count']}")
        ...
        if comparison['resolved_vulnerabilities_count'] > 0:
            summary.append(f"✅ RESOLVED VULNERABILITIES: {comparison['resolved_vulnerabilities_count']}")
            for severity in ['critical', 'high', 'medium', 'low', 'unknown']:
        ...
        if comparison['persisting_vulnerabilities_count'] > 0:
            summary.append(f"⚠️  PERSISTING VULNERABILITIES: {comparison['persisting_vulnerabilities_count']}")
            for severity in ['critical', 'high', 'medium', 'low', 'unknown']:
        ...
        if comparison['high_priority_new_count'] > 0:
            ...
            for vuln in comparison['high_priority_new']:
                summary.append(f"  - {vuln.get('name')} {vuln.get('version')}: {vuln.get('id')}")
                if vuln.get('fix_versions'):
                    summary.append(f"    Fix available: {', '.join(vuln.get('fix_versions', []))}")
        
        return "\n".join(summary)


def main():
    parser = argparse.ArgumentParser(description='Compare vulnerability scan results with baseline')
    parser.add_argument('--current', required=True, help='Path to current vulnerability scan JSON')
    parser.add_argument('--baseline', required=True, help='Path to baseline vulnerability scan JSON')
    parser.add_argument('--output', required=True, help='Path to output comparison JSON')
