...
class PulumiESCManager:
    ...
    def __init__(self, org: str = "scoobyjava-org", environment: str = "default/sophia-ai-production"):
        self.org = org
    ...
    def check_pulumi_cli(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error("❌ Pulumi CLI not authenticated. Run 'pulumi login' first.")
                return False
    ...
    def check_esc_cli(self) -> bool:
        ...
        try:
            ...
            logger.error("❌ ESC CLI not found. Install from https://get.pulumi.com/esc/install.sh")
            return False
    ...
    def create_environment(self) -> bool:
        ...
        try:
            ...
            template_path = Path(__file__).parent.parent.parent / "pulumi" / "esc" / "sophia-ai-production.yaml"
    ...
    def export_environment_template(self) -> bool:
        ...
        try:
            ...
            output_path = Path(__file__).parent.parent.parent / "pulumi" / "esc" / "generated-template.yaml"
    ...
    def cleanup_placeholder_secrets(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ['esc', 'env', 'get', self.full_env, '--show-secrets', '--format', 'json'],
                capture_output=True,
                text=True
            )
            ...
            if placeholders_found:
                logger.warning(f"⚠️  Found {len(placeholders_found)} placeholder values:")
                for key in placeholders_found:
                    logger.warning(f"   - {key}")
                
                logger.info("These should be set to real values using the interactive setup.")
            else:
