...
class LambdaLabsValidator:
    ...
    def validate_instances(self) -> bool:
        ...
        try:
            ...
            instances_active = sum(1 for inst in active_instances.values() if inst['status'] == 'active')
            ips_correct = sum(1 for inst in active_instances.values() if inst['ip_matches'])
            ...
            # Success criteria
            success = (instances_found == 5 and 
                      instances_active >= 4 and 
                      ips_correct >= 4 and
                      critical_instances_active == 2)
    ...
    def validate_ssh_connectivity(self) -> bool:
        ...
        print(f"   📊 SSH Success Rate: {successful_connections}/5 ({success_rate*100:.1f}%)")
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if not self.results['api_access']:
            recommendations.append("🔑 Fix Lambda Labs API access - check API key in secrets")
            
        # Check for failed instances
        failed_instances = [
            name for name, data in self.results['instances'].items() 
            if data.get('status') != 'active'
        ]
        
        if failed_instances:
            recommendations.append(f"🖥️  Restart failed instances: {', '.join(failed_instances)}")
            
        # Check SSH failures
        ssh_failures = [
            name for name, success in self.results['ssh_connectivity'].items() 
            if not success
        ]
        
        if ssh_failures:
            recommendations.append(f"🔐 Fix SSH connectivity to: {', '.join(ssh_failures)}")
            
        if not self.results['secret_management']:
            recommendations.append("☁️  Fix secret management pipeline - check Pulumi ESC access")
            
        # Cost optimization
        if self.results['total_cost'] > 5.0:
            recommendations.append("💰 Consider stopping non-critical instances to reduce costs")
    ...
    def print_summary(self):
        ...
        print(f"\n{'='*70}")
        print(f"🎯 LAMBDA LABS INFRASTRUCTURE VALIDATION SUMMARY")
        print(f"{'='*70}")
        
        # Determine overall status
        api_ok = self.results['api_access']
        instances_ok = len([i for i in self.results['instances'].values() if i.get('status') == 'active']) >= 4
        ...
        print(f"💰 Total Cost: ${self.results['total_cost']:.2f}/hour (${self.results['total_cost']*24:.2f}/day)")
...
def main():
    ...
    try:
        ...
        if is_healthy:
            ...
            print(f"✅ Lambda Labs infrastructure is operational")
            return 0
        else:
