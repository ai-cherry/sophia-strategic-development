...
class SophiaRealtimeDashboard:
    ...
    def __init__(self):
        self.services = {
            "api_gateway": {"url": "http://localhost:8000", "name": "API Gateway", "icon": "🌐"},
            "ai_memory": {"url": "http://localhost:9001", "name": "AI Memory", "icon": "🧠"},
    ...
    def get_average_response_time(self, service_key: str) -> float:
        ...
        response_times = [m["response_time"] for m in history if m["status"] == "healthy"]
    ...
    def render_dashboard(self, metrics: Dict[str, Any]):
        ...
        print("=" * 80)
        print(f"⏰ Uptime: {uptime_str} | 🔄 Updated: {datetime.now().strftime('%H:%M:%S')}")
        print()
        
        # Overall status
        healthy_services = len([m for m in metrics.values() if m["status"] == "healthy"])
        total_services = len(metrics)
        overall_health = (healthy_services / total_services) * 100
        
        health_icon = "🟢" if overall_health == 100 else "🟡" if overall_health >= 80 else "🔴"
        print(f"🏥 OVERALL HEALTH: {health_icon} {overall_health:.1f}% ({healthy_services}/{total_services} services)")
        
        # Average response time
        avg_response_times = [m["response_time"] for m in metrics.values() if m["status"] == "healthy"]
        if avg_response_times:
            overall_avg = sum(avg_response_times) / len(avg_response_times)
            grade, grade_icon = self.get_response_time_grade(overall_avg)
            print(f"⚡ AVERAGE RESPONSE: {grade_icon} {overall_avg:.1f}ms (Grade: {grade})")
        ...
        for service_key, service in self.services.items():
            ...
            if response_times:
                ...
                print(f"{service['icon']} {service['name']:<15} [{chart:<10}] "
        ...
        healthy_measurements = sum(len([m for m in history if m["status"] == "healthy"]) 
                                 for history in self.metrics_history.values())
        
        if total_measurements > 0:
            system_reliability = (healthy_measurements / total_measurements) * 100
            print(f"📊 System Reliability: {system_reliability:.2f}% ({healthy_measurements}/{total_measurements} checks)")
    ...
    async def run_dashboard(self, refresh_interval: int = 2):
        ...
        try:
            ...
            if total_measurements > 0:
                ...
                print(f"   Monitoring Duration: {int((time.time() - self.start_time) // 60)} minutes")
