...
class TypeAnnotationAdder(ast.NodeTransformer):
    ...
    def _infer_param_type(self, param_name: str, node: ast.FunctionDef) -> Optional[str]:
        """Infer parameter type based on name and usage."""
...
def add_imports_if_needed(content: str) -> str:
    ...
    if 'Dict[' in content or 'List[' in content or 'Optional[' in content or 'Any' in content or 'Tuple[' in content:
        needs_typing = True
    ...
    if imports_to_add:
        ...
        if insert_index < len(lines) - 1 and lines[insert_index + len(imports_to_add)].strip():
...
def process_file(file_path: Path) -> bool:
    ...
    try:
        ...
        if transformer.changes_made > 0:
            ...
            logger.info(f"✓ Added {transformer.changes_made} type annotations to {file_path}")
            return True
        else:
...
def main() -> None:
    ...
    if not report_path.exists():
        logger.error("type_safety_audit_report.json not found. Run type_safety_audit.py first.")
        return
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Type annotation addition complete!")
