...
class VenvCleanup:
    ...
    def find_venv_directories(self):
        ...
        for pattern in patterns:
            for path in self.project_root.rglob(pattern):
                if path.is_dir() and "site-packages" in [
    ...
    def _is_likely_custom_code(self, file_path, relative_path):
        ...
        if any(
            x in relative_path
            for x in ["site-packages", "lib/python", "bin/", "Scripts/"]
        ):
            # But check for sophia/backend/agents etc in the path
            if any(
                x in relative_path.lower()
                for x in ["sophia", "backend", "agents", "mcp"]
            ):
                return True
            return False
    ...
    def _determine_correct_location(self, file_path):
        ...
        if "agent" in file_name.lower() or "from backend.agents" in file_content:
            ...
            if "specialized" in file_content:
                return (
                    self.project_root / "backend" / "agents" / "specialized" / file_name
                )
...
"""
    ...
    def create_summary_report(self):
        ...
        report = {
            ...
            "moved_files": [
                {"from": str(src), "to": str(dst)} for src, dst in self.moved_files
            ],
        }
    ...
    def run_cleanup(self):
        ...
        print(
            "2. Delete the venv directories: rm -rf sophia_admin_api/venv sophia_venv"
        )
        print("3. Run: git add .")
        print(
            "4. Run: git commit -m 'Fix: Remove venv from Git and recover misplaced files'"
        )
