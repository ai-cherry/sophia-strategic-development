...
class V2DeploymentValidator:
    ...
    async def test_tools(self, server: str, port: int, expected_tools: List[str]) -> Dict:
        ...
        with Progress(
            ...
            for server, port, tools in V2_SERVERS:
                ...
                self.results[server] = {
                    ...
                    "overall": self._calculate_overall_status(health, tools_result, performance, integration)
                }
    ...
    def _calculate_overall_status(self, health: Dict, tools: Dict, perf: Dict, integ: Dict) -> str:
    ...
    def print_summary(self):
        ...
        for server, data in self.results.items():
            ...
            tools_status = "✅" if data["tools"]["status"] == "pass" else "⚠️" if data["tools"]["status"] == "partial" else "❌"
            perf_status = "✅" if data["performance"]["status"] == "pass" else "⚠️" if data["performance"]["status"] == "degraded" else "❌"
            integ_status = "✅" if data["integration"]["status"] == "pass" else "⏭️" if data["integration"]["status"] == "skip" else "❌"
        ...
        for server, data in self.results.items():
            if data["performance"]["status"] in ["pass", "degraded"]:
                ...
                console.print(f"  {server}: avg={perf['avg_ms']}ms, p95={perf['p95_ms']}ms")
        
        # Overall deployment status
        unhealthy_count = sum(1 for d in self.results.values() if d["overall"] == "unhealthy")
        degraded_count = sum(1 for d in self.results.values() if d["overall"] == "degraded")
    ...
    def save_report(self, output_path: str):
        ...
        report = {
            ...
            "summary": {
                "healthy": sum(1 for d in self.results.values() if d["overall"] == "healthy"),
                "warning": sum(1 for d in self.results.values() if d["overall"] == "warning"),
                "degraded": sum(1 for d in self.results.values() if d["overall"] == "degraded"),
                "unhealthy": sum(1 for d in self.results.values() if d["overall"] == "unhealthy")
            }
        }
...
@click.option('--output', default='reports/v2_deployment_validation.json', help='Output report path')
