...
class TestNaturalLanguageCommands(unittest.TestCase):
    ...
    def test_context_awareness(self):
        ...
        for command in commands:
            with self.subTest(command=command):
                result = self._process_natural_language_command(
    ...
    def _process_natural_language_command(
        ...
        if " and " in command or " then " in command:
            ...
            result["steps"] = [
                {"action": self._classify_single_command(step), "command": step}
                for step in steps
            ]
...
class TestGitHubIntegrationFeatures(unittest.TestCase):
    ...
    def test_branch_protection_simulation(self):
        ...
        protection_rules = {
            ...
            "develop": {"required_reviews": 1, "required_status_checks": ["ci"]},
        }
...
async def run_async_tests():
    ...
    try:
        ...
        logger.info(
            f"✅ Async tests completed: {len(async_results)} commands processed"
        )
        logger.info(
            f"✅ Concurrent tests completed: {len(concurrent_results)} task groups"
        )
