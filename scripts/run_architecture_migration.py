...
class ArchitectureMigration:
    ...
    async def check_architecture_consistency(
        self, output_file: str = "architecture_consistency_report.md"
    ) -> Dict[str, Any]:
    ...
    async def run_migration(self) -> Dict[str, Any]:
        ...
        self.results["check_before"] = await self.check_architecture_consistency(
            "architecture_consistency_before.md"
        )
        ...
        self.results[
            "vector_store_migration"
        ] = await self.migrate_vector_store_access()
        ...
        self.results["check_after"] = await self.check_architecture_consistency(
            "architecture_consistency_after.md"
        )
        ...
        self.results["success"] = (
            self.results["check_before"]["success"]
            and self.results["vector_store_migration"]["success"]
        )

        logger.info(
            f"Architecture migration {'completed successfully' if self.results['success'] else 'failed'}."
        )
        return self.results
    ...
    def generate_report(self) -> str:
        ...
        if "report" in self.results["check_before"]:
            ...
            report += self.extract_summary_from_report(
                self.results["check_before"]["report"]
            )
            report += "\n"
        ...
        if "report" in self.results["check_after"]:
            ...
            report += self.extract_summary_from_report(
                self.results["check_after"]["report"]
            )
            report += "\n"
        ...
        if (
            "report" in self.results["check_before"]
            and "report" in self.results["check_after"]
        ):
            before_inconsistencies = self.extract_inconsistencies_from_report(
                self.results["check_before"]["report"]
            )
            after_inconsistencies = self.extract_inconsistencies_from_report(
                self.results["check_after"]["report"]
            )
        ...
        if self.results["success"]:
            ...
            report += (
                "1. Review the error messages to identify the cause of the failure.\n"
            )
    ...
    def extract_summary_from_report(self, report: str) -> str:
        ...
        for i, line in enumerate(lines):
            if line.startswith("Scanned") or line.startswith("Found"):
                ...
                line.startswith("### ") and i + 1 < len(lines) and lines[i + 1].strip()
            ):
                summary.append(line)
