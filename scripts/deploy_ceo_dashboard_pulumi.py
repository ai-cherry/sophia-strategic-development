...
from backend.core.config_manager import get_secret, get_config
...
class CEODashboardPulumiDeployer:
    ...
    async def check_infrastructure_health(self) -> Dict[str, Any]:
        ...
        # Check MCP Servers
        mcp_servers = [
            "gong", "slack", "snowflake", "pinecone", "linear", 
            "claude", "retool", "ai_memory", "docker", "pulumi"
        ]
    ...
    async def deploy_retool_dashboard_direct(self) -> Dict[str, Any]:
        ...
        app_result = await self.retool_integration.create_app(
            "sophia_ceo_dashboard",
            "Sophia AI CEO Command Center"
        )
    ...
    async def _create_dashboard_components(self, app_id: str) -> List[Dict]:
        ...
        # KPI Statistics
        kpis = [
            {"name": "revenue_stat", "label": "Revenue", "query": "getDashboardSummary.data.revenue"},
            {"name": "client_health_stat", "label": "Client Health", "query": "getDashboardSummary.data.clientHealth"},
            {"name": "agent_performance_stat", "label": "AI Performance", "query": "getDashboardSummary.data.agentPerformance"},
            {"name": "system_health_stat", "label": "System Health", "query": "getDashboardSummary.data.systemHealth"}
        ]
...
"""
    ...
    async def deploy(self):
        ...
        try:
            ...
            # Get backend URL
            backend_config = await get_config("backend")
            deployment_results["backend_url"] = backend_config.get("url", "http://localhost:8000")
            ...
            # Summary
            healthy_servers = len([s for s in health_status.values() if s.get("status") == "healthy"])
            ...
            print(f"Dashboard: {'✅ Deployed' if deployment_results.get('app_id') else '❌ Failed'}")
