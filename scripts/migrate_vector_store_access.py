...
class VectorStoreMigrator:
    ...
    async def scan_files(self):
        ...
        logger.info(f"Scanning files in {self.root_dir} for direct vector store access...")
        ...
        for file_path in python_files:
            try:
                ...
                has_vector_store_import = any(re.search(pattern, content) for pattern in self.vector_store_imports)
                
                # Check for direct access patterns
                has_direct_access = any(re.search(pattern, content) for pattern in self.direct_access_patterns)
                ...
                self.migration_results["errors"].append(f"Error scanning file {file_path}: {e}")
        
        logger.info(f"Found {len(self.files_to_migrate)} files with direct vector store access.")
    ...
    async def migrate_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.migration_results["errors"].append(f"Error migrating file {file_path}: {e}")
            return False
    ...
    async def migrate_files(self):
        ...
        logger.info(f"Migration complete. {self.migration_results['migrated_files']} files migrated, {self.migration_results['skipped_files']} files skipped.")
...
async def main():
    ...
    print("\nMigration complete!")
    print("All direct vector store access has been migrated to use the ComprehensiveMemoryManager.")
