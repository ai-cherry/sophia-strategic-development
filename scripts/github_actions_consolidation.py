...
class GitHubActionsConsolidator:
    ...
    def _analyze_single_workflow(self, workflow_file: Path) -> WorkflowAnalysis:
        ...
        complexity_score = (
            lines_of_code + len(jobs) * 50 + len(secrets_used) * 10 + len(triggers) * 25
        )

        # Determine consolidation group
        consolidation_group = self._determine_consolidation_group(
            name, triggers, jobs, content
        )
    ...
    def _determine_consolidation_group(
        ...
        if any(
            ...
            for keyword in ["deploy", "deployment", "infrastructure", "pulumi"]
        ):
            return "deployment"
        ...
        if any(
            keyword in name_lower
            for keyword in ["test", "quality", "lint", "validation"]
        ):
            return "testing"

        # Monitoring and health check workflows
        if any(
            keyword in name_lower for keyword in ["monitor", "health", "alert", "check"]
        ):
            return "monitoring"

        # Secret and configuration management
        if any(
            keyword in name_lower for keyword in ["secret", "sync", "config", "esc"]
        ):
        ...
        if any(
            keyword in name_lower
            for keyword in ["doc", "analysis", "report", "enhancement"]
        ):
        ...
        if any(
            keyword in name_lower
            for keyword in ["gong", "estuary", "etl", "pipeline", "data"]
        ):
    ...
    def generate_consolidation_report(self) -> dict:
        ...
        consolidatable_workflows = sum(
            len(workflows) for workflows in groups.values() if len(workflows) > 1
        )
        estimated_templates = len(
            [group for group, workflows in groups.items() if len(workflows) > 1]
        )
