...
class VSAMigrationManager:
    ...
    def __init__(self, config_path: str = "config/agno_vsa_configuration.yaml"):
        ...
        self.backup_dir = (
            self.project_root
            / "backups"
            / f"pre_vsa_migration_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    async def analyze_current_structure(self) -> Dict[str, List[str]]:
        ...
        logger.info("📊 Current structure analysis:")
    ...
    async def create_migration_plans(self) -> List[MigrationPlan]:
        ...
        migration_plans = []
        feature_slices = self.config.get("vertical_slice_architecture", {}).get(
            "feature_slices", {}
        )

        for feature_name, feature_config in feature_slices.items():
            plan = await self._create_feature_migration_plan(
    ...
    async def _create_feature_migration_plan(
        self, feature_name: str, feature_config: Dict
    ) -> MigrationPlan:
        ...
        target_structure = {
            "agents": [
                f"features/{feature_name}/agents/{agent}.py"
                for agent in feature_config.get("agents", [])
            ],
            "integrations": [
                f"features/{feature_name}/integrations/{integration}.py"
                for integration in feature_config.get("integrations", [])
            ],
            "tools": [
                f"features/{feature_name}/tools/{tool}.py"
                for tool in feature_config.get("tools", [])
            ],
            "workflows": [
                f"features/{feature_name}/workflows/{workflow}.py"
                for workflow in feature_config.get("workflows", [])
            ],
            "knowledge": [
                f"features/{feature_name}/knowledge/{kb}/"
    ...
    async def create_backup(self) -> bool:
        ...
        try:
            ...
            manifest = {
                ...
                "total_files": sum(
                    len(list(Path(self.backup_dir / d).rglob("*")))
                    for d in critical_dirs
                    if (self.backup_dir / d).exists()
                ),
            }
    ...
    async def create_feature_structure(self, migration_plan: MigrationPlan) -> bool:
        ...
        try:
            ...
            for directory in directories:
                ...
                if not init_file.exists():
                    with open(init_file, "w") as f:
                        f.write(
                            f'"""Feature: {migration_plan.feature_name} - {directory}"""\n'
                        )
            ...
            logger.error(
                f"❌ Failed to create structure for {migration_plan.feature_name}: {e}"
            )
            return False
    ...
    async def migrate_files(self, migration_plan: MigrationPlan) -> bool:
                ...
                if target_path:
                    ...
                    if success:
                        ...
                        self.failed_operations.append(
                            f"Failed to migrate {source_file}"
                        )
        ...
        logger.info(
            f"📊 Migration complete: {success_count}/{total_files} files migrated"
        )
        return success_count == total_files

    async def _determine_target_path(
        self, source_file: str, feature_name: str
    ) -> Optional[str]:
    ...
    async def _update_imports(self, content: str, target_file: str) -> str:
        ...
        for line in lines:
            if line.strip().startswith("from backend.") or line.strip().startswith(
                "import backend."
            ):
        ...
        try:
            ...
            for feature_dir in features_dir.iterdir():
                if feature_dir.is_dir():
                    ...
                    migrated_files += len(
                        [f for f in python_files if f.stat().st_size > 100]
                    )  # Non-empty files
        ...
        try:
            ...
            for plan in migration_plans:
                ...
                if not structure_success:
                    logger.error(
                        f"❌ Failed to create structure for {plan.feature_name}"
                    )
                    return False
            ...
            if overall_success:
                ...
                logger.info(
                    f"  - Files processed: {sum(len(plan.source_files) for plan in migration_plans)}"
                )
