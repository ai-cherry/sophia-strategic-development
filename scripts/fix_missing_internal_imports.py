...
class ImportFixer:
    ...
    def _get_missing_internal_imports(self) -> Dict[str, List[str]]:
        ...
        if "missing_imports" in self.audit_report:
            for import_name, details in self.audit_report["missing_imports"]["details"].items():
    ...
    def _find_module_file(self, parts: List[str]) -> List[str]:
        ...
        for i in range(len(parts)):
            ...
            # Check as .py file
            py_file = self.project_root / Path(*path_parts[:-1]) / f"{path_parts[-1]}.py"
            if py_file.exists():
                module_path = '.'.join(path_parts)
                possible_modules.append(module_path)
    ...
    def _fix_import_in_file(self, file_path: str, old_import: str, new_import: str) -> bool:
        ...
        try:
            ...
            patterns = [
                # from x import y
                (f"from {re.escape(old_import)} import (\\w+)", f"from {new_import} import \\1"),
                # from x import y as z
                (f"from {re.escape(old_import)} import (\\w+) as (\\w+)", f"from {new_import} import \\1 as \\2"),
                # import x
                (f"import {re.escape(old_import)}\\b", f"import {new_import}"),
                # import x as y
                (f"import {re.escape(old_import)} as (\\w+)", f"import {new_import} as \\1"),
            ]
            ...
            if len(old_parts) > 1:
                ...
                child = old_parts[-1]
                patterns.extend([
                    (f"from {re.escape(parent_module)} import {re.escape(child)}", f"from {new_import} import {child}"),
                    (f"from {re.escape(parent_module)} import .*{re.escape(child)}", f"from {new_import} import {child}"),
                ])
...
def main():
    ...
    summary = {
        ...
        "success_rate": fixer.fixed_count / max(fixer.fixed_count + fixer.error_count, 1) * 100
    }
