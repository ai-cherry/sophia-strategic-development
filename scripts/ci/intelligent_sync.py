...
def get_all_org_secrets() -> list[str]:
    ...
    if not GITHUB_ORG or not GITHUB_TOKEN:
        logger.error("Missing GITHUB_REPOSITORY_OWNER or GITHUB_TOKEN environment variables.")
        return []
    ...
    try:
        ...
        logger.info(f"Discovered {len(secret_names)} secrets in the '{GITHUB_ORG}' organization.")
        return secret_names
...
def sync_secret_to_pulumi_esc(secret_name: str):
    """Syncs a single secret from the environment to Pulumi ESC."""
    secret_value = os.getenv(secret_name)
    if not secret_value:
        logger.warning(f"Secret '{secret_name}' is in GitHub but not in the environment. Skipping.")
        return False
        
    logger.info(f"Syncing secret '{secret_name}' to Pulumi ESC...")
    try:
        command = ["pulumi", "env", "set", PULUMI_ESC_ENV, secret_name, "--secret", "--non-interactive"]
        subprocess.run(
            command,
            input=secret_value,
            text=True,
