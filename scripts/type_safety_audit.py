...
def analyze_file(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        functions_with_complete_types = sum(
            1 for f in analyzer.functions 
            if f['has_return_type'] and all(p['has_type'] or p['name'] == 'self' 
        ...
        return {
            ...
            'missing_annotations': [
                f for f in analyzer.functions 
                if f['missing_annotations']
            ]
        }
...
def scan_codebase(root_dir: Path, exclude_dirs: Optional[List[str]] = None) -> Dict[str, Any]:
...
def main():
    ...
    logger.info(f"Functions with complete types: {analysis['functions_with_complete_types']}")
    ...
    if priority_files:
        ...
        for pf in sorted(priority_files, key=lambda x: x['coverage']):
            logger.info(f"{pf['file']}: {pf['coverage']:.1f}% coverage, "
