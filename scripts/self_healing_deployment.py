    ...
    async def _fix_snowflake_indentation(self):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if "cursor = self.connection.cursor()" in line and line.startswith(
                    "cursor"
                ):
...
'''
                ...
                try:
                    ...
                    if await self._is_port_available(port):
                        ...
                        if await self._check_service_health("localhost", port):
                            logger.info(
                                f"   ✅ {server_name} MCP server deployed on port {port}"
                            )
                            self.deployment_status["services_deployed"] += 1
                        else:
                            logger.warning(
                                f"   ⚠️ {server_name} MCP server health check failed"
                            )
                    else:
                        logger.warning(
                            f"   ⚠️ Port {port} not available for {server_name}"
                        )
    ...
    async def _validate_deployment(self):
        ...
        if services_checked > 0:
            self.deployment_status["health_score"] = (
                services_healthy / services_checked
            ) * 100

        logger.info(
            f"   📊 Deployment health score: {self.deployment_status['health_score']:.1f}%"
        )
