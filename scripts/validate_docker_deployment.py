    ...
    def get_service_status(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not service_containers:
                return {"status": "not_found", "error": f"No containers found for service {service_name}"}
...
class HealthEndpointValidator:
    ...
    async def check_endpoint(self, url: str, expected_status: int = 200) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "timeout", "error": f"Request timed out after {self.timeout}s"}
        except requests.exceptions.ConnectionError:
            return {"status": "connection_error", "error": "Could not connect to endpoint"}
        except Exception as e:
            return {"status": "error", "error": str(e)}
...
class DockerDeploymentValidator:
    ...
    async def _validate_docker_services(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_status": "healthy" if healthy_services == len(self.expected_services) else "degraded"
        }
    ...
    async def _validate_health_endpoints(self) -> Dict[str, Any]:
        ...
        for endpoint in self.health_endpoints:
            ...
            endpoint_results[endpoint["name"]] = await self.health_validator.check_endpoint(
                endpoint["url"]
            )
            
        # Count healthy endpoints
        healthy_endpoints = sum(1 for result in endpoint_results.values() 
        ...
        return {
            ...
            "overall_status": "healthy" if healthy_endpoints == len(self.health_endpoints) else "degraded"
        }
    ...
    def _validate_resource_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_cpu_percent": round(total_cpu / container_count, 2) if container_count > 0 else 0,
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        service_status = results.get("docker_services", {}).get("overall_status", "unknown")
        network_status = results.get("network_connectivity", {}).get("status", "unknown")
        endpoint_status = results.get("health_endpoints", {}).get("overall_status", "unknown")
        resource_status = results.get("resource_usage", {}).get("status", "unknown")
        
        healthy_components = sum(1 for status in [service_status, network_status, endpoint_status, resource_status]
                                if status == "healthy")
    ...
    def print_summary(self, report: Dict[str, Any]) -> None:
        ...
        print(f"Components: {summary['healthy_components']}/{summary['total_components']} healthy")
        ...
        for name, status in components:
            emoji = {"healthy": "✅", "degraded": "⚠️", "unhealthy": "❌", "warning": "⚠️", "error": "❌"}.get(status, "❓")
            print(f"  {emoji} {name}: {status}")
        
        # Detailed service information
        services = report["detailed_results"].get("docker_services", {}).get("services", {})
        if services:
            print("\n🔧 Service Details:")
            for service_name, service_info in services.items():
                status = service_info.get("status", "unknown")
                emoji = {"running": "✅", "exited": "❌", "not_found": "❓"}.get(status, "❓")
                print(f"  {emoji} {service_name}: {status}")
        
        # Health endpoint details
        endpoints = report["detailed_results"].get("health_endpoints", {}).get("endpoints", {})
        if endpoints:
            print("\n🌐 Health Endpoints:")
            for endpoint_name, endpoint_info in endpoints.items():
                status = endpoint_info.get("status", "unknown")
                emoji = {"healthy": "✅", "unhealthy": "❌", "timeout": "⏰", "connection_error": "🔌"}.get(status, "❓")
...
async def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Sophia AI Docker Deployment Validation")
