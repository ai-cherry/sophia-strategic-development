...
try:
    ...
    logging.warning("⚠️  Warning: Could not import Lambda Labs config")
...
class SophiaIntelAIDeployer:
    ...
    def __init__(self):
        ...
        self.namecheap_api_key = os.getenv("NAMECHEAP_API_KEY")

        if not self.vercel_token or not self.namecheap_api_key:
            logging.error(
                "❌ Error: VERCEL_API_TOKEN and NAMECHEAP_API_KEY environment variables are required."
            )
            sys.exit(1)
