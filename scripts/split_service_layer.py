...
class ServiceAnalyzer(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        for alias in node.names:
            ...
            if any(lib in alias.name for lib in ['requests', 'httpx', 'boto3', 'snowflake', 'redis', 'psycopg2', 'aiohttp', 'gong_api_client']):
                self.has_external_imports = True
                
    def visit_ImportFrom(self, node):
        if node.module:
            self.imports.append(node.module)
            if any(lib in node.module for lib in ['requests', 'httpx', 'boto3', 'snowflake', 'redis', 'psycopg2', 'aiohttp', 'gong_api_client']):
                self.has_external_imports = True
