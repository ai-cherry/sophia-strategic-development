        ...
        for phase in MigrationPhase:
            ...
            for service in services:
                for dep in service.get("dependencies", []):
                    if dep not in deployed and dep not in ["kafka", "postgres", "redis", "eventbridge", "api-gateway"]:
                        issues.append(f"{service['name']} depends on {dep} which is not yet deployed")
...
def main():
    ...
    for phase_name, phase_data in plan["phases"].items():
        ...
        print(f"Duration: {phase_data['start_date'][:10]} to {phase_data['end_date'][:10]}")
        print(f"Services to migrate: {len(phase_data['services'])}")
        
        for service in phase_data["services"]:
            print(f"  - {service['name']} ({service['type'].value}) [Priority: {service['priority']}]")
