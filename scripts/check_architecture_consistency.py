...
class ArchitectureConsistencyChecker:
            ...
            for inconsistency_type, patterns in self.patterns.items():
                ...
                for pattern in patterns:
                    if re.search(pattern, content):
                        ...
                        if file_path not in self.check_results["files"][inconsistency_type]:
                            self.check_results["files"][inconsistency_type].append(file_path)
                        break
    ...
    async def check_files(self):
        ...
        logger.info(f"Checking files in {self.root_dir} for architecture inconsistencies...")
        
        # Find all relevant files
        python_files = glob.glob(f"{self.root_dir}/**/*.py", recursive=True)
        shell_files = glob.glob(f"{self.root_dir}/**/*.sh", recursive=True)
        js_files = glob.glob(f"{self.root_dir}/**/*.js", recursive=True) + glob.glob(f"{self.root_dir}/**/*.jsx", recursive=True)
        ...
        logger.info(f"Found {self.check_results['inconsistent_files']} files with architecture inconsistencies.")
    ...
    async def generate_report(self) -> str:
        ...
        if self.check_results["inconsistencies"]["old_ui_references"] > 0:
            ...
            report += "Replace the old `sophia_admin_frontend` UI with Retool dashboards.\n\n"
            report += "Example:\n\n"
            report += "```python\n"
            report += "from backend.mcp.mcp_client import MCPClient\n\n"
