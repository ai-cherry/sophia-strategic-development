...
class UnifiedAPITester:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    self.test_results["failed"].append(
                        {"test": "Service health", "status_code": response.status_code}
                    )

        except Exception as e:
            logger.error(f"Health check failed: {e}")
            self.test_results["failed"].append(
                {"test": "Service health", "error": str(e)}
            )
            ...
            for route in test_routes:
                try:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        logger.error(
                            f"❌ {route['description']}: {response.status_code}"
                        )
                        self.test_results["failed"].append(
                            {
                                "route": route["path"],
    ...
    async def _test_performance(self):
        ...
        # Test response times
        endpoints_to_test = ["/health", "/", "/api/v3/chat/status"]
        ...
        async with httpx.AsyncClient() as client:
            for endpoint in endpoints_to_test:
                try:
                    ...
                    self.test_results["failed"].append(
                        {"test": "Performance", "endpoint": endpoint, "error": str(e)}
                    )
    ...
    def _generate_test_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- Success rate: {len(self.test_results['passed']) / max(total_tests, 1) * 100:.1f}%\n\n"
            )
