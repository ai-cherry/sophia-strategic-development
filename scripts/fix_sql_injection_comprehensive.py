...
class SQLInjectionFixer:
    ...
    def find_sql_files(self) -> List[Path]:
        ...
        sql_files = [
            ...
            if "dead_code" not in str(f) and ".venv" not in str(f)
        ]
    ...
    def fix_sql_injection(self, file_path: Path) -> bool:
        ...
        try:
            ...
            pattern3 = r'(execute|query)\s*\(\s*["\']([^"\']+)["\'][\s\n]*\+[\s\n]*([^,\)]+)'
            ...
            if content != original_content:
                ...
                if "# SQL injection fixes applied" not in content:
                    ...
                    lines.insert(insert_pos, "# SQL injection fixes applied - using parameterized queries")
                    content = '\n'.join(lines)
    ...
    def run(self):
        ...
        for file_path in sql_files:
            if self.fix_sql_injection(file_path):
                self.fixed_count += 1
                print(f"✅ Fixed SQL injection in {file_path.relative_to(self.project_root)}")
