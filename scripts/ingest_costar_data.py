...
async def process_single_file(server: CoStarMCPServer, file_path: Path) -> CoStarImportResult:
    ...
    try:
        ...
        if result.import_status == 'success':
            logger.info(f"✅ Successfully imported {result.records_imported} records from {file_path.name}")
        elif result.import_status == 'partial':
            logger.warning(f"⚠️  Partially imported {result.records_imported}/{result.records_processed} records from {file_path.name}")
...
async def process_directory(server: CoStarMCPServer, directory_path: Path, recursive: bool = False) -> List[CoStarImportResult]:
    ...
    try:
        ...
        for req_col in required_columns:
            ...
            for col in df.columns:
                if col.lower().replace(' ', '_') in [req_col, 'market', 'metro', 'date', 'period']:
                    column_found = True
                    break
        ...
        if len(df) == 0:
            ...
            validation_result['warnings'].append(f"Large file with {len(df)} records - processing may take time")
...
Examples:
  ...
  python scripts/ingest_costar_data.py --directory data/ --verbose
    ...
    input_group.add_argument(
        '--file', '-f',
        type=Path,
        help='Process a single CoStar data file'
    )
    input_group.add_argument(
        '--directory', '-d',
        type=Path,
        ...
        if args.file:
            ...
            if args.validate_only:
                ...
                for file_path in sorted(files):
                    ...
                    if validation['valid']:
                        logger.info(f"✅ {file_path.name} - {validation['record_count']} records")
                        valid_count += 1
                    else:
                ...
                results = await process_directory(server, args.directory, args.recursive)
