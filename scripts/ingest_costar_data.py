...
async def ingest_costar_file(file_path: Path, market_area: str, data_year: int):
    ...
    try:
        ...
        logger.info(f"Successfully ingested '{file_path.name}' into the knowledge base.")
        logger.info("You can now query this data using the 'knowledge' MCP server or agent.")

    except Exception as e:
        logger.error(f"An error occurred during the ingestion process: {e}", exc_info=True)
        logger.error("Please ensure your PINECONE_API_KEY and other environment variables are set correctly.")
