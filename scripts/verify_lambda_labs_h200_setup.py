...
class LambdaLabsH200Verifier:
    """Verifies Lambda Labs H200 GPU setup for Sophia AI"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if h200_types:
                    ...
                    print(f"{YELLOW}⚠️  H200 not available, checking alternatives:{RESET}")
                    gpu_types = [
                        inst for inst in instance_types
                    ...
                    if gpu_types:
                        ...
                        for inst in gpu_types[:5]:  # Show top 5
                            gpu_mem = inst['instance_type']['description'].get('gpu_memory_gb', 0)
                            price = inst['instance_type']['price_cents_per_hour'] / 100
                            print(f"   - {inst['name']}: {gpu_mem}GB @ ${price:.2f}/hour")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(h200_instances) >= 3:
                    print(f"{GREEN}✅ Found {len(h200_instances)} H200 instances:{RESET}")
                    ...
                    print(f"{RED}❌ Only {len(h200_instances)} instances found (need 3){RESET}")
                    self.results['instances']['count'] = len(h200_instances)
    ...
    def verify_ssh_access(self) -> bool:
            ...
            try:
                result = subprocess.run(
                    ['ssh', '-o', 'ConnectTimeout=5', '-o', 'StrictHostKeyChecking=no',
                     '-i', str(ssh_key_path), f'ubuntu@{ip}', 'echo "SSH OK"'],
                    capture_output=True,
                    text=True,
                    timeout=10
                )
        ...
        self.results['network']['ssh_access'] = success_count == len(self.instance_ips[:3])
        ...
        try:
            ...
            result = subprocess.run(
                ['ssh', '-o', 'StrictHostKeyChecking=no', '-i', str(ssh_key_path),
                 f'ubuntu@{master_ip}', 'nvidia-smi --query-gpu=name,memory.total,driver_version,cuda_version --format=csv,noheader'],
                capture_output=True,
                text=True,
                timeout=15
            )
            ...
            if result.returncode == 0:
                ...
                if 'H200' in gpu_info or 'H100' in gpu_info:
                    ...
                    print(f"   {YELLOW}⚠️  GPU is not H200 but may be compatible{RESET}")
    ...
    def generate_summary_report(self):
        ...
        print(f"Overall Status: {'READY FOR KUBERNETES' if overall_ready else 'NOT READY'}")
        print(f"\nChecklist:")
        print(f"  {self.check_mark(self.results.get('account', {}).get('api_valid', False))} API Credentials")
        print(f"  {self.check_mark(self.results.get('instances', {}).get('h200_available', False))} H200 Availability")
        print(f"  {self.check_mark(self.results.get('instances', {}).get('running', False))} 3+ Instances Running")
        print(f"  {self.check_mark(self.results['network'].get('ssh_access', False))} SSH Access")
        print(f"  {self.check_mark(self.results.get('gpu', {}).get('configured', False))} GPU Configuration")
        print(f"  {self.check_mark(self.results.get('gpu', {}).get('cuda_correct', False))} CUDA 12.3+")
        print(f"  {self.check_mark(self.results.get('secrets', {}).get('complete', False))} Environment Secrets")
        ...
        if overall_ready:
            ...
            print(f"You can now proceed with Kubernetes deployment.")
