...
def analyze_json_file(file_path: Path) -> Dict[str, Any]:
    ...
    if isinstance(data, list):
        ...
        if data:
            ...
            first_record = data[0]
            analysis['fields'] = list(first_record.keys()) if isinstance(first_record, dict) else []
        ...
        analysis['sample_data'] = {k: v for k, v in list(data.items())[:5]}
...
def find_correlations(analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for field, files in all_fields.items():
        if len(files) > 1:
            correlations['common_fields'].append({
                'field': field,
                'appears_in': files
            })
...
def generate_schema_suggestions(analyses: List[Dict[str, Any]], correlations: Dict[str, Any]) -> str:
    ...
    if any('employee' in a['file_name'].lower() for a in analyses):
        ...
        suggestions.append("    EMPLOYEE_ID VARCHAR(255) PRIMARY KEY DEFAULT UUID_STRING(),")
        
        # Add email if found
        for file, field in correlations['email_fields'].items():
            if 'employee' in file.lower():
                suggestions.append(f"    EMAIL VARCHAR(255) UNIQUE NOT NULL, -- from {field}")
                break
            ...
            if 'employee' in file.lower():
                for field in fields:
                    if 'first' in field.lower():
                        suggestions.append(f"    FIRST_NAME VARCHAR(255), -- from {field}")
                    elif 'last' in field.lower():
                        suggestions.append(f"    LAST_NAME VARCHAR(255), -- from {field}")
        
        # Add ID fields for integration
        for file, fields in correlations['id_fields'].items():
            if 'gong' in file.lower():
                suggestions.append(f"    GONG_USER_ID VARCHAR(255), -- from {fields[0]}")
            elif 'slack' in file.lower():
                suggestions.append(f"    SLACK_USER_ID VARCHAR(255), -- from {fields[0]}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python analyze_sample_data.py <directory_path>")
        print("Example: python analyze_sample_data.py ./sample_data/")
        sys.exit(1)
    ...
    print(f"Analyzing data files in: {data_dir}")
    print("=" * 60)
    ...
    for file_path in data_dir.iterdir():
        if file_path.suffix == '.csv':
            ...
            print(f"  Potential IDs: {analysis['potential_id_fields']}")
            ...
            if high_nulls:
                print(f"  High null fields: {', '.join(high_nulls)}")
        
        elif file_path.suffix == '.json':
            print(f"\nAnalyzing JSON: {file_path.name}")
    ...
    for common in correlations['common_fields']:
        print(f"  '{common['field']}' appears in: {', '.join(common['appears_in'])}")
    ...
    schema = generate_schema_suggestions(analyses, correlations)
    print(schema)
