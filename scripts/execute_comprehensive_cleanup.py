...
def create_cleanup_report(initial_size: float, final_size: float, 
                         removed_items: List[str], fixed_items: List[str],
                         obsolete_scripts: List[str]) -> None:
...
def main():
    ...
    if obsolete_scripts:
        print_status(f"Found {len(obsolete_scripts)} potentially obsolete scripts", "WARNING")
    ...
    # Create report
    create_cleanup_report(initial_size, final_size, all_removed, all_fixed, obsolete_scripts)
    ...
    print_status("4. Commit changes: git add . && git commit -m 'chore: comprehensive codebase cleanup'", "INFO")
...
if __name__ == "__main__":
    ...
    print_status("This will remove vendored dependencies and clean up the repository.", "WARNING")
