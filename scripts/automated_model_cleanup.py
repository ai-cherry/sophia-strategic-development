...
# Airbyte to Estuary replacements
AIRBYTE_REPLACEMENTS = {
    "RAW_AIRBYTE": "RAW_ESTUARY",
    "STG_TRANSFORMED": "STG_ESTUARY",
...
def update_yaml_file(file_path: Path, dry_run: bool = False) -> int:
    ...
    try:
        ...
        # Replace Airbyte references
        for old_term, new_term in AIRBYTE_REPLACEMENTS.items():
            if old_term in ["airbyte", "Airbyte", "AIRBYTE"]:
...
def update_sql_file(file_path: Path, dry_run: bool = False) -> int:
    ...
    try:
        ...
        for old_schema, new_schema in AIRBYTE_REPLACEMENTS.items():
            if old_schema in ["RAW_AIRBYTE", "STG_TRANSFORMED"]:
...
def main(dry_run: bool = True):
    ...
    if dry_run:
        ...
        print(f"Backups created in: cleanup_backup/")
