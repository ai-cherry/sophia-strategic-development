        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if (
                        "MCPServer" in node.name
                        and node.name != "StandardizedMCPServer"
                    ):
                        ...
                        if not has_proper_base:
                            logger.warning(
                                f"⚠️  {file_path}: {node.name} doesn't inherit from StandardizedMCPServer"
                            )
                            self.issues_found.append(
                                f"Missing StandardizedMCPServer inheritance: {file_path} - {node.name}"
                            )
