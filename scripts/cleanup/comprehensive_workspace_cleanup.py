...
class WorkspaceCleanup:
    ...
    def safe_remove(self, path: Path, item_type: str = "file") -> bool:
        ...
        try:
            if path.exists():
                ...
                logger.info(f"✅ Removed {item_type}: {path.name} ({self.format_size(size)})")
                return True
    ...
    def is_archive_directory(self, path: Path) -> bool:
        ...
        archive_indicators = [
            "backup", "old", "archive", "deprecated", "legacy",
            "migration", "upgrade", "temp"
        ]
        ...
        for pattern in script_patterns:
            for script in self.workspace_root.rglob(pattern):
                if script.is_file() and script.suffix == '.py':
                    ...
                    if any(part in str(script).lower() for part in ['test', 'backup', 'old', 'temp', 'copy']):
