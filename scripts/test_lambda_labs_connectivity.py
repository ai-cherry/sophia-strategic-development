...
class LambdaLabsConnectivityTester:
    ...
    async def test_http_endpoint(self, port: int, timeout: float = 5.0) -> Tuple[bool, Optional[str], float]:
    ...
    async def test_health_endpoint(self, port: int) -> Tuple[bool, Optional[str], float]:
                ...
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        try:
                            ...
                            return True, "Health endpoint responding (non-JSON)", response_time
                    else:
                        return False, f"Health check failed: {response.status}", response_time
    ...
    async def run_connectivity_tests(self):
        ...
        for service_name, port in self.test_ports.items():
            ...
            if success:
                ...
                health_success, health_info, health_time = await self.test_health_endpoint(port)
                if health_success:
                    print(f"    Health endpoint: ✅ {health_time:6.1f}ms - {health_info}")
                else:
                ...
                results.append({
                    ...
                    "info": info,
                    "health_status": "healthy" if health_success else "no_health_endpoint",
    ...
    def generate_report(self, results: List[dict]):
        ...
        healthy_services = [r for r in accessible_services if r.get("health_status") == "healthy"]
        ...
        if accessible_services:
            ...
            for service in accessible_services:
                ...
                print(f"   {health_icon} {service['service']:20} (:{service['port']}) - {service['response_time']:6.1f}ms")
        ...
        if inaccessible_services:
            ...
            for service in inaccessible_services:
                print(f"   ❌ {service['service']:20} (:{service['port']}) - {service['error']}")
