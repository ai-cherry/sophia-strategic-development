        ...
        if test_val == "default_value":
            ...
            results.append(
                (
                    "Config Access",
                    True,
                    f"✅ Configuration system working (got: {test_val})",
                )
            )
...
def test_enhanced_search_imports() -> list[tuple[str, bool, str]]:
    ...
    try:
        ...
        try:
            ...
            results.append(
                ("SearchTier Enum", False, f"❌ SearchTier import failed: {e}")
            )
        ...
        try:
            ...
            results.append(
                ("SearchProvider Enum", True, "✅ SearchProvider enum imported")
            )
        except ImportError as e:
            results.append(
                ("SearchProvider Enum", False, f"❌ SearchProvider import failed: {e}")
            )
        ...
        try:
            ...
            results.append(
                ("SearchRequest Model", True, "✅ SearchRequest model imported")
            )
        except ImportError as e:
            results.append(
                ("SearchRequest Model", False, f"❌ SearchRequest import failed: {e}")
            )
        ...
        try:
            ...
            results.append(
                (
                    ...
                    False,
                    f"⚠️ Service import failed (expected): {e}",
                )
            )
    ...
    try:
        ...
        results.append(
            (
                "MCP Compatibility",
                True,
                f"✅ MCP compatibility layer working (class: {server_class.__name__})",
            )
        )
    except ImportError as e:
        results.append(
...
def run_all_tests() -> bool:
    ...
    logger.info(
        f"🎯 Critical Dependencies: {critical_passed}/{len(critical_deps)} ({critical_rate:.1f}%)"
    )
