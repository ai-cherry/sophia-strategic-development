...
class SecretSyncFixer:
    ...
    def check_environment_variables(self) -> Dict[str, str]:
        ...
        for github_name in self.critical_secrets.keys():
            ...
            if value:
                available[github_name] = value[:10] + "..." if len(value) > 10 else value
    ...
    def generate_report(self, present: List[str], missing: List[str], 
                       available_env: Dict[str, str], synced: int):
    ...
    def run(self):
        ...
        # Generate report
        report = self.generate_report(present, missing, available_env, synced)
