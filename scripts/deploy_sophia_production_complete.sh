...
phase1_checks() {
    ...
    if ! ssh -i "$SSH_KEY" -o ConnectTimeout=5 ubuntu@$SERVER_IP "echo 'SSH connection successful'" > /dev/null 2>&1; then
...
phase3_deploy_frontend() {
    ...
    scp -i "$SSH_KEY" sophia-frontend-deploy.tar.gz ubuntu@$SERVER_IP:/tmp/
...
EOF
    ...
    scp -i "$SSH_KEY" nginx-sophia-production.conf ubuntu@$SERVER_IP:/tmp/
...
EOF
    ...
    scp -i "$SSH_KEY" health_check.sh ubuntu@$SERVER_IP:~/
    
    ssh -i "$SSH_KEY" ubuntu@$SERVER_IP "chmod +x ~/health_check.sh"
...
main() {
    ...
    echo "ssh -i $SSH_KEY ubuntu@$SERVER_IP './health_check.sh'"
    echo
}
