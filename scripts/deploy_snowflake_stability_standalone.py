...
class SnowflakeStabilityDeployer:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            cursor.execute("SELECT CURRENT_TIMESTAMP(), CURRENT_USER(), CURRENT_WAREHOUSE()")
            result = cursor.fetchone()
            logger.info(f"Connected as {result[1]} using warehouse {result[2]} at {result[0]}")
            cursor.close()
            return True
    ...
    def execute_query(self, query: str, description: str = "") -> bool:
        ...
        try:
            ...
            logger.error(f"❌ Failed to execute query{' - ' + description if description else ''}: {e}")
    ...
    def deploy_resource_monitors(self) -> bool:
        ...
        for query, description in resource_monitor_queries:
            if self.execute_query(query, description):
                success_count += 1
                self.deployment_status["resource_monitors"]["details"].append(description)
    ...
    def deploy_specialized_warehouses(self) -> bool:
        ...
        monitor_assignments = [
            ("ALTER WAREHOUSE SOPHIA_AI_CHAT_WH SET RESOURCE_MONITOR = SOPHIA_AI_PROD_MONITOR", "Chat warehouse monitor assignment"),
            ("ALTER WAREHOUSE SOPHIA_AI_ANALYTICS_WH SET RESOURCE_MONITOR = SOPHIA_AI_ANALYTICS_MONITOR", "Analytics warehouse monitor assignment"), 
            ("ALTER WAREHOUSE SOPHIA_AI_ETL_WH SET RESOURCE_MONITOR = SOPHIA_AI_PROD_MONITOR", "ETL warehouse monitor assignment"),
            ("ALTER WAREHOUSE SOPHIA_AI_ML_WH SET RESOURCE_MONITOR = SOPHIA_AI_PROD_MONITOR", "ML warehouse monitor assignment")
        ]
    ...
    def deploy_security_roles(self) -> bool:
        ...
        security_queries = [
            ...
            ("CREATE ROLE IF NOT EXISTS SOPHIA_AI_ANALYTICS_SERVICE", "Analytics service role"),
            ...
            ("GRANT USAGE ON DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_CHAT_SERVICE", "Chat service database access"),
            ("GRANT USAGE ON DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ANALYTICS_SERVICE", "Analytics service database access"),
            ("GRANT USAGE ON DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ETL_SERVICE", "ETL service database access"),
            ("GRANT ALL ON DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ADMIN_SERVICE", "Admin service database access"),
            
            # Grant warehouse access
            ("GRANT USAGE ON WAREHOUSE SOPHIA_AI_CHAT_WH TO ROLE SOPHIA_AI_CHAT_SERVICE", "Chat service warehouse access"),
            ("GRANT USAGE ON WAREHOUSE SOPHIA_AI_ANALYTICS_WH TO ROLE SOPHIA_AI_ANALYTICS_SERVICE", "Analytics service warehouse access"),
            ("GRANT USAGE ON WAREHOUSE SOPHIA_AI_ETL_WH TO ROLE SOPHIA_AI_ETL_SERVICE", "ETL service warehouse access"),
            ("GRANT USAGE ON WAREHOUSE SOPHIA_AI_ML_WH TO ROLE SOPHIA_AI_ADMIN_SERVICE", "Admin service ML warehouse access"),
            
            # Schema-level permissions
            ("GRANT USAGE ON SCHEMA SOPHIA_AI_PROD.UNIVERSAL_CHAT TO ROLE SOPHIA_AI_CHAT_SERVICE", "Chat service schema access"),
            ("GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA SOPHIA_AI_PROD.UNIVERSAL_CHAT TO ROLE SOPHIA_AI_CHAT_SERVICE", "Chat service table permissions"),
            ("GRANT SELECT, INSERT, UPDATE ON ALL VIEWS IN SCHEMA SOPHIA_AI_PROD.UNIVERSAL_CHAT TO ROLE SOPHIA_AI_CHAT_SERVICE", "Chat service view permissions"),
            
            # Analytics permissions
            ("GRANT USAGE ON ALL SCHEMAS IN DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ANALYTICS_SERVICE", "Analytics service all schemas"),
            ("GRANT SELECT ON ALL TABLES IN DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ANALYTICS_SERVICE", "Analytics service read access"),
            ("GRANT SELECT ON ALL VIEWS IN DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ANALYTICS_SERVICE", "Analytics service view access"),
            
            # ETL permissions
            ("GRANT USAGE ON ALL SCHEMAS IN DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ETL_SERVICE", "ETL service all schemas"),
            ("GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN DATABASE SOPHIA_AI_PROD TO ROLE SOPHIA_AI_ETL_SERVICE", "ETL service full data access"),
        ]
    ...
    def deploy_performance_optimization(self) -> bool:
        ...
        optimization_queries = [
            ...
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.KNOWLEDGE_ENTRIES CLUSTER BY (CREATED_AT, CATEGORY_ID)", "Knowledge entries clustering"),
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.CONVERSATION_MESSAGES CLUSTER BY (SESSION_ID, CREATED_AT)", "Conversation messages clustering"),
            
            # Clustering for AI Memory (if exists)
            ("ALTER TABLE SOPHIA_AI_PROD.AI_MEMORY.BUSINESS_MEMORIES CLUSTER BY (CREATED_AT, IMPORTANCE_SCORE)", "Business memories clustering"),
            
            # Automatic clustering
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.KNOWLEDGE_ENTRIES SET ENABLE_AUTOMATIC_CLUSTERING = TRUE", "Knowledge entries auto-clustering"),
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.CONVERSATION_MESSAGES SET ENABLE_AUTOMATIC_CLUSTERING = TRUE", "Conversation messages auto-clustering"),
            ("ALTER TABLE SOPHIA_AI_PROD.AI_MEMORY.BUSINESS_MEMORIES SET ENABLE_AUTOMATIC_CLUSTERING = TRUE", "Business memories auto-clustering")
        ]
        ...
        for query, description in optimization_queries:
            if self.execute_query(query, description):
                success_count += 1
                self.deployment_status["performance_optimization"]["details"].append(description)
    ...
    def deploy_backup_recovery(self) -> bool:
        ...
        backup_queries = [
            ...
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.KNOWLEDGE_ENTRIES SET DATA_RETENTION_TIME_IN_DAYS = 7", "Knowledge entries 7-day retention"),
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.CONVERSATION_MESSAGES SET DATA_RETENTION_TIME_IN_DAYS = 7", "Conversation messages 7-day retention"),
            ("ALTER TABLE SOPHIA_AI_PROD.AI_MEMORY.BUSINESS_MEMORIES SET DATA_RETENTION_TIME_IN_DAYS = 7", "Business memories 7-day retention"),
            
            # Schema evolution
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.KNOWLEDGE_ENTRIES SET ENABLE_SCHEMA_EVOLUTION = TRUE", "Knowledge entries schema evolution"),
            ("ALTER TABLE SOPHIA_AI_PROD.UNIVERSAL_CHAT.CONVERSATION_MESSAGES SET ENABLE_SCHEMA_EVOLUTION = TRUE", "Conversation messages schema evolution")
        ]
    ...
    def deploy_monitoring_schemas(self) -> bool:
        ...
        monitoring_queries = [
            # Monitoring schemas
            ("CREATE SCHEMA IF NOT EXISTS SOPHIA_AI_PROD.MONITORING", "Monitoring schema"),
            ...
            # Grant permissions
            ("GRANT ALL ON SCHEMA SOPHIA_AI_PROD.MONITORING TO ROLE SOPHIA_AI_ADMIN_SERVICE", "Monitoring schema permissions"),
            ("GRANT ALL ON SCHEMA SOPHIA_AI_PROD.QUALITY TO ROLE SOPHIA_AI_ADMIN_SERVICE", "Quality schema permissions"),
            ("GRANT ALL ON SCHEMA SOPHIA_AI_PROD.BACKUPS TO ROLE SOPHIA_AI_ADMIN_SERVICE", "Backups schema permissions"),
        ]
        ...
        for query, description in monitoring_queries:
            if self.execute_query(query, description):
                success_count += 1
                self.deployment_status["monitoring_schemas"]["details"].append(description)
