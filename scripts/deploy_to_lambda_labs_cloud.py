...
class LambdaLabsCloudDeployer:
    ...
    def __init__(self, environment: str = "prod", dry_run: bool = False):
        ...
        print(f"🚀 Initializing Lambda Labs Cloud Deployment")
    ...
    def _check_lambda_labs_connectivity(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"   ✅ Lambda Labs connectivity: {self.lambda_labs_config['ip_address']}")
                return True
            else:
                print(f"   ⚠️  Lambda Labs instance may not be ready: {self.lambda_labs_config['ip_address']}")
    ...
    def _check_registry_access(self) -> bool:
        ...
        try:
            if self.dry_run:
                print(f"   🔍 DRY RUN: Would check registry access to {self.docker_registry}")
                return True
    ...
    def _create_docker_secret(self, secret_name: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "secret", "ls", "--filter", f"name={secret_name}", "--format", "{{.Name}}"],
                capture_output=True,
                text=True,
                check=True
            )
    ...
    def verify_deployment(self) -> bool:
        ...
        try:
            ...
            for i in range(max_retries):
                result = subprocess.run(
                    ["docker", "stack", "services", self.stack_name, "--format", "{{.Replicas}}"],
                    capture_output=True,
                    text=True,
                    check=True
                )
                ...
                all_ready = all('/' in replica and replica.split('/')[0] == replica.split('/')[1] 
                              for replica in replicas if replica.strip())
