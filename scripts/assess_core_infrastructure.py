...
class InfrastructureAssessment:
    ...
    def assess_pulumi_esc(self):
        ...
        try:
            from backend.core.auto_esc_config import config, get_config_value
            ...
            self.results["pulumi_esc"] = {
                ...
                "details": {
                    ...
                    "missing_critical": [
                        k for k, v in available_secrets.items() if not v
                    ],
                },
            }
    ...
    def assess_docker(self):
        ...
        try:
            ...
            self.results["docker"] = {"status": "error", "details": {"error": str(e)}}
    ...
    async def assess_vercel(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    "https://api.vercel.com/v2/user", headers=headers
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        self.results["vercel"] = {
            ...
            self.results["vercel"] = {"status": "error", "details": {"error": str(e)}}
            ...
            if not missing:
                ...
                try:
                    ...
                    self.results["snowflake"] = {
                        "status": "connected",
                        "details": {"version": version, "all_config_present": True},
                    }
    ...
    def assess_mcp_servers(self):
        ...
        try:
            ...
            working_servers = sum(
                1 for s in server_status.values() if s.get("server_file_exists")
            )
        ...
        try:
            ...
            quality_tools = {
                ...
                "black": subprocess.run(
                    ["which", "black"], capture_output=True
                ).returncode
                == 0,
                "mypy": subprocess.run(
                    ["which", "mypy"], capture_output=True
                ).returncode
                == 0,
            }
    ...
    def generate_report(self):
        ...
        report = {
            "assessment_summary": {
                ...
                "healthy_components": sum(
                    ...
                    if r["status"] in ["connected", "available", "assessed"]
                ),
                "critical_issues": [],
            },
        ...
        if self.results["docker"]["status"] != "available":
            report["recommendations"].append(
                "HIGH: Install/start Docker for MCP server deployment"
            )

        if self.results["mcp_servers"]["details"].get("working_servers", 0) < 3:
            report["recommendations"].append(
                "HIGH: Fix MCP server configurations for core development workflow"
            )
