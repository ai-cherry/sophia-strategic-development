...
class SophiaProductionDeployer:
    ...
    def __init__(self):
        ...
        try:
            self.docker_config = {
                ...
                "username": get_config_value("docker_hub_username", default="scoobyjava15"),
        ...
        self.mcp_servers = [
            {"name": "ai-memory", "port": 9000, "image": "scoobyjava15/sophia-ai-memory:latest"},
            {"name": "snowflake", "port": 9001, "image": "scoobyjava15/sophia-snowflake:latest"},
            {"name": "hubspot", "port": 9006, "image": "scoobyjava15/sophia-hubspot:latest"}
        ]
    ...
    async def deploy_complete_stack(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_count = sum(1 for phase in results["phases"].values() if phase.get("success", False))
            total_phases = len(results["phases"])
            results["overall_success"] = success_count >= (total_phases * 0.8)  # 80% success threshold
    ...
    async def _deploy_infrastructure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            postgres_cmd = [
                "ssh", "-i", self.ssh_key_path, f"ubuntu@{self.lambda_ip}",
                "docker", "run", "-d",
    ...
    async def _deploy_core_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            network_cmd = [
                "ssh", "-i", self.ssh_key_path, f"ubuntu@{self.lambda_ip}",
                "docker", "network", "create", "sophia-network", "||", "true"
            ]
                ...
                try:
                    ...
                    if server['name'] == 'ai-memory':
                        deploy_cmd = [
                            "ssh", "-i", self.ssh_key_path, f"ubuntu@{self.lambda_ip}",
                            "cd", "/opt/sophia-ai", "&&",
                            "python3", "-m", "mcp-servers.ai_memory.simple_ai_memory_server", "&"
                        ]
                    else:
                        # Placeholder for other servers
                        deploy_cmd = [
                            "ssh", "-i", self.ssh_key_path, f"ubuntu@{self.lambda_ip}",
                            "echo", f"'{server['name']} MCP server placeholder'"
                        ]
...
'''
    ...
    async def _validate_deployment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_cmd = [
                "ssh", "-i", self.ssh_key_path, f"ubuntu@{self.lambda_ip}",
                "curl", "-f", f"http://localhost:8000/health", "||", "echo", "health_failed"
            ]
            
            result = subprocess.run(health_cmd, check=False, capture_output=True, text=True)
            backend_healthy = "health_failed" not in result.stdout
            
            # Test database connections
            db_cmd = [
                "ssh", "-i", self.ssh_key_path, f"ubuntu@{self.lambda_ip}",
                "docker", "ps", "|", "grep", "postgres", "||", "echo", "postgres_failed"
            ]
...
async def main():
    ...
    try:
        ...
        if results["overall_success"]:
            ...
            print(f"   🌐 Backend API: http://192.222.58.232:8000")
            print(f"   🏥 Health Check: http://192.222.58.232:8000/health")
            print(f"   📚 API Documentation: http://192.222.58.232:8000/docs")
            print("="*70)
            print("📋 Next Steps:")
