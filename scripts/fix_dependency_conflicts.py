...
class DependencyFixer:
        ...
        try:
            ...
            for dep in dependencies:
                if dep.startswith('mcp-python'):
                    ...
                    self.fixes_applied.append(f"MCP-Python: {old_version} → {new_version}")
                    mcp_fixed = True
                else:
    ...
    def fix_cchardet_compilation(self) -> bool:
        ...
        try:
            ...
            dependencies = config.get('project', {}).get('dependencies', [])
            performance_deps = config.get('project', {}).get('optional-dependencies', {}).get('performance', [])
            
            # Remove cchardet from dependencies
            updated_deps = [dep for dep in dependencies if not dep.startswith('cchardet')]
            updated_performance = [dep for dep in performance_deps if not dep.startswith('cchardet')]
            
            # Add chardet as alternative (pure Python, no compilation needed)
            if any(dep.startswith('cchardet') for dep in dependencies + performance_deps):
                updated_performance.append("chardet>=5.0.0")
                logger.info("  📝 Replaced cchardet with chardet (pure Python)")
                self.fixes_applied.append("Replaced cchardet with chardet (compilation-free)")
            
            # Update config
            config['project']['dependencies'] = updated_deps
            if 'optional-dependencies' not in config['project']:
                config['project']['optional-dependencies'] = {}
            config['project']['optional-dependencies']['performance'] = updated_performance
    ...
    def fix_python_version_compatibility(self) -> bool:
        ...
        try:
            ...
            if current_python == ">=3.12":
                ...
                logger.info(f"  📝 Updated Python requirement: {current_python} → {new_python}")
                self.fixes_applied.append(f"Python version: {current_python} → {new_python}")
    ...
    def install_system_dependencies(self) -> bool:
        ...
        try:
            if sys.platform.startswith('linux'):
                ...
                logger.warning("⚠️  Unsupported OS for automatic system dependency installation")
                return False
            ...
            if sys.platform.startswith('linux'):
                logger.info("  Run: sudo apt-get update && sudo apt-get install -y build-essential python3-dev libffi-dev libssl-dev pkg-config")
            elif sys.platform == 'darwin':
                logger.info("  Run: xcode-select --install")
                logger.info("  Or install via Homebrew: brew install python@3.12")
            
            self.fixes_applied.append("System dependencies: Manual installation required")
            return True
    ...
    def test_dependency_resolution(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(f"❌ Dependency resolution failed:")
...
'''
    ...
    def generate_requirements_lock(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(f"⚠️  Failed to generate requirements.lock: {result.stderr}")
                return False
    ...
    def run_all_fixes(self) -> bool:
        ...
        try:
            ...
            return success_count >= 5 and resolution_success  # Require most fixes to succeed
    ...
    def generate_summary(self, success_count: int, total_fixes: int, resolution_success: bool):
        ...
        logger.info(f"🧪 Resolution Test: {'PASSED' if resolution_success else 'FAILED'}")
        ...
        if success_count >= 5:
            ...
            logger.info("2. Test: python -c 'from backend.services.enhanced_search_service import EnhancedSearchService'")
