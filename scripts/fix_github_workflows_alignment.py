...
def fix_environment_defaults(workflow_path: str) -> bool:
    ...
    try:
        ...
        content = re.sub(r"default:\s*['\"]staging['\"]", "default: 'prod'", content)

        # Fix ENVIRONMENT variable assignments
        content = re.sub(r'ENVIRONMENT="staging"', 'ENVIRONMENT="prod"', content)
...
def remove_database_url_dependencies(workflow_path: str) -> bool:
    ...
    try:
        ...
        content = re.sub(r"^\s*DATABASE_URL:.*$", "", content, flags=re.MULTILINE)

        # Remove DATABASE_URL validation checks
        content = re.sub(r".*DATABASE_URL.*not set.*$", "", content, flags=re.MULTILINE)
...
def analyze_workflow_health() -> dict[str, list[str]]:
    ...
    for workflow_file in workflows_dir.glob("*.yml"):
        ...
        try:
            ...
            has_old_entry_points = any(
                pattern in content for pattern in ["python app.py", "flask run"]
            )
...
def main():
    ...
    database_url_workflows = ["deploy-phase2.yml", "sync_secrets.yml"]
    ...
    alignment_percentage = (
        len(health_status["aligned"])
        / (len(health_status["aligned"]) + len(health_status["needs_updates"]))
    ) * 100
