...
class MCPPerformanceTester:
    ...
    async def capability_test(self, session: aiohttp.ClientSession, endpoint: str) -> Dict[str, Any]:
        """Test specific capability endpoint"""
        start_time = time.time()
        
        try:
            # Use HEAD request for testing availability
            async with session.head(f"{self.base_url}{endpoint}", timeout=5) as response:
                response_time = time.time() - start_time
    ...
    async def load_test(self, duration: int, concurrent_requests: int) -> PerformanceMetrics:
        """Perform load test on MCP server"""
        logger.info(f"🚀 Starting load test for {self.server_name}")
        logger.info(f"Duration: {duration}s, Concurrent requests: {concurrent_requests}")
    ...
    async def stress_test(self, max_concurrent: int = 100, ramp_up_time: int = 30) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            for concurrent in range(5, max_concurrent + 1, 5):
                ...
                if total_requests > 0:
                    ...
                    if error_rate > 0.1 or avg_response_time > 5.0:  # 10% error rate or 5s response time
                        stress_results["breaking_point"] = concurrent
                        logger.warning(f"Breaking point reached at {concurrent} concurrent requests")
                        break
    ...
    def generate_performance_report(self, metrics: PerformanceMetrics, 
                                  ...
                                  stress_results: Optional[Dict[str, Any]] = None) -> str:
        ...
        if capability_metrics:
            ...
            for endpoint, cap_metrics in capability_metrics.items():
                ...
                report.append(f"- **Avg Response**: {cap_metrics.avg_response_time:.3f}s")
        ...
        if stress_results:
            ...
            if stress_results['breaking_point']:
                report.append(f"- **Breaking Point**: {stress_results['breaking_point']} concurrent requests")
            else:
                report.append(f"- **Breaking Point**: Not reached (tested up to {stress_results['max_concurrent']})")
            report.append("")
        ...
        if metrics.avg_response_time > 0.5:
            report.append("- ⚡ Optimize response time (consider caching, async processing)")
        
        if metrics.requests_per_second < 50:
            report.append("- 📈 Improve throughput (connection pooling, load balancing)")
        
        if stress_results and stress_results.get('breaking_point') and stress_results['breaking_point'] < 50:
...
async def main():
    ...
    parser.add_argument("--config", default="cursor_mcp_config.json", help="MCP config file")
    ...
    parser.add_argument("--capability-test", action="store_true", help="Test individual capabilities")
    parser.add_argument("--stress-test", action="store_true", help="Run stress test")
    parser.add_argument("--max-concurrent", type=int, default=100, help="Max concurrent for stress test")
    ...
    try:
        ...
        # Generate report
        report = tester.generate_performance_report(metrics, capability_metrics, stress_results)
