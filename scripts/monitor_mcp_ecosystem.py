...
class EcosystemMonitor:
    ...
    async def find_mcp_pods(self) -> list[dict[str, Any]]:
        ...
        try:
            result_json = await self._run_command(
                f"kubectl get pods -n {self.namespace} -l app.kubernetes.io/component=mcp-server -o json"
            )
        ...
        while True:
            ...
            try:
                ...
                stdscr.addstr(
                    0,
                    1,
                    f"Phoenix 1.4 Ecosystem Monitor - Namespace: {self.namespace} (Press 'q' to quit)",
                    curses.A_BOLD,
                )
                ...
                if not pods:
                    ...
                    for pod in pods:
                        ...
                        req_total = metrics.get(
                            f"sophia_mcp_{pod['name'].split('-')[0]}_requests_total", 0
                        )
                        latency_avg = metrics.get(
                            f"sophia_mcp_{pod['name'].split('-')[0]}_request_duration_seconds_avg",
                            0,
                        )

                        metrics_str = f"    Requests: {req_total:.0f} | Avg Latency: {latency_avg*1000:.2f}ms"
                        if y < height - 1:
                            stdscr.addstr(y, 1, metrics_str)
                            y += 1
