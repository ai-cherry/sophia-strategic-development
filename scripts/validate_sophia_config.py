...
def validate_capabilities(config: Dict) -> bool:
    ...
    for tier, servers in config.get('mcp_servers', {}).items():
        for server_name, server_config in servers.items():
            if isinstance(server_config, dict) and server_config.get('status') == 'active':
...
def validate_resources(config: Dict) -> bool:
        ...
        for server_name, server_config in servers.items():
            if isinstance(server_config, dict):
                ...
                try:
                    ...
                    issues.append(f"{server_name}: Invalid memory format '{memory_str}'")
    ...
    if total_memory > recommended_memory * 1.5:
        print(f"  ⚠️  Memory allocation seems high (recommended: ~{recommended_memory} GB)")
    if total_cpu > recommended_cpu * 1.5:
        print(f"  ⚠️  CPU allocation seems high (recommended: ~{recommended_cpu / 1000:.1f} cores)")
        ...
        for server_name, server_config in servers.items():
            if isinstance(server_config, dict):
                ...
                if not version:
                    ...
                    version_distribution[major_version].append(f"{server_name} ({version})")
...
def main():
    ...
    if all(results):
        ...
        print(f"❌ {total_checks - passed_checks} of {total_checks} validation checks failed")
        print("\n🔧 Next steps:")
