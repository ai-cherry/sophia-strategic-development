...
def simple_connectivity_test():
    ...
    try:
        result = subprocess.run(
            ["ping", "-c", "1", "165.1.69.44"], capture_output=True, timeout=10
        )
...
def main():
    ...
    print(
        f"🌐 Lambda Labs Server: {'✅ Reachable' if server_reachable else '❌ Not reachable'}"
    )
    ...
    print("   Codacy MCP: http://165.1.69.44:3008 (when ready)")

    print("\n💡 Manual Check Commands:")
    print("   python scripts/monitor_codacy_mcp_server.py")
    print("   python scripts/test_lambda_labs_connectivity.py")
