...
class CleanArchitectureAnalyzer:
    ...
    def analyze_architecture_compliance(self) -> List[ArchitectureViolation]:
        ...
        logger.info(f"Analyzing {len(backend_files)} backend files for architecture compliance")
        ...
        self.violations.sort(key=lambda x: (severity_order.get(x.severity, 4), x.file_path))
    ...
    def _check_direct_database_access(self, file_path: Path, tree: ast.AST, content: str):
    ...
    def _check_hardcoded_configurations(self, file_path: Path, tree: ast.AST, content: str):
    ...
    def _check_business_logic_in_controllers(self, file_path: Path, tree: ast.AST, content: str):
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    if function_content:
                        for indicator in business_logic_indicators:
                            if indicator in function_content and len(function_content.split('\n')) > 20:
                                self.violations.append(ArchitectureViolation(
                                    file_path=relative_path,
    ...
    def _check_dependency_injection_usage(self, file_path: Path, tree: ast.AST, content: str):
    ...
    def _is_infrastructure_import(self, module_name: str) -> bool:
        ...
        return any(pattern in module_name.lower() for pattern in infrastructure_patterns)
...
class CleanArchitectureCompliance:
    ...
    def apply_compliance_fixes(self, violations: List[ArchitectureViolation], max_fixes: int = 30) -> List[ComplianceResult]:
        ...
        priority_violations = [v for v in violations if v.severity in ["CRITICAL", "HIGH"]][:max_fixes]
        ...
        for violation in priority_violations:
            try:
                ...
                if result.success:
                    logger.info(f"✅ Fixed {violation.violation_type} in {violation.file_path}")
                else:
                    logger.warning(f"⚠️  Failed to fix {violation.violation_type}: {result.error}")
                    
            except Exception as e:
                logger.error(f"❌ Error fixing {violation.violation_type}: {e}")
                self.compliance_results.append(ComplianceResult(
    ...
    def _fix_violation(self, violation: ArchitectureViolation) -> ComplianceResult:
        ...
        try:
            if violation.fix_strategy == "dependency_injection":
                return self._apply_dependency_injection_fix(violation, file_path, content)
                ...
                return self._apply_use_case_extraction_fix(violation, file_path, content)
            else:
    ...
    def _apply_dependency_injection_fix(self, violation: ArchitectureViolation, file_path: Path, content: str) -> ComplianceResult:
        ...
        if "snowflake" in violation.description.lower():
            ...
            changes_made.append("Replaced Snowflake import with DataRepository interface")
        
        elif "requests" in violation.description.lower() or "httpx" in violation.description.lower():
    ...
    def _apply_repository_pattern_fix(self, violation: ArchitectureViolation, file_path: Path, content: str) -> ComplianceResult:
        ...
        if "snowflake.connector" in content:
            ...
            changes_made.append("Marked direct Snowflake connection for repository replacement")
        ...
        if "cursor.execute" in content:
            ...
            changes_made.append("Marked direct SQL execution for repository replacement")
    ...
    def _apply_config_injection_fix(self, violation: ArchitectureViolation, file_path: Path, content: str) -> ComplianceResult:
    ...
    def _apply_use_case_extraction_fix(self, violation: ArchitectureViolation, file_path: Path, content: str) -> ComplianceResult:
        ...
        for line in lines:
            if any(indicator in line.lower() for indicator in ["calculate", "process", "transform", "validate", "generate"]):
                new_lines.append(f"    # TODO: Extract business logic to use case")
...
class Week4CleanArchitectureCompliance:
    ...
    def run_week4_clean_architecture_compliance(self) -> Dict[str, any]:
        ...
        try:
            ...
            if total_critical + total_high > 0:
                fixed_percentage = ((summary["critical_violations_fixed"] + summary["high_violations_fixed"]) / 
                                  (total_critical + total_high)) * 100
    ...
    def _generate_week4_report(self, summary: Dict[str, any], violations: List[ArchitectureViolation], results: List[ComplianceResult]):
...
def main():
    ...
    try:
        ...
        logger.info(f"Critical Violations Fixed: {summary['critical_violations_fixed']}")
