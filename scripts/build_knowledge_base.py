    ...
    try:
        ...
        for topic in RESEARCH_TOPICS:
            ...
            if research_result.status != "success":
                logger.error(
                    f"Research failed for topic: {topic}. Error: {research_result.output}"
                )
                continue
            ...
            logger.info(
                f"Ingesting brief into Knowledge Base from temp file: {temp_file_path}"
            )
            ...
            if ingest_result.get("success"):
                ...
                logger.error(
                    f"Failed to ingest brief for '{topic}'. Reason: {ingest_result.get('error')}"
                )
