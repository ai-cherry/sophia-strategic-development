                ...
                try:
                    async with self.session.post(
                        ...
                        if response.status == 200:
                            ...
                            logger.warning(f"Workflow {workflow_name} failed with status {response.status}")
...
async def main():
    ...
    async with N8NWorkflowAutomation(base_url=args.base_url, api_key=args.api_key) as automation:
        if args.workflow == 'salesforce_migration':
            result = await automation.run_salesforce_migration(input_data.get('salesforce_data', {}))
