...
class DeploymentStatusChecker:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                        if response.status == 200:
                            ...
                            print(f"   ✅ {image_name} - Latest: {latest_tag}, Total tags: {len(tags)}")
                        else:
                            image_status[image_name] = {
                                "exists": False,
            ...
            for commit_line in commits:
                if commit_line.strip():
                    ...
                    recent_commits.append({
                        ...
                        "age_minutes": (datetime.now() - commit_date).total_seconds() / 60
                    })
                    
                    print(f"   📌 {commit_hash}: {commit_msg} ({commit_date.strftime('%H:%M:%S')})")
            
            return {
                "commits": recent_commits,
                "latest_commit_age_minutes": recent_commits[0]["age_minutes"] if recent_commits else 0
            }
    ...
    async def quick_connectivity_test(self) -> Dict[str, Any]:
        ...
        for service_name, port in self.expected_services.items():
            try:
                ...
                timeout = aiohttp.ClientTimeout(total=3.0)  # Shorter timeout for quick test
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    ...
                    async with session.get(url) as response:
                        ...
                        if response.status == 200:
                            ...
                            print(f"   ✅ {service_name} ({port}) - {response_time:.1f}ms")
                        else:
                            results[service_name] = {
                            ...
                            print(f"   ⚠️ {service_name} ({port}) - HTTP {response.status}")
    ...
    def analyze_deployment_status(self, docker_status: dict, git_status: dict, connectivity_status: dict) -> dict:
        ...
        available_images = len([img for img in docker_status.values() if img.get("exists", False)])
        
        total_services = len(connectivity_status)
        healthy_services = len([svc for svc in connectivity_status.values() if "✅" in svc.get("status", "")])
        ...
        if phase == "Not Started":
            analysis["recommendations"].append("Check if GitHub Actions workflow is triggered")
            analysis["recommendations"].append("Verify Docker Hub credentials in GitHub secrets")
        elif phase == "Building Images":
            analysis["recommendations"].append("Wait for Docker image builds to complete")
            analysis["recommendations"].append("Check GitHub Actions workflow logs")
        elif phase == "Deploying Services":
            analysis["recommendations"].append("Wait for services to start (usually 2-5 minutes)")
            analysis["recommendations"].append("Monitor connectivity for service health")
        elif phase == "Partial Deployment":
            analysis["recommendations"].append("Check logs for failed services")
            analysis["recommendations"].append("Verify all required secrets are available")
    ...
    async def run_comprehensive_check(self):
        ...
        # Analyze results
        analysis = self.analyze_deployment_status(docker_status, git_status, connectivity_status)
