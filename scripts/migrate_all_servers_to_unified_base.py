...
def analyze_server_file(file_path: Path) -> dict[str, Any]:
    ...
    analysis = {
        ...
        "has_enhanced_import": "enhanced_standardized_mcp_server" in content
        or "StandardizedMCPServer" in content,
...
def migrate_server(server_dir: Path, backup: bool = True):
    ...
    if analysis["class_name"] and analysis["base_class"]:
        ...
        if any(
            term in server_name_lower
            for term in ["snowflake", "ai", "cortex", "openai", "claude"]
        ):
            new_base = "AIEngineMCPServer"
        elif any(
            term in server_name_lower for term in ["pulumi", "lambda", "infrastructure"]
        ):
    ...
    content = re.sub(
        r"app\s*=\s*FastAPI\([^)]*\)", "# FastAPI app created by unified base", content
    )
    ...
    for server_dir in servers:
        try:
            ...
            if validate_migration(server_dir):
                ...
                print("   ⚠️  Validation failed - may need manual review")
