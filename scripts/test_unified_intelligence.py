...
from backend.services.unified_intelligence_service import get_unified_intelligence_service
...
async def test_unified_intelligence():
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        try:
            ...
            print(f"   - Constitutional Compliance: {result['constitutional_compliance']:.2f}")
            print(f"   - Confidence Score: {result['confidence_score']:.2f}")
            
            if result.get('memory_context'):
                print(f"   - Memory Context: {len(result['memory_context'])} memories found")
            
            if result.get('business_data'):
                print(f"   - Business Data Sources: {list(result['business_data'].keys())}")
...
async def test_self_optimization():
    ...
    opportunities = await optimizer.identify_optimization_opportunities(performance_data)
    
    print(f"\n   Found {len(opportunities)} optimization opportunities:")
    for opp in opportunities:
        print(f"   • {opp['type']} (Priority: {opp['priority']})")
        print(f"     Current: {opp['current_value']:.3f} → Target: {opp['target_value']:.3f}")
    ...
    for opp in opportunities[:2]:  # Test first two
        ...
        print(f"   • {opp['type']}: {status} (Score: {validation['compliance_score']:.2f})")
