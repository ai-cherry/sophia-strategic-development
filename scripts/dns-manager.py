...
class PulumiESCIntegration:
    ...
    def __init__(
        ...
        project: str = "default",
        environment: str = "sophia-intelligence-platform",
    ):
    ...
    async def get_configuration(self) -> dict:
        ...
        try:
            cmd = [
                ...
                "get",
                "--stack",
                f"{self.organization}/{self.project}/{self.environment}",
                "--json",
            ]
...
class NamecheapDNSManager:
    ...
    def _update_record_in_list(
        ...
        filtered_records = [
            ...
            if not (
                r.name == new_record.name and r.record_type == new_record.record_type
            )
        ]
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sophia Intelligence Platform DNS Manager"
    )
    parser.add_argument(
        "action", choices=["setup", "validate", "status"], help="Action to perform"
    )
    parser.add_argument("--domain", default="sophia-intel.ai", help="Domain to manage")
    parser.add_argument(
        "--ip", help="Server IP address (uses Pulumi ESC if not specified)"
    )
