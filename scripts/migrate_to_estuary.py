...
class SophiaAIMigrator:
    ...
    async def migrate_data_pipeline(self):
        ...
        try:
            ...
            self.migration_status["steps_completed"].append("estuary_captures_configured")
    ...
    async def _create_snowflake_schemas(self):
        ...
        self.migration_status["warnings"].append("Execute snowflake_cortex_setup.sql manually in Snowflake")
    ...
    def _update_fastapi_routes(self):
        ...
        if fastapi_app_path.exists():
            ...
            if "from backend.api.cortex_routes import router as cortex_router" not in content:
                import_line = "from backend.api.cortex_routes import router as cortex_router"
...
async def main():
    ...
    parser.add_argument(
        "--phase",
        choices=["data-pipeline", "ai-agents", "api-modernization", "business-logic", "all"],
        required=True,
