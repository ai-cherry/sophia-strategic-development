    ...
    async def build_admin_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not mcp_status_result.get("success", False):
                logger.warning(
                    f"Failed to add MCP Server Status widget: {mcp_status_result.get('error')}"
                )
            ...
            if not user_mgmt_result.get("success", False):
                logger.warning(
                    f"Failed to add User Management widget: {user_mgmt_result.get('error')}"
                )
            ...
            if not health_result.get("success", False):
                logger.warning(
                    f"Failed to add System Health widget: {health_result.get('error')}"
                )
            ...
            if not integration_result.get("success", False):
                logger.warning(
                    f"Failed to add Integration Status widget: {integration_result.get('error')}"
                )
            ...
            if not kb_result.get("success", False):
                logger.warning(
                    f"Failed to add Knowledge Base Management widget: {kb_result.get('error')}"
                )
            ...
            if not gong_result.get("success", False):
                logger.warning(
                    f"Failed to add Gong Call Analytics widget: {gong_result.get('error')}"
                )
            ...
            if not memory_result.get("success", False):
                logger.warning(
                    f"Failed to add AI Memory Inspector widget: {memory_result.get('error')}"
                )

            # Step 9: Publish the dashboard
            logger.info("Publishing dashboard...")
            publish_result = await self.mcp_client.call_tool(
                "retool", "publish_dashboard", {"app_id": app_id}
            )

            if not publish_result.get("success", False):
                logger.warning(
                    f"Failed to publish dashboard: {publish_result.get('error')}"
                )
