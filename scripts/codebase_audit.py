...
class CodebaseAuditor:
    ...
    def audit_documentation(self) -> Dict[str, Dict]:
            ...
            try:
                with open(py_file, "r") as f:
                    ...
                    if '"""' in content or "'''" in content:
                        ...
                        doc_status["missing_docs"].append(str(py_file.relative_to(self.root_path)))
            except Exception:
                pass
        ...
        if docs_dir.exists():
            for doc in docs_dir.rglob("*.md"):
                ...
                if (datetime.now() - datetime.fromtimestamp(doc.stat().st_mtime)).days > 90:
                    doc_status["outdated_docs"].append(str(doc.relative_to(self.root_path)))
    ...
    def _analyze_agent(self, agent_file: Path) -> Dict:
        ...
        try:
            ...
            capability_keywords = ["gong", "slack", "linear", "apollo", "competitive", "nmhc"]
    ...
    def generate_recommendations(self):
        ...
        doc_coverage = doc_status["documented_modules"] / max(doc_status["total_modules"], 1) * 100
        if doc_coverage < 80:
            recommendations.append({
                "priority": "MEDIUM",
        ...
        if len(self.audit_results["integrations"]) > 5:
            recommendations.append({
                ...
                "current_integrations": list(self.audit_results["integrations"].keys())
            })
    ...
    def run_full_audit(self) -> Dict:
        ...
        print(f"   Found {len(self.audit_results['compliance_sensitive'])} sensitive flows")
        
        print("5. Auditing documentation...")
        self.audit_documentation()
        doc_status = self.audit_results["documentation_status"]
        print(f"   Documentation coverage: {doc_status['documented_modules']}/{doc_status['total_modules']}")
