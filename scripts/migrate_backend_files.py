...
class BackendMigrator:
    ...
    def load_service_split(self):
        ...
        if os.path.exists('reports/service_split_report.txt'):
            with open('reports/service_split_report.txt', 'r') as f:
                content = f.read()
                
            # Parse core services
            if 'Core Services' in content:
                core_section = content.split('Core Services')[1].split('Infrastructure Services')[0]
            ...
            if 'Infrastructure Services' in content:
                infra_section = content.split('Infrastructure Services')[1].split('Manual Review')[0]
    ...
    def get_service_destination(self, file_path, service_split):
        ...
        if file_path in service_split:
            if service_split[file_path] == 'infrastructure':
                return file_path.replace('backend/services/', 'infrastructure/services/')
            else:
        ...
        if self.dry_run:
            ...
            try:
                ...
                try:
                    ...
                    self.migration_log.append(f"MOVED (non-git): {old_path} -> {new_path}")
                    return True
                except Exception as e2:
                    self.migration_log.append(f"ERROR: Failed to move {old_path}: {e} / {e2}")
                    return False
