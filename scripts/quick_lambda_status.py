...
def check_instance(name, ip):
    ...
    try:
        ...
        ssh_result = subprocess.run(ssh_cmd, shell=True, capture_output=True, text=True, timeout=10)
        ...
        http_result = subprocess.run(http_cmd, shell=True, capture_output=True, text=True, timeout=15)
