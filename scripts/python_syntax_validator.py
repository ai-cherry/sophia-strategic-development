...
class PythonSyntaxValidator:
        ...
        try:
            ...
            try:
                ...
                issues.append(SyntaxIssue(
                    ...
                    f"Python syntax error: {e.msg}", "error"
                ))
                return issues
            ...
            for i, line in enumerate(lines):
                for pattern, _, issue_type in self.DOCSTRING_PATTERNS:
                    if re.search(pattern, line):
                        issues.append(SyntaxIssue(
                            str(file_path), i + 1, issue_type,
                            "Docstring not properly separated from following code"
                        ))
            
            # Check import spacing
            for i in range(len(lines) - 1):
                if lines[i].strip().endswith('"""') and lines[i+1].strip().startswith(('import ', 'from ')):
                    issues.append(SyntaxIssue(
                        str(file_path), i + 2, "import_spacing",
            ...
            for i, line in enumerate(lines):
                if re.search(r'\.\s*$', line) and not line.strip().endswith(('...', '."', ".'", '.)')):
                    ...
                    if re.search(r'(=\s*[^.]+|\.clear\(\)|\.append\([^)]+\))\.\s*$', line):
                        issues.append(SyntaxIssue(
                            str(file_path), i + 1, "trailing_period",
            ...
            for i, line in enumerate(lines):
                if line.strip() == 'try:':
                    ...
                    if not found_handler:
                        issues.append(SyntaxIssue(
                            str(file_path), i + 1, "incomplete_try",
                            "Try block without except or finally clause", "error"
                        ))
    ...
    def validate_directory(self, directory: Path, fix: bool = False) -> List[SyntaxIssue]:
...
def main():
    ...
    parser.add_argument('path', nargs='?', default='.', help='Path to validate (file or directory)')
    parser.add_argument('--fix', action='store_true', help='Automatically fix issues')
    parser.add_argument('--json', action='store_true', help='Output results as JSON')
    parser.add_argument('--severity', choices=['all', 'error', 'warning'], 
    ...
    if args.severity != 'all':
        issues = [i for i in issues if i.severity == args.severity or 
                 (args.severity == 'warning' and i.severity == 'error')]
    ...
    if args.json:
        ...
        for file_path, file_issues in by_file.items():
            ...
            for issue in sorted(file_issues, key=lambda x: x.line_number):
                ...
                print(f"  {severity_icon} Line {issue.line_number}: {issue.description} ({issue.issue_type})")
