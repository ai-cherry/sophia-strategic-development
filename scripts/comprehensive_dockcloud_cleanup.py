...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("dockcloud_cleanup.log"), logging.StreamHandler()],
)
...
class DockcloudAwareCleanup:
    ...
    def _update_ci_workflows(self) -> None:
        ...
        for workflow_file in self.ci_workflows:
            ...
            try:
                ...
                content = re.sub(
                    r"python\s+scripts/deploy_to_lambda_labs\.py.*\n", "", content
                )
    ...
    def _update_documentation(self) -> None:
        ...
        handbook_path = (
            self.project_root / "docs/system_handbook/00_SOPHIA_AI_SYSTEM_HANDBOOK.md"
        )
    ...
    def _generate_cleanup_report(self) -> None:
        ...
        report = {
            ...
            "dockcloud_validations": {
                ...
                "ci_workflows_validated": len(
                    [f for f in self.ci_workflows if (self.project_root / f).exists()]
                ),
        ...
        logger.info(
            f"   Storage Freed: {round(self.stats.bytes_freed / (1024 * 1024), 2)} MB"
        )
        logger.info(
            f"   Docker References Updated: {self.stats.docker_references_updated}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Dockcloud-Aware Comprehensive Cleanup"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
