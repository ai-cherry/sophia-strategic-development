...
class LambdaLabsManager:
    ...
    def initialize(self) -> bool:
        ...
        # Test connection
        try:
            response = requests.get(f"{self.base_url}/instance-types", headers=self.headers, timeout=10)
            if response.status_code == 200:
                print("✅ Lambda Labs connection successful")
                return True
            else:
                print(f"❌ Lambda Labs connection failed: {response.status_code} - {response.text}")
                return False
        except Exception as e:
...
def main():
    ...
    if not manager.initialize():
        ...
        print("1. Set environment variable: export LAMBDA_API_KEY='your_key_here'")
