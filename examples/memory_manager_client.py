...
class MemoryManagerClient:
    ...
    async def store_memory(self, agent_id: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    async def retrieve_memory(self, agent_id: str, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.success:
                logger.info(f"Retrieved {len(response.data.get('vector_memories', []))} vector memories and {len(response.data.get('persistent_memories', []))} persistent memories for agent {agent_id}")
                return {
                    "success": True,
    ...
    async def delete_memory(self, agent_id: str, memory_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.success:
                logger.info(f"Memory {memory_id} deleted successfully for agent {agent_id}")
                return {
                    "success": True,
                    "memory_id": memory_id,
                    "processing_time": response.processing_time
    ...
    async def semantic_search(self, agent_id: str, query: str, top_k: int = 5) -> Dict[str, Any]:
        """
