...
async def store_memory(content: str, category: str = "general", tags: list = [], importance_score: float = 0.5) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            payload = {"content": content, "category": category, "tags": tags, "importance_score": importance_score}
...
async def execute_snowflake_query(query: str) -> dict:
    ...
    logger.info(f"Gateway: Executing Snowflake query.")
    try:
...
@mcp.tool()
async def create_asana_task(name: str, notes: str, workspaceId: str, projectId: str) -> dict:
    """Creates a new task in Asana."""
    logger.info(f"Gateway: Creating Asana task '{name}'")
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            payload = {"name": name, "notes": notes, "workspaceId": workspaceId, "projectId": projectId}
