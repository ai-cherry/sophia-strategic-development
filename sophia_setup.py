...
def setup_environment():
    ...
    if not os.path.exists(".env"):
        ...
        if os.path.exists("env.example"):
            ...
            print(
                "⚠️ No env.example found. You'll need to fill in the .env file manually."
            )
    ...
    if not success:
        ...
        print(
            "   You can run './secrets_manager.py detect-missing' to see what's missing."
        )
    else:
...
def start_mcp_servers():
    ...
    success, _ = run_command(
        ["docker-compose", "--version"], "Checking Docker Compose installation"
    )
    if not success:
...
def run_health_check():
    ...
    if not os.path.exists("automated_health_check.py") and os.path.exists(
...
def run_command_interface():
    ...
    if not os.path.exists("unified_command_interface.py") and os.path.exists(
...
def main():
    ...
    print(
        "No need to remember multiple commands or procedures - just sit back and relax!"
    )
