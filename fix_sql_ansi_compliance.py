...
class SQLANSIComplianceFixer:
    ...
    def __init__(self):
        ...
        self.patterns_to_fix = [
            ...
            (
                ...
                "-- EXCEPTION WHEN OTHERS THEN (Snowflake-specific)",
            ),
            # GET DIAGNOSTICS (not standard)
            (
                r"GET\s+DIAGNOSTICS\s+(\w+)\s*=\s*ROW_COUNT;",
                r"-- GET DIAGNOSTICS \1 = ROW_COUNT; (Snowflake-specific)",
            ),
            # VARIANT data type
    ...
    def should_skip_file(self, file_path: Path) -> bool:
        ...
        skip_patterns = ["test", "backup", "temp", ".git", "__pycache__"]
    ...
    def fix_stored_procedure_syntax(self, content: str) -> str:
        ...
        content = re.sub(
            r"^(\s*RETURNS\s+\w+)",
            r"-- \1 -- Snowflake stored procedure syntax",
            content,
            flags=re.MULTILINE,
        )
        ...
        content = re.sub(
            r"^(\s*BEGIN\s*$)",
            r"\1 -- Snowflake stored procedure",
            content,
            flags=re.MULTILINE,
        )
    ...
    def fix_snowflake_specific_functions(self, content: str) -> str:
        ...
        content = re.sub(
            r"CONCAT\s*\(([^)]+)\)",
            lambda m: self._convert_concat_to_pipes(m.group(1)),
            content,
        )
    ...
    def fix_snowflake_tags_and_policies(self, content: str) -> str:
        ...
        content = re.sub(
            r"^(ALTER\s+TABLE\s+[^;]*MODIFY\s+COLUMN\s+[^;]*SET\s+TAG[^;]*;)",
            r"-- \1 -- Snowflake-specific TAG",
            content,
            flags=re.MULTILINE | re.IGNORECASE,
        )
        ...
        content = re.sub(
            r"^([^;]*MASKING\s+POLICY[^;]*;)",
            r"-- \1 -- Snowflake-specific MASKING POLICY",
            content,
            flags=re.MULTILINE | re.IGNORECASE,
        )
        ...
        # SEARCH OPTIMIZATION
        content = re.sub(
            r"^(ALTER\s+TABLE\s+[^;]*ADD\s+SEARCH\s+OPTIMIZATION[^;]*;)",
            r"-- \1 -- Snowflake-specific SEARCH OPTIMIZATION",
            content,
            flags=re.MULTILINE | re.IGNORECASE,
        )
...
def main():
    ...
    for directory in directories_to_fix:
        if os.path.exists(directory):
            ...
            logger.info(
                f"✅ Completed {directory}: {stats['files_fixed']}/{stats['files_processed']} files fixed"
            )
        else:
    ...
    if total_stats["files_fixed"] > 0:
        ...
        logger.info(
            "🔍 Review changes before committing to ensure Snowflake functionality is preserved"
        )
