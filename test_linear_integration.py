...
async def test_linear_integration():
    ...
    try:
        ...
        if success:
            ...
            print("   ⚠️  Linear integration initialization failed (expected without credentials)")
    ...
    try:
        ...
        for project in projects:
            print(f"      • {project.id}: {project.name} ({project.progress}% complete)")
    except Exception as e:
    ...
    try:
        ...
        if config_path.exists():
            ...
            if 'linear' in config.get('mcpServers', {}):
                ...
                print(f"   ✅ Linear MCP server configured")
                print(f"      • Command: {linear_config.get('command')}")
                print(f"      • Args: {linear_config.get('args')}")
            else:
                print(f"   ❌ Linear MCP server not found in config")
        else:
    ...
    try:
        ...
        if compose_path.exists():
            ...
            if 'linear:' in content:
                print(f"   ✅ Linear service configured in Docker Compose")
                if 'LINEAR_API_TOKEN' in content:
                    print(f"      • Environment variables configured")
                if 'healthcheck:' in content:
                    print(f"      • Health checks configured")
            else:
                print(f"   ❌ Linear service not found in Docker Compose")
        else:
