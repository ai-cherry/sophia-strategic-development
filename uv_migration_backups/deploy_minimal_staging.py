...
class MinimalStagingDeployment:
    ...
    async def deploy_minimal_staging(self) -> Dict[str, Any]:
        ...
        try:
            ...
            running_services = len([s for s in deployment_result["components"] if s.get("status") == "running"])
            ...
            logger.info(f"✅ Minimal staging deployment completed: {running_services}/{total_services} services running")
        ...
        for service_config in minimal_services:
            try:
                ...
                docker_cmd = [
                    ...
                    "-p", service_config["port"],
                    "--restart", "unless-stopped"
                ]
                
                if service_config["command"]:
                    docker_cmd.extend([service_config["image"]] + service_config["command"].split())
                else:
                ...
                if result.returncode == 0:
                    ...
                    status_result = subprocess.run([
                        "docker", "inspect", service_config["name"], "--format", "{{.State.Status}}"
                    ], capture_output=True, text=True)
                    ...
                    if status_result.returncode == 0:
                        ...
                        if container_status == "running":
                            ...
                            logger.warning(f"      ⚠️ {service_config['name']} status: {container_status}")
                    else:
                        services.append({
                            "service": service_config["name"],
                            "status": "unknown",
                    ...
                    logger.error(f"      ❌ {service_config['name']} failed: {result.stderr}")
            ...
            for container in containers:
                ...
                if result.returncode == 0:
                    ...
                    monitors.append({
                        ...
                        "status": "active",
                        "health": health_status if health_status else "no_healthcheck",
    ...
    async def _validate_minimal_deployment(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run([
                "docker", "ps", "--filter", "network=sophia-staging", "--format", "{{.Names}}\t{{.Status}}"
            ], capture_output=True, text=True)
...
async def main():
    ...
    print(f"Status: {'✅ SUCCESS' if result['status'] == 'success' else '⚠️ PARTIAL' if result['status'] == 'partial_success' else '❌ FAILED'}")
    ...
    if result["infrastructure"]:
        ...
        for component in result["infrastructure"]:
            status_icon = "✅" if component["status"] in ["available", "exists"] else "⚠️"
