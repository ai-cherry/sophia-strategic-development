...
from backend.integrations.enhanced_microsoft_gong_integration import enhanced_microsoft_gong_integration
...
async def test_riley_coaching_analysis():
    ...
    try:
        ...
        for insight in result['insights']:
            priority_emoji = "🚨" if insight['priority'] == 'critical' else "⚠️" if insight['priority'] == 'high' else "ℹ️"
        ...
        print(f"  Personalization Score: {email_summary.get('avg_personalization_score', 0):.2f}")
        ...
        print(f"  Calls Needing Coaching: {call_summary.get('calls_needing_coaching', 0)}")
        ...
        print(f"✅ Real-time coaching provided")
        print(f"🔍 Insights Generated: {len(real_time_result.get('real_time_insights', []))}")
        ...
        print(f"✅ Improvement tracking completed")
        ...
        print(f"🎯 Coaching Effectiveness: {effectiveness.get('effectiveness_score', 0):.2f}")
...
async def test_microsoft_gong_integration():
    ...
    try:
        ...
        analysis = await enhanced_microsoft_gong_integration.analyze_sales_rep_performance(
            sales_rep="Riley Martinez",
            days=7
        )
