...
class FixedStagingDeployment:
    ...
    async def deploy_to_staging(self) -> Dict[str, Any]:
        ...
        try:
            ...
            deployment_result["status"] = "success" if all_phases_success else "partial_success"
    ...
    async def _start_core_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for service in services:
                ...
                result = subprocess.run([
                    "docker-compose", 
                    "-f", "docker-compose.staging-simple.yml",
                    "up", "-d", "--build", service
            ...
            logger.info(f"   ✅ Service startup: {len(services_started)} services processed")
...
'''
    ...
    async def _validate_deployment(self) -> Dict[str, Any]:
        ...
        success = len(errors) == 0
        logger.info(f"   ✅ Deployment validation: {len(validation_checks)} checks, {len(errors)} errors")
...
async def main():
    ...
    if result["status"] in ["success", "partial_success"]:
        ...
        print("\n🔧 Management Commands:")
        print("   • View logs: docker-compose -f docker-compose.staging-simple.yml logs -f")
        print("   • Stop services: docker-compose -f docker-compose.staging-simple.yml down")
        print("   • Restart services: docker-compose -f docker-compose.staging-simple.yml restart")
    else:
