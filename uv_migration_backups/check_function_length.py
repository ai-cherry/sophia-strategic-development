...
class FunctionLengthChecker:
    ...
    def scan_directory(
        self, directory: str, exclude_patterns: list[str] | None = None
    ...
    def generate_report(self, results: dict[str, list[FunctionMetrics]]) -> str:
        ...
        compliance_rate = (
            ((self.total_functions - violation_count) / self.total_functions * 100)
...
"""
        ...
        for file_path, functions in results.items():
            ...
            file_types[file_type]["violations"] += len(
                [f for f in functions if f.line_count > self.max_length]
            )
