...
GONG_REDIRECT_URI = os.getenv(
    "GONG_OAUTH_REDIRECT_URI"
)  # e.g., https://your-app.com/api/auth/gong/callback
GONG_AUTHORIZATION_URL = (
    "https://app.gong.io/oauth2/authorize"  # Check Gong docs for exact URL
)
...
class GongOAuthHandler:
    def __init__(self, client_id=None, client_secret=None, redirect_uri=None):
        ...
        if not all([self.client_id, self.client_secret, self.redirect_uri]):
            logger.error(
                "Gong OAuth client ID, secret, or redirect URI not configured."
            )
    ...
    def get_authorization_url(self, state: str = None) -> str:
        ...
        if not all([self.client_id, self.redirect_uri]):
            logger.error(
                "Cannot generate authorization URL: Client ID or Redirect URI missing."
            )
    ...
    def exchange_code_for_tokens(self, authorization_code: str) -> dict:
        ...
        if not all([self.client_id, self.client_secret, self.redirect_uri]):
            logger.error(
                "Cannot exchange code: Client ID, Secret or Redirect URI missing."
            )
        ...
        try:
                ...
                def store_tokens_sync():
                    ...
                    try:
                        loop.run_until_complete(
                            pulumi_esc_client.set_secret(
                                "gong_access_token", tokens.get("access_token")
                            )
                        )
                        loop.run_until_complete(
                            pulumi_esc_client.set_secret(
                                "gong_refresh_token", tokens.get("refresh_token")
                            )
                        )
                        ...
                        loop.run_until_complete(
                            pulumi_esc_client.set_configuration(
                                "gong_token_metadata", token_metadata
                            )
                        )
                    finally:
                        loop.close()
            ...
            logger.error(
                f"Error exchanging code for tokens: {e}. Response: {e.response.text if e.response else 'No response'}"
            )
    ...
    def refresh_access_token(self, refresh_token: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                def update_tokens_sync():
                    ...
                    try:
                        loop.run_until_complete(
                            pulumi_esc_client.set_secret(
                                "gong_access_token", new_tokens.get("access_token")
                            )
                        )
            ...
            logger.error(
                f"Error refreshing access token: {e}. Response: {e.response.text if e.response else 'No response'}"
            )
    ...
    def store_tokens(
        ...
        logger.info(
            f"Storing tokens for user_id: {user_id} (access_token: {access_token[:10]}..., refresh_token: {refresh_token[:10] if refresh_token else 'None'}...)"
        )
        ...
        if hasattr(self, "_temp_token_storage"):
            if self._temp_token_storage["expires_at"] > datetime.utcnow().timestamp():
                ...
                new_tokens = self.refresh_access_token(
                    self._temp_token_storage["refresh_token"]
                )
...
if __name__ == "__main__":
    ...
    if not all([GONG_CLIENT_ID, GONG_CLIENT_SECRET, GONG_REDIRECT_URI]):
        print(
            "Please set GONG_OAUTH_CLIENT_ID, GONG_OAUTH_CLIENT_SECRET, and GONG_OAUTH_REDIRECT_URI environment variables to run this example."
        )
    else:
        ...
        if mock_auth_code:
            ...
            if "access_token" in tokens:
                ...
                if tokens.get("refresh_token"):
                    ...
                    handler._temp_token_storage["expires_at"] = (
                    ...
                    if refreshed_access_token:
                        print(
                            f"5. Refreshed access token: {refreshed_access_token[:20]}..."
                        )
                    else:
