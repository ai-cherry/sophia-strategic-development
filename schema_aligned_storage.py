            ...
            return {
                ...
                "apartment_intelligence": {
                    ...
                    "relevance_percentage": round(
                        (apartment_calls / max(calls_count, 1)) * 100, 2
                    ),
                },
                "sample_data": {
...
async def main():
    ...
    if storage_result["success"]:
        print("✅ Gong data stored successfully!")
    ...
    if intelligence_result["success"]:
        ...
        print(
            f"❌ Intelligence enhancement failed: {intelligence_result.get('error', 'Unknown error')}"
        )
    ...
    if verification_result["success"]:
        ...
        for user in verification_result["sample_data"]["users"][:5]:
            print(
                f"   {user['first_name']} {user['last_name']} ({user['title']}) - {user['email_address']}"
            )

        print("\n📞 RECENT CALLS:")
        for call in verification_result["sample_data"]["calls"][:5]:
            duration_min = (
                call["duration_seconds"] // 60 if call["duration_seconds"] else 0
            )
