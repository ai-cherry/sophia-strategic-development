...
def update_requirements(requirements_path: str, updated_requirements: Dict[str, str]) -> bool:
        ...
        for line in lines:
            ...
            if match:
                ...
                if package in updated_requirements:
                    updated_lines.append(f"{package}{updated_requirements[package]}{comment}\n")
                else:
                    updated_lines.append(original_line)
            else:
                # Handle requirements without version specifiers
...
def fix_pydantic_mcp_conflict() -> bool:
    ...
    try:
        ...
        logger.info(f"Downgrading Pydantic to version {COMPATIBLE_VERSIONS['pydantic']}")
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", f"pydantic=={COMPATIBLE_VERSIONS['pydantic']}"],
            capture_output=True,
            text=True
        )
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", f"mcp=={COMPATIBLE_VERSIONS['mcp']}"],
            capture_output=True,
            text=True
        )
...
def main():
    ...
    parser.add_argument("--requirements", default="requirements.txt", help="Path to requirements.txt file (default: requirements.txt)")
    ...
    print("   python -c \"import pydantic; print(f'Pydantic version: {pydantic.__version__}')\"")
