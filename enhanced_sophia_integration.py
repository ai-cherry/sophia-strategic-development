...
class EnhancedSophiaIntegration:
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                for channel in channels:
                    ...
                    business_relevance = (
                        await self._calculate_channel_business_relevance(channel)
                    )
                    apartment_focus = await self._detect_apartment_industry_focus(
                        channel
                    )
        ...
        for channel in channels:
            try:
                ...
                response = await self.slack_client.conversations_history(
                    channel=channel["id"], oldest=str(oldest_timestamp), limit=1000
                )
                ...
                async with self.db_pool.acquire() as conn:
                    for message in messages:
                        if message.get("type") == "message" and "text" in message:
                            ...
                            intelligence = await self._analyze_message_intelligence(
                                message
                            )
                ...
                logger.error(
                    f"Failed to import messages for channel {channel['name']}: {e.response['error']}"
                )
        ...
        async with aiohttp.ClientSession() as session:
            while True:
                async with session.get(
                    f"{base_url}/calls", headers=headers, params=params
                ) as response:
                    ...
                    async with self.db_pool.acquire() as conn:
                        for call in calls:
                            ...
                            apartment_analysis = await self._analyze_apartment_context(
                                call
                            )
    ...
    async def create_conversation_threads(self):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            for thread_id, group in thread_groups.items():
                await self._create_conversation_intelligence_record(
                    conn, thread_id, group
                )
    ...
    async def _detect_apartment_industry_focus(self, channel: Dict) -> bool:
        ...
        return any(
            keyword in name or keyword in purpose for keyword in apartment_keywords
        )
    ...
    async def _create_conversation_intelligence_record(
        self, conn, thread_id: str, interactions: List
    ):
