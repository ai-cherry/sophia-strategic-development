...
class SecretAuditor:
        ...
        for ext in extensions:
            for file_path in self.workspace_root.rglob(f"*{ext}"):
                ...
                if any(
                    ...
                    for skip in [".git", "node_modules", "__pycache__", ".env"]
                ):
                    continue
                ...
                try:
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            ...
                            if (
                                re.match(r"^[A-Z][A-Z_]*[A-Z]$", secret_name)
    ...
    def analyze_secret_consistency(self) -> None:
        ...
        all_secrets = (
            self.github_secrets
            | self.pulumi_esc_secrets
            | {ref.secret_name for ref in self.code_references}
        )
    ...
    def _analyze_individual_secret(self, secret_name: str) -> SecretAnalysis:
        ...
        references = [
            ref for ref in self.code_references if ref.secret_name == secret_name
        ]
        ...
        recommended_name = self._generate_recommended_name(
            service, credential_type, secret_name
        )
    ...
    def _generate_recommended_name(
        self, service: str, credential_type: str, current_name: str
    ) -> str:
    ...
    def generate_migration_plan(self) -> dict:
        ...
        rename_secrets = [
            ...
            if a.current_name != a.recommended_name and a.action_required == "keep"
        ]
        if rename_secrets:
            migration_steps.append(
                {
                    ...
                    "secrets": [
                        (a.current_name, a.recommended_name) for a in rename_secrets
                    ],
                    "commands": [
                        f"# Rename {old} to {new}"
                        for old, new in [
        ...
        delete_secrets = [
            a for a in self.analysis_results if a.action_required == "delete"
        ]
        if delete_secrets:
            migration_steps.append(
                {
                    ...
                    "commands": [
                        f"# Delete unused secret: {a.current_name}"
                        for a in delete_secrets
                    ],
                }
            )
        ...
        add_to_github = [
            a for a in self.analysis_results if a.action_required == "add_to_github"
        ]
        add_to_pulumi = [
            a for a in self.analysis_results if a.action_required == "add_to_pulumi"
        ]
