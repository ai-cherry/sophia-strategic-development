...
class SecretStandardizer:
    ...
    def generate_cleanup_list(self) -> dict:
        ...
        add_to_pulumi = self.analysis_report["actions_required"].get(
            "add_to_pulumi", []
        )
        ...
        delete_candidates = self.analysis_report["actions_required"].get(
            "add_to_both", []
        )
    ...
    def generate_github_actions_cleanup(self) -> dict:
        ...
        cleanup_recommendations = {
            "high_complexity_workflows": [
                name
                for name, data in workflow_analysis.items()
                if data["complexity_score"] > 1000
            ],
            "duplicate_secret_patterns": {
                secret: files
                for secret, files in secret_usage.items()
                if len(files) > 3
            },
    ...
    def create_standardization_report(self) -> None:
        ...
        print(
            f"  High complexity workflows: {len(github_cleanup['high_complexity_workflows'])}"
        )
        print(
            f"  Consolidation candidates: {len(github_cleanup['consolidation_candidates'])}"
        )
