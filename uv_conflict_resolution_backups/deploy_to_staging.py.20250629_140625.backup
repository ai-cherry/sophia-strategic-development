...
class StagingDeployment:
    ...
    async def _validate_pre_deployment(self) -> dict[str, Any]:
        ...
        try:
            ...
            validation_checks.append(
                {"check": "docker_availability", "status": "failed", "details": str(e)}
            )
        ...
        logger.info(
            f"   ✅ Pre-deployment validation: {len(validation_checks)} checks, {len(errors)} errors"
        )
        ...
        return {
            ...
            "passed_checks": len(
                [c for c in validation_checks if c["status"] == "passed"]
            ),
        }
    ...
    async def _setup_staging_infrastructure(self) -> dict[str, Any]:
        ...
        success = len(errors) == 0
        logger.info(
            f"   ✅ Infrastructure setup: {len(setup_steps)} steps, {len(errors)} errors"
        )

        return {"success": success, "steps": setup_steps, "errors": errors}
    ...
    def _create_staging_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "monitoring": {"health_check_interval": 30, "metrics_collection": True},
        }
    ...
    async def _deploy_services(self) -> dict[str, Any]:
            ...
            if result.returncode == 0:
                ...
                for service in service_order:
                    ...
                    if service_status["status"] == "running":
                        ...
                        logger.warning(
                            f"      ⚠️ {service} deployment issue: {service_status['details']}"
                        )
        ...
        success = (
            len(errors) == 0
            and len([s for s in services_deployed if s["status"] == "running"]) > 0
        )
        logger.info(
            f"   ✅ Service deployment: {len(services_deployed)} services, {len(errors)} errors"
        )

        return {"success": success, "services": services_deployed, "errors": errors}
        ...
        return {
            ...
            "services": {
                ...
                "ai-memory-mcp": {
                    ...
                    "environment": ["MCP_SERVER_PORT=9001", "ENVIRONMENT=staging"],
                    "ports": ["9001:9001"],
                    "networks": ["sophia-staging-network"],
                },
            },
            "networks": {"sophia-staging-network": {"external": True}},
            "volumes": {
    ...
    async def _validate_deployment_health(self) -> dict[str, Any]:
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"      ⚠️ {endpoint['name']} returned HTTP {response.status_code}"
                    )
                ...
                logger.warning(
                    f"      ⚠️ {endpoint['name']} not reachable (may still be starting)"
                )
                ...
                health_checks.append(
                    {"endpoint": endpoint["name"], "status": "error", "details": str(e)}
                )
        ...
        logger.info(
            f"   ✅ Health validation: {healthy_services}/{total_services} services healthy"
        )
    ...
    async def _run_integration_tests(self) -> dict[str, Any]:
        ...
        try:
            ...
            test_results.append(
                {"test": "api_connectivity", "status": "failed", "details": str(e)}
            )
        ...
        logger.info(
            f"   ✅ Integration tests: {passed_tests}/{total_tests} passed, {len(errors)} errors"
        )
