...
class DocumentationIndexGenerator:
    ...
    def __init__(
        self,
        docs_root: str = "docs",
        output_file: str = "SOPHIA_AI_DOCUMENTATION_MASTER_INDEX.md",
    ):
    ...
    def _determine_category(self, relative_path: Path) -> str:
        ...
        for category_key in self.categories:
            if first_dir == category_key or first_dir.startswith(
                category_key.split("-")[0]
            ):
                return category_key
    ...
    def _parse_markdown_header(self, file_path: Path) -> tuple[str, str, list[str]]:
        ...
        try:
            ...
            description = (
                " ".join(description_lines)[:200] + "..." if description_lines else ""
            )

            # Extract tags from content
            tags_match = re.search(
                r"(?:Tags?|Keywords?):\s*(.+)", content, re.IGNORECASE
            )
            if tags_match:
...
"""
    ...
    def _generate_table_of_contents(self) -> str:
        ...
        categories_with_docs = {doc.category for doc in self.documents}

        for category_key, category_info in sorted(
            self.categories.items(), key=lambda x: x[1]["priority"]
        ):
    ...
    def _generate_featured_section(self) -> str:
        ...
        featured_docs = [
            ...
            if doc.path.split("/")[-1] in self.featured_documents
        ]
        featured_docs.sort(
            key=lambda x: (
                self.featured_documents.index(x.path.split("/")[-1])
                if x.path.split("/")[-1] in self.featured_documents
                else 999
            )
        )
        ...
        for category_key, category_info in sorted(
            ...
            if category_key in docs_by_category:
                ...
                category_docs = sorted(
                    ...
                    key=lambda x: (x.priority, x.title.lower()),
                )
    ...
    def _generate_statistics(self) -> str:
        ...
        for category_key, count in sorted(
            ...
            category_name = self.categories.get(category_key, {}).get(
                "name", category_key
            )
        ...
        # Recent updates
        recent_docs = sorted(
            self.documents, key=lambda x: x.last_modified, reverse=True
        )[:5]
