...
try:
    ...
    logger.error(
        "Please install required packages: pip install aiohttp snowflake-connector-python"
    )
    sys.exit(1)
...
class IntegrationTester:
    ...
    async def test_all_integrations(self):
        ...
        try:
            ...
            service_statuses = [
                s.get("status") for s in self.results["services"].values()
            ]
    ...
    async def test_gong_integration(self):
        ...
        if not gong_api_key or not gong_api_secret:
            ...
            result[
                "error"
            ] = "Missing required environment variables: GONG_API_KEY and/or GONG_API_SECRET"
    ...
    async def test_snowflake_integration(self):
        ...
        if not snowflake_account or not snowflake_user or not snowflake_password:
            ...
            result[
                "error"
            ] = "Missing required environment variables for Snowflake connection"
            self.results["services"][service_name] = result
            return
            ...
            async with self.session.get(url, headers=headers) as response:
                ...
                if status_code == 200:
                    ...
                    result["details"] = {
                        ...
                        "account_type": data.get("user", {})
                        .get("account", {})
                        .get("type"),
                        "team": data.get("user", {}).get("team", {}).get("name")
                        if data.get("user", {}).get("team")
                        else None,
                    }
                    ...
                    async with self.session.get(
                        ...
                        if projects_response.status == 200:
                            ...
                            result["details"]["projects_count"] = len(
