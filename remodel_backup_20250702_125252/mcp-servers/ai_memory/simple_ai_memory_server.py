...
class SimpleMemoryStore:
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "categories": list({m["category"] for m in self.memories}),
            "average_importance": (
                sum(m["importance_score"] for m in self.memories) / len(self.memories)
                if self.memories
                else 0
            ),
        }
...
async def main():
    ...
    try:
        ...
        # Start server
        config = uvicorn.Config(app=app, host="0.0.0.0", port=9001, log_level="info")
