...
class ComprehensivePulumiESCUpdater:
    ...
    def _get_github_secrets_mapping(self) -> dict[str, dict[str, str]]:
        ...
        return {
            "infrastructure": {
                ...
                "load_balancer": {"host": "${LOAD_BALANCER_HOST}"},
            },
            "data_pipeline": {
                "estuary_flow": {
            ...
            "ai_services": {
                ...
                "huggingface": {"api_token": "${HUGGINGFACE_API_TOKEN}"},
                "langchain": {"api_key": "${LANGCHAIN_API_KEY}"},
                "langsmith": {
                    "api_key": "${LANGSMITH_API_KEY}",
                    "org_id": "${LANGSMITH_ORG_ID}",
                },
                "llama": {"api_key": "${LLAMA_API_KEY}"},
            },
            "security": {
                "encryption": {
                    "key": "${ENCRYPTION_KEY}",
            ...
            "productivity_tools": {
                ...
                "asana": {"api_token": "${ASANA_API_TOKEN}"},
                "figma": {"pat": "${FIGMA_PAT}", "project_id": "${FIGMA_PROJECT_ID}"},
            },
            "external_services": {
    ...
    def update_pulumi_esc_environment(self) -> bool:
        ...
        try:
            ...
            esc_config = {"values": self.github_secrets_mapping}
