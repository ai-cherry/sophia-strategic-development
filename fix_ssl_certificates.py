    ...
    try:
        ...
        if not ssl_cert_file_set:
            ...
            if config_section_index != -1:
                ...
                lines.insert(
                    config_section_index + 1, f"SSL_CERT_FILE={certifi_path}\n"
                )
            else:
...
def update_shell_profile() -> bool:
    ...
    try:
        ...
        if not os.path.exists(profile_path):
            logger.warning(
                f"Profile file not found: {profile_path}, skipping profile update"
            )
            return False
...
def test_ssl_verification() -> bool:
    ...
    try:
        ...
        if response.status == 200:
            ...
            logger.warning(
                f"SSL certificate verification returned status code: {response.status}"
            )
            return False
    except Exception as e:
...
def main():
    ...
    if update_shell_profile():
        ...
        print(
            "   You need to restart your shell or run 'source ~/.bashrc' or 'source ~/.zshrc'"
        )
    else:
