...
class SophiaDataImporter:
    ...
    def __init__(self):
        ...
        self.gong_access_key_secret = os.getenv(
    ...
    async def test_api_connectivity(self):
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if data.get("ok"):
                            ...
                            results["slack"][
                                "details"
                            ] = f"Connected as {data.get('user', 'unknown')}"
                        else:
                            results["slack"]["status"] = "error"
                            results["slack"]["details"] = data.get(
                                "error", "Unknown error"
                            )
                    else:
                        results["slack"]["status"] = "error"
        ...
        try:
            auth_string = base64.b64encode(
                f"{self.gong_access_key}:{self.gong_access_key_secret}".encode()
            ).decode()
            headers = {
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://api.gong.io/v2/users", headers=headers
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        results["gong"][
                            "details"
                        ] = f"Connected - {user_count} users found"
                    else:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            for contact in sample_contacts:
                ...
                """,
                    ...
                    contact["apartment_portfolio_size"],
            ...
            for channel in sample_channels:
                ...
                """,
                    ...
                    channel["name"],
                    channel["purpose"],
                    channel["business_relevance_score"],
                    channel["apartment_industry_focus"],
                )
            ...
            for interaction in sample_interactions:
                ...
                """,
                    ...
                    interaction.get("interaction_duration_seconds"),
                    interaction["subject_title"],
                    interaction["content_summary"],
                    interaction.get("slack_channel_id"),
                    interaction["sentiment_score"],
                    interaction["apartment_industry_relevance"],
                    interaction["business_impact_score"],
                    interaction.get("gong_call_id"),
                    interaction.get("gong_call_duration_seconds"),
                )
            ...
            logger.info(
                f"Sample data imported successfully - {len(contact_ids)} contacts, {len(interaction_ids)} interactions"
            )
    ...
    async def generate_intelligence_report(self):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            report = {
                ...
                "interaction_summary": [
                    dict(summary) for summary in interaction_summary
                ],
                "top_slack_channels": [dict(channel) for channel in top_channels],
                "total_conversations": len(conversations),
                "avg_sentiment_score": sum(
                    c["overall_sentiment_score"] or 0 for c in conversations
                )
                / len(conversations)
                if conversations
                else 0,
                "avg_success_probability": sum(
                    c["success_probability_score"] or 0 for c in conversations
                )
                / len(conversations)
                if conversations
                else 0,
            ...
            logger.info(
                f"Intelligence report generated: {report['total_conversations']} conversations analyzed"
            )
            return report
    ...
    async def run_full_import(self):
        ...
        try:
            ...
            logger.info("Intelligence report saved to sophia_intelligence_report.json")
...
async def main():
    ...
    print(
        f"✅ Sample Data: {report.get('total_conversations', 0)} conversations imported"
    )
    print(
        f"✅ Intelligence Analysis: Average sentiment {report.get('avg_sentiment_score', 0):.2f}"
    )
    print(f"✅ Success Probability: {report.get('avg_success_probability', 0):.2f}")
    print("✅ Report Generated: sophia_intelligence_report.json")
    print("=" * 60)
    print(
        "🚀 Sophia AI is now ready for production with enhanced conversation intelligence!"
    )
