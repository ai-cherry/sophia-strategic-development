...
class GoSlackMCPBridge:
    ...
    def __init__(self, config: GoSlackConfig = None):
        ...
        self.base_url = f"http://{self.config.go_server_host}:{self.config.go_server_port}"
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "-port", str(self.config.go_server_port)
            ]
    ...
    async def _build_go_server(self):
        ...
        try:
            build_cmd = ["go", "build", "-o", "cmd/slack-mcp-server/slack-mcp-server", "./cmd/slack-mcp-server"]
        ...
        try:
            ...
            async with self.session.request(method, url, **kwargs) as response:
                ...
                if response.status == 200:
                    ...
                    raise RuntimeError(f"Go server error {response.status}: {error_text}")
    ...
    async def get_conversations(self, channel_id: str, limit: int = 100, 
    ...
    async def send_message(self, channel: str, text: str, thread_ts: str = None) -> Dict[str, Any]:
    ...
    async def export_conversations_csv(self, channel_id: str, start_date: str = None, 
...
class EnhancedSlackService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("✅ Enhanced Slack service initialized with Go performance boost")
        except Exception as e:
            logger.warning(f"⚠️ Failed to initialize Go bridge, using Python fallback: {e}")
            self.go_bridge = None
    ...
    async def get_channels(self, **kwargs) -> List[Dict[str, Any]]:
        ...
        from backend.services.enhanced_slack_integration_service import SlackIntegrationService
