...
class FaissManager:
    ...
    def _create_new_index(self, date_str: str) -> None:
        ...
        logger.info(
            f"Created new empty index for {date_str} with dimension {dimension}"
        )
    ...
    def save_today_index(self) -> None:
        ...
        if today in self.indexes:
            ...
            logger.debug(
                f"Today's index contains {index_size} vectors and {metadata_count} metadata items"
            )
    ...
    def add_data(
        ...
        logger.debug(
            f"Vector data shape: {vectors.shape}, metadata list length: {len(metadata_list)}"
        )
        ...
        logger.debug(
            f"Current index size before addition: {self.indexes[today].ntotal}"
        )
        ...
        logger.debug(
            f"Current metadata count before addition: {current_metadata_count}"
        )
        ...
        logger.info(
            f"Added {len(vectors)} vectors to index for {today}, new total: {self.indexes[today].ntotal}"
        )
