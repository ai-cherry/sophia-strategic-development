...
class TicketClient:
    ...
    def _create_default_criteria_filters(self) -> list[dict[str, Any]]:
        ...
        """
        one_day_ago = (datetime.now() - timedelta(days=1)).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
        ...
        close_date_filter = {
            "filters": [
                {"propertyName": "closedate", "operator": "GT", "value": one_day_ago}
            ]
        }
    ...
    def _execute_ticket_search_with_retry(
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Executing ticket search with filter groups: {json.dumps(search_request.filter_groups)}"
                )
                ...
                if tickets_dict:
                    logger.debug(
                        f"First ticket pipeline stage: {tickets_dict[0].get('properties', {}).get('hs_pipeline_stage')}"
                    )
                    logger.debug(
                        f"First ticket status: {tickets_dict[0].get('properties', {}).get('hs_ticket_status')}"
                    )

                # Get pagination information
                next_after = None
                if hasattr(search_response, "paging") and hasattr(
                    search_response.paging, "next"
                ):
                    next_after = search_response.paging.next.after

                return {
                    "results": converted_tickets,
                    "pagination": {"next": {"after": next_after}},
                ...
                if e.status == 429 or (e.status >= 500 and e.status < 600):
                    ...
                    logger.warning(
                        f"Rate limit hit or server error ({e.status}). Retrying in {sleep_time:.2f} seconds (attempt {retry_count}/{max_retries})"
                    )
    ...
    def get_conversation_threads(self, ticket_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Found {len(thread_ids)} conversation threads associated with ticket {ticket_id}"
            )
    ...
    def _extract_thread_ids(
        ...
        for conversation in associated_conversations.get("results", []):
            ...
            if "toObjectId" in conversation:
                ...
                logger.warning(
                    f"No 'id' or 'toObjectId' field in conversation: {json.dumps(conversation)}"
                )
    ...
    def _get_thread_messages(
        ...
        for thread_id in thread_ids:
            try:
                ...
                actual_messages = [
                    msg for msg in message_results if msg.get("type") == "MESSAGE"
                ]

                # Format thread with its messages
                formatted_thread = {"id": thread_id, "messages": []}
    ...
    def _determine_sender_type(self, msg: dict[str, Any]) -> str:
        ...
        if msg.get("senders") and len(msg.get("senders")) > 0:
            ...
            if sender.get("senderField") == "FROM" and sender.get(
                "actorId", ""
            ).startswith(("0-1", "0-2")):
                sender_type = "AGENT"
            else:
