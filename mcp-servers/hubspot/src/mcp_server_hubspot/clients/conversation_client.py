...
class ConversationClient:
    ...
    def __init__(
        self, hubspot_client: HubSpot, access_token: str, thread_storage: ThreadStorage
    ):
    ...
    def _format_email_batch(self, batch_response: Any) -> List[Dict[str, Any]]:
        ...
        for email in batch_response.results:
            ...
            formatted_email = {
                ...
                or properties.get("hs_email_html", ""),
            }
    ...
    def _get_threads_data(
        ...
        if (
            not refresh_cache
            and not after
            and self.thread_storage.get_cached_threads().get("results")
        ):
    ...
    def _create_empty_threads_response(
        ...
        return {
            ...
            "pagination": {
                "next": {
                    "after": threads_data.get("paging", {}).get("next", {}).get("after")
                }
            },
        }
    ...
    def _extract_sender_info(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if msg.get("senders") and len(msg.get("senders")) > 0:
            ...
            sender_info = {
                ...
                "email": (
                    sender.get("deliveryIdentifier", {}).get("value", "")
                    if sender.get("deliveryIdentifier", {}).get("type")
                    == "HS_EMAIL_ADDRESS"
                    else ""
                ),
            }
    ...
    def _extract_recipients_info(self, msg: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for recipient in msg.get("recipients", []):
            if (
                recipient.get("deliveryIdentifier", {}).get("type")
                == "HS_EMAIL_ADDRESS"
            ):
                recipients_info.append(
                    {
                        "recipient_field": recipient.get("recipientField", ""),
                        "email": recipient.get("deliveryIdentifier", {}).get(
                            "value", ""
                        ),
                    }
                )
