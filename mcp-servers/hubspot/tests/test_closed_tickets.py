...
class TicketTester:
    ...
    def test_direct_search_api(self):
        ...
        filter_groups_approaches = [
            ...
            [
                {
                    "filters": [
                        {"propertyName": "status", "operator": "EQ", "value": "CLOSED"}
                    ]
                }
            ],
        ...
        for idx, filter_groups in enumerate(filter_groups_approaches):
            ...
            try:
                ...
                if tickets_dict:
                    ...
                    logger.info(
                        f"First ticket sample: {json.dumps(tickets_dict[0], indent=2)}"
                    )
    ...
    def test_get_pipelines(self):
        ...
        try:
            pipelines = self.direct_client.crm.pipelines.pipelines_api.get_all(
                "tickets"
            )
    ...
    if isinstance(lowercase_results, str):
        try:
            ...
            if "error" in lowercase_dict:
                ...
                print(
                    f"Lowercase method: {len(lowercase_dict.get('results', []))} tickets"
                )
