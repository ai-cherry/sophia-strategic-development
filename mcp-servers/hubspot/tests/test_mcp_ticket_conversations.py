...
def test_get_closed_tickets(hubspot_client: HubSpotClient) -> list[dict[str, Any]]:
    ...
    tickets_response = hubspot_client.get_tickets(
        criteria="Closed", limit=20, max_retries=3, retry_delay=1.0
    )
    ...
    for i, ticket in enumerate(tickets):
        ...
        pipeline_stage = ticket.get("properties", {}).get(
...
def test_get_ticket_conversation_threads(
    ...
    """
    logger.info(
        f"=== Testing hubspot_get_ticket_conversation_threads for ticket {ticket_id} ==="
    )

    # Get conversation threads
    try:
        threads_response = hubspot_client.get_ticket_conversation_threads(
            ticket_id=ticket_id
        )
        ...
        logger.info(
            f"Found {total_threads} conversation threads with {total_messages} total messages"
        )
...
def main():
    ...
    for ticket in closed_tickets:
        ...
        logger.info(
            f"Testing conversation threads for ticket: {subject} (ID: {ticket_id})"
        )
