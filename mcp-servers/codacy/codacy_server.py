...
class AdvancedSecurityAnalyzer:
    ...
    async def analyze(self, code: str, filename: str) -> list[CodeIssue]:
        ...
        issues.extend(
            self._analyze_patterns(code, filename, self.infrastructure_patterns)
        )
    ...
    def _analyze_patterns(
        self, code: str, filename: str, patterns: list[dict]
    ) -> list[CodeIssue]:
            ...
            for line_num, line in enumerate(lines, 1):
                if re.search(pattern_info["pattern"], line, re.IGNORECASE):
                    issues.append(
                        CodeIssue(
                            category=IssueCategory(
                                pattern_info.get("category", "security")
                            ),
...
class UnifiedCodeAnalyzer:
    ...
    def _calculate_metrics(
        self, code: str, issues: list[CodeIssue], complexity_metrics: dict
    ) -> CodeMetrics:
        ...
        overall_score = max(
            0, 100 - (critical_issues * 25 + high_issues * 15 + medium_issues * 5)
        )
    ...
    def _generate_suggestions(
        ...
        if critical_issues:
            suggestions.append(
                f"🚨 URGENT: Fix {len(critical_issues)} critical issue(s)"
            )
        ...
        if complexity_issues:
            suggestions.append(
                f"📊 Reduce complexity in {len(complexity_issues)} area(s)"
            )

        infra_issues = [i for i in issues if i.category == IssueCategory.INFRASTRUCTURE]
        if infra_issues:
            suggestions.append(
                f"🏗️ Review {len(infra_issues)} infrastructure pattern(s)"
            )
...
async def analyze_code(
    ...
    try:
        result = await analyzer.analyze_code(
            request.code, request.filename, request.language
        )
...
async def security_scan(
    request: SecurityScanRequest, analyzer: UnifiedCodeAnalyzer = Depends(get_analyzer)
):
    ...
    try:
        ...
        for issue in security_issues:
            severity_summary[issue.severity] = (
                severity_summary.get(issue.severity, 0) + 1
            )
        ...
        if severity_summary.get(SeverityLevel.CRITICAL, 0) > 0:
            recommendations.append(
                f"🚨 URGENT: {severity_summary[SeverityLevel.CRITICAL]} critical security issue(s) require immediate attention"
            )
        if severity_summary.get(SeverityLevel.HIGH, 0) > 0:
            recommendations.append(
                f"⚠️ {severity_summary[SeverityLevel.HIGH]} high-severity security issue(s) should be addressed soon"
            )
    ...
    return {
        ...
        "analysis_stats": {
            ...
            "security_patterns": len(
                app_state.analyzer.security_analyzer.sophia_patterns
            ),
            "infrastructure_patterns": len(
                app_state.analyzer.infrastructure_analyzer.dev_patterns
            ),
            "supported_languages": [
                "python",
                "javascript",
...
async def main():
    ...
    config = uvicorn.Config(
        app=app, host="127.0.0.1"  # Changed from 0.0.0.0 for security. Use environment variable for production, port=3008, log_level="info", access_log=True
    )
