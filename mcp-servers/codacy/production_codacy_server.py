...
class ComplexityAnalyzer:
    ...
    async def analyze(self, code: str, filename: str) -> tuple[List[CodeIssue], Dict[str, int]]:
...
class ProductionCodeAnalyzer:
    ...
    def _calculate_metrics(self, code: str, issues: List[CodeIssue], complexity_metrics: Dict[str, int]) -> CodeMetrics:
        ...
        security_penalty = sum(
            {"critical": 25, "high": 15, "medium": 10, "low": 5, "info": 2}.get(issue.severity.value, 0)
            for issue in issues if issue.category == IssueCategory.SECURITY
        )
        ...
        return CodeMetrics(
            ...
            cyclomatic_complexity=total_complexity / max(complexity_metrics.get("functions", 1), 1),
    ...
    def _generate_suggestions(self, issues: List[CodeIssue], metrics: CodeMetrics) -> List[str]:
        ...
        if security_issues:
            critical_count = len([i for i in security_issues if i.severity == SeverityLevel.CRITICAL])
            if critical_count > 0:
                suggestions.append(f"🚨 URGENT: Fix {critical_count} critical security issue(s)")
            suggestions.append(f"🔒 Address {len(security_issues)} security issue(s) for better protection")
        
        # Complexity suggestions
        complexity_issues = [i for i in issues if i.category == IssueCategory.COMPLEXITY]
        if complexity_issues:
            suggestions.append(f"🔧 Refactor {len(complexity_issues)} complex function(s) for better maintainability")
        ...
        if metrics.overall_score > 80:
            ...
            suggestions.append("⚠️ Consider significant refactoring to improve code quality")
...
async def security_scan(
    ...
    try:
        ...
        if high_count > 0:
            recommendations.append(f"⚠️ {high_count} high-severity security issue(s) should be addressed soon")
