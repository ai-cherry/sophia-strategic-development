...
api_errors = Counter(
    'v0dev_api_errors_total',
    'Total number of V0.dev API errors',
    ['error_type']
)
...
class ComponentRequest(BaseModel):
    ...
    design_context: Optional[Dict[str, Any]] = Field(None, description="Figma design tokens and context")
    component_type: str = Field("react", description="Type of component to generate")
    styling: str = Field("tailwind", description="Styling approach (tailwind, css-modules, styled-components)")
...
class V0DevService:
    ...
    async def stream_component(self, request: StreamComponentRequest):
        ...
        try:
            ...
            async with self.client.stream("POST", V0DEV_API_ENDPOINT, json=payload) as response:
                response.raise_for_status()
    ...
    def _build_enhanced_prompt(self, request) -> str:
        ...
        if hasattr(request, 'design_context') and request.design_context:
            ...
            if "colors" in request.design_context:
                prompt_parts.append(f"Color Palette: {json.dumps(request.design_context['colors'])}")
            
            if "typography" in request.design_context:
                prompt_parts.append(f"Typography: {json.dumps(request.design_context['typography'])}")
            
            if "spacing" in request.design_context:
                prompt_parts.append(f"Spacing: {json.dumps(request.design_context['spacing'])}")
            
            if "components" in request.design_context:
                prompt_parts.append(f"Related Components: {json.dumps(request.design_context['components'])}")
    ...
    def _get_system_prompt(self, request) -> str:
        ...
        type_suffix = "TypeScript" if getattr(request, 'typescript', True) else "JavaScript"
...
Return only the component code without markdown formatting."""
    ...
    async def _generate_tests(self, component_code: str, request: ComponentRequest) -> str:
...
async def startup_event():
    ...
    try:
        ...
        @mcp_server.tool()
        async def streamComponent(prompt: str, design_context: Optional[Dict] = None) -> Dict:
            """Stream component generation for live preview."""
        ...
        @mcp_server.tool()
        async def deployComponent(project_id: str, component_code: str, component_name: str) -> Dict:
...
async def stream_component(request: StreamComponentRequest):
    ...
    return StreamingResponse(
        v0dev_service.stream_component(request),
