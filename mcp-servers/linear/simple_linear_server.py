...
class SimpleLinearManager:
    ...
    def __init__(self):
        self.mock_projects = [
            {"id": "SOPH", "name": "Sophia AI Platform", "status": "active", "progress": 75},
            {"id": "MCP", "name": "MCP Server Infrastructure", "status": "active", "progress": 60},
            {"id": "DEPLOY", "name": "Deployment Pipeline", "status": "completed", "progress": 100}
        ]
        
        self.mock_issues = [
            {"id": "SOPH-001", "title": "Deploy AI Memory MCP server", "status": "done", "priority": "high", "project": "SOPH"},
            {"id": "SOPH-002", "title": "Deploy Codacy MCP server", "status": "in_progress", "priority": "high", "project": "SOPH"},
            {"id": "SOPH-003", "title": "Deploy GitHub MCP server", "status": "in_progress", "priority": "high", "project": "SOPH"},
            {"id": "SOPH-004", "title": "Deploy Linear MCP server", "status": "todo", "priority": "high", "project": "SOPH"},
            {"id": "MCP-001", "title": "Standardize MCP server pattern", "status": "done", "priority": "medium", "project": "MCP"},
            {"id": "DEPLOY-001", "title": "Setup GitHub Actions", "status": "done", "priority": "medium", "project": "DEPLOY"}
        ]
    ...
    async def get_project_info(self, project_id: str) -> Dict[str, Any]:
        ...
        return {
            **project,
            ...
            "completed_issues": len([i for i in project_issues if i["status"] == "done"]),
            "in_progress_issues": len([i for i in project_issues if i["status"] == "in_progress"]),
    ...
    async def get_issues(self, status: str = None, project: str = None) -> List[Dict[str, Any]]:
    ...
    async def create_issue(self, title: str, description: str, priority: str = "medium", project: str = "SOPH") -> Dict[str, Any]:
        """Create a new issue."""
        new_issue = {
    ...
    async def get_team_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_velocity": sum(t["velocity"] for t in self.mock_teams) / len(self.mock_teams),
            "active_projects": len([p for p in self.mock_projects if p["status"] == "active"])
        }
    ...
    async def get_project_health(self, project_id: str = None) -> Dict[str, Any]:
        ...
        for project in projects:
            project_issues = [i for i in self.mock_issues if i.get("project") == project["id"]]
        ...
        return {
            ...
            "overall_health": round(sum(h["health_score"] for h in health_scores) / len(health_scores), 1) if health_scores else 0
        }
