...
class CodeModifierMCPServer(StandardizedMCPServer):
    ...
    async def server_specific_health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                status=HealthStatus.HEALTHY if workspace_exists else HealthStatus.DEGRADED,
                response_time_ms=10,
    ...
    async def modify_file(
        ...
        """
        logger.info(f"Modifying file {file_path} with instructions: {instructions[:100]}...")
    ...
    async def analyze_file(
        ...
        try:
            ...
            validation = await self.code_service._validate_modifications(
                file_path,
                content,
                content  # Same content for analysis
            )
            
            # Calculate metrics
            metrics = self.code_service._calculate_change_metrics(
                content,
                content  # Same content for baseline
            )
