...
class PromptOptimizationRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(default=None, description="Context for optimization")
    optimization_level: str = Field(default="balanced", description="Optimization level: minimal, balanced, aggressive")
...
class PromptOptimizerService:
    ...
    async def analyze_prompt(self, prompt: str) -> PromptAnalysisResult:
        ...
        has_clear_instruction = any(keyword in prompt.lower() for keyword in [
            'explain', 'describe', 'list', 'compare', 'analyze', 'create', 'write'
        ])
        ...
        if structure_score < 0.7:
            suggestions.append("Use bullet points or numbered lists for complex requests")
        if word_count < 10:
    ...
    async def update_template_performance(
        ...
        template.performance_score = (
            template.performance_score * (1 - weight) +
    ...
    try:
        ...
        return {
            ...
            "improvement": {
                ...
                "specificity": analysis_after.specificity_score - analysis_before.specificity_score,
                "structure": analysis_after.structure_score - analysis_before.structure_score,
