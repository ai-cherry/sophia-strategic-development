...
class ConversationHandler(BaseHandler):
    ...
    def get_recent_conversations_schema(self) -> dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "refresh_cache": {
                    "type": "boolean",
                    "description": "Whether to refresh the threads cache (default: false)",
                },
            },
        }
    ...
    def get_recent_conversations(
        ...
        refresh_cache = self.get_argument_with_default(
            arguments, "refresh_cache", False
        )
        ...
        self.logger.debug(
            f"Getting recent conversations with limit={limit}, after={after}, refresh_cache={refresh_cache}"
        )
        results = self.hubspot.get_recent_conversations(
            limit=limit, after=after, refresh_cache=refresh_cache
        )
    ...
    def _store_conversations_in_faiss(
        ...
        try:
            ...
            if data:
                ...
                self.logger.debug(
                    f"Preparing to store {len(data)} conversation threads in FAISS individually"
                )
                for i, thread in enumerate(data):
                    ...
                    self.logger.debug(
                        f"Storing thread {i+1}/{len(data)} with ID {thread_metadata['thread_id']}"
                    )
        ...
        for thread in truncated_results.get("results", []):
            for message in thread.get("messages", []):
                ...
                if "rich_text" in message:
                    message["rich_text"] = (
                        message["rich_text"][:200] if message["rich_text"] else ""
                    )
