...
def store_in_faiss(
    ...
    try:
        logger.debug(
            f"Starting store_in_faiss for {data_type} with {len(data) if data else 0} items"
        )
        ...
        metadata_list = []
        for item in data:
            metadata = {"type": data_type, "data": item}
            if metadata_extras:
...
def search_in_faiss(
    ...
    try:
        ...
        metadata_list, distances = faiss_manager.search(
            query_vector=np.array(query_embedding), k=limit
        )
            ...
            results.append(
                {
                    ...
                    - (distance / 2.0),  # Convert distance to similarity score (0-1)
