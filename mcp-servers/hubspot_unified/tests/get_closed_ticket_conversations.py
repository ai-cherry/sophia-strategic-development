...
def main():
    ...
    # Print summary
    total_threads = sum(
        conv.get("conversation_data", {}).get("total_threads", 0)
        for conv in all_conversations
    )
    total_messages = sum(
        conv.get("conversation_data", {}).get("total_messages", 0)
        for conv in all_conversations
    )

    logger.info(
        f"Summary: {len(closed_tickets)} tickets, {total_threads} threads, {total_messages} messages"
    )
