...
async def setup_claude_environment():
    ...
    if not validation["valid"]:
        print(
            f"❌ Configuration validation failed: {validation.get('error', 'Unknown error')}"
        )
        return False
...
async def test_claude_integration():
    ...
    try:
        response = await claude_integration.send_message(
            "Hello, Claude! Please respond with 'Integration test successful.'"
        )
...
async def start_claude_mcp_server():
    ...
    try:
        # Run the MCP server
        async with claude_mcp_server.server.stdio_server() as (
            read_stream,
            write_stream,
        ):
            await claude_mcp_server.server.run(
                read_stream,
                write_stream,
                claude_mcp_server.server.create_initialization_options(),
            )
    except Exception as e:
