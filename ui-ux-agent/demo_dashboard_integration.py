    ...
    async def _demo_design_analysis(self):
        ...
        try:
            ...
            if analysis_response.status_code == 200:
                ...
                logger.info(f"      ⏱️  Implementation Time: {analysis['estimated_implementation_time']}")
                logger.info(f"      🛠️  Recommended Approach: {analysis['recommended_approach']}")
                logger.info(f"      ♿ Accessibility Score: {len(analysis['accessibility_considerations'])} items")
                logger.info(f"      🚀 Performance Score: {len(analysis['performance_recommendations'])} optimizations")
            else:
                logger.warning(f"   ⚠️  Analysis failed: Status {analysis_response.status_code}")
    ...
    async def _demo_component_generation(self):
        ...
        try:
            ...
            if generation_response.status_code == 200:
                ...
                logger.info(f"      🔧 TypeScript Types: {'typescript_types' in component}")
                ...
                if len(component['component_code'].split('\n')) > 10:
                    logger.info(f"      ... ({len(component['component_code'].split('\n')) - 10} more lines)")
                    
            else:
                logger.warning(f"   ⚠️  Generation failed: Status {generation_response.status_code}")
    ...
    async def _demo_design_system_validation(self):
        ...
        try:
            ...
            if validation_response.status_code == 200:
                ...
                logger.info(f"      🏆 Overall Score: {validation['overall_score']}/100")
                
                for check_name, check_result in validation['compliance_checks'].items():
                    status_icon = "✅" if check_result['status'] == 'passed' else "❌"
                    logger.info(f"      {status_icon} {check_name.title()}: {check_result['score']}/100")
                
                logger.info(f"      🔧 Automated Fixes Available: {validation['automated_fixes_available']}")
                logger.info(f"      💡 Recommendations: {len(validation['recommendations'])} items")
                
            else:
                logger.warning(f"   ⚠️  Validation failed: Status {validation_response.status_code}")
