...
class SnowflakeManager(IntegrationManager):
    ...
    def rotate(self) -> Dict[str, Any]:
        ...
        console.print(
            "[yellow]Snowflake credential rotation must be done manually.[/yellow]"
        )
        ...
        if Confirm.ask("Have you reset the password in Snowflake?"):
            ...
            if new_password:
                if self._update_env_file({"SNOWFLAKE_PASSWORD": new_password}):
                    ...
                    result["details"][
                        "message"
                    ] = "Failed to update password in .env file"
    ...
    def configure(self) -> Dict[str, Any]:
        ...
        config = {}
        config["SNOWFLAKE_ACCOUNT"] = Prompt.ask(
            "Snowflake Account", default=os.getenv("SNOWFLAKE_ACCOUNT", "")
        )
        config["SNOWFLAKE_USER"] = Prompt.ask(
            "Snowflake User", default=os.getenv("SNOWFLAKE_USER", "")
        )
        config["SNOWFLAKE_PASSWORD"] = Prompt.ask(
            "Snowflake Password", password=True, default=""
        )
        config["SNOWFLAKE_WAREHOUSE"] = Prompt.ask(
            "Snowflake Warehouse",
            default=os.getenv("SNOWFLAKE_WAREHOUSE", "SOPHIA_DEV_WH"),
        )
        config["SNOWFLAKE_DATABASE"] = Prompt.ask(
            "Snowflake Database", default=os.getenv("SNOWFLAKE_DATABASE", "SOPHIA_DEV")
        )
        config["SNOWFLAKE_SCHEMA"] = Prompt.ask(
            "Snowflake Schema", default=os.getenv("SNOWFLAKE_SCHEMA", "SOPHIA_MAIN")
        )
        config["SNOWFLAKE_ROLE"] = Prompt.ask(
            "Snowflake Role", default=os.getenv("SNOWFLAKE_ROLE", "SOPHIA_DEV_ROLE")
        )
...
class GongManager(IntegrationManager):
    ...
    def __init__(self):
        super().__init__("gong")
        self.required_vars = ["GONG_API_KEY", "GONG_API_SECRET"]
    ...
    def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get("https://api.gong.io/v2/users", headers=headers)
            ...
            if response.status_code == 200:
                ...
                result["status"] = "connected"
                logger.info(
                    f"Gong API connection successful: {result['details']['users_count']} users found"
                )
            else:
                result["status"] = "error"
                result["details"][
                ...
                logger.error(
                    f"Gong API connection failed: {response.status_code} {response.text}"
                )
    ...
    def rotate(self) -> Dict[str, Any]:
        ...
        console.print(
            "[yellow]Gong credential rotation must be done manually.[/yellow]"
        )
        console.print("Please follow these steps:")
        ...
        if Confirm.ask("Have you generated new credentials in the Gong dashboard?"):
            ...
            if new_api_key and new_api_secret:
                ...
                if self._update_env_file(updates):
                    ...
                    result["details"][
                        "message"
                    ] = "Failed to update credentials in .env file"
    ...
    def configure(self) -> Dict[str, Any]:
        ...
        # Collect configuration
        config = {}
        config["GONG_API_KEY"] = Prompt.ask(
            "Gong API Key", default=os.getenv("GONG_API_KEY", "")
        )
        config["GONG_API_SECRET"] = Prompt.ask(
            "Gong API Secret", password=True, default=""
        )
...
class VercelManager(IntegrationManager):
    ...
    def __init__(self):
        ...
        self.required_vars = ["VERCEL_API_TOKEN", "VERCEL_TEAM_ID", "VERCEL_PROJECT_ID"]
            ...
            if response.status_code == 200:
                ...
                if os.getenv("VERCEL_TEAM_ID"):
                    ...
                    if team_response.status_code == 200:
                        ...
                        result["details"][
                            "team_error"
                        ] = f"API returned {team_response.status_code}: {team_response.text}"

                result["status"] = "connected"
                logger.info(
                    f"Vercel API connection successful: {result['details']['user']}"
                )
            else:
                result["status"] = "error"
                result["details"][
                ...
                logger.error(
                    f"Vercel API connection failed: {response.status_code} {response.text}"
                )
    ...
    def rotate(self) -> Dict[str, Any]:
        ...
        console.print(
            "[yellow]Vercel credential rotation must be done manually.[/yellow]"
        )
        console.print("Please follow these steps:")
    ...
    def configure(self) -> Dict[str, Any]:
        ...
        # Collect configuration
        config = {}
        config["VERCEL_API_TOKEN"] = Prompt.ask(
            "Vercel API Token", password=True, default=""
        )
        config["VERCEL_TEAM_ID"] = Prompt.ask(
            "Vercel Team ID", default=os.getenv("VERCEL_TEAM_ID", "")
        )
        config["VERCEL_PROJECT_ID"] = Prompt.ask(
            "Vercel Project ID", default=os.getenv("VERCEL_PROJECT_ID", "")
        )
...
class EstuaryManager(IntegrationManager):
    ...
    def __init__(self):
        ...
        self.required_vars = ["ESTUARY_API_KEY", "ESTUARY_API_URL"]
    ...
    def check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(f"{api_url}/v1/collections", headers=headers)
            ...
            if response.status_code == 200:
                ...
                result["status"] = "connected"
                logger.info(
                    f"Estuary API connection successful: {result['details']['total_files']} files"
                )
            else:
                result["status"] = "error"
                result["details"][
                ...
                logger.error(
                    f"Estuary API connection failed: {response.status_code} {response.text}"
                )
    ...
    def rotate(self) -> Dict[str, Any]:
        ...
        console.print(
            "[yellow]Estuary credential rotation must be done manually.[/yellow]"
        )
        console.print("Please follow these steps:")
    ...
    def configure(self) -> Dict[str, Any]:
        ...
        # Collect configuration
        config = {}
        config["ESTUARY_API_KEY"] = Prompt.ask(
            "Estuary API Key", password=True, default=""
        )
        config["ESTUARY_API_URL"] = Prompt.ask(
            "Estuary API URL",
            default=os.getenv("ESTUARY_API_URL", "https://api.estuary.tech"),
        )
...
class MCPManager(IntegrationManager):
    ...
    def __init__(self):
        super().__init__("mcp")
        self.required_vars = ["MCP_CONFIG_PATH"]
    ...
    def rotate(self) -> Dict[str, Any]:
        ...
        console.print(
            "MCP configuration does not contain credentials that need rotation."
        )
    ...
    def configure(self) -> Dict[str, Any]:
        ...
        # Collect configuration
        config = {}
        config["MCP_CONFIG_PATH"] = Prompt.ask(
            "MCP Config Path", default=os.getenv("MCP_CONFIG_PATH", "mcp_config.json")
        )
        ...
        if not os.path.exists(config["MCP_CONFIG_PATH"]):
            ...
            if Confirm.ask(
                f"Config file {config['MCP_CONFIG_PATH']} does not exist. Create it?"
            ):
                try:
                    ...
                    result["details"][
                        "error"
                    ] = f"Failed to create config file: {str(e)}"
                    return result
    ...
    for name, result in results.items():
        ...
        if status == "connected":
            if name == "snowflake":
                ...
                details.append(
                    f"Warehouse: {result['details'].get('warehouse', 'N/A')}"
                )
            elif name == "gong":
                ...
                details.append(
                    f"Servers: {result['details'].get('servers_count', 'N/A')}"
                )

        if "missing_vars" in result["details"]:
            details.append(
                f"[yellow]Missing: {', '.join(result['details']['missing_vars'])}[/yellow]"
            )
...
def main():
    ...
    if not os.path.exists(".env"):
        ...
        if os.path.exists("integration.env.example"):
            try:
                with open("integration.env.example", "r") as src, open(
                    ".env", "w"
                ) as dst:
                    dst.write(src.read())
                console.print("[green]Created .env file from template.[/green]")
                console.print(
                    "[yellow]Please edit .env file to add your credentials.[/yellow]"
                )
                dotenv.load_dotenv()
            except Exception as e:
    ...
    if args.action == "check":
        console.print(
            f"[blue]Checking {'all' if args.service == 'all' else args.service} integration(s)...[/blue]"
        )
        ...
        if args.service == "all":
            console.print(
                "[yellow]Rotating credentials for all services is not recommended.[/yellow]"
            )
        ...
        if manager:
            ...
            if result["status"] == "rotated":
                console.print(
                    f"[green]Credentials for {args.service} rotated successfully.[/green]"
                )
            elif result["status"] == "not_supported":
                console.print(
                    f"[yellow]Automatic credential rotation not supported for {args.service}.[/yellow]"
                )
            else:
                console.print(
                    f"[red]Failed to rotate credentials for {args.service}: {result['details'].get('message', 'Unknown error')}[/red]"
                )
        ...
        if args.service == "all":
            console.print(
                "[yellow]Configuring all services at once is not supported.[/yellow]"
            )
        ...
        if manager:
            ...
            if result["status"] == "configured":
                ...
                console.print(
                    f"[red]Failed to configure {args.service}: {result['details'].get('error', 'Unknown error')}[/red]"
                )
