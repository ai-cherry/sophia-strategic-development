...
class AlignmentIssuesFixer:
    ...
    def fix_estuary_credentials(self) -> bool:
        ...
        try:
            ...
            self.execute_sql(
                "DELETE FROM PLATFORM_CREDENTIALS WHERE PLATFORM_NAME = 'ESTUARY'"
            )
            ...
            credentials = [
                (
                    ...
                    "Estuary Client ID for API access",
                ),
                (
                    "ESTUARY",
                    "CLIENT_SECRET",
                    self.estuary_credentials["client_secret"],
                    "Estuary Client Secret for API access",
                ),
                (
                    "ESTUARY",
                    "ACCESS_TOKEN",
                    self.estuary_credentials["access_token"],
                    "Estuary Access Token for API access",
                ),
                (
                    "ESTUARY",
                    "CLIENT_ID_NEW",
                    self.estuary_credentials["client_id_new"],
                    "Estuary New Client ID for API access",
                ),
                (
                    "ESTUARY",
                    "CLIENT_SECRET_NEW",
                    self.estuary_credentials["client_secret_new"],
                    "Estuary New Client Secret for API access",
                ),
            ]
            ...
            for platform, cred_type, cred_value, description in credentials:
                ...
                self.execute_sql(
                    insert_sql, [platform, cred_type, cred_value, metadata]
                )
            ...
            # Fix API endpoints
            self.execute_sql(
                "DELETE FROM API_ENDPOINTS WHERE PLATFORM_NAME = 'ESTUARY'"
            )
            ...
            for platform, endpoint_name, url, method, description in endpoints:
                ...
                self.execute_sql(
                    insert_sql, [platform, endpoint_name, url, method, description]
                )
    ...
    def validate_fixes(self) -> bool:
        ...
        try:
            ...
            # Query test memory
            test_query = (
                "SELECT COUNT(*) as count FROM MEMORY_RECORDS WHERE CATEGORY = 'TEST'"
            )
