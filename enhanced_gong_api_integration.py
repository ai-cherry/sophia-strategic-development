...
class EnhancedGongIntegration:
    ...
    async def setup_enhanced_schema(self):
        ...
        try:
            ...
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_transcripts_call_id ON conversation_transcripts(call_id)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_transcripts_apartment_score ON conversation_transcripts(apartment_relevance_score)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_media_call_id ON conversation_media(call_id)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_stats_call_id ON interaction_statistics(call_id)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_webhooks_event_type ON webhook_events(event_type)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_analytics_call_id ON enhanced_call_analytics(call_id)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_analytics_apartment_score ON enhanced_call_analytics(apartment_relevance_score)"
            )
    ...
    async def get_conversation_transcript(
        ...
        try:
            ...
            result = await self.oauth_manager.make_authenticated_request(
                f"/v2/calls/{call_id}/transcript", "GET"
            )
            ...
            if result["success"]:
                ...
                logger.error(
                    f"Failed to get transcript for call {call_id}: {result['error']}"
                )
                return None
    ...
    async def get_conversation_media(self, call_id: str) -> Optional[MediaFile]:
        ...
        try:
            ...
            result = await self.oauth_manager.make_authenticated_request(
                f"/v2/calls/{call_id}/media-url", "GET"
            )
            ...
            if result["success"]:
                ...
                logger.error(
                    f"Failed to get media for call {call_id}: {result['error']}"
                )
                return None
    ...
    async def get_interaction_statistics(
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.error(
                    f"Failed to get interaction stats for call {call_id}: {result['error']}"
                )
                return None

        except Exception as e:
            logger.error(
                f"Error getting interaction stats for call {call_id}: {str(e)}"
            )
            return None
    ...
    async def get_extensive_call_data(self, call_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await self.oauth_manager.make_authenticated_request(
                f"/v2/calls/{call_id}/extensive", "GET"
            )
            ...
            if result["success"]:
                ...
                logger.error(
                    f"Failed to get extensive data for call {call_id}: {result['error']}"
                )
                return None
    ...
    async def process_all_conversations_enhanced(
        self, limit: int = 100
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": "Processing failed", "details": str(e)}
    ...
    async def _store_extensive_call_data(
        self, call_id: str, extensive_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            apartment_score = await self._calculate_apartment_relevance_from_data(
                extensive_data
            )
    ...
    async def _calculate_apartment_relevance_from_data(
        ...
        for topic in topics:
            ...
            if any(
                ...
                for keyword in ["apartment", "rental", "lease", "property"]
            ):
        ...
        for participant in participants:
            ...
            if any(
                ...
                for indicator in ["property", "apartment", "management", "real estate"]
            ):
                apartment_indicators += 1
    ...
    def _extract_deal_stage(self, data: Dict[str, Any]) -> str:
        ...
        for topic in topics:
            ...
            if any(
                keyword in topic_text
                for keyword in ["demo", "presentation", "overview"]
            ):
                return "Discovery"
            elif any(
                keyword in topic_text for keyword in ["proposal", "pricing", "quote"]
            ):
                return "Evaluation"
            elif any(
                keyword in topic_text for keyword in ["contract", "agreement", "terms"]
            ):
                return "Negotiation"
            elif any(
                keyword in topic_text for keyword in ["signature", "close", "finalize"]
            ):
                return "Closing"
    ...
    def _calculate_win_probability(self, data: Dict[str, Any]) -> float:
        ...
        for topic in topics:
            ...
            if any(
                positive in topic_text
                for positive in ["interested", "excited", "perfect", "exactly"]
            ):
                probability_factors.append(0.8)
            elif any(
                neutral in topic_text
                for neutral in ["consider", "think about", "discuss"]
            ):
                probability_factors.append(0.5)
            elif any(
                negative in topic_text
                for negative in ["expensive", "budget", "concern"]
            ):
            ...
            if "transcript" in endpoint:
                return {
                    ...
                    "data": {
                        "entries": [
                            {
                                "text": "We're looking for apartment management software",
                                "speaker": "Customer",
                            },
                            {
                                "text": "Pay Ready can help with that",
                                "speaker": "Sales Rep",
                            },
                        ],
                        "speakers": [
