...
class DocumentationCleaner:
            ...
            for group_name, group_config in self.consolidation_groups.items():
                if re.match(group_config["pattern"], file_name, re.IGNORECASE):
                    ...
                    if file_name not in group_config["keep_separate"]:
                        analysis["consolidation_candidates"][group_name].append(str(relative_path))
    ...
    def consolidate_documentation_group(self, group_name: str, files: List[str]) -> bool:
        ...
        try:
            ...
            combined_content = []
            combined_content.append(f"# {group_name.title().replace('_', ' ')} Master Guide")
            ...
            for directory in directories:
                ...
                if not readme_path.exists():
                    with open(readme_path, 'w') as f:
                        ...
                        f.write(f"Documentation for {directory.replace('-', ' ').lower()}.\n")
