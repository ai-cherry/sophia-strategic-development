...
class SophiaGongDataExtractorFixed:
    ...
    def __init__(self):
        ...
        if not self.access_key or not self.access_secret:
            logger.error(
                "GONG_ACCESS_KEY or GONG_CLIENT_SECRET environment variables not set."
            )
        ...
        try:
            ...
            extraction_results["extraction_summary"] = {
                ...
                "api_endpoints_working": [
                    endpoint
                    for endpoint in ["users", "calls", "workspaces"]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for user in users:
                    ...
                    enhanced_user[
                        "apartment_relevance"
                    ] = self.calculate_user_apartment_relevance_safe(user)
                    enhanced_user[
                        "sophia_profile"
                    ] = self.generate_user_sophia_profile_safe(user)
    ...
    async def extract_calls_fixed(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/calls", headers=self.headers, json=params, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                for call in calls:
                    call[
                        "apartment_relevance"
                    ] = self.calculate_call_apartment_relevance_safe(call)
                    call["sophia_insights"] = self.generate_sophia_insights_safe(call)
                    call[
                        "business_intelligence"
                    ] = self.extract_business_intelligence_safe(call)
            ...
            if response.status_code == 200:
                ...
                for workspace in workspaces:
                    workspace_analysis[workspace.get("id", "unknown")] = {
                        ...
                        "apartment_focus": self.analyze_workspace_apartment_focus_safe(
                            workspace
                        ),
                    }
    ...
    def extract_business_intelligence_safe(
    ...
    def generate_user_sophia_profile_safe(self, user: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Role categorization
            if any(
                role in title for role in ["sales", "account", "business development"]
            ):
                profile["role_category"] = "sales"
            elif any(
                role in title
                for role in ["manager", "director", "vp", "president", "ceo"]
            ):
            ...
            for user in users:
                ...
                if profile.get("apartment_focus", False):
                    role_analysis["apartment_focused_roles"][role_category] = (
                        role_analysis["apartment_focused_roles"].get(role_category, 0)
                        + 1
                    )
    ...
    def analyze_workspace_apartment_focus_safe(
    ...
    async def store_data_in_sophia(
        ...
        try:
            ...
            storage_summary = {"users_stored": 0, "calls_stored": 0, "errors": []}
