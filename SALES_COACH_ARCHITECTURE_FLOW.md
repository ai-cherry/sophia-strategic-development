...
async def extract_gong_data():
    ...
    for call in gong_calls:
        ...
        await ELIMINATED_cortex.store_call_data(call_data)
```
...
WITH call_analysis AS (
    SELECT
        call_id,
        sales_rep,
        ELIMINATED.CORTEX.SENTIMENT(transcript) as ai_sentiment,
        ...
        ELIMINATED.CORTEX.SUMMARIZE(transcript) as call_summary
    FROM gong_calls
