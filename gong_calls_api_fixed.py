...
class GongCallsAPIFixed:
            ...
            if response.status_code == 200:
                ...
                for call in calls[:10]:  # Limit to first 10 for testing
                    ...
                    if call_id:
                        ...
                        call[
                            "apartment_relevance"
                        ] = self.calculate_apartment_relevance(call)
                ...
                return {
                    ...
                    "apartment_relevant_calls": len(
                        [
                            c
                            for c in enhanced_calls
                            if c.get("apartment_relevance", 0) > 0.5
                        ]
                    ),
                    "high_value_calls": len(
                        [
                            c
                            for c in enhanced_calls
                            if c.get("apartment_relevance", 0) > 0.8
                        ]
                    ),
                }
            else:
    ...
    async def get_extensive_call_data(self, call_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Could not get extensive data for call {call_id}: {response.status_code}"
                )
                return None
    ...
    def generate_sophia_insights(self, call: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if apartment_relevance > 0.8:
                ...
                insights["opportunity_indicators"].append(
            ...
            if any(word in title for word in ["demo", "demonstration", "presentation"]):
                ...
                word in title for word in ["discovery", "initial", "introduction"]
            ):
            ...
            if any(
                word in title
                for word in ["decision", "maker", "owner", "ceo", "president"]
            ):
    ...
    def extract_business_intelligence(self, call: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(
                word in title for word in ["luxury", "high-end", "premium", "class a"]
            ):
                bi_data["market_segment"] = "luxury"
            elif any(
                word in title
                for word in ["affordable", "budget", "low-income", "class c"]
            ):
            ...
            if any(
                word in title for word in ["enterprise", "large", "portfolio", "reit"]
            ):
                bi_data["company_size"] = "enterprise"
            elif any(
                word in title for word in ["small", "independent", "single", "family"]
            ):
    ...
    async def store_calls_in_database(
        self, calls_data: Dict[str, Any]
    ) -> Dict[str, Any]:
                            ...
                            if isinstance(party, dict):
                                try:
                                    ...
                                    storage_summary["errors"].append(
                                        f"Error storing participant: {str(e)}"
                                    )
...
async def test_fixed_gong_calls_api():
    ...
    if calls_result["success"]:
        ...
        logger.info(
            f"Enhanced {calls_result['enhanced_count']} calls with apartment analysis"
        )
        logger.info(
            f"Found {calls_result['apartment_relevant_calls']} apartment-relevant calls"
        )
        logger.info(
            f"Found {calls_result['high_value_calls']} high-value opportunities"
        )
        ...
        if storage_result["success"]:
            ...
            logger.info(
                f"✅ Stored {storage_result['participants_stored']} participants"
            )
            logger.info(
                f"✅ Generated {storage_result['insights_stored']} business insights"
            )
        else:
            logger.error(
        ...
        logger.error(
            f"❌ Calls extraction failed: {calls_result.get('error', 'Unknown error')}"
        )
