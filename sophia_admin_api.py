...
try:
    ...
    logger.warning(
        "gong_oauth_application.py not found or GongOAuthHandler could not be imported. OAuth endpoints will not function."
    )
...
class SophiaDatabase:
        ...
        try:
            ...
            if filters:
                ...
                if filters.get("min_relevance"):
                    count_param_count += 1
                    count_sql += (
                        f" AND c.apartment_relevance_score >= ${count_param_count}"
                    )
                ...
                if filters.get("deal_stage"):
                    count_param_count += 1
                    count_sql += (
                        f" AND ds.deal_progression_stage = ${count_param_count}"
                    )
                ...
                conversations.append(
                    {
                        ...
                        "apartment_relevance_score": float(
                            row["apartment_relevance_score"]
                        )
                        if row["apartment_relevance_score"]
                        else 0,
                        "business_impact_score": float(row["business_impact_score"])
                        if row["business_impact_score"]
                        else 0,
                        "ai_summary": row["ai_summary"],
                        "deal_health_score": float(row["deal_health_score"])
                        if row["deal_health_score"]
                        else 0,
    ...
    async def get_conversation_details(self, call_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            conversation = {
                ...
                "url": call_row["url"],
                "started": call_row["started"].isoformat()
                if call_row["started"]
                else None,
                ...
                "apartment_relevance_score": float(
                    call_row["apartment_relevance_score"]
                )
                if call_row["apartment_relevance_score"]
                else 0,
                "business_impact_score": float(call_row["business_impact_score"])
                if call_row["business_impact_score"]
                else 0,
                "intelligence": {
                    "ai_summary": call_row["ai_summary"],
                    "confidence_level": float(call_row["confidence_level"])
                    if call_row["confidence_level"]
                    else 0,
                    "key_insights": json.loads(call_row["key_insights"])
                    if call_row["key_insights"]
                    else {},
                    "recommended_actions": json.loads(call_row["recommended_actions"])
                    if call_row["recommended_actions"]
                    else [],
                    "deal_health_score": float(call_row["deal_health_score"])
                    if call_row["deal_health_score"]
                    else 0,
                },
                "apartment_analysis": {
                    "market_segment": call_row["market_segment"],
                    "apartment_terminology_count": call_row[
                        "apartment_terminology_count"
                    ],
                    "industry_relevance_factors": json.loads(
                        call_row["industry_relevance_factors"]
                    )
                    if call_row["industry_relevance_factors"]
                    else {},
                },
                "deal_signals": {
                    "positive_signals": json.loads(call_row["positive_signals"])
                    if call_row["positive_signals"]
                    else [],
                    "negative_signals": json.loads(call_row["negative_signals"])
                    if call_row["negative_signals"]
                    else [],
                    "deal_stage": call_row["deal_progression_stage"],
                    "win_probability": float(call_row["win_probability"])
                    if call_row["win_probability"]
                    else 0,
                },
                "competitive_intelligence": {
                    "competitors_mentioned": call_row["competitors_mentioned"]
                    if call_row["competitors_mentioned"]
                    else [],
                    "threat_level": call_row["competitive_threat_level"],
                    "win_probability_impact": float(call_row["win_probability_impact"])
                    if call_row["win_probability_impact"]
                    else 0,
                },
                "participants": [
                    {
                        ...
                        "talk_time_percentage": float(p["talk_time_percentage"])
    ...
    async def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["total_emails"] = await self.connection.fetchval(
                "SELECT COUNT(*) FROM gong_emails"
            )
            ...
            stats["deal_stages"] = {
                row["deal_progression_stage"]: row["count"] for row in deal_stages
            }
...
current_schema_config = {
    ...
    "mappings": [
        {
            "gong_field": "call.title",
            "sophia_field": "conversations.title",
            "type": "string",
            "transformation": "direct",
        },
        {
            "gong_field": "call.duration",
            "sophia_field": "conversations.duration_minutes",
            "type": "integer",
            "transformation": "seconds_to_minutes",
        },
    ],
    "definitions": {
...
def get_schema_config():
    ...
    try:
        ...
        return jsonify({"success": True, "config": current_schema_config})
    except Exception as e:
    ...
    try:
        ...
        if "mappings" not in data or "definitions" not in data:
            return (
                jsonify(
                    {
                        "error": "Missing 'mappings' or 'definitions' in request",
                        "success": False,
                    }
                ),
                400,
        ...
        current_schema_config["version"] = str(
            float(current_schema_config.get("version", "1.0")) + 0.1
...
def process_natural_language_schema_query():
    ...
    try:
        ...
        if "map gong call titles to apartment relevance scores" in nl_query.lower():
            ...
            interpretation["entities"] = [
                {"type": "condition", "value": "competitor mentions"}
            ]
            interpretation["parsed_action"] = {
    ...
    def gong_oauth_login():
        ...
        if not gong_oauth_handler.client_id or not gong_oauth_handler.redirect_uri:
            return (
                jsonify(
                    {
                        "error": "Gong OAuth is not configured properly on the server.",
                        "success": False,
                    }
                ),
                500,
        ...
        if authorization_url == "/error_oauth_misconfigured":
            return (
                jsonify(
                    {
                        "error": "Gong OAuth is not configured properly on the server (URL generation failed).",
                        "success": False,
                    }
                ),
                500,
    ...
    def gong_oauth_callback():
        ...
        if error:
            ...
            return (
                jsonify(
                    {
                        "error": f"Gong OAuth failed: {error}",
        ...
        if "error" in tokens:
            logger.error(
                f"Failed to exchange Gong auth code for tokens: {tokens.get('details', tokens['error'])}"
            )
            return (
                jsonify(
                    {
                        "error": "Failed to obtain Gong tokens.",
                        "details": tokens.get("details"),
                        "success": False,
                    }
                ),
                500,
        ...
        logger.info(
            "Gong OAuth successful. Tokens obtained (details omitted for security)."
        )
