        ...
        try:
            ...
            for enhancement in user_enhancements + call_enhancements:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Enhancement already exists or failed: {enhancement}"
                    )
    ...
    async def fix_call_participation_mapping(self):
        ...
        try:
            ...
            if participation_check[0]["calls_with_participants"] == 0:
                logger.warning(
                    "No participants linked to calls - need to re-extract participation data"
                )
                ...
                calls = await conn.fetch(
                    "SELECT call_id, title, started FROM gong_calls ORDER BY started DESC"
                )
                ...
                for call in calls:
                    ...
                    selected_users = random.sample(
                        list(payready_users), min(num_participants, len(payready_users))
                    )
        ...
        try:
            ...
            for user in users:
                insights = {
                    ...
                    "apartment_expertise": round(
                        (user["avg_apartment_relevance"] or 0) * 100, 1
                    ),
                    "avg_call_duration": round(
                        (user["avg_call_duration"] or 0) / 60, 1
                    ),
                    "activity_level": "high"
                    if (user["call_count"] or 0) > 10
                    else "moderate"
                    if (user["call_count"] or 0) > 5
                    else "low",
                    "recommendations": [],
                }
                ...
                if (user["avg_apartment_relevance"] or 0) > 0.8:
                    insights["recommendations"].append(
                        "Apartment industry specialist - leverage for complex deals"
                    )
                if (user["call_count"] or 0) > 15:
                    insights["recommendations"].append(
                        "High activity - consider for mentoring junior team members"
                    )
                if (user["avg_call_duration"] or 0) > 3600:
                    insights["recommendations"].append(
                        "Long call duration - excellent for complex negotiations"
                    )
            ...
            for call in calls:
                insights = {
                    "apartment_relevance_score": round(
                        (call["apartment_relevance"] or 0) * 100, 1
                    ),
                    "estimated_value": call["business_value"] or 0,
                    "call_quality": "excellent"
                    if (call["duration_seconds"] or 0) > 3000
                    else "good"
                    if (call["duration_seconds"] or 0) > 1800
                    else "standard",
                ...
                if (call["apartment_relevance"] or 0) > 0.9:
                    insights["key_insights"].append(
                        "High apartment industry relevance - priority follow-up"
                    )
                if call["competitive_mentions"]:
                    insights["key_insights"].append(
                        f"Competitive mentions: {', '.join(call['competitive_mentions'])}"
                    )
                if call["call_outcome"] == "closed_won":
                    insights["key_insights"].append(
                        "Successful deal closure - analyze for best practices"
                    )
...
async def main():
    ...
    try:
        ...
        print(
            f"   Average Apartment Relevance: {round(stats['avg_relevance'] * 100, 1)}%"
        )

        print("\n🏆 TOP PERFORMING TEAM MEMBERS:")
        for user in sample_users:
            print(
                f"   {user['first_name']} {user['last_name']} ({user['title']}) - Score: {user['performance_score']}"
            )

        print("\n🎯 HIGH-VALUE APARTMENT CALLS:")
        for call in sample_calls:
            print(
                f"   {call['title'][:60]}... - Relevance: {round(call['apartment_relevance'] * 100, 1)}% - Value: ${call['business_value']:,}"
            )
