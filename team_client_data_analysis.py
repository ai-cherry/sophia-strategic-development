    ...
    async def analyze_user_data(self) -> dict:
        ...
        try:
            ...
            for domain, data in domain_analysis.items():
                if any(
                    keyword in domain.lower() for keyword in ["payready", "pay-ready"]
                ):
...
async def main():
    ...
    if "error" not in user_analysis:
        ...
        for domain, data in user_analysis["domain_analysis"].items():
            ...
            if data["likely_type"] == "pay_ready_team":
                print("      🏢 Potential Pay Ready team members:")
        ...
        for participant in user_analysis["participation_analysis"][:10]:
            print(
                f"   {participant['first_name']} {participant['last_name']}: {participant['total_calls']} calls"
            )
            print(
                f"      Pay Ready calls: {participant['payready_calls']}, Apartment calls: {participant['apartment_calls']}"
            )
    ...
    if "error" not in schema_review:
        print("✅ Schema review complete!")
        print("📋 CURRENT TABLES:")

        for table, info in schema_review["current_schema"].items():
            print(
                f"   {table}: {info['record_count']} records, {len(info['columns'])} columns"
            )
    ...
    print(
        "1. What email domains do Pay Ready team members use? (@payready.com, @payready.ai, etc.)"
    )
    print(
        "2. Can you provide names of key Pay Ready team members to verify our classification?"
    )
    print("3. Should we create separate tables for internal team vs external clients?")
    print("4. Do you want to track call outcomes and sales pipeline stages?")
    print("5. Should we implement automatic apartment industry client identification?")
