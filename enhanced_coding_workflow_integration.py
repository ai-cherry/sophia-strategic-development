...
- Executive summary interpretation""",
    ...
    async def generate_enhanced_prompt(
        ...
        # Combine all context
        full_context = {**context, **sophia_context, **(platform_context or {})}
        ...
        try:
            ...
            if "business_query" in context:
                bi_response = requests.post(
                    ...
                    json={
                        "message": context["business_query"],
                        "context": "business_intelligence",
                    },
                    timeout=10,
                )
                if bi_response.status_code == 200:
                    business_context["business_intelligence"] = bi_response.json().get(
                        "response", ""
                    )
    ...
    async def enhanced_code_generation(
        ...
        enhanced_prompt = await self.generate_enhanced_prompt(
            prompt_name, context, platform_context
        )
...
async def main():
    ...
    gen_parser = subparsers.add_parser(
        "generate", help="Generate code with enhanced prompts"
    )
