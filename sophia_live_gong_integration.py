        ...
        try:
            async with self.session.get(f"{self.base_url}/v2/workspaces") as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"✅ Retrieved {len(data.get('workspaces', []))} workspaces"
                    )
                    return data
                else:
                    error_text = await response.text()
                    logger.error(
                        f"❌ Workspaces request failed: {response.status} - {error_text}"
                    )
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Users request failed: {response.status} - {error_text}"
                    )
                    return {"error": f"HTTP {response.status}: {error_text}"}
        except Exception as e:
    ...
    async def get_calls(
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/v2/calls", json=payload
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Calls request failed: {response.status} - {error_text}"
                    )
                    return {"error": f"HTTP {response.status}: {error_text}"}
        except Exception as e:
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"❌ Call details request failed: {response.status} - {error_text}"
                    )
                    return {"error": f"HTTP {response.status}: {error_text}"}
        except Exception as e:
...
class ApartmentIndustryAnalyzer:
            ...
            if competitor.lower() in text_lower:
                ...
                for match in matches:
                    competitive_contexts.append(
                        {"competitor": competitor, "context": match.strip()}
                    )
    ...
    def analyze_deal_signals(self, text: str) -> Dict[str, Any]:
        ...
        found_positive = [
            signal for signal in self.positive_signals if signal in text_lower
        ]
        found_negative = [
            signal for signal in self.negative_signals if signal in text_lower
        ]
        ...
        win_probability = max(
            0.0, min(1.0, base_probability + positive_weight + negative_weight)
        )

        # Determine deal stage
        deal_stage = "discovery"
        if any(
            signal in text_lower for signal in ["budget approved", "timeline confirmed"]
        ):
            deal_stage = "evaluation"
        if any(
            signal in text_lower
            for signal in ["ready to move forward", "when can we start"]
        ):
            deal_stage = "negotiation"
        if any(
            signal in text_lower
            for signal in ["implementation", "next steps", "contract"]
        ):
            deal_stage = "closing"
    ...
    def calculate_business_impact(
        ...
        if competitive_analysis["competitive_intensity"] > 0:
            competitive_score = min(
                0.3, competitive_analysis["competitive_intensity"] * 0.1
            )
    ...
    def _calculate_confidence(
        ...
        total_signals = len(deal_signals.get("positive_signals", [])) + len(
            deal_signals.get("negative_signals", [])
        )
        if total_signals >= 3:
            confidence_factors.append(0.9)
...
class SophiaDataPopulator:
    ...
    async def populate_workspaces(self, workspaces_data: Dict[str, Any]) -> int:
        ...
        for workspace in workspaces:
            try:
                ...
                logger.error(
                    f"❌ Error populating workspace {workspace.get('id', 'unknown')}: {e}"
                )
    ...
    async def populate_users(
        self, users_data: Dict[str, Any], workspace_id: str = None
    ) -> int:
        ...
        for user in users:
            try:
                ...
                logger.error(
                    f"❌ Error populating user {user.get('id', 'unknown')}: {e}"
                )
    ...
    async def populate_calls(
        self, calls_data: Dict[str, Any], workspace_id: str = None
    ) -> int:
        ...
        for call in calls:
            try:
                ...
                apartment_relevance = self.analyzer.analyze_apartment_relevance(
                    analysis_text
                )
                competitive_analysis = self.analyzer.analyze_competitive_mentions(
                    analysis_text
                )
                ...
                await self.create_conversation_intelligence(
                    call_id,
                    analysis_text,
                    apartment_relevance,
                ...
                populated_count += 1
                logger.info(
                    f"📞 Populated call: {title[:50]}... (Relevance: {apartment_relevance:.2f})"
                )

            except Exception as e:
                logger.error(
                    f"❌ Error populating call {call.get('id', 'unknown')}: {e}"
                )

        logger.info(f"✅ Populated {populated_count} calls")
        return populated_count

    async def populate_call_participants(
        self, call_id: str, participants: List[Dict[str, Any]]
    ) -> int:
        ...
        for participant in participants:
            try:
                participant_id = (
                    f"{call_id}_{participant.get('userId', str(uuid.uuid4()))}"
                )
    ...
    async def create_conversation_intelligence(
        self,
        call_id: str,
        analysis_text: str,
        apartment_relevance: float,
        ...
        try:
            ...
            if apartment_relevance > 0.7:
                recommended_actions.append(
                    "High apartment industry relevance - prioritize follow-up"
                )
            if competitive_analysis["threat_level"] != "none":
                recommended_actions.append(
                    f"Competitive threat level: {competitive_analysis['threat_level']} - prepare competitive response"
                )
            if deal_signals["win_probability"] > 0.7:
                recommended_actions.append(
                    "High win probability - accelerate deal progression"
                )
            ...
            await self.create_apartment_analysis(
                call_id, analysis_text, apartment_relevance
            )

            # Create competitive intelligence record if competitors mentioned
            if competitive_analysis["competitors_mentioned"]:
                await self.create_competitive_intelligence(
    ...
    async def create_apartment_analysis(
        self, call_id: str, analysis_text: str, relevance_score: float
    ):
        ...
        try:
            ...
            apartment_terms = sum(
                1
                for keyword in self.analyzer.apartment_keywords
                ...
                json.dumps(
                    {
                        ...
                        "keywords_found": apartment_terms,
                    }
                ),
            )
    ...
    async def create_competitive_intelligence(
        self, call_id: str, competitive_analysis: Dict[str, Any]
    ):
...
async def execute_live_gong_integration():
    ...
    try:
        ...
        async with GongAPIClient() as gong_client:
            ...
            results_file = (
                f"/home/ubuntu/sophia_live_integration_results_{timestamp}.json"
            )
            ...
            logger.info(
                f"📊 Populated: {workspaces_count} workspaces, {users_count} users, {calls_count} calls"
            )
