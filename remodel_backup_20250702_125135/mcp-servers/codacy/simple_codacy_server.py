    ...
    async def analyze_code(
        ...
        return {
            ...
            "recommendations": self._get_recommendations(
                security_issues, complexity_score, comment_lines, non_empty_lines
            ),
        }
...
async def analyze_file(data: dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"Analyzed file {file_path}: {result['quality_score']}/100 quality score"
        )
...
async def security_scan(data: dict[str, Any]):
    ...
    try:
        ...
        return {
            ...
            "severity_summary": {
                "high": len(
                    [i for i in result["security_issues"] if i["severity"] == "high"]
                ),
                "medium": len(
                    [i for i in result["security_issues"] if i["severity"] == "medium"]
                ),
                "low": len(
                    [i for i in result["security_issues"] if i["severity"] == "low"]
                ),
            },
            "recommendations": [
                r
                for r in result["recommendations"]
                if "security" in r.lower() or "fix" in r.lower()
            ],
        }
    except Exception as e:
...
async def main():
    ...
    try:
        ...
        # Start server
        config = uvicorn.Config(app=app, host="0.0.0.0", port=3008, log_level="info")
