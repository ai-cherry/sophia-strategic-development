...
class MultiTenantGongManager:
    ...
    def __init__(self):
        ...
        # Feature tiers
        self.feature_tiers = {
            "basic": ["conversation_search", "basic_analytics", "email_upload"],
            "professional": [
                "conversation_search",
                "basic_analytics",
                "email_upload",
    ...
    async def setup_multitenant_schema(self):
        ...
        try:
            ...
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_customer_conversations_relevance ON customer_conversations(apartment_relevance_score)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_customer_transcripts_customer_id ON customer_transcripts(customer_id)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_customer_media_customer_id ON customer_media_files(customer_id)"
            )
            await conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_customer_webhooks_customer_id ON customer_webhook_events(customer_id)"
            )
            await conn.execute(
    ...
    async def create_customer(
        self, company_name: str, contact_email: str, subscription_tier: str = "trial"
    ) -> Customer:
        ...
        try:
            ...
            features = self.feature_tiers.get(
                subscription_tier, self.feature_tiers["basic"]
            )
    ...
    async def update_customer_oauth_config(
        self,
        customer_id: str,
        gong_workspace_id: str,
        oauth_client_id: str,
    ...
    async def store_customer_oauth_tokens(
        self, customer_id: str, tokens: CustomerOAuthTokens
    ):
    ...
    async def get_customer_oauth_tokens(
        self, customer_id: str
    ) -> Optional[CustomerOAuthTokens]:
    ...
    async def track_feature_usage(
        self, customer_id: str, feature_name: str, metadata: Dict = None
    ):
    ...
    async def get_customer_usage_stats(
        ...
        try:
            ...
            for row in rows:
                usage_stats[row["feature_name"]] = {
                    ...
                    "avg_daily_usage": round(
                        row["total_usage"] / max(row["days_used"], 1), 2
                    ),
                }
...
async def register_customer():
    ...
    try:
        ...
        if not company_name or not contact_email:
            return (
                jsonify({"error": "Company name and contact email are required"}),
                400,
            )

        # Check if customer already exists
        existing_customer = await multitenant_manager.get_customer_by_email(
            contact_email
        )
...
async def setup_customer_oauth(customer_id):
    ...
    try:
        ...
        if not all([gong_workspace_id, oauth_client_id, oauth_client_secret]):
            return (
                jsonify({"error": "All OAuth configuration fields are required"}),
                400,
            )
        ...
        return jsonify({"error": "OAuth setup failed", "details": str(e)}), 500
...
async def get_customer_features(customer_id):
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to get customer features", "details": str(e)}),
            500,
        )
...
async def upgrade_customer(customer_id):
    ...
    try:
        ...
        if new_tier not in multitenant_manager.feature_tiers:
            return jsonify({"error": f"Invalid subscription tier: {new_tier}"}), 400
        ...
        return jsonify({"error": "Upgrade failed", "details": str(e)}), 500
...
async def test_multitenant_system():
    ...
    customer = await multitenant_manager.create_customer(
        "Test Apartment Management", "test@apartments.com", "professional"
    )
