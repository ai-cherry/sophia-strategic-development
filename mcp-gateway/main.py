...
async def health_check():
    return {
        "status": "healthy",
        "discovered_servers": list(app.state.mcp_servers.keys()),
    }
...
async def get_servers():
    if not hasattr(app.state, "mcp_servers") or not app.state.mcp_servers:
        return JSONResponse(
            status_code=404,
            content={
                "error": "No MCP servers configured or configuration file not loaded."
            },
        )

    server_list = [
        {"name": name, "url": url} for name, url in app.state.mcp_servers.items()
    ]
...
async def tool_call(request: Request):
    ...
    if not server_url:
        return JSONResponse(
            status_code=404,
            content={"error": f"Server '{server_name}' not found in config."},
        )
    try:
        async with app.state.http_session.post(
            f"{server_url}/mcp", json=payload
        ) as response:
        ...
        return JSONResponse(
            status_code=502,
            content={"error": f"Gateway could not connect to server '{server_name}'."},
        )
