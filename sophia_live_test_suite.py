...
class SophiaLiveTestSuite:
    ...
    async def setup_test_database(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Test database already exists: {self.db_config['database']}"
                )
    ...
    async def test_gong_api_connectivity(self):
        ...
        if not self.gong_access_key or not self.gong_access_key_secret:
            ...
            self.test_results["api_responses"][
                "gong_connectivity"
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        calls_payload = {
                            ...
                            "cursor": {"limit": 10},  # Small sample for testing
                        }
                        ...
                        async with session.post(
                            ...
                            if calls_response.status == 200:
                                ...
                                logger.info("✅ Gong.io API connected successfully")
        ...
        try:
            ...
            if auth_response["ok"]:
                ...
                channels_response = await self.slack_client.conversations_list(
                    types="public_channel,private_channel", limit=10
                )
                ...
                if channels_response["ok"]:
                    ...
                    if channels:
                        ...
                        messages_response = (
                            await self.slack_client.conversations_history(
                        ...
                        if messages_response["ok"]:
                            ...
                            logger.info("✅ Slack API connected successfully")
    ...
    async def test_database_operations(self):
        ...
        if not self.db_pool:
            ...
            self.test_results["schema_validations"][
                "database_operations"
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                """,
                    interaction_id,
                    "Test conversation about apartment management software",
                    0.6,
                    "High potential customer showing interest in property management solutions",
                )
                ...
                interaction_count = await conn.fetchval(
                    "SELECT COUNT(*) FROM unified_interactions"
                )
                intelligence_count = await conn.fetchval(
                    "SELECT COUNT(*) FROM conversation_intelligence"
                )
                ...
                # Clean up test data
                await conn.execute(
                    "DELETE FROM conversation_intelligence WHERE id = $1",
                    intelligence_id,
                )
                await conn.execute(
                    "DELETE FROM unified_interactions WHERE id = $1", interaction_id
                )
                await conn.execute(
                    "DELETE FROM unified_contacts WHERE id = $1", contact_id
                )
    ...
    async def test_data_processing_pipeline(self):
        ...
        try:
            ...
            logger.info(
                f"   - Intelligence generated: {len(intelligence_result)} insights"
            )
    ...
    async def process_conversation_intelligence(
        self, gong_data: Dict, slack_data: Dict
    ) -> Dict:
    ...
    async def generate_test_report(self):
        ...
        if self.test_results["tests_failed"] == 0:
            ...
            report["recommendations"].append(
                "⚠️ Some tests failed - review failed components before production"
            )

        if (
            self.test_results["performance_metrics"].get("gong_api_response_time", 0)
            > 5
        ):
            report["recommendations"].append(
                "⚠️ Gong API response time is high - consider caching strategy"
            )

        if (
            self.test_results["performance_metrics"].get("slack_api_response_time", 0)
            > 3
        ):
            report["recommendations"].append(
                "⚠️ Slack API response time is high - consider rate limiting optimization"
            )

        return report

    async def run_comprehensive_test(
        self, gong_key: str, gong_secret: str, slack_token: str
    ):
