...
export class StorageComponents extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: StorageArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        if (args.dynamoConfig?.tables && args.dynamoConfig.tables.length > 0) {
            ...
            args.dynamoConfig.tables.forEach((tableConfig, i) => {
                ...
                if (args.dynamoConfig?.enableAutoScaling) {
                    ...
                    // Set up GSI auto scaling
                    if (tableConfig.globalSecondaryIndexes) {
                        tableConfig.globalSecondaryIndexes.forEach((gsi, gsiIndex) => {
                            // GSI read capacity auto scaling
                            const gsiReadCapacityTarget = new aws.appautoscaling.Target(`${name}-dynamo-gsi-read-target-${i + 1}-${gsiIndex + 1}`, {
                                maxCapacity: 100,
                                minCapacity: gsi.readCapacity || 5,
                                resourceId: pulumi.interpolate`table/${tableName}/index/${gsi.name}`,
                                scalableDimension: "dynamodb:index:ReadCapacityUnits",
                                serviceNamespace: "dynamodb",
                            }, { parent: this });

                            readCapacityTargets.push(gsiReadCapacityTarget);

                            // GSI read capacity scaling policy
                            const gsiReadScalingPolicy = new aws.appautoscaling.Policy(`${name}-dynamo-gsi-read-policy-${i + 1}-${gsiIndex + 1}`, {
                                policyType: "TargetTrackingScaling",
                                resourceId: gsiReadCapacityTarget.resourceId,
                                scalableDimension: gsiReadCapacityTarget.scalableDimension,
                                serviceNamespace: gsiReadCapacityTarget.serviceNamespace,
                                targetTrackingScalingPolicyConfiguration: {
                                    predefinedMetricSpecification: {
                                        predefinedMetricType: "DynamoDBReadCapacityUtilization",
                                    },
                            ...
                            readScalingPolicies.push(gsiReadScalingPolicy);

                            // GSI write capacity auto scaling
                            const gsiWriteCapacityTarget = new aws.appautoscaling.Target(`${name}-dynamo-gsi-write-target-${i + 1}-${gsiIndex + 1}`, {
                                maxCapacity: 100,
                                minCapacity: gsi.writeCapacity || 5,
                                resourceId: pulumi.interpolate`table/${tableName}/index/${gsi.name}`,
                                scalableDimension: "dynamodb:index:WriteCapacityUnits",
                                serviceNamespace: "dynamodb",
                            }, { parent: this });

                            writeCapacityTargets.push(gsiWriteCapacityTarget);

                            // GSI write capacity scaling policy
                            const gsiWriteScalingPolicy = new aws.appautoscaling.Policy(`${name}-dynamo-gsi-write-policy-${i + 1}-${gsiIndex + 1}`, {
                                policyType: "TargetTrackingScaling",
                                resourceId: gsiWriteCapacityTarget.resourceId,
                                scalableDimension: gsiWriteCapacityTarget.scalableDimension,
                                serviceNamespace: gsiWriteCapacityTarget.serviceNamespace,
                                targetTrackingScalingPolicyConfiguration: {
                                    predefinedMetricSpecification: {
                                        predefinedMetricType: "DynamoDBWriteCapacityUtilization",
                                    },
                            ...
                            writeScalingPolicies.push(gsiWriteScalingPolicy);
                        });
                    }
                }
            });
        ...
        // Create EFS file system if configured
        if (args.efsConfig) {
            // Create EFS file system
            const modelFileSystem = new aws.efs.FileSystem(`${name}-model-fs`, {
                ...
                provisionedThroughputInMibps: args.efsConfig.throughputMode === "provisioned" ? args.efsConfig.provisionedThroughputInMibps : undefined,
            ...
            args.subnetIds.forEach((subnetId, i) => {
                const mountTarget = new aws.efs.MountTarget(`${name}-mount-target-${i + 1}`, {
                    fileSystemId: modelFileSystem.id,
                    subnetId: subnetId,
            ...
            // Assign EFS resources
            this.efsFileSystems = {
                modelFs: modelFileSystem,
        ...
        if (args.elastiCacheConfig) {
            ...
            const modelCache = new aws.elasticache.Cluster(`${name}-model-cache`, {
                ...
                engineVersion: "6.2",
                nodeType: args.elastiCacheConfig.nodeType || "cache.r6g.large", // Memory-optimized for ML caching
                numCacheNodes: args.elastiCacheConfig.numNodes || 1,
                parameterGroupName: args.elastiCacheConfig.parameterGroupName || "default.redis6.x",
                port: 6379,
        ...
        if (args.k8sProvider) {
            ...
            // Create EFS storage class if EFS is configured
            let efsStorageClass: k8s.storage.v1.StorageClass | undefined;
            if (this.efsFileSystems) {
                efsStorageClass = new k8s.storage.v1.StorageClass(`${name}-efs-storage-class`, {
                    metadata: {
                        name: `${name}-efs-storage`,
                    },
                    provisioner: "efs.csi.aws.com",
                    parameters: {
                        fileSystemId: this.efsFileSystems.modelFs.id,
...
/**
 * Create S3 bucket with optimized settings for ML workloads
 */
export function createMlOptimizedBucket(
    name: string,
    environment: string,
    bucketType: string,
    options: {
        ...
        enableReplication?: boolean;
        replicationBucket?: aws.s3.Bucket;
        replicationRoleArn?: pulumi.Input<string>;
        tags?: { [key: string]: string };
    } = {},
    parent?: pulumi.Resource,
): aws.s3.Bucket {
    // Set default lifecycle days
    const lifecycleDays = {
        standardIa: options.lifecycleDays?.standardIa || 30,
        glacier: options.lifecycleDays?.glacier || 90,
        deepArchive: options.lifecycleDays?.deepArchive || 180,
        expiration: options.lifecycleDays?.expiration || 365,
    };
    ...
    const tags = {
        ...
        CreatedAt: new Date().toISOString(),
        ResourceType: "S3Bucket",
        StorageType: bucketType,
        ...options.tags,
    };

    // Create bucket
    const bucket = new aws.s3.Bucket(`${name}-${bucketType}`, {
        acl: "private",
        versioning: {
            enabled: options.enableVersioning !== false, // Default to true
        },
        serverSideEncryptionConfiguration: {
            rule: {
                applyServerSideEncryptionByDefault: {
                    sseAlgorithm: options.kmsKeyId ? "aws:kms" : "AES256",
        ...
        // Intelligent tiering for optimal cost management
        ...(options.enableIntelligentTiering ? {
            intelligentTieringConfigurations: [
                {
                    name: `${bucketType}-tiering`,
                    status: "Enabled",
                    tierings: [
                        {
                            accessTier: "ARCHIVE_ACCESS",
        ...
        tags: {
            ...tags,
            Name: `${name}-${bucketType}-${environment}`,
        },
    }, { parent });

    // Set up replication if enabled
    if (options.enableReplication && options.replicationBucket && options.replicationRoleArn) {
        new aws.s3.BucketReplicationConfiguration(`${name}-${bucketType}-replication`, {
            bucket: bucket.id,
            role: options.replicationRoleArn,
            rules: [
                {
                    id: `${bucketType}-replication-rule`,
                    status: "Enabled",
                    priority: 0,
                    filter: {
                        prefix: "",
                    ...
                    destination: {
                        bucket: options.replicationBucket.arn,
                        storageClass: "STANDARD_IA",
                    },
                },
            ],
