...
def mock_portkey_client():
    with patch.object(
        PortkeyClient, "llm_call", new_callable=AsyncMock
    ) as mock_llm_call:
        ...
        """
        mock_llm_call.return_value = {"choices": [{"message": {"content": mock_plan}}]}
        yield mock_llm_call
...
async def test_iac_manager_agent_full_execution(
    ...
    mock_filesystem.read_file.assert_called_once_with(
        file_path="infrastructure/lambda_labs.py"
    )
