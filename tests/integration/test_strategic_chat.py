...
class TestStrategicChat:
    ...
    async def _mock_mcp_responses(self, server, tool, **kwargs):
        ...
        if server == "knowledge" and tool == "search":
            ...
            return {
                "memories": [
                    {"content": "Previous strategic decision", "timestamp": "2024-01-15"}
                ]
            }
            ...
            return {
                "results": [
                    {"title": "PropTech Market Trends 2024", "url": "https://example.com"}
                ]
            }
        elif server == "huggingface" and tool == "analyze":
            return {
                "analysis": "AI market insights and predictions"
            }
    ...
    @pytest.mark.asyncio
    async def test_strategic_chat_internal_mode(self, mock_openrouter_client, mock_mcp_client):
        """Test strategic chat in internal only mode"""
        with patch('backend.app.routes.retool_executive_routes.get_openrouter_client', 
                  return_value=mock_openrouter_client):
            with patch('backend.app.routes.retool_executive_routes.get_mcp_client',
    ...
    @pytest.mark.asyncio
    async def test_strategic_chat_external_mode(self, mock_openrouter_client, mock_mcp_client):
    ...
    @pytest.mark.asyncio
    async def test_strategic_chat_combined_mode(self, mock_openrouter_client, mock_mcp_client):
        ...
        with patch('backend.app.routes.retool_executive_routes.get_openrouter_client', 
            ...
            with patch('backend.app.routes.retool_executive_routes.get_mcp_client',
                ...
                assert mock_mcp_client.call_tool.call_count >= 5  # 3 internal + 2 external
    ...
    async def test_model_presets(self, mock_openrouter_client):
        ...
        with patch('backend.app.routes.retool_executive_routes._get_latest_model',
    ...
    async def test_optimal_model_selection(self, mock_openrouter_client):
        ...
        with patch('backend.app.routes.retool_executive_routes.get_openrouter_client', 
            ...
            mock_openrouter_client.select_optimal_model.return_value = "openai/o1-preview"
            
            message = StrategicChatMessage(
                message="Develop a 5-year strategic plan",
                mode="internal"
            )
    ...
    async def test_performance_tracking(self, mock_mcp_client):
        ...
        with patch('backend.app.routes.retool_executive_routes.get_mcp_client',
            ...
            from backend.app.routes.retool_executive_routes import track_model_performance
    ...
    async def test_context_window_handling(self, mock_openrouter_client):
        ...
        with patch('backend.app.routes.retool_executive_routes.get_openrouter_client', 
            ...
            mock_openrouter_client.select_optimal_model.return_value = "anthropic/claude-3.5-sonnet"
            
            result = await strategic_chat(message, UserRole.CEO)
            
            # Verify context size was considered
            mock_openrouter_client.select_optimal_model.assert_called_with(
                query_type=pytest.Any(str),
                context_size=150000,
                max_cost_per_token=None
            )
...
class TestPerformance:
    ...
    async def test_parallel_search_execution(self, mock_mcp_client):
        ...
        with patch('backend.app.routes.retool_executive_routes.get_mcp_client',
            ...
            from backend.app.routes.retool_executive_routes import _build_synthesis_prompt
