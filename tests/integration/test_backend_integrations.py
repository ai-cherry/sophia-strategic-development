...
from backend.core.integration_registry import IntegrationRegistry, integration_registry
...
class TestIntegrationConfig:
    ...
    async def test_service_registry_loading(self, config_manager):
        ...
        expected_services = ["snowflake", "gong", "vercel", "estuary", "pinecone", "openai"]
...
class TestLambdaLabsIntegration:
    ...
    async def test_lambda_labs_client_creation(self, mock_session, lambda_labs_integration):
...
# Test utilities
def create_mock_service_config(service_name: str, config: Dict[str, Any] = None, secrets: Dict[str, str] = None) -> ServiceConfig:
...
def create_mock_response(status: int = 200, json_data: Dict[str, Any] = None) -> AsyncMock:
