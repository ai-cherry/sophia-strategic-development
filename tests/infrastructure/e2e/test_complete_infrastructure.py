...
class TestCompleteInfrastructure:
    ...
    async def test_full_infrastructure_deployment(self):
        ...
        try:
            ...
            logger.info(
                f"Total resources created: {up_result.summary.resource_changes.get('create', 0)}"
            )
    ...
    async def test_ai_agent_vector_storage(self, mock_pinecone_client):
        ...
        mock_pinecone_client.generate_test_vectors(
            count=100, index_name="test-knowledge-base"
        )

        # Test vector search
        search_vector = [0.1] * 128  # Assuming 128-dimensional vectors
        results = mock_pinecone_client.search(
            vector=search_vector, top_k=10, index_name="test-knowledge-base"
        )
    ...
    async def test_complete_system_health(self):
        ...
        logger.info(
            f"System health: {healthy_count}/{len(health_status)} components healthy"
        )
