...
class TestSnowflakeGongIntegration:
    ...
    @pytest.mark.asyncio
    async def test_gong_data_flow_to_snowflake(self, mock_snowflake_client, mock_gong_client):
        """
        ...
        assert result[0]["transcript"] == test_data["transcript"], "Transcript does not match"
    ...
    async def test_gong_webhook_configuration(self, mock_gong_client):
        ...
        assert gong_outputs["webhook_url"].startswith("https://"), "Webhook URL is not HTTPS"
    ...
    async def test_snowflake_schema_for_gong_data(self, mock_snowflake_client):
        ...
        for column in expected_columns:
            assert column in column_names, f"{column} column not found in GONG_CALLS table"
