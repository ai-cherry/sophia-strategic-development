...
class TestSnowflakeGongLoader:
    ...
    async def test_load_raw_calls(self, snowflake_loader):
        ...
        with patch.object(
            snowflake_loader, "_ensure_raw_calls_table", return_value=None
        ):
...
class TestSnowflakeGongConnector:
    ...
    async def test_get_calls_for_coaching(self, gong_connector):
        ...
        result = await gong_connector.get_calls_for_coaching(
            sales_rep="John Smith", date_range_days=7
        )
...
class TestSnowflakeCortexService:
    ...
    async def test_analyze_gong_call_sentiment(self):
        ...
        with patch(
            "backend.utils.snowflake_cortex_service.get_cortex_service"
        ) as mock_service:
    ...
    async def test_find_similar_gong_calls(self):
        ...
        with patch(
            ...
            result = await find_similar_gong_calls(
                query_text="pricing discussion", top_k=5, similarity_threshold=0.7
            )
...
class TestSalesCoachAgent:
    ...
    async def test_analyze_call_performance_snowflake_success(self, sales_coach_agent):
        ...
        with patch(
            "backend.agents.specialized.sales_coach_agent.analyze_gong_call_sentiment"
        ) as mock_sentiment, patch(
            "backend.agents.specialized.sales_coach_agent.summarize_gong_call_with_context"
        ) as mock_summary, patch(
            "backend.agents.specialized.sales_coach_agent.get_gong_connector"
        ) as mock_connector:
    ...
    async def test_generate_rep_coaching(self, sales_coach_agent):
        ...
        with patch(
            "backend.agents.specialized.sales_coach_agent.get_gong_coaching_insights"
        ) as mock_insights, patch(
            "backend.agents.specialized.sales_coach_agent.get_gong_connector"
        ) as mock_connector:
    ...
    async def test_real_time_coaching(self, sales_coach_agent):
        ...
        assert result["success"] is True
        assert (
            len(result["coaching_suggestions"]) >= 2
        )  # Should have sentiment and talk ratio suggestions

        # Check for sentiment coaching
        sentiment_suggestions = [
            s for s in result["coaching_suggestions"] if s["type"] == "sentiment"
        ]
        assert len(sentiment_suggestions) == 1
        assert sentiment_suggestions[0]["priority"] == "high"

        # Check for talk ratio coaching
        talk_ratio_suggestions = [
            s for s in result["coaching_suggestions"] if s["type"] == "talk_ratio"
        ]
...
class TestCallAnalysisAgent:
    ...
    async def test_analyze_individual_call(self, call_analysis_agent):
        ...
        with patch(
            "backend.agents.specialized.call_analysis_agent.get_gong_connector"
        ) as mock_connector, patch(
            "backend.agents.specialized.call_analysis_agent.analyze_gong_call_sentiment"
        ) as mock_sentiment, patch(
            "backend.agents.specialized.call_analysis_agent.summarize_gong_call_with_context"
        ) as mock_summary:
            ...
            mock_summary.return_value = {"ai_summary": "Successful discovery call"}
    ...
    async def test_batch_call_analysis(self, call_analysis_agent):
        ...
        with patch(
            ...
            with patch.object(
                call_analysis_agent, "_analyze_individual_call"
            ) as mock_analyze:
    ...
    def test_calculate_call_score(self, call_analysis_agent):
        ...
        sentiment_analysis = {"call_sentiment_score": 0.7}

        score = asyncio.run(
            call_analysis_agent._calculate_call_score(call_details, sentiment_analysis)
        )
    ...
    def test_determine_call_priority(self, call_analysis_agent):
        ...
        priority = call_analysis_agent._determine_call_priority(
            overall_score=25.0, sentiment_analysis={"call_sentiment_score": -0.6}
        )
        assert priority == CallPriority.CRITICAL

        # Test high priority
        priority = call_analysis_agent._determine_call_priority(
            overall_score=45.0, sentiment_analysis={"call_sentiment_score": -0.2}
        )
        assert priority == CallPriority.HIGH

        # Test low priority
        priority = call_analysis_agent._determine_call_priority(
            overall_score=85.0, sentiment_analysis={"call_sentiment_score": 0.8}
        )
...
class TestIntegrationScenarios:
    ...
    async def test_full_etl_to_agent_workflow(self):
        ...
        with patch(
            "backend.etl.gong.ingest_gong_data.GongAPIClient"
        ) as mock_gong_client, patch(
            "backend.etl.gong.ingest_gong_data.SnowflakeGongLoader"
...
def sample_call_data():
    ...
    return {
        ...
        "analytics": {"talkRatio": 0.6, "interactivity": 0.8, "questionsAsked": 5},
    }
