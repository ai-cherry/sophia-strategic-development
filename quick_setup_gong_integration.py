...
class GongIntegrationSetup:
    ...
    def __init__(self):
        ...
        self.required_secrets = ["GONG_ACCESS_KEY", "GONG_CLIENT_SECRET"]
    ...
    def check_prerequisites(self):
        ...
        if python_version.major >= 3 and python_version.minor >= 8:
            self.log_step(
                ...
                True,
                f"Version: {python_version.major}.{python_version.minor}",
            )
        else:
            self.log_step(
                "Python 3.8+ required",
        ...
        for package in required_packages:
            try:
                ...
                self.log_step(
                    f"Package {package} missing", False, f"Run: pip install {package}"
                )
    ...
    def validate_gong_credentials(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_step(
                    "Gong API authentication", True, f"Access to {user_count} users"
                )
                ...
                if response.status_code == 200:
                    ...
                    self.log_step(
                        "Gong calls access", True, f"Access to {call_count} calls"
                    )
                else:
                    self.log_step(
                        "Gong calls access", False, f"Status: {response.status_code}"
                    )

            else:
                self.log_step(
                    "Gong API authentication", False, f"Status: {response.status_code}"
                )
    ...
    def setup_database(self):
        ...
        try:
            ...
            # Create database
            result = subprocess.run(
                ["sudo", "-u", "postgres", "createdb", "sophia_enhanced"],
                capture_output=True,
                text=True,
            )
            if result.returncode == 0:
                ...
                self.log_step(
                    "Database sophia_enhanced exists", True, "Already created"
                )
            else:
            ...
            if os.path.exists("sophia_enhanced_schema.py"):
                ...
                self.log_step(
                    "Schema file not found", False, "sophia_enhanced_schema.py missing"
                )
    ...
    def test_api_integration(self):
        ...
        if os.path.exists("enhanced_gong_api_tester.py"):
            try:
                result = subprocess.run(
                    ["python3", "enhanced_gong_api_tester.py"],
                    capture_output=True,
                    text=True,
                    timeout=60,
                )
            ...
            self.log_step(
                "API tester not found", False, "enhanced_gong_api_tester.py missing"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_step(
                    "Flask API health check", False, f"Status: {response.status_code}"
                )
        except requests.exceptions.RequestException:
            self.log_step(
                "Flask API not running",
                False,
                "Start with: python sophia_admin_api/src/main.py",
            )
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_step(
                    "React frontend health check", True, "Frontend responding"
                )
            else:
                self.log_step(
                    "React frontend health check",
                    False,
            ...
            self.log_step(
                ...
                False,
                "Start with: cd sophia_admin_frontend && npm run dev",
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "sophia-main" in result.stdout:
                    ...
                    self.log_step(
                        "GitHub repository", False, f"Remote: {result.stdout.strip()}"
                    )

                # Check for workflow files
                if os.path.exists(".github/workflows/deploy-secure-gong.yml"):
                    self.log_step(
                        "GitHub Actions workflow", True, "deploy-secure-gong.yml found"
                    )
                else:
                    self.log_step(
                        "GitHub Actions workflow",
                        False,
                        "deploy-secure-gong.yml missing",
                    )
    ...
    def run_complete_setup(self):
        ...
        successful_steps = sum(
            1 for step in self.setup_results["steps_completed"] if step["success"]
        )
