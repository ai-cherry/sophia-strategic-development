                ...
                try:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            total_records = calls_data.get("records", {}).get(
                                "totalRecords", 0
                            )

                            logger.info(
                                f"    ✅ Success: {call_count} calls returned, {total_records} total available"
                            )
                            ...
                            self.test_results["gong_api_tests"][scenario["name"]] = {
                                ...
                                "has_pagination": bool(
                                    calls_data.get("records", {}).get("cursor")
                                ),
                            }
                            ...
                            if (
                                ...
                                self.test_results["data_samples"]["sample_call"] = {
                                    ...
                                    "participants": [
                                        ...
                                        for p in sample_call.get("participants", [])[
                                            :3
                    ...
                    if endpoint["method"] == "GET":
                        async with session.get(
                            ...
                            if response.status == 200:
                                ...
                                logger.info(
                                    f"    ✅ {endpoint['name']} endpoint successful"
                                )
                                ...
                                if endpoint["name"] == "Users":
                                    ...
                                    self.test_results["data_samples"][
                                        ...
                                        [u for u in users if u.get("active", False)]
                                    )
                                ...
                                logger.error(
                                    f"    ❌ {endpoint['name']} failed: {response.status}"
                                )
        ...
        try:
            ...
            test_queries = [
                ...
                (
                    ...
                    "SELECT COUNT(*) FROM gong_conversation_intelligence",
                ),
            ...
            for query_name, query_sql in test_queries:
                try:
                    ...
                    query_results[query_name] = (
                        len(result) if query_name != "Join test" else result
                    )
                    logger.info(
                        f"    ✅ {query_name}: {len(result) if query_name != 'Join test' else 'Success'}"
                    )
                except Exception as e:
    ...
    async def generate_integration_recommendations(self):
        ...
        if successful_scenarios:
            recommendations.append(
                f"✅ Gong API connectivity confirmed - {len(successful_scenarios)} scenarios successful"
            )
            ...
            if best_scenario and max_records > 0:
                recommendations.append(
                    f"🚀 Optimal data range: {best_scenario} with {max_records} total records available"
                )
                recommendations.append(
                    f"💡 Recommend starting with {best_scenario} for initial data import"
                )

        else:
            recommendations.append(
                "⚠️ No successful Gong API calls - investigate authentication or API permissions"
            )
        ...
        if (
            ...
            recommendations.append(
                "✅ Database schema validated - ready for production deployment"
            )
            recommendations.append(
                "🏗️ Recommended tables: gong_users, gong_calls, gong_participants, gong_conversation_intelligence"
            )
            recommendations.append(
                "🔗 Cross-platform correlation ready via unified_contacts and unified_interactions tables"
            )
        ...
        if total_users > 0:
            ...
            if total_users > 50:
                recommendations.append(
                    "📈 Large team detected - implement user segmentation for analysis"
                )
