...
class SchemaMigrationSystem:
    ...
    def get_existing_columns(self, table_name: str) -> Dict[str, str]:
        ...
        with self.get_connection() as conn:.
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute(
                    """SELECT column_name, data_type, character_maximum_lengthFROM information_schemacolumns
    ...
    def table_exists(self, table_name: str) -> bool:
        ...
        with self.get_connection() as conn:.
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    """SELECT EXISTS (SELECT FROM information_schematables
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                for col_name, col_type in new_columns.items():
                    ...
                    self.logger.info(
                        f"Added column {col_name} ({col_type}) to {table_name}"
                    )
    ...
    def record_migration(
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """INSERT INTO schema_migrations(table_name, migration_type, column_changes, data_sample,
        ...
        try:.
            ...
            if migration_summary["migration_type"] != "no_changes":
                migration_hash = self.record_migration(
                    table_name,
                    migration_summary["migration_type"],
                    migration_summary["changes"],
                    data,
                    quality_score,
        ...
        try:.
            ...
            with self.get_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                    ...
                    cursor.execute(
                        """SELECT * FROM schema_migrationsWHERE migration_hash = %s AND status = 'completed'
                        """,(migration_hash,),
                    )
                    ...
                    if not migration:
                        self.logger.warning(
                            f"Migration {migration_hash} not found or already rolled back"
                        )
                        return False
                    ...
                    if migration["rollback_sql"]:
                        ...
                        cursor.execute(
                            """UPDATE schema_migrationsSET status = 'rolled_back'
                        ...
                        conn.commit()
                        self.logger.info(
                            f"Successfully rolled back migration {migration_hash}"
                        )
                        return True

        except Exception as e:
            self.logger.error(
                f"Rollback failed for migration {migration_hash}: {str(e)}"
            )
            return False
    ...
    def get_migration_history(self, table_name: Optional[str] = None) -> List[Dict]:
        ...
        with self.get_connection() as conn:.
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                if table_name:
                    cursor.execute(
                        """SELECT * FROM schema_migrationsWHERE table_name = %s
                                                                        ORDER BY created_at DESC
                        """,(table_name,),
                    )
                else:
                    cursor.execute(
                        """SELECT * FROM schema_migrationsORDER BY created_at DESC
                        """).
            ...
            with conn.cursor() as cursor:
                ...
                for col_name in index_candidates:
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to create index on {col_name}: {str(e)}"
                        )
