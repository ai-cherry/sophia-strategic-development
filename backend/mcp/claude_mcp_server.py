...
class ClaudeMCPServer(BaseMCPServer):
    ...
    async def list_resources(self, request: ListResourcesRequest) -> List[Resource]:
        ...
        return [
            Resource(uri="claude://health", name="Claude Health", mimeType="application/json"),
            Resource(uri="claude://config", name="Claude Config", mimeType="application/json"),
            Resource(uri="claude://models", name="Claude Models", mimeType="application/json"),
        ]
    ...
    async def get_resource(self, request: GetResourceRequest) -> str:
        ...
        if uri == "claude://health":
            ...
            if config and "api_key" in config:
                config["api_key"] = f"{config['api_key'][:8]}..." if config['api_key'] else None
            data = config or {}
        elif uri == "claude://models":
            data = {
                "available_models": ["claude-3-5-sonnet-20241022", "claude-3-haiku-20240307", "claude-3-opus-20240229"],
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "analysis_type": {"type": "string", "enum": ["review", "explain", "optimize"], "default": "review"},
                    }, "required": ["code"]}
            ),
            Tool(
                name="refactor_code",
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "refactor_goal": {"type": "string", "description": "Goal of refactoring", "default": "improve readability"},
                    }, "required": ["code"]}
            )
        ]
    ...
    async def call_tool(self, request: CallToolRequest) -> List[TextContent]:
        ...
        if tool_name == "generate_code":
            ...
            result = await self.claude.refactor_code(
                code=args["code"], refactor_goal=args.get("refactor_goal", "improve readability")
            )
        else:
