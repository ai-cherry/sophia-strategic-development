...
from backend.integrations.apify_integration import apify_integration # Assuming this will be created
    ...
    async def call_tool(self, request: CallToolRequest) -> List[TextContent]:
        ...
        try:
            if tool_name == "google_search_and_scrape":
                ...
                result = await apify_integration.run_actor_and_get_results(actor_id, run_input)
            else:
            ...
            self.logger.error(f"Error calling Apify tool {tool_name}: {e}", exc_info=True)
