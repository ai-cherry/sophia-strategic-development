...
class CodebaseAwarenessMCPServer(BaseMCPServer):
        ...
        return [
            Tool(
                ...
                inputSchema={
                    "type": "object",
                    "properties": {
                        "query": {"type": "string", "description": "Natural language query describing the desired code."},
                        "top_k": {"type": "integer", "default": 5},
                        "item_type": {"type": "string", "description": "Optional filter by item type (e.g., 'python_function', 'api_endpoint')."}
                    }, "required": ["query"]}
            ),
            Tool(
    ...
    async def call_tool(self, request: CallToolRequest) -> List[TextContent]:
        ...
        if tool_name == "find_relevant_code":
            ...
            result = [f"Found in {res['metadata']['file_path']}:\nScore: {res['score']:.4f}\nContent: {res['content']}\n---" for res in search_results]
            ...
            result = {"status": "success", "message": "Codebase ingestion process started in the background."}
            ...
            if not file_path.exists():
                ...
                components = ArchitectureParser.parse_python_code(content, str(file_path))
                result = {
                    "file_path": str(file_path),
                    "summary": f"File contains {len(components)} functions/classes.",
                    "components": [f"{item['type']}: {item['name']}" for item in components]
                }
        else:
