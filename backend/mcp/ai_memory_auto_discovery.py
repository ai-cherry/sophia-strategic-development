...
class IntelligentContextDetector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if api_key:
                ...
                logger.info("✅ Intelligent Context Detector initialized with OpenAI")
            else:
                logger.warning("OpenAI API key not available, using pattern-based detection only")
    ...
    def _initialize_patterns(self) -> List[ContextPattern]:
        ...
        return [
            ContextPattern(
                ...
                keywords=["architecture", "design", "structure", "pattern", "approach", "strategy"],
                phrases=[
                    "we decided to", "architectural decision", "design choice", "we chose",
                    "the approach is", "pattern we're using", "architectural pattern",
                    "design decision", "we're implementing", "the structure will be"
                ],
                code_indicators=["class", "interface", "abstract", "@dataclass", "def __init__"],
            ...
            ContextPattern(
                ...
                keywords=["bug", "error", "issue", "problem", "fix", "solution", "debug"],
                phrases=[
                    "the bug was", "error occurs", "issue is", "problem with",
                    "fixed by", "solution is", "root cause", "debugging showed",
                    "the fix is", "resolved by", "error happens when"
                ],
                code_indicators=["try:", "except:", "raise", "assert", "if __debug__", "logging"],
            ...
            ContextPattern(
                ...
                keywords=["pattern", "implementation", "method", "function", "class", "algorithm"],
                phrases=[
                    "code pattern", "implementation approach", "we implement",
                    "the method is", "algorithm we use", "coding standard",
                    "best practice", "pattern for", "way to implement"
                ],
                code_indicators=["def ", "class ", "async def", "@decorator", "lambda", "yield"],
            ...
            ContextPattern(
                ...
                keywords=["performance", "optimization", "speed", "memory", "efficiency", "bottleneck"],
                phrases=[
                    "performance improvement", "optimization technique", "bottleneck is",
                    "memory usage", "speed up", "efficient way", "performance issue",
                    "optimize by", "faster approach", "reduce latency"
                ],
                code_indicators=["async", "await", "cache", "pool", "batch", "concurrent"],
                importance_weight=0.8,
                confidence_threshold=0.7
            ),
            ...
            ContextPattern(
                ...
                phrases=[
                    "security consideration", "authentication method", "secure way",
                    "vulnerability in", "encryption approach", "security risk",
                    "authorization pattern", "secure implementation", "security best practice"
                ],
                code_indicators=["auth", "encrypt", "hash", "token", "secret", "validate"],
                importance_weight=0.9,
            ...
            ContextPattern(
                ...
                keywords=["integration", "api", "service", "connector", "client", "interface"],
                phrases=[
                    "integration with", "api client", "service connection", "external service",
                    "integration pattern", "connector for", "interface to", "api wrapper",
                    "service integration", "external api", "third party"
                ],
                code_indicators=["requests", "aiohttp", "client", "api", "endpoint", "async def"],
                importance_weight=0.8,
                confidence_threshold=0.6
            ),
            ...
            ContextPattern(
                ...
                keywords=["business", "logic", "rule", "process", "workflow", "requirement"],
                phrases=[
                    "business logic", "business rule", "process flow", "workflow step",
                    "business requirement", "logic for", "business process",
                    "rule is", "requirement is", "business case"
                ],
            ...
            ContextPattern(
                ...
                keywords=["error", "exception", "failure", "resolve", "handle", "recovery"],
                phrases=[
                    "error handling", "exception occurs", "failure mode", "error recovery",
                    "handle the error", "exception handling", "error case",
                    "failure scenario", "error resolution", "exception strategy"
                ],
                code_indicators=["try:", "except", "finally:", "raise", "Exception", "Error"],
                importance_weight=0.8,
                confidence_threshold=0.6
            ),
            ...
            ContextPattern(
                ...
                keywords=["deployment", "deploy", "production", "staging", "environment", "release"],
                phrases=[
                    "deployment strategy", "deploy to", "production deployment",
                    "staging environment", "release process", "deployment approach",
                    "environment setup", "deploy with", "production ready"
                ],
                code_indicators=["docker", "kubernetes", "config", "env", "production", "staging"],
                importance_weight=0.8,
                confidence_threshold=0.7
            )
        ]
    
    async def detect_context(self, conversation: str, file_context: Optional[Dict[str, Any]] = None) -> List[DetectedContext]:
        """
    ...
    def _detect_with_patterns(self, conversation: str, file_context: Optional[Dict[str, Any]] = None) -> List[DetectedContext]:
        ...
        for pattern in self.patterns:
            ...
            if confidence >= pattern.confidence_threshold:
                ...
                detected.append(DetectedContext(
                    context_type=pattern.context_type,
                    confidence=min(confidence, 1.0),
                    key_points=key_points,
                    suggested_category=self._map_context_to_category(pattern.context_type),
                    suggested_tags=tags,
    ...
    async def _detect_with_ai(self, conversation: str, file_context: Optional[Dict[str, Any]] = None) -> List[DetectedContext]:
        ...
        try:
            ...
            try:
                ...
                for ctx_data in contexts_data:
                    ...
                    if context_type:
                        ai_contexts.append(DetectedContext(
                            context_type=context_type,
                            confidence=float(ctx_data.get("confidence", 0.5)),
                            key_points=ctx_data.get("key_points", []),
                            suggested_category=self._map_context_to_category(context_type),
                            suggested_tags=ctx_data.get("suggested_tags", []),
                            importance_score=float(ctx_data.get("importance_score", 0.5)),
        ...
        for context_type, ctx_list in grouped.items():
            if len(ctx_list) == 1:
                ...
                merged_ctx = DetectedContext(
                    ...
                    reasoning=f"Merged from {len(ctx_list)} detections: " + "; ".join(ctx.reasoning for ctx in ctx_list)[:200],
    ...
    async def analyze_and_store_conversation(
        ...
        try:
            ...
            return {
                ...
                "high_confidence_contexts": len([ctx for ctx in detected_contexts if ctx.confidence >= 0.7]),
    ...
    async def get_discovery_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.discovery_stats,
            ...
            "ai_memory_available": self.ai_memory.initialized if self.ai_memory else False
        }
        ...
        try:
            ...
            for memory in memories:
                ...
                contextual_memories.append({
                    **memory,
                    "contextual_relevance": min(relevance_score, 1.0)
                })
