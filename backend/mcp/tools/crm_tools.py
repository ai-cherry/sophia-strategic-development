...
class CrmSyncTool(MCPTool):
        ...
        try:
            ...
            if crm_type == "hubspot":
                result = await self._sync_hubspot(
                    entity_type, data, operation, external_id
                )
            elif crm_type == "salesforce":
                result = await self._sync_salesforce(
                    entity_type, data, operation, external_id
                )
            else:
    ...
    async def _sync_hubspot(
        ...
        if operation == "create":
            ...
            return {"id": api_response.id, "created": True}

        elif operation == "update":
            # Update existing entity
            api_response = self.hubspot_client.crm[hubspot_entity].basic_api.update(
                record_id=external_id, body=hubspot_data
            )

            return {"id": external_id, "updated": True}
            ...
            try:
                ...
                return {"id": external_id, "updated": True, "created": False}

            except hubspot.exceptions.ApiException as e:
                if e.status == 404:
                    # Entity not found, create new
                    api_response = self.hubspot_client.crm[
                        hubspot_entity
                    ].basic_api.create(body=hubspot_data)

                    return {"id": api_response.id, "updated": False, "created": True}
                else:
                    # Other error
                    raise
    ...
    async def _sync_salesforce(
        ...
        if not self.salesforce_client:
            ...
            security_token = await secret_manager.get_secret(
                "security_token", "salesforce"
            )
        ...
        if operation == "create":
            # Create new entity
            result = self.salesforce_client.__getattr__(salesforce_object).create(
                salesforce_data
            )
            ...
            # Update existing entity
            result = self.salesforce_client.__getattr__(salesforce_object).update(
                external_id, salesforce_data
            )
            ...
            try:
                ...
                if "NOT_FOUND" in str(e):
                    # Entity not found, create new
                    result = self.salesforce_client.__getattr__(
                        salesforce_object
                    ).create(salesforce_data)

                    if result["success"]:
                        return {"id": result["id"], "updated": False, "created": True}
                    else:
                        raise ValueError(
                            f"Failed to create {entity_type}: {result['errors']}"
                        )
                else:
                    # Other error
                    raise
...
class CrmQueryTool(MCPTool):
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if crm_type == "hubspot":
                results = await self._query_hubspot(
                    entity_type, query_type, query_params, limit
                )
            elif crm_type == "salesforce":
                results = await self._query_salesforce(
                    entity_type, query_type, query_params, limit
                )
            else:
                raise ValueError(f"Unsupported CRM type: {crm_type}")
    ...
    async def _query_hubspot(
        ...
        if query_type == "get_by_id":
            ...
            api_response = self.hubspot_client.crm[hubspot_entity].basic_api.get_by_id(
                record_id=entity_id, properties=query_params.get("properties", [])
            )
            ...
            api_response = self.hubspot_client.crm[hubspot_entity].basic_api.get_page(
                limit=limit, properties=query_params.get("properties", [])
            )
    ...
    async def _query_salesforce(
        ...
        if query_type == "get_by_id":
            ...
            result = self.salesforce_client.__getattr__(salesforce_object).get(
                entity_id
            )
