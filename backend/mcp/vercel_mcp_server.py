...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        ...
        if name == "get_projects":
            ...
            if not project_id:
                return [TextContent(type="text", text=json.dumps({"error": "Project ID required"}))]
            
            project = await vercel_integration.get_project(project_id)
            
            return [TextContent(type="text", text=json.dumps(project or {"error": "Project not found"}, indent=2))]
            ...
            if not deployment_id:
                return [TextContent(type="text", text=json.dumps({"error": "Deployment ID required"}))]
            
            deployment = await vercel_integration.get_deployment(deployment_id)
            
            return [TextContent(type="text", text=json.dumps(deployment or {"error": "Deployment not found"}, indent=2))]
            ...
            if not name or not files:
                return [TextContent(type="text", text=json.dumps({"error": "Name and files required"}))]
            
            deployment = await vercel_integration.create_deployment(name, files, project_settings)
            
            return [TextContent(type="text", text=json.dumps(deployment or {"error": "Failed to create deployment"}, indent=2))]
            ...
            if not name or not project_id:
                return [TextContent(type="text", text=json.dumps({"error": "Name and project ID required"}))]
            
            domain = await vercel_integration.add_domain(name, project_id)
            
            return [TextContent(type="text", text=json.dumps(domain or {"error": "Failed to add domain"}, indent=2))]
            ...
            if not project_id or not key or not value:
                return [TextContent(type="text", text=json.dumps({"error": "Project ID, key, and value required"}))]
            
            env_var = await vercel_integration.set_environment_variable(project_id, key, value, target)
            
            return [TextContent(type="text", text=json.dumps(env_var or {"error": "Failed to set environment variable"}, indent=2))]
