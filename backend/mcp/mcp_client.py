...
class MCPClient:
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(
                        f"Discovered {len(tools.get('tools', []))} tools for {server_name}"
                    )
    ...
    async def call_tool(self, server: str, tool: str, **parameters) -> Dict[str, Any]:
        ...
        # Make the tool call
        payload = {"server": server, "tool": tool, "parameters": parameters}

        try:
            async with self.session.post(
                f"{self.gateway_url}/tool-call", json=payload
            ) as response:
    ...
    async def batch_call(self, calls: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for call in calls:
            task = self.call_tool(
                call["server"], call["tool"], **call.get("parameters", {})
            )
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append({"error": str(result), "call": calls[i]})
            else:
    ...
    async def execute_workflow(
        ...
        for step in workflow:
            ...
            result = await self.call_tool(
                step["server"], step["tool"], **resolved_params
            )
...
class MCPToolWrapper:
    ...
    def as_langchain_tool(self):
        ...
        return Tool(name=self.name, description=self.description, func=sync_wrapper)
