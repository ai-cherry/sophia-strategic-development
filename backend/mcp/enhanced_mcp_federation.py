...
class QueryClassifier:
    ...
    def __init__(self):
        ...
        self.server_keywords = {
            "gong": ["call", "meeting", "conversation", "sales", "revenue", "deal", "prospect"],
            "slack": ["message", "channel", "team", "notification", "chat", "communication"],
            "linear": ["issue", "ticket", "bug", "feature", "project", "development", "task"],
            "vercel": ["deployment", "build", "frontend", "website", "domain", "hosting"],
            "lambda_labs": ["gpu", "compute", "training", "model", "instance", "server"],
            "agno": ["agent", "ai", "assistant", "automation", "workflow", "orchestration"],
            "knowledge": ["document", "search", "knowledge", "information", "content", "file"]
        }
        
        self.query_patterns = {
            "data_retrieval": ["get", "find", "search", "list", "show", "retrieve"],
            "data_modification": ["create", "update", "delete", "modify", "change", "edit"],
            "analysis": ["analyze", "report", "summarize", "insights", "trends", "metrics"],
            "workflow": ["automate", "process", "execute", "run", "trigger", "orchestrate"]
        }
    ...
    def classify_query(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if server_scores:
            ...
            priority_servers = [
                server for server, score in server_scores.items() 
        ...
        return {
            ...
            "confidence": max(server_scores.values()) / len(self.server_keywords) if server_scores else 0.1,
...
class ResultAggregator:
    ...
    def _rank_results(self, results: List[FederatedQueryResult]) -> List[FederatedQueryResult]:
        ...
        def calculate_score(result: FederatedQueryResult) -> float:
            ...
            time_score = 1.0 - (result.execution_time_ms / max_time) if max_time > 0 else 1.0
    ...
    def _combine_data(self, ranked_results: List[FederatedQueryResult]) -> Dict[str, Any]:
...
class MCPFederation:
        ...
        try:
            ...
            self.servers = {
                "gong": MCPServerInfo(
                    ...
                    capabilities=["call_analysis", "sales_data", "conversation_insights"],
                    priority=1,
                    timeout_ms=5000
                ),
                "slack": MCPServerInfo(
                ...
                "linear": MCPServerInfo(
                    ...
                    capabilities=["issue_tracking", "project_management", "development"],
                    priority=1,
                    timeout_ms=4000
                ),
                ...
                "knowledge": MCPServerInfo(
                    ...
                    capabilities=["document_search", "knowledge_retrieval", "content_analysis"],
                    priority=1,
                    timeout_ms=3000
                )
            }
            ...
            for server_info in self.servers.values():
                try:
                    ...
                    logger.error(f"Failed to initialize MCP server {server_info.name}: {e}")
                    server_info.health_status = "unhealthy"
            
            # Start health monitoring
            self.health_monitor_task = asyncio.create_task(self._monitor_server_health())
    ...
    async def federated_query(
        ...
        try:
            ...
            logger.info(f"Query classified: type={classification['query_type']}, "
                       f"servers={classification['priority_servers']}, "
                       f"confidence={classification['confidence']:.2f}")
            
            # Filter servers based on health and classification
            target_servers = self._get_healthy_servers(classification['priority_servers'])
            ...
            if stream:
                return self._stream_federated_results(query, target_servers, context, classification)
            else:
                return await self._execute_parallel_queries(query, target_servers, context, classification)
    ...
    async def _execute_parallel_queries(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                valid_results.append(FederatedQueryResult(
        ...
        self._update_performance_metrics(total_time, target_servers, aggregated["success"])
    ...
    async def _query_single_server(
        ...
        try:
            ...
            tool_name = self._get_tool_for_query(server_name, classification["query_type"])
            ...
            if hasattr(server_info.server_instance, tool_name):
                ...
                result = await self._generic_server_query(server_info.server_instance, query, context)
            
            execution_time = (time.perf_counter() - start_time) * 1000
            
            # Calculate confidence score
            confidence = self._calculate_confidence_score(result, server_name, classification)
    ...
    async def _generic_server_query(self, server_instance: Any, query: str, context: Dict[str, Any]) -> Any:
    ...
    def _calculate_confidence_score(self, result: Any, server_name: str, classification: Dict[str, Any]) -> float:
        ...
        max_server_score = max(classification.get("server_scores", {}).values()) if classification.get("server_scores") else 1
        
        relevance_factor = server_score / max_server_score if max_server_score > 0 else 0.5
        
        # Adjust based on result quality
        quality_factor = self.result_aggregator._calculate_data_quality(result)
        
        # Combined confidence score
        confidence = (base_confidence * 0.4 + relevance_factor * 0.3 + quality_factor * 0.3)
                ...
                for server_name, server_info in self.servers.items():
                    try:
                        ...
                        if hasattr(server_info.server_instance, 'health_check'):
                            health_result = await server_info.server_instance.health_check()
                            server_info.health_status = health_result.get("status", "unknown")
                        else:
                            server_info.health_status = "healthy"  # Assume healthy if no health check
    ...
    def _update_performance_metrics(self, execution_time_ms: float, servers: List[str], success: bool):
        ...
        for server in servers:
            if server not in self.metrics["server_success_rates"]:
                self.metrics["server_success_rates"][server] = {"total": 0, "successful": 0}
    ...
    def get_federation_stats(self) -> Dict[str, Any]:
        ...
        server_stats = {}
        for server_name, server_info in self.servers.items():
            success_data = self.metrics["server_success_rates"].get(server_name, {"total": 0, "successful": 0})
            success_rate = (
        ...
        return {
            ...
            "federation_health": {
                "healthy_servers": sum(1 for s in self.servers.values() if s.health_status == "healthy"),
