...
class EnhancedIngestionService:
    ...
    async def create_ingestion_job(
        ...
        logger.info(
            f"Created ingestion job {job_id} for file {filename} ({file_size} bytes)"
        )
        return job
    ...
    async def process_file_async(self, job_id: str, file_content: bytes) -> None:
        ...
        try:
            ...
            text_content = await self._extract_text_content(
                file_content, job.file_type, job.filename
            )
            ...
            logger.info(
                f"Completed ingestion job {job_id}: {len(chunks)} chunks processed"
            )
    ...
    async def _process_chunk_to_knowledge_entry(
        self, chunk: DocumentChunk, job: IngestionJob
    ) -> str:
