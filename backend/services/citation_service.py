...
class CitationService:
    ...
    async def _create_citation_from_source(
        self, citation_id: int, source: dict[str, Any]
    ) -> Citation:
    ...
    async def _extract_inline_citation(
        self, citation_id: int, reference: str, full_response: str
    ) -> Citation:
    ...
    async def _verify_against_sources(
        self, citation: Citation, sources: list[dict[str, Any]]
    ) -> float:
        ...
        for source in sources:
            ...
            if source.get("title") == citation.title:
                ...
                source.get("title")
                and citation.title.lower() in source["title"].lower()
            ):
                score += 0.2
            ...
            if "content" in source and citation.excerpt:
                if citation.excerpt in source["content"]:
                    ...
                    word in source["content"] for word in citation.excerpt.split()[:5]
                ):
                    score += 0.1
