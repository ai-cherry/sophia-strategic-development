...
class AIaCIntentClassifier:
    ...
    def __init__(self):
        self.infrastructure_keywords = {
            ...
            "database", "kubernetes", "k8s", "pulumi", "docker",
            "restart", "reboot", "provision", "configure"
        }
        
        self.read_only_keywords = {
            "show", "list", "describe", "status", "check",
            "preview", "simulate", "what if", "would", "view",
            "get", "inspect", "monitor", "health"
        }
    ...
    async def classify(self, message: str) -> IntentClassification:
        ...
        if not is_infrastructure:
            ...
            if "how" in message_lower and any(k in message_lower for k in ["deploy", "scale", "infrastructure"]):
...
class ExecutionPlanGenerator:
    ...
    async def generate_plan(
        ...
        handler = self.service_handlers.get(
            target,
        ...
        if intent.intent_type == "deploy":
            steps.extend([
                ExecutionStep(
                    ...
                    params={"stack": intent.extracted_params.get("name", "production")},
...
class AIaCChatIntegration:
            ...
            return {
                ...
                "data": {
                    "pulumi_stacks": [
                        ...
                        {"name": "staging", "status": "update-available", "resources": 38}
                    ],
                    "kubernetes_clusters": [
                        {"name": "primary", "nodes": 5, "pods": 127, "health": "healthy"}
                    ]
                }
            }
    ...
    async def approve_plan(self, plan_id: str, user_id: str) -> Dict[str, Any]:
        ...
        await self.mem0_service.store_conversation_memory(
            ...
            conversation=[
                {
                    "role": "user",
                    "content": f"Approved plan {plan_id}"
                },
                {
                    "role": "assistant",
    ...
    async def reject_plan(self, plan_id: str, user_id: str, reason: str = "") -> Dict[str, Any]:
        """Reject a plan"""
        if plan_id not in self.active_plans:
            raise ValueError(f"Plan {plan_id} not found or expired")
        ...
        await self.mem0_service.store_conversation_memory(
            ...
            conversation=[
                {
                    "role": "user",
                    "content": f"Rejected plan {plan_id}: {reason}"
                },
                {
                    "role": "assistant",
                    "content": "Plan rejected and cancelled"
                }
            ],
            metadata={
                "category": "aiac",
