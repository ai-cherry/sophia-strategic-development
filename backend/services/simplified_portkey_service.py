...
class SimplifiedPortkeyService:
    ...
    def __init__(self):
        ...
        self.virtual_key = get_config_value("portkey_virtual_key_prod") or get_config_value("values_sophia_ai_portkey_virtual_key_prod")
    ...
    async def complete(self, request: SimplifiedLLMRequest) -> SimplifiedLLMResponse:
        ...
        try:
            ...
            async with self.session.post(
                ...
                cost_estimate = (tokens_used / 1000) * tier_config["estimated_cost_per_1k"]
    ...
    async def stream_complete(self, request: SimplifiedLLMRequest) -> AsyncGenerator[str, None]:
...
class SophiaLLM:
    ...
    @classmethod
    async def analyze_business(cls, query: str, context: Optional[Dict[str, Any]] = None) -> SimplifiedLLMResponse:
...
"""
    ...
    @classmethod
    async def generate_code(cls, requirements: str, language: str = "python") -> SimplifiedLLMResponse:
...
"""
        return await cls.chat(code_prompt, TaskType.CODE_GENERATION, max_tokens=4000, temperature=0.3)
