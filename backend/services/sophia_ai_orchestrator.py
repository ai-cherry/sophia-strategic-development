...
from backend.agents.specialized.interactive_sales_coach_agent import InteractiveSalesCoachAgent
...
class SophiaAIOrchestrator:
    ...
    async def process_request(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            logger.info(f"Processing orchestration request: {request.request_type.value}")
            ...
            logger.info(f"Request processed in {processing_time:.2f}ms: {response.success}")
            return response
    ...
    async def _process_knowledge_query(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            if is_sales_query and kb_result.get("success"):
                ...
                if coaching_result.get("success"):
                    ...
                    coaching_insights_generated = len(coaching_result.get("coaching_insights", []))
            
            # Generate suggestions
            suggestions = await self._generate_suggestions(request.query, kb_result)
            
            # Calculate confidence score
            confidence_score = self._calculate_confidence_score(kb_result, supporting_responses)
            ...
            response = OrchestrationResponse(
                ...
                services_used=["knowledge_base"] + (["sales_coach"] if is_sales_query else []),
    ...
    async def _process_knowledge_ingestion(self, request: OrchestrationRequest) -> OrchestrationResponse:
        """Process knowledge ingestion request"""
        try:
            # Ingest knowledge
            ingestion_result = await self.knowledge_base.interactive_knowledge_ingestion(
    ...
    async def _process_sales_coaching_request(self, request: OrchestrationRequest) -> OrchestrationResponse:
        """Process sales coaching request"""
        try:
            # Process sales coaching request
            coaching_result = await self.sales_coach.provide_real_time_coaching(
                sales_rep_id=request.user_id,
                context=request.context
            )
            ...
            response = OrchestrationResponse(
                ...
                supporting_responses=supporting_responses,
                services_used=["sales_coach"] + (["knowledge_base"] if supporting_responses else []),
                processing_time_ms=0.0,
                confidence_score=0.85,  # Sales coaching typically high confidence
                knowledge_items_accessed=knowledge_items_accessed,
                coaching_insights_generated=coaching_result.get("insights_generated", 0),
                memories_processed=0,
                suggested_actions=[
    ...
    async def _process_memory_preservation(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            if operation == "preserve_all":
                preservation_result = await self.memory_preservation.preserve_all_memories()
            elif operation == "validate_integrity":
                preservation_result = await self.memory_preservation.validate_memory_integrity()
            elif operation == "incremental_sync":
                from backend.services.memory_preservation_service import MemorySourceSystem
                source_system = MemorySourceSystem(request.context.get("source_system", "ai_memory_mcp"))
                preservation_result = await self.memory_preservation.incremental_memory_sync(source_system)
            else:
                preservation_result = {"success": False, "error": f"Unknown operation: {operation}"}
            ...
            response = OrchestrationResponse(
                ...
                knowledge_items_accessed=0,
                coaching_insights_generated=0,
                memories_processed=preservation_result.get("total_records_processed", 0),
                suggested_actions=[
            ...
            self.orchestration_analytics["service_utilization"]["memory_preservation"] += 1
    ...
    async def _process_teaching_session(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            kb_teaching_result = await self.knowledge_base.knowledge_teaching_interface(teaching_data)
            ...
            response = OrchestrationResponse(
                ...
                supporting_responses=supporting_responses,
                services_used=["knowledge_base"] + (["sales_coach"] if supporting_responses else []),
                processing_time_ms=0.0,
    ...
    async def _process_analytics_request(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            if analytics_type in ["comprehensive", "memory"]:
                analytics_tasks.append(self.memory_preservation.get_migration_analytics())
            ...
            if analytics_type in ["comprehensive", "knowledge"] and len(analytics_results) > 0:
                combined_analytics["service_analytics"]["knowledge_base"] = analytics_results[0]
            
            if analytics_type in ["comprehensive", "memory"] and len(analytics_results) > 1:
                combined_analytics["service_analytics"]["memory_preservation"] = analytics_results[-1]
    ...
    async def _process_health_check(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            health_results = {
                ...
                "knowledge_base": {
                    "status": "healthy" if self.knowledge_base and self.knowledge_base.initialized else "unhealthy",
                    "initialized": self.knowledge_base.initialized if self.knowledge_base else False
                },
                "sales_coach": {
                    "status": "healthy" if self.sales_coach and self.sales_coach.initialized else "unhealthy",
                    "initialized": self.sales_coach.initialized if self.sales_coach else False
                },
                "memory_preservation": {
                    "status": "healthy" if self.memory_preservation and self.memory_preservation.initialized else "unhealthy",
                    "initialized": self.memory_preservation.initialized if self.memory_preservation else False
                }
            }
            ...
            response = OrchestrationResponse(
                ...
                primary_response={
                    ...
                    "system_info": {
                        "initialized_services": sum(1 for s in health_results.values() if s["initialized"]),
                        "total_services": len(health_results),
                        "uptime_requests": self.orchestration_analytics["total_requests"]
                    }
                },
                supporting_responses=[],
                services_used=["orchestrator"],
                ...
                coaching_insights_generated=0,
                memories_processed=0,
                suggested_actions=[] if overall_health else ["Check service configurations", "Restart unhealthy services"],
                related_queries=[]
            )
    ...
    async def _process_unified_intelligence(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            response = OrchestrationResponse(
                ...
                supporting_responses=supporting_responses,
                services_used=["knowledge_base"] + (["sales_coach"] if is_sales_relevant else []),
                processing_time_ms=0.0,
                confidence_score=self._calculate_confidence_score(kb_result, supporting_responses),
                knowledge_items_accessed=kb_result.get("results_count", 0) if kb_result.get("success") else 0,
                coaching_insights_generated=coaching_insights_generated,
                memories_processed=0,
                suggested_actions=await self._generate_unified_suggestions(request, supporting_responses),
                related_queries=await self._generate_related_queries(request.query, supporting_responses)
            )
    ...
    def _is_sales_related_query(self, query: str) -> bool:
        ...
        sales_keywords = [
            "sales", "deal", "prospect", "lead", "customer", "revenue",
            "close", "pipeline", "quota", "forecast", "objection",
            "coaching", "training", "performance", "call", "meeting"
        ]
    ...
    def _calculate_confidence_score(self, primary_result: Dict, supporting_results: List[Dict]) -> float:
    ...
    async def _generate_suggestions(self, query: str, result: Dict) -> Dict[str, List[str]]:
    ...
    async def _synthesize_unified_response(self, request: OrchestrationRequest, kb_result: Dict, supporting_responses: List[Dict]) -> Dict[str, Any]:
        ...
        if kb_result.get("success") and kb_result.get("knowledge_items"):
            ...
            unified_response["knowledge_summary"] = f"Found {len(knowledge_items)} relevant knowledge items"
            
            # Create unified answer from top knowledge item
            if knowledge_items:
                top_item = knowledge_items[0]
                unified_response["unified_answer"] = f"Based on our knowledge base: {top_item.get('content', '')[:200]}..."
        
        # Add coaching insights if available
        for response in supporting_responses:
            if response.get("coaching_insights"):
                unified_response["coaching_insights"].extend(response["coaching_insights"])
    ...
    async def _generate_unified_suggestions(self, request: OrchestrationRequest, supporting_responses: List[Dict]) -> List[str]:
    ...
    async def _generate_related_queries(self, original_query: str, supporting_responses: List[Dict]) -> List[str]:
    ...
    async def get_orchestration_analytics(self) -> Dict[str, Any]:
        ...
        return {
            **self.orchestration_analytics,
            "service_health": {
                "knowledge_base": self.knowledge_base.initialized if self.knowledge_base else False,
                "sales_coach": self.sales_coach.initialized if self.sales_coach else False,
                "memory_preservation": self.memory_preservation.initialized if self.memory_preservation else False
            },
