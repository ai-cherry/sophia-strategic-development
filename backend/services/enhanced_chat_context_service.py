...
class ContextWindow:
    ...
    def truncate_context(self, context_items: List[Dict], max_tokens: int) -> List[Dict]:
        """Intelligently truncate context to fit within token limits"""
        if not context_items:
            return []
        
        # Calculate current token usage
        total_tokens = sum(self.estimate_tokens(item.get('content', '')) for item in context_items)
...
class EnhancedChatContextService:
    ...
    async def get_enhanced_context(
        ...
        optimized_context = self.context_window.truncate_context(all_context, max_context_tokens)
        ...
        return {
            ...
            'stats': context_stats,
            'total_tokens': sum(self.context_window.estimate_tokens(item['content']) for item in optimized_context)
        }
    
    async def _get_conversation_context(self, session_id: str, user_id: str) -> List[Dict]:
    ...
    async def _get_knowledge_context(self, query: str, user_id: str, limit: int = 10) -> List[Dict]:
    ...
    async def _get_cross_document_context(self, query: str, user_id: str, limit: int = 5) -> List[Dict]:
    ...
    def _calculate_context_stats(self, context_items: List[Dict]) -> Dict[str, Any]:
        ...
        # Date range
        timestamps = [item['timestamp'] for item in context_items if item.get('timestamp')]
        if timestamps:
            stats['date_range'] = {
    ...
    def _generate_context_summary(self, context_items: List[Dict]) -> str:
        ...
        if type_counts.get('cross_document', 0) > 0:
            summary_parts.append(f"{type_counts['cross_document']} cross-document references")
...
Once you upload these documents, I'll be able to provide detailed, contextualized responses based on your specific business information."""
        ...
        knowledge_items = [item for item in context_items if item.get('type') == 'knowledge']
        conversation_items = [item for item in context_items if item.get('type') == 'conversation']
        cross_doc_items = [item for item in context_items if item.get('type') == 'cross_document']
        ...
        if cross_doc_items:
            ...
            for item in cross_doc_items[:2]:  # Limit to top 2
                ...
                content_preview = item['content'][:250] + "..." if len(item['content']) > 250 else item['content']
                response_parts.append(f"• **{title}**: {content_preview}")
    ...
    async def save_enhanced_message(
        ...
        metadata = {
            'context_items_used': len(context_metadata.get('context_items', [])) if context_metadata else 0,
            'context_tokens': context_metadata.get('total_tokens', 0) if context_metadata else 0,
            'context_summary': context_metadata.get('context_summary', '') if context_metadata else '',
        ...
        """, (
            message_id, session_id, user_id, content, message_type, 
    ...
    async def get_session_analytics(self, session_id: str, user_id: str) -> Dict[str, Any]:
