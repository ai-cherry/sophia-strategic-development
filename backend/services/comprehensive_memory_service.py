...
class ComprehensiveMemoryService:
    ...
    async def get_raw_slack_data(self, limit: int = 100) -> List[dict]:
        ...
        try:
            ...
            """
            results = await self.cortex_service.execute_query(query)
            logger.info(f"Retrieved {len(results) if results else 0} Slack message records")
            return results or []
        except Exception as e:
        ...
        try:
            ...
            for call_data in raw_gong_data:
                try:
                    ...
                    logger.error(f"Failed to process Gong call {call_data.get('call_id', 'unknown')}: {e}")
                    continue
        ...
        try:
            ...
            for message_data in raw_slack_data:
                try:
                    ...
                    logger.error(f"Failed to process Slack message {message_data.get('ts', 'unknown')}: {e}")
                    continue
            
            logger.info(f"Successfully processed {processed_count} Slack message records")
            return processed_count
    ...
    async def create_integrated_conversation_memories(self, limit: int = 50) -> int:
        ...
        try:
            ...
            for conv_data in results or []:
                try:
                    from backend.agents.enhanced.data_models import IntegratedConversationRecord
                    ...
                    insights = await self._generate_conversation_insights(integrated_conv)
                    ...
                    logger.debug(f"Created integrated memory: {integrated_conv.conversation_id}")
                    
                except Exception as e:
                    logger.error(f"Failed to create integrated memory for {conv_data.get('conversation_id', 'unknown')}: {e}")
                    continue
            
            logger.info(f"Successfully created {created_count} integrated conversation memories")
            return created_count
            
        except Exception as e:
            logger.error(f"Failed to create integrated conversation memories: {e}", exc_info=True)
            return created_count

    async def _generate_conversation_insights(self, conversation: 'IntegratedConversationRecord') -> dict:
        ...
        try:
            ...
            analysis_result = await self.cortex_service.generate_completion(analysis_prompt)
            
            # Parse the analysis result (this would need more sophisticated parsing in production)
            return {
                'insights': ['Insight 1', 'Insight 2'],  # Placeholder - would parse from analysis_result
                'action_items': ['Action 1'],  # Placeholder - would parse from analysis_result
                'sentiment_score': 0.7,  # Placeholder - would extract from analysis_result
                'summary': analysis_result[:100] if analysis_result else 'No summary available'
            }
            
        except Exception as e:
            logger.error(f"Failed to generate insights for conversation {conversation.conversation_id}: {e}")
            return {
                'insights': [],
                'action_items': [],
    ...
    async def search_cross_platform_memories(self, query: str, platforms: List[str] = None, top_k: int = 10) -> List[MemoryRecord]:
        """
        Search for memories across multiple platforms (Gong, Slack, etc.).
        """
        logger.info(f"Searching cross-platform memories for: '{query}' across platforms: {platforms}")
        
        try:
            # Build platform filter
            platform_filter = ""
            if platforms:
                platform_conditions = [f"source_system = '{platform}'" for platform in platforms]
            ...
            logger.error(f"Failed to search cross-platform memories: {e}", exc_info=True)
            return []
