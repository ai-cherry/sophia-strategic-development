...
class DataTieringManager:
    ...
    async def _migrate_cold_to_warm(self, stats: dict[str, int]) -> None:
        ...
        try:
            ...
            results = await self.memory_service.execute_snowflake_query(
                query,
                (self.config.COLD_TIER_ACCESS_THRESHOLD, self.config.BATCH_SIZE)
            )
    ...
    async def _migrate_warm_to_hot(self, stats: dict[str, int]) -> None:
        ...
        try:
            ...
            results = await self.memory_service.execute_snowflake_query(
                query,
                (self.config.HOT_TIER_ACCESS_COUNT, self.config.BATCH_SIZE)
            )
    ...
    async def _compress_for_cold_storage(
        self,
        row: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        compressed = {
            ...
            "embedding_quantized": self._quantize_embedding(
                row.get("EMBEDDING", [])
            ),
            "metadata_minimal": {
    ...
    async def _update_cold_storage(
        ...
        metadata["compression_ratio"] = len(compressed_data["content_compressed"]) / len(
            compressed_data.get("original_size", 1)
        )
