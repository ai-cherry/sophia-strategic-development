...
class EnhancedSlackIntegrationService:
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        logger.info(
            "✅ Enhanced Slack Integration Service initialized with Events API/Socket Mode"
        )
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Authenticated as {auth_response['user']} in team {auth_response['team']}"
            )
    ...
    def _register_socket_mode_handlers(self):
        ...
        async def handle_socket_mode_request(
    ...
    async def _assess_business_impact(
        ...
        if any(
            channel in channel_info.get("name", "").lower()
        ...
        keyword_count = sum(
            1 for keyword in high_impact_keywords if keyword in text_lower
        )
    ...
    async def get_channel_analytics(
        ...
        try:
            ...
            if not history["ok"]:
                raise Exception(
                    f"Failed to get channel history: {history.get('error')}"
                )
            ...
            return SlackAnalytics(
                ...
                business_impact_score=await self._calculate_channel_business_impact(
                    messages
                ),
    ...
    async def _calculate_channel_business_impact(self, messages: List[Dict]) -> float:
        ...
        for msg in messages:
            ...
            if text:
                ...
                keyword_count = sum(
                    1 for keyword in impact_keywords if keyword.lower() in text.lower()
                )
