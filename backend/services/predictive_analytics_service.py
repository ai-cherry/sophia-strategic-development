...
class PredictiveAnalyticsService:
    ...
    async def initialize_prediction_models(self) -> bool:
        ...
        try:
            models_to_create = [
                {
                    'model_id': 'customer_churn_prediction', 'model_type': 'classification', 'target': 'churn_risk',
                    'features': ['last_activity_days', 'support_tickets', 'sentiment_score', 'contract_value'], 'horizon': '30_days'
                },
                {
                    'model_id': 'revenue_forecasting', 'model_type': 'regression', 'target': 'monthly_revenue',
                    'features': ['pipeline_value', 'closed_deals', 'market_indicators', 'seasonality'], 'horizon': '90_days'
                },
                {
                    'model_id': 'sales_conversion_prediction', 'model_type': 'classification', 'target': 'deal_outcome',
                    'features': ['deal_size', 'sales_stage', 'customer_engagement', 'competitor_presence'], 'horizon': '60_days'
                }
            ]
    ...
    async def generate_prediction(self, model_id: str, input_data: Dict[str, Any]) -> PredictionResult:
        ...
        try:
            ...
            contributing_factors = await self._analyze_contributing_factors(model_id, input_data, prediction_value)
            recommendation = await self._generate_prediction_recommendation(model_id, prediction_value, contributing_factors)
            return PredictionResult(model_id=model_id, prediction_value=prediction_value, confidence_interval=(prediction_value*0.8, prediction_value*1.2), confidence_score=confidence_score, contributing_factors=contributing_factors, recommendation=recommendation)
        except Exception as e:
    ...
    async def _analyze_contributing_factors(self, model_id: str, input_data: Dict[str, Any], prediction: Any) -> List[Dict[str, Any]]:
