...
class FileProcessor:
    ...
    async def process_file_content(self, file_content: bytes, file_type: str, filename: str) -> str:
        ...
        if file_type in ["text/plain", "text/markdown"]:
        ...
        elif file_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document" and HAS_DOCX:
            ...
            return await self._process_excel(file_content, filename)
                          ...
                          "application/vnd.ms-powerpoint"] and HAS_PPTX:
...
class LargeFileIngestionService:
    ...
    async def create_ingestion_job(
        ...
        if file_size > self.processor.max_file_size:
            raise ValueError(f"File too large: {file_size} bytes (max: {self.processor.max_file_size})")
        ...
        logger.info(f"Created ingestion job {job_id} for {filename} ({file_size:,} bytes)")
        return job_id
    ...
    async def process_job_async(self, job_id: str):
        ...
        try:
            ...
            await self._save_job_status(job_id, JobStatus.STORING, 20, total_chunks, 0, 0)
            ...
            for i, chunk in enumerate(chunks):
                ...
                await self._save_job_status(job_id, JobStatus.STORING, progress, total_chunks, i + 1, entries_created)
                
                # Small delay to prevent overwhelming
                await asyncio.sleep(0.05)
            
            # Complete job
            await self._save_job_status(job_id, JobStatus.COMPLETED, 100, total_chunks, total_chunks, entries_created)
            logger.info(f"Completed job {job_id}: {entries_created} entries created from {total_chunks} chunks")
    ...
    async def _create_knowledge_entry(self, chunk: Dict[str, Any], filename: str) -> str:
        ...
        """, (
            ...
            json.dumps(chunk['metadata']), datetime.now(), datetime.now()
        ))
        cursor.close()
    ...
    def _categorize_content(self, filename: str, content: str) -> str:
        ...
        categories = {
            "customers": ["customer", "client", "contact", "company", "tenant", "account"],
            "products": ["product", "service", "feature", "solution", "offering", "tool"],
            "employees": ["employee", "staff", "team", "personnel", "hr", "person"],
            "financial": ["financial", "revenue", "budget", "accounting", "payment", "invoice"],
            "processes": ["process", "procedure", "workflow", "manual", "instruction", "guide"],
    ...
    async def _ensure_category_exists(self, category_id: str):
        """Ensure category exists"""
        cursor = self.connection.cursor()
        cursor.execute("SELECT CATEGORY_ID FROM KNOWLEDGE_CATEGORIES WHERE CATEGORY_ID = ?", (category_id,))
