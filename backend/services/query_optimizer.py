...
class QueryOptimizer:
    ...
    def analyze_query(self, query: str) -> QueryPlan:
        ...
        cache_key = self._generate_cache_key(query, metadata_filters) if use_cache else None
    ...
    def _classify_query(self, query: str) -> QueryType:
        ...
        if any(word in query_lower for word in ["analyze", "compare", "trend", "statistics"]):
    ...
    def _determine_strategy(self, query_type: QueryType, query: str) -> ExecutionStrategy:
        """
    ...
    def _estimate_cost(self, strategy: ExecutionStrategy, tier_order: list[str]) -> float:
        """
        ...
        if strategy == ExecutionStrategy.CACHE_ONLY:
            ...
            cost = self.cost_weights["snowflake_vector"]
        elif strategy == ExecutionStrategy.BM25_ONLY:
            cost = self.cost_weights["snowflake_bm25"]
        elif strategy == ExecutionStrategy.HYBRID_PARALLEL:
            # Both searches run in parallel
            cost = max(
                self.cost_weights["snowflake_vector"],
                self.cost_weights["snowflake_bm25"]
            )
            ...
            cost = (
                ...
                self.cost_weights["snowflake_vector"] * 0.5
            )
            
        return round(cost, 2)
        
    def _estimate_latency(self, strategy: ExecutionStrategy, tier_order: list[str]) -> float:
        """
        ...
        if strategy == ExecutionStrategy.CACHE_ONLY:
            ...
            latency = self.latency_estimates["snowflake_vector"]
        elif strategy == ExecutionStrategy.BM25_ONLY:
            latency = self.latency_estimates["snowflake_bm25"]
        elif strategy == ExecutionStrategy.HYBRID_PARALLEL:
            # Parallel execution takes the max
            latency = max(
                self.latency_estimates["snowflake_vector"],
                self.latency_estimates["snowflake_bm25"]
            )
        elif strategy == ExecutionStrategy.HYBRID_SEQUENTIAL:
            # Sequential adds up
            latency = (
            ...
            latency = (
                ...
                self.latency_estimates["snowflake_vector"] * 0.4
            )
