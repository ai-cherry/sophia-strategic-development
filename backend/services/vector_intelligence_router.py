...
class VectorIntelligenceRouter:
    ...
    async def intelligent_search(
        ...
        if self.ai_memory and (
            query_analysis.get("requires_context")
            or query_analysis.get("requires_realtime")
        ):
            search_tasks.append(self.search_ai_memory(query, context, query_analysis))
    ...
    async def search_snowflake_cortex(
        ...
        try:
            ...
            table_name = table_mapping.get(
                query_analysis.get("query_type", "general"), "UNIFIED_KNOWLEDGE_BASE"
            )

            # Execute vector search
            results = await self.snowflake_cortex.search_business_context(
                query=query, table_name=table_name, limit=context.get("limit", 10)
            )
    ...
    async def search_ai_memory(
        ...
        try:
            ...
            category = category_mapping.get(
                query_analysis.get("query_type", "general"), "general"
            )

            # Search memories
            memories = await self.ai_memory.search_memories(
                query=query, category=category, limit=context.get("limit", 10)
            )
