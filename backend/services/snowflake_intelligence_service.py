...
class SnowflakeIntelligenceService:
    ...
    async def process_natural_language_query(self, query: IntelligenceQuery) -> IntelligenceResponse:
        ...
        try:
            ...
            structured_data = await self._retrieve_structured_data(query_analysis, query.query_text)
            unstructured_data = await self._retrieve_unstructured_data(query_analysis, query.query_text)
            ...
            follow_ups = await self._generate_follow_up_suggestions(query.query_text, synthesis_result)
    ...
    async def _analyze_query_intent(self, query_text: str) -> Dict[str, Any]:
        ...
        cortex_query = "SELECT SNOWFLAKE.CORTEX.COMPLETE('mixtral-8x7b', %s) as analysis"
        
        try:
            result = await self.semantic_service._execute_query(cortex_query, [analysis_prompt])
    ...
    async def _retrieve_structured_data(self, query_analysis: Dict[str, Any], query_text: str) -> Dict[str, Any]:
        ...
        if 'customer' in query_analysis.get('entities', []):
            ...
            structured_results['customers'] = await self.semantic_service._execute_query(customer_query)
        ...
        if 'employee' in query_analysis.get('entities', []):
            ...
            structured_results['employees'] = await self.semantic_service._execute_query(employee_query)
        ...
        if query_analysis.get('intent') == 'metrics':
            ...
            structured_results['metrics'] = await self.semantic_service._execute_query(metrics_query)
    ...
    async def _retrieve_unstructured_data(self, query_analysis: Dict[str, Any], query_text: str) -> Dict[str, Any]:
        """Retrieve relevant unstructured data using vector search"""
        unstructured_results = {}
        
        search_terms = query_analysis.get('entities', []) + [query_analysis.get('intent', '')]
        ...
        LIMIT 10;
        """
        unstructured_results['slack'] = await self.semantic_service._execute_query(slack_search_sql, [search_query])
        ...
        LIMIT 10;
        """
        unstructured_results['gong'] = await self.semantic_service._execute_query(gong_search_sql, [search_query])
        logger.info(f"Retrieved unstructured data for query: '{query_text}'")
        return unstructured_results
    
    async def _synthesize_response(self, query: str, structured: Dict, unstructured: Dict, context: Dict) -> Dict[str, Any]:
        ...
        cortex_query = "SELECT SNOWFLAKE.CORTEX.COMPLETE('mixtral-8x7b', %s) as synthesis"
        
        try:
            result = await self.semantic_service._execute_query(cortex_query, [synthesis_prompt])
    ...
    def _summarize_unstructured_data(self, data: Dict) -> str:
        ...
        for key, value in data.items():
             if isinstance(value, list) and len(value) > 0:
                summary_parts.append(f"- Found {len(value)} relevant documents from '{key}'.")
    ...
    async def _generate_follow_up_suggestions(self, query: str, synthesis_result: Dict) -> List:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        healthy = semantic_health['status'] == 'healthy' and vector_health['status'] == 'healthy'
