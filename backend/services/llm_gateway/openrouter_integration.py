...
class OpenRouterIntegration:
    ...
    async def execute(
        ...
        request_data = self._prepare_request(
            query, context, selected_model, settings
        )
        ...
        logger.info(f"OpenRouter execution completed in {execution_time:.2f}s with {selected_model}")
    ...
    async def _select_quality_optimal_model(
        ...
        candidates = self.model_catalog.get(task_type, self.model_catalog["general_purpose"])
        ...
        for model in candidates:
            if model in self.model_characteristics:
                ...
                matching_strengths = sum(
                    1 for strength in characteristics["strengths"]
    ...
    def _analyze_task_type(self, query: str, context: Optional[str]) -> str:
        """Analyze query to determine task type"""
        query_lower = query.lower()
        
        # Check for apartment industry keywords
        apartment_keywords = [
            "apartment", "tenant", "lease", "rent", "property",
            "payment", "maintenance", "resident", "unit"
        ]
    ...
    def _get_task_keywords(self, query: str) -> list[str]:
        ...
        capability_keywords = [
            "analyze", "code", "reason", "explain", "create",
            "write", "calculate", "optimize", "debug", "design"
        ]
    ...
    def _prepare_request(
        ...
        request_data = {
            ...
            "frequency_penalty": settings.get("frequency_penalty", 0.0) if settings else 0.0,
            "presence_penalty": settings.get("presence_penalty", 0.0) if settings else 0.0,
            "stream": False
        }
