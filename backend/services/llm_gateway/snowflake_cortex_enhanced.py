...
class SnowflakeCortexEnhanced:
    ...
    def __init__(self):
        ...
        logger.info("Snowflake Cortex Enhanced initialized with data-local AI capabilities")
    ...
    async def _is_data_related_query(
        self,
        query: str,
        ...
        data_keywords = [
            "data", "report", "analyze", "metrics", "kpi", "dashboard",
            "revenue", "payment", "customer", "tenant", "trend", "pattern"
        ]
    ...
    async def _prepare_data_context(self, query: str, bi_area: str) -> str:
        ...
        if bi_area == "revenue_analysis":
            context_parts.append("Focus: Payment trends, collection rates, revenue optimization")
        elif bi_area == "customer_intelligence":
            context_parts.append("Focus: Tenant behavior, satisfaction, retention strategies")
        elif bi_area == "operational_metrics":
            context_parts.append("Focus: Efficiency, cost reduction, performance optimization")
        elif bi_area == "market_intelligence":
            context_parts.append("Focus: Competitive positioning, market opportunities, pricing strategy")
    ...
    def _needs_sql_generation(self, query: str) -> bool:
        ...
        sql_indicators = [
            "show me", "list", "count", "sum", "average",
            "group by", "filter", "where", "between",
    ...
    def _prepare_cortex_prompt(self, query: str, context: Optional[str]) -> str:
    ...
    def _calculate_cost_savings(self, is_data_query: bool, response_length: int) -> float:
    ...
    async def search_business_data(
        ...
        embeddings = await self.cortex_service.generate_embedding(query)
