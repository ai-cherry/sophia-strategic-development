...
Uses Snowflake Cortex for AI-driven intent analysis and optimal routing
"""
...
class IntelligentQueryRouter:
    ...
    def __init__(self):
        ...
        self.domain_keywords = {
            "customer_management": ["customer", "client", "account", "churn", "expansion", "relationship", "deal"],
            "project_management": ["project", "task", "sprint", "milestone", "asana", "linear", "notion", "slack"],
            "financial_analysis": ["revenue", "profit", "cost", "budget", "financial", "money", "pricing"],
            "operational_metrics": ["performance", "efficiency", "utilization", "capacity", "resource"],
            "strategic_planning": ["strategy", "goal", "objective", "okr", "roadmap", "vision", "growth"],
            "infrastructure": ["system", "server", "database", "deployment", "infrastructure", "pulumi"],
            "development": ["code", "github", "pull request", "deployment", "bug", "feature"],
            "intelligence": ["insights", "analytics", "trends", "prediction", "forecast", "analysis"]
        }
        ...
        self.system_capabilities = {
            "mcp_servers": {
                ...
                "strengths": ["real-time", "operational", "immediate_actions", "live_updates"],
                "use_cases": ["task_creation", "status_updates", "notifications", "real_time_monitoring"]
            },
            "snowflake_cortex": {
                "response_time": 3.0,  # seconds
                "strengths": ["analytics", "historical_analysis", "predictions", "complex_queries"],
                "use_cases": ["trend_analysis", "forecasting", "semantic_search", "data_insights"]
            }
        }
    
    async def analyze_and_route(self, query: str, user_context: Dict[str, Any]) -> QueryAnalysis:
        """
        ...
        try:
            ...
            return QueryAnalysis(
                query_type=QueryType(analysis_data.get("query_type", "ANALYTICAL").lower()),
                urgency=UrgencyLevel(analysis_data.get("urgency", "MEDIUM").lower()),
                confidence=float(analysis_data.get("confidence", 0.7)),
                target_systems=analysis_data.get("target_systems", ["snowflake_cortex"]),
                processing_strategy=analysis_data.get("processing_strategy", "sequential"),
                estimated_response_time=self._estimate_response_time(
                    analysis_data.get("target_systems", ["snowflake_cortex"]),
    ...
    async def _fallback_analysis(self, query: str, user_context: Dict[str, Any]) -> QueryAnalysis:
        """Rule-based fallback when Cortex analysis fails"""
        query_lower = query.lower()
        
        # Determine query type based on keywords
        operational_keywords = ["create", "update", "delete", "send", "notify", "execute", "run", "start", "stop"]
        analytical_keywords = ["analyze", "trend", "forecast", "predict", "insights", "report", "dashboard"]
        ...
        if any(keyword in query_lower for keyword in system_keywords):
            ...
            target_systems = ["snowflake_cortex"]
        ...
        return QueryAnalysis(
            ...
            estimated_response_time=self._estimate_response_time(target_systems, "parallel"),
            context_requirements=["user_role", "business_context"],
            business_domain=business_domain,
            recommended_model="balanced"
        )
    
    def _estimate_response_time(self, target_systems: List[str], strategy: str) -> float:
    ...
    async def get_routing_recommendations(self, business_domain: str) -> Dict[str, Any]:
        ...
        try:
            recommendations = {
                "customer_management": {
                    "preferred_systems": ["mcp_servers", "snowflake_cortex"],
                    "strategy": "hybrid",
                    "real_time_priority": ["interaction_updates", "deal_changes", "churn_alerts"],
                    "analytical_priority": ["sentiment_analysis", "expansion_predictions", "relationship_health"]
                },
                "project_management": {
                    "preferred_systems": ["mcp_servers"],
                    "strategy": "operational_first",
                    "real_time_priority": ["task_updates", "status_changes", "notifications"],
                    "analytical_priority": ["team_performance", "project_health", "resource_planning"]
                },
                "financial_analysis": {
                    "preferred_systems": ["snowflake_cortex"],
                    "strategy": "analytical_first", 
                    "real_time_priority": ["deal_alerts", "revenue_milestones"],
                    "analytical_priority": ["revenue_trends", "forecasting", "profitability_analysis"]
                },
                "infrastructure": {
                    "preferred_systems": ["mcp_servers"],
                    "strategy": "operational_first",
                    "real_time_priority": ["system_health", "deployments", "scaling"],
                    "analytical_priority": ["cost_analysis", "utilization_trends", "capacity_planning"]
                }
            }
            
            return recommendations.get(business_domain, {
                "preferred_systems": ["snowflake_cortex"],
                "strategy": "balanced",
                "real_time_priority": [],
