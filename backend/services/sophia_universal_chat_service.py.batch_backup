...
class SophiaUniversalChatService:
        ...
        try:
            ...
            if exa_api_key:
                self.exa_client = httpx.AsyncClient(
                    base_url="https://api.exa.ai", headers={"x-api-key": exa_api_key}
                )
    ...
    async def process_chat_message(
        ...
        try:
            ...
            search_request = SearchRequest(
                ...
                competitor_focus=user_profile.custom_context.get(
                    "priority_competitors", []
                ),
            )
            ...
            result = await self._apply_personality(
                result, user_profile.preferred_personality
            )
    ...
    async def _determine_search_context(
        self, message: str, user_profile: UserProfile
    ) -> SearchContext:
        ...
        has_internet_indicators = any(
            keyword in message_lower for keyword in internet_keywords
        )
        has_internal_indicators = any(
            keyword in message_lower for keyword in internal_keywords
        )
    ...
    async def _execute_blended_search(self, request: SearchRequest) -> SearchResult:
        ...
        try:
            ...
            all_sources = []
            all_sources.extend(
                [{"type": "internal", **source} for source in internal_results]
            )
            all_sources.extend(
                [{"type": "internet", **source} for source in internet_results]
            )
            ...
            for schema in request.internal_schemas:
                try:
                    ...
                    schema_results = await self.cortex_service.search_with_context(
                        query=request.query, schema=schema, limit=5
                    )
    ...
    async def _search_with_exa(self, request: SearchRequest) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.exa_client.post(
                "/search",
                json={
...
"""
            ...
            return response.get(
                "content",
                "I apologize, but I couldn't synthesize the search results at this time.",
            )
    ...
    def _format_results_for_synthesis(self, results: List[Dict[str, Any]]) -> str:
        ...
        for i, result in enumerate(results[:5]):  # Limit to top 5 for context
            formatted.append(
                f"{i + 1}. {result.get('title', 'Untitled')}: {result.get('content', '')[:200]}..."
            )

        return "\n".join(formatted) if formatted else "No results found."

    async def _apply_personality(
        self, result: SearchResult, personality: SophiaPersonality
    ) -> SearchResult:
    ...
    def _add_executive_insights(self, content: str, result: SearchResult) -> str:
        ...
        if result.internal_results and result.internet_results:
            insights.append(
                "📊 This analysis combines your internal company data with current market intelligence."
            )

        if len(result.sources) > 5:
            insights.append(
                f"🔍 I've analyzed {len(result.sources)} sources to provide this comprehensive view."
            )
    ...
    def _add_technical_details(self, content: str, result: SearchResult) -> str:
        ...
        if result.internal_results:
            technical_notes.append(
                f"Internal data sources: {len(result.internal_results)} schemas queried"
            )

        if result.internet_results:
            technical_notes.append(
                f"External intelligence: {len(result.internet_results)} sources analyzed"
            )

        if result.synthesis_quality > 0.9:
            technical_notes.append(
                f"Analysis confidence: {result.synthesis_quality:.1%}"
            )
    ...
    async def create_user_profile(
        ...
        try:
            ...
            user_profile = UserProfile(
                ...
                accessible_schemas=self.schema_access_map[
                    UserAccessLevel(user_data["access_level"])
                ],
                search_permissions=user_data.get(
                    "search_permissions", [SearchContext.INTERNAL_ONLY]
                ),
                preferred_personality=SophiaPersonality(
                    user_data.get("preferred_personality", "friendly_assistant")
                ),
        ...
        try:
            if user_id:
                ...
                return {
                    ...
                    "usage_percentage": (
                        user_profile.api_usage_today / user_profile.api_quota_daily
                    )
                    * 100,
                    "last_active": user_profile.last_active.isoformat(),
                    "accessible_schemas": user_profile.accessible_schemas,
                    "search_permissions": [
                        perm.value for perm in user_profile.search_permissions
                    ],
                }
            else:
                # All users analytics
                ...
                total_usage = sum(
                    profile.api_usage_today for profile in self.user_profiles.values()
                )
