...
class SnowflakeAdminIntentClassifier:
    ...
    def __init__(self):
        self.admin_patterns = {
            SnowflakeAdminIntent.SCHEMA_MANAGEMENT: {
                ...
                "phrases": ["create schema", "show schemas", "describe schema", "grant on schema"],
                "patterns": [r"create\s+schema", r"show\s+schemas", r"describe\s+schema"]
            },
            SnowflakeAdminIntent.WAREHOUSE_MANAGEMENT: {
                "keywords": ["warehouse", "compute", "create warehouse", "alter warehouse"],
                "phrases": ["create warehouse", "show warehouses", "alter warehouse", "suspend warehouse"],
                "patterns": [r"create\s+warehouse", r"show\s+warehouses", r"alter\s+warehouse"]
            },
            ...
            SnowflakeAdminIntent.GRANTS_MANAGEMENT: {
                ...
                "phrases": ["grant usage", "revoke access", "show grants", "grant select"],
            ...
            SnowflakeAdminIntent.OBJECT_INSPECTION: {
                ...
                "phrases": ["show tables", "describe table", "show columns", "list objects"],
                "patterns": [r"show\s+\w+", r"describe\s+\w+", r"list\s+\w+"]
            },
            SnowflakeAdminIntent.CONFIRMATION: {
                "keywords": ["confirm", "yes", "proceed", "execute"],
                "phrases": ["yes confirm", "proceed with", "execute sql", "confirm operation"],
    ...
    def is_snowflake_admin_query(self, query: str) -> bool:
        ...
        snowflake_keywords = [
            "snowflake", "schema", "warehouse", "role", "grant", "revoke",
            "create", "drop", "alter", "show", "describe", "sql"
        ]
        
        admin_keywords = [
            "admin", "administration", "manage", "configure", "setup"
        ]
        ...
        for intent_patterns in self.admin_patterns.values():
            ...
            if any(phrase in query_lower for phrase in intent_patterns["phrases"]):
                return True
            if any(re.search(pattern, query_lower) for pattern in intent_patterns["patterns"]):
                return True
        
        return has_snowflake and any(keyword in query_lower for keyword in admin_keywords)
    ...
    def extract_entities(self, query: str, intent: SnowflakeAdminIntent) -> Dict[str, Any]:
        """Extract entities based on intent"""
        entities = {}
        query_lower = query.lower()
        
        # Extract common entities
        schema_match = re.search(r"schema\s+([A-Za-z_][A-Za-z0-9_]*)", query, re.IGNORECASE)
        if schema_match:
            entities["schema_name"] = schema_match.group(1)
        
        warehouse_match = re.search(r"warehouse\s+([A-Za-z_][A-Za-z0-9_]*)", query, re.IGNORECASE)
        if warehouse_match:
        ...
        if intent == SnowflakeAdminIntent.CONFIRMATION:
            confirm_match = re.search(r"confirm[_\s]+([A-Za-z0-9_]+)", query, re.IGNORECASE)
            if confirm_match:
...
class SnowflakeAdminChatIntegration:
    ...
    async def _handle_confirmation(
        self,
        parsed_query: SnowflakeAdminQuery,
        user_id: str
    ) -> Dict[str, Any]:
        """Handle confirmation queries"""
    ...
    def _format_admin_response(
        ...
        result = {
            ...
            "environment": response.environment.value if response.environment else "unknown",
            "execution_time": f"{response.execution_time:.2f}s"
        }
        ...
        if response.results:
            ...
            if len(response.results) > 10:
                result["results_note"] = f"Showing 10 of {len(response.results)} results"
...
async def process_snowflake_admin_query(
    ...
    Returns:
        Formatted response
    """
    return await snowflake_admin_integration.process_admin_query(query, user_id, user_context)
