...
from backend.services.migration_orchestrator_client import (
    get_migration_orchestrator_client,
)
...
class EnhancedUnifiedChatService:
    ...
    async def _handle_migration_control(
        self, request: UnifiedChatRequest
    ) -> UnifiedChatResponse:
        ...
        try:
            ...
            if result.get("success"):
                ...
                response_message = await self._generate_executive_response(
    ...
    async def _handle_business_intelligence(
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["migration", "salesforce", "hubspot"]
            ):
                summary = await self.migration_client.get_executive_summary()

                response_message = await self._format_executive_summary(
    ...
    async def _handle_system_monitoring(
        ...
        try:
            ...
            healthy_systems = [
                name
                for name, status in health_indicators.items()
                if status == "healthy"
            ]
            degraded_systems = [
                name
            ...
            if healthy_systems:
                response_parts.append(
                    f"✅ **Healthy Systems ({len(healthy_systems)}):** {', '.join(healthy_systems)}"
                )

            if degraded_systems:
                response_parts.append(
                    f"⚠️ **Degraded Systems ({len(degraded_systems)}):** {', '.join(degraded_systems)}"
                )

            if offline_systems:
                response_parts.append(
                    f"❌ **Offline Systems ({len(offline_systems)}):** {', '.join(offline_systems)}"
                )
            ...
            if migration_status.get("status") != "error":
                ...
                if migration_status.get("metrics"):
                    ...
                    response_parts.append(
                        f"**Progress:** {metrics.get('overall_progress', 0):.1f}%"
                    )
                    response_parts.append(
    ...
    async def _handle_strategic_planning(
        ...
        try:
            ...
            if any(
                keyword in request.message.lower()
                for keyword in ["migration", "automation", "ai", "platform"]
            ):
                summary = await self.migration_client.get_executive_summary()
    ...
    async def _generate_executive_response(
        self, result: dict[str, Any], original_command: str
    ) -> str:
    ...
    async def _format_executive_summary(
        self, summary: dict[str, Any], query: str
    ) -> str:
        ...
        if overview:
            ...
            response_parts.append(
                f"**Progress:** {overview.get('overall_progress', 0):.1f}%"
            )
            response_parts.append(
        ...
        if business_impact:
            ...
            if cost_savings:
                response_parts.append("\n**💰 Business Impact:**")
                response_parts.append(
                    f"• Annual Savings: ${cost_savings.get('annual_salesforce_savings', 0):,}"
                )
                response_parts.append(
                    f"• ROI vs Consulting: {cost_savings.get('migration_cost_vs_consulting', 'N/A')}"
                )
                response_parts.append(
                    f"• Time Efficiency: {cost_savings.get('time_savings', 'N/A')}"
                )
        ...
        if risk_assessment:
            response_parts.append("\n**🎯 Risk Assessment:**")
            response_parts.append(
                f"• Risk Level: {risk_assessment.get('risk_level', 'Unknown').title()}"
            )
            response_parts.append(
                f"• Total Issues: {risk_assessment.get('total_issues', 0)}"
            )
            response_parts.append(
                f"• Critical Issues: {risk_assessment.get('critical_issues', 0)}"
            )
    ...
    async def _generate_strategic_insights(self, summary: dict[str, Any]) -> str:
        ...
        if business_impact:
            ...
            if strategic_value:
                insights.append(
                    "• **AI Capabilities:** Demonstrated AI-enhanced business process automation"
                )
                insights.append(
                    "• **Competitive Advantage:** Built industry-leading migration platform internally"
                )
                insights.append(
                    "• **Internal Expertise:** Developed core AI and automation competencies"
                )

        insights.append(
            "• **Scalability:** Platform can be extended to other business transformations"
        )
        insights.append(
            "• **Market Position:** Positions Pay Ready as AI-native and technically sophisticated"
        )
        insights.append(
            "• **Future Opportunities:** Foundation for additional enterprise automation projects"
        )
