...
class OptimizedDashboardService:
    ...
    def __init__(self):
        ...
        self.metrics_collector = (
            None  # Will be initialized from unified_connection_manager
        )

    @HierarchicalCacheManager().cache(
        ttl=300, key_pattern="dashboard:performance:{user_id}"
    )
    async def get_performance_dashboard(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "service_metrics": (
                service_metrics if not isinstance(service_metrics, Exception) else {}
            ),
    ...
    async def _get_system_health(self) -> dict[str, Any]:
        ...
        return {
            "overall_status": (
                "healthy" if healthy_services == len(services) else "degraded"
            ),
    ...
    async def _get_service_metrics(self) -> dict[str, Any]:
        ...
        return {
            "api_response_times": {"avg_ms": 125, "p95_ms": 200, "p99_ms": 500},
