...
class LargeDataImportService:
    ...
    async def create_import_job(
        ...
        logger.info(f"Created import job {job_id} for {data_type.value}: {total_records} estimated records")
        return job
    ...
    async def process_import_job(self, job_id: str) -> ImportJob:
        ...
        try:
            ...
            logger.info(f"Completed import job {job_id}: {job.processed_records} records processed")
            return job
        ...
        with zipfile.ZipFile(file_path, 'r') as zip_file:
            ...
            if 'channels.json' in zip_file.namelist():
                ...
                for channel in channels:
                    ...
                    channel_files = [f for f in zip_file.namelist() if f.startswith(f'{channel_name}/')]
    ...
    async def _extract_document_archive_data(self, file_path: str) -> List[Dict[str, Any]]:
    ...
    async def _process_email_batch(self, emails: List[Dict[str, Any]], job: ImportJob) -> None:
        ...
        for email in emails:
            try:
                ...
                logger.error(f"Failed to process email {email.get('email_id')}: {str(e)}")
    
    async def _process_calendar_batch(self, events: List[Dict[str, Any]], job: ImportJob) -> None:
        ...
        for event in events:
            try:
                ...
                logger.error(f"Failed to process calendar event {event.get('event_id')}: {str(e)}")
    
    async def _process_slack_channel_batch(self, channel_data: Dict[str, Any], job: ImportJob) -> None:
    ...
    async def _process_slack_message_batch(self, messages: List[Dict[str, Any]], channel_info: Dict[str, Any], job: ImportJob) -> None:
            ...
            try:
                if message.get('type') == 'message' and message.get('text'):
                    ...
                    document = {
                        ...
                        'tags': ['slack', 'message', 'communication', job.data_type.value],
                        'category': 'communication',
    ...
    async def _process_generic_batch(self, records: List[Dict[str, Any]], job: ImportJob) -> None:
        ...
        for record in records:
            try:
                ...
                document = {
                    ...
                    'title': record.get('title', f"Imported Record from {job.data_type.value}"),
    ...
    async def _process_document_batch(self, documents: List[Dict[str, Any]], job: ImportJob) -> None:
        ...
        for doc in documents:
            try:
                ...
                logger.error(f"Failed to process document {doc.get('file_name')}: {str(e)}")
    
    async def _estimate_record_count(self, file_path: str, data_type: ImportDataType) -> int:
