...
class EnhancedAsanaIntegrationTestSuite:
    ...
    async def initialize_services(self) -> None:
        ...
        try:
            ...
            self.intelligence_agent = AsanaProjectIntelligenceAgent({
                "agent_id": "test_asana_intelligence",
    ...
    async def run_transformation_tests(self) -> List[TestResult]:
        ...
        try:
            ...
            validation_result = await self.cortex_service.execute_query(validation_query)
                ...
                if transformed_records > 0:
                    results.append(TestResult(
                        ...
                        metrics={
                            ...
                            "metadata_coverage": records_with_metadata / max(transformed_records, 1),
            ...
            if not task_result.empty:
                ...
                results.append(TestResult(
                    ...
                    metrics={
                        ...
                        "assignment_rate": assigned_tasks / max(transformed_tasks, 1),
    ...
    async def run_ai_enrichment_tests(self) -> List[TestResult]:
        ...
        try:
            ...
            await self.cortex_service.execute_query("CALL GENERATE_ASANA_PROJECT_AI_EMBEDDINGS()")
            execution_time = time.time() - start_time
            ...
            enrichment_result = await self.cortex_service.execute_query(enrichment_query)
            ...
            if not enrichment_result.empty:
                ...
                if embedding_coverage >= self.test_suite.performance_thresholds["min_ai_embedding_coverage"]:
                    results.append(TestResult(
                        ...
                        metrics={
                            ...
                            "summary_coverage": projects_with_summaries / max(total_projects, 1),
    ...
    async def run_intelligence_tests(self) -> List[TestResult]:
        ...
        start_time = time.time()
        try:
            report = await self.intelligence_agent.generate_project_intelligence_report()
            execution_time = time.time() - start_time
        ...
        try:
            ...
            if risk_assessments:
                high_risk_count = sum(1 for r in risk_assessments if r.overall_risk.value in ["high", "critical"])
    ...
    async def run_chat_integration_tests(self) -> List[TestResult]:
        ...
        try:
            ...
            query_result = await self.chat_service.process_unified_query(
                "Show me the status of all active Asana projects",
                context
            )
            execution_time = time.time() - start_time
            ...
            if query_result and query_result.confidence_score > 0.5:
                ...
                results.append(TestResult(
                    ...
                    details={"query_result": query_result.__dict__ if query_result else None}
                ))
    ...
    async def run_performance_tests(self) -> List[TestResult]:
        ...
        try:
            ...
            if avg_query_time <= self.test_suite.performance_thresholds["query_response_max_time"]:
                results.append(TestResult(
        ...
        try:
            ...
            for test_function in test_functions:
                try:
                    ...
                    logger.error(f"❌ Test category failed: {test_function.__name__}: {e}")
                    # Add a failure result for the entire category
                    self.test_suite.results.append(TestResult(
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        for category in TestCategory:
            category_results = [r for r in self.test_suite.results if r.category == category]
            if category_results:
        ...
        if self.test_suite.start_time and self.test_suite.end_time:
            total_execution_time = (self.test_suite.end_time - self.test_suite.start_time).total_seconds()
        
        return {
            "test_suite_summary": {
                "environment": self.test_suite.environment,
                "start_time": self.test_suite.start_time.isoformat() if self.test_suite.start_time else None,
            ...
            "failed_tests": [
                ...
                for r in self.test_suite.results if r.status == "FAIL"
            ]
        }

async def main():
    """Main test execution function"""
    parser = argparse.ArgumentParser(description="Enhanced Asana Integration Test Suite")
    ...
    parser.add_argument("--skip-data-validation", action="store_false", dest="include_data_validation")
