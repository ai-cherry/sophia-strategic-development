...
def validate_ddl_file(file_path: str) -> dict:
    ...
    try:
        ...
        for statement in statements:
            ...
            if "CREATE TABLE" in statement_upper:
                table_match = re.search(
                    r"CREATE TABLE\s+(?:IF NOT EXISTS\s+)?(\S+)", statement_upper
                )
                if table_match:
            ...
            if "CREATE OR REPLACE PROCEDURE" in statement_upper:
                proc_match = re.search(
                    r"CREATE OR REPLACE PROCEDURE\s+(\S+)", statement_upper
                )
                if proc_match:
            ...
            if "CREATE OR REPLACE TASK" in statement_upper:
                task_match = re.search(
                    r"CREATE OR REPLACE TASK\s+(\S+)", statement_upper
                )
                if task_match:
            ...
            if "CREATE INDEX" in statement_upper:
                index_match = re.search(
                    r"CREATE INDEX\s+(?:IF NOT EXISTS\s+)?(\S+)", statement_upper
                )
                if index_match:
        ...
        for procedure in expected_procedures:
            if not any(
                procedure in found_proc for found_proc in results["procedures_found"]
            ):
