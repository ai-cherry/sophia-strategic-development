...
class SnowflakeApplicationLayerDeployer:
    ...
    def __init__(self, environment: str = "DEV", dry_run: bool = False):
        ...
        # Deployment tracking
        self.deployment_id = (
            f"DEPLOY_{self.environment}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _define_deployment_steps(self) -> List[DeploymentStep]:
        ...
        return [
            ...
            DeploymentStep(
                ...
                dependencies=[
                    "setup_gong_transformation",
                    "setup_hubspot_transformation",
                ],
            ),
            DeploymentStep(
    ...
    async def _execute_step(self, step: DeploymentStep) -> None:
        ...
        try:
            ...
            logger.info(
                f"Step {step.name} completed with status: {step.status.value} in {step.duration_seconds:.2f}s"
            )
    ...
    async def _initialize_services(self) -> None:
        ...
        self.config_manager = SnowflakeConfigManager(
            environment=self.environment, application_name="SOPHIA_AI"
        )
    ...
    async def _setup_gong_transformation(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Gong transformation test: {result[0] if result else 'Success'}"
            )
        finally:
            cursor.close()
    ...
    async def _setup_hubspot_transformation(self) -> None:
        ...
        try:
            ...
            result = cursor.fetchone()
            logger.info(
                f"HubSpot transformation test: {result[0] if result else 'Success'}"
            )
        finally:
            cursor.close()
    ...
    async def _setup_embedding_columns(self) -> None:
        ...
        for table in tables:
            try:
                success = await self.cortex_service.ensure_embedding_columns_exist(
                    table
                )
                logger.info(
                    f"Embedding columns setup for {table}: {'Success' if success else 'Failed'}"
                )
            except Exception as e:
    ...
    async def _generate_sample_embeddings(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Sample embeddings generated: {stats.successful_embeddings}/{stats.total_records}"
            )
    ...
    async def _validate_configuration(self) -> None:
        ...
        test_flags = [
            "enhanced_ai_memory",
            "cortex_auto_embeddings",
            "debug_verbose_logging",
        ]

        for flag_name in test_flags:
            try:
                enabled = await self.config_manager.evaluate_feature_flag(
                    flag_name, user_id="test_user"
                )
    ...
    async def _test_data_queries(self) -> None:
        ...
        test_queries = [
            (
                "STG_HUBSPOT_DEALS count",
                "SELECT COUNT(*) FROM STG_TRANSFORMED.STG_HUBSPOT_DEALS",
            ),
            (
                "STG_GONG_CALLS count",
                "SELECT COUNT(*) FROM STG_TRANSFORMED.STG_GONG_CALLS",
            ),
            (
                "AI_MEMORY records count",
                "SELECT COUNT(*) FROM AI_MEMORY.MEMORY_RECORDS",
            ),
            (
                "CONFIG settings count",
                "SELECT COUNT(*) FROM CONFIG.APPLICATION_SETTINGS",
            ),
        ]
    ...
    async def _test_ai_memory_integration(self) -> None:
        ...
        test_content = (
            "This is a test deal analysis for high-value enterprise opportunity"
        )
            ...
            try:
                ...
                deal_results = (
                    await self.cortex_service.search_hubspot_deals_with_ai_memory(
                        query_text=query, top_k=3, similarity_threshold=0.6
                    )
                )
                ...
                logger.info(
                    f"Vector search '{query}': {len(deal_results)} deals, {len(call_results)} calls"
                )
    ...
    async def _test_langgraph_workflow(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"LangGraph workflow test: {result.get('status', 'Unknown status')}"
            )

            if result.get("consolidated_findings"):
                health_score = result["consolidated_findings"].get(
                    "deal_health_score", 0
                )
    ...
    async def _health_check(self) -> None:
        ...
        if self.cortex_service:
            try:
                ...
                test_sentiment = (
                    await self.cortex_service.analyze_sentiment_with_cortex(
                        "This is a positive test message"
                    )
                )
                health_results["cortex_service"] = (
                    "healthy" if test_sentiment else "degraded"
                )
            except Exception as e:
    ...
    async def _generate_deployment_report(self) -> None:
        ...
        logger.info(
            f"Steps: {self.report.completed_steps}/{self.report.total_steps} completed"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Deploy Sophia AI Snowflake Application Layer"
    )
    parser.add_argument(
        "--environment",
    ...
    parser.add_argument(
        "--deploy-all", action="store_true", help="Deploy all components"
    )
    parser.add_argument(
        "--test-only", action="store_true", help="Run only testing and validation"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
