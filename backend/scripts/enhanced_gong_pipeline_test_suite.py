...
class EnhancedGongPipelineTestSuite:
    ...
    def __init__(self, environment: TestEnvironment = TestEnvironment.DEV):
        ...
        self.database = (
            "SOPHIA_AI" if environment == TestEnvironment.DEV else "SOPHIA_AI_STAGING"
        )
        ...
        logger.info(
            f"🧪 Enhanced Gong Pipeline Test Suite initialized for {environment.value}"
        )
    ...
    async def _run_connectivity_tests(self):
        ...
        await self._run_test(
            "test_gong_api_connection", "connectivity", self._test_gong_api_connection
        )
        await self._run_test(
            "test_snowflake_connection", "connectivity", self._test_snowflake_connection
        )
        await self._run_test(
            "test_pulumi_esc_secrets", "connectivity", self._test_pulumi_esc_secrets
        )
        await self._run_test(
        ...
        async with SnowflakeDataLoader(self.test_config) as snowflake:
            ...
            try:
                ...
                for schema in required_schemas:
                    cursor.execute(
                        f"SHOW SCHEMAS LIKE '{schema}' IN DATABASE {self.database}"
                    )
    ...
    async def _run_ingestion_tests(self):
        ...
        await self._run_test(
            "test_raw_data_landing", "ingestion", self._test_raw_data_landing
        )
        await self._run_test(
            "test_batch_processing", "ingestion", self._test_batch_processing
        )
        await self._run_test(
    ...
    async def _run_transformation_tests(self):
        ...
        await self._run_test(
            "test_stg_table_population",
            "transformation",
            self._test_stg_table_population,
        )
        await self._run_test(
    ...
    async def _run_ai_enrichment_tests(self):
        ...
        await self._run_test(
            "test_sentiment_analysis", "ai_enrichment", self._test_sentiment_analysis
        )
        await self._run_test(
    ...
    async def _test_throughput(self) -> dict[str, Any]:
        ...
        calls_per_second = (
            results["metrics"]["calls_processed"] / duration if duration > 0 else 0
        )
        ...
        return {
            ...
            "meets_performance_target": calls_per_second
    ...
    async def _test_latency(self) -> dict[str, Any]:
        ...
        async with GongAPIClient(self.test_config) as gong:
            ...
            await gong.get_calls(
                datetime.now() - timedelta(hours=1), datetime.now(), limit=1
            )
    ...
    def generate_test_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "success_rate": (
                    (
                        self.suite_results.passed_tests
                        / self.suite_results.total_tests
                        * 100
                    )
                    if self.suite_results.total_tests > 0
                    else 0
                ),
            },
            "test_results": [
                asdict(result) for result in self.suite_results.test_results
            ],
        }
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Enhanced Gong Pipeline Test Suite")
    ...
    parser.add_argument(
        "--output-file", type=str, help="Output file for test results (JSON format)"
    )
