...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('snowflake_deployment.log')
    ]
)
...
class PhoenixSchemaDeployer:
    ...
    def load_schema_ddl(self) -> str:
        ...
        """
        try:
            schema_file = Path(__file__).parent.parent / 'snowflake_setup' / 'unified_platform_schema.sql'
    ...
    def get_statement_type(self, statement: str) -> str:
        ...
        elif statement_upper.startswith('CREATE TABLE') or statement_upper.startswith('CREATE OR REPLACE TABLE'):
            return 'CREATE TABLE'
        ...
        elif statement_upper.startswith('CREATE TASK') or statement_upper.startswith('CREATE OR REPLACE TASK'):
            return 'CREATE TASK'
    ...
    async def deploy_schema(self) -> bool:
        ...
        try:
            ...
            logger.info(f"📊 Success rate: {(successful_statements / len(statements)) * 100:.1f}%")
    ...
    async def verify_deployment(self) -> Dict[str, any]:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) as registry_count FROM SOPHIA_CORE.MCP_SERVER_REGISTRY")
...
async def main():
    ...
    try:
        ...
        # Save report to file
        report_file = Path(__file__).parent.parent.parent / 'PHOENIX_DEPLOYMENT_REPORT.md'
