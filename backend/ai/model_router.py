...
class OpenRouterClient:
    ...
    async def chat_completion(self, 
        ...
        try:
            ...
            if not model:
                model = self._select_optimal_model(messages, kwargs.get("capabilities", []))
    ...
    def _select_optimal_model(self, messages: List[Dict[str, str]], capabilities: List[str]) -> str:
        """
        ...
        for model_key, config in self.models.items():
            if not capabilities or any(cap in config.capabilities for cap in capabilities):
...
class PortkeyGateway:
    """
    Portkey AI Gateway for advanced routing, caching, and observability
    """
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        result = await response.json()
                        logger.info(f"Successful Portkey completion with model: {model}")
                        return result
                    else:
                        error_text = await response.text()
        ...
        if strategy == "cost":
            ...
            for rule in self.routing_rules:
                if self._evaluate_condition(rule.condition, messages, kwargs):
                    logger.info(f"Applied routing rule: {rule.condition} -> {rule.target_model}")
...
class SophiaAIModelRouter:
    ...
    def __init__(self):
        """Initialize Sophia AI model router"""
    ...
    async def chat(self,
        ...
        try:
            ...
            if provider == "portkey":
                ...
                if "error" in result and self.config["fallback_provider"] == "openrouter":
...
def get_sophia_router() -> SophiaAIModelRouter:
    """Get or create global Sophia AI model router"""
...
async def sophia_analyze(text: str, analysis_type: str = "general", **kwargs) -> Dict[str, Any]:
    """Convenience function for text analysis"""
    messages = [
        {"role": "system", "content": f"You are Sophia AI, an expert analyst. Perform {analysis_type} analysis."},
        {"role": "user", "content": text}
    ]
    router = get_sophia_router()
    return await router.chat(messages, type="analysis", **kwargs)

async def sophia_code(prompt: str, language: str = "python", **kwargs) -> Dict[str, Any]:
    """Convenience function for code generation"""
    messages = [
        {"role": "system", "content": f"You are Sophia AI, an expert {language} developer."},
