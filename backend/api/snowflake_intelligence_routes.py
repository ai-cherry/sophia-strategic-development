...
async def process_natural_language_query(
    request: NaturalLanguageQueryRequest, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        logger.info(
            f"Natural language query processed for user {current_user.get('id')}: {request.query}"
        )
...
async def get_enhanced_dashboard_metrics(
    request: DashboardMetricsRequest, current_user: dict = Depends(get_current_user)
):
    """Get enhanced dashboard metrics with AI insights"""
    try:
        base_metrics = (
            await intelligence_service.semantic_service.get_dashboard_metrics(
                time_range=request.timeRange, metrics=request.metrics
            )
        )
        insights = await insights_service.get_insights_for_metrics(
            request.metrics or list(base_metrics.keys())
        )
        predictions = {}
            ...
            for metric in request.metrics or ["revenue", "satisfaction", "pipeline"]:
                try:
                    prediction = await predictive_service.generate_prediction(
                        f"{metric}_prediction", {"time_range": request.timeRange}
                    )
...
async def get_automated_insights(
    ...
    try:
        insights = await insights_service.get_active_insights(
            priority_filter=priority_filter, limit=limit
        )
        return {
            "success": True,
            "insights": [i.__dict__ for i in insights],
...
async def get_dashboard_predictions(current_user: dict = Depends(get_current_user)):
    ...
    prediction_models = [
        ...
        "revenue_forecasting",
        "sales_conversion_prediction",
    ]
    predictions = {}
    for model_id in prediction_models:
        try:
            prediction = await predictive_service.generate_prediction(
                model_id, {"current_date": datetime.now().isoformat()}
            )
