...
async def get_market_analytics_summary(metro_area: str):
    ...
    try:
        ...
        property_types = list(set(record.get('property_type') for record in market_data if record.get('property_type')))
        latest_data = market_data[0] if market_data else {}
        
        # Calculate averages (simplified for demo)
        avg_vacancy = sum(record.get('vacancy_rate', 0) or 0 for record in market_data) / len(market_data) if market_data else 0
        avg_rent = sum(record.get('asking_rent_psf', 0) or 0 for record in market_data) / len(market_data) if market_data else 0
