...
from backend.api.agui_protocol import agui_protocol, EventType
...
class MCPQueryRequest(BaseModel):
    ...
    query: str = Field(..., description="The query string")
    context: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional context")
    target_servers: Optional[List[str]] = Field(None, description="Specific servers to query")
    stream: bool = Field(default=False, description="Whether to stream the response")
...
async def query_hybrid_rag(request: QueryRequest):
    ...
    try:
        ...
        session_id = None
        if request.user_id:
            session_id = await agui_protocol.create_session(request.user_id, request.context)
    ...
    try:
        ...
        return {
            "system_overview": {
                ...
                "timestamp": time.time(),
                "components": {
                    "hybrid_rag_router": "initialized" if hybrid_rag_router.initialized else "not_initialized",
                    "agno_integration": "initialized" if enhanced_agno_integration.initialized else "not_initialized",
                    "mcp_federation": "initialized" if mcp_federation.initialized else "not_initialized",
            ...
            "performance_summary": {
                ...
                "avg_routing_time_ms": router_stats["routing_metrics"]["avg_routing_time_ms"],
                "avg_agno_instantiation_us": agno_stats["overall"]["avg_instantiation_us"],
                "active_agui_sessions": agui_stats["sessions"]["active"],
                "mcp_federation_health": mcp_stats["federation_health"]["overall_success_rate"]
            },
            "detailed_stats": {
                "router": router_stats,
                "agno": agno_stats,
    ...
    try:
        ...
        try:
            if not enhanced_agno_integration.initialized:
                await enhanced_agno_integration.initialize()
            validation = await enhanced_agno_integration.validate_performance_targets()
            health_status["components"]["agno_integration"] = "healthy" if validation["validation"]["overall_performance"] == "excellent" else "degraded"
        except Exception as e:
