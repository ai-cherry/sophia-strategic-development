...
async def query_knowledge(request: KnowledgeQueryRequest):
    ...
    try:
        ...
        if not response.success:
            raise HTTPException(
                status_code=400, detail=response.error or "Knowledge query failed"
            )
...
async def ingest_knowledge(request: KnowledgeIngestionRequest):
    ...
    try:
        ...
        if not response.success:
            raise HTTPException(
                status_code=400, detail=response.error or "Knowledge ingestion failed"
            )
        ...
        return {
            ...
            "enhancement_suggestions": response.primary_response.get(
...
async def provide_sales_coaching(request: SalesCoachingRequest):
    ...
    try:
        ...
        if not response.success:
            raise HTTPException(
                status_code=400, detail=response.error or "Sales coaching failed"
            )

        return {
            "success": True,
            "sales_rep_id": request.sales_rep_id,
            "coaching_insights": response.primary_response.get(
                "immediate_insights", []
            ),
...
async def handle_slack_event(request: SlackEventRequest):
    ...
    try:
        ...
        if not sales_coach:
            raise HTTPException(
                status_code=503, detail="Sales coach service not available"
            )
...
async def conduct_teaching_session(request: TeachingSessionRequest):
    ...
    try:
        ...
        if not response.success:
            raise HTTPException(
                status_code=400, detail=response.error or "Teaching session failed"
            )
...
async def preserve_memories(
    request: MemoryPreservationRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if request.operation in ["preserve_all", "full_migration"]:
            background_tasks.add_task(
                _background_memory_preservation, orch, orchestration_request
            )
            ...
            if not response.success:
                raise HTTPException(
                    status_code=400,
                    detail=response.error or "Memory preservation failed",
                )
...
async def unified_intelligence_query(request: UnifiedIntelligenceRequest):
    ...
    try:
        ...
        orchestration_request = OrchestrationRequest(
            ...
            user_id=request.user_id,
            query=request.query,
            context={**request.context, "source": "api_unified_intelligence"},
        ...
        if not response.success:
            raise HTTPException(
                status_code=400,
                detail=response.error or "Unified intelligence query failed",
            )
...
async def get_analytics(analytics_type: str = "comprehensive"):
    ...
    try:
        ...
        orchestration_request = OrchestrationRequest(
            ...
            user_id="system",
            context={"analytics_type": analytics_type, "source": "api_analytics"},
        )
        ...
        if not response.success:
            raise HTTPException(
                status_code=400, detail=response.error or "Analytics retrieval failed"
            )
...
async def get_system_status():
    ...
    try:
        ...
        service_status = {
            "knowledge_base": {
                "initialized": (
                    orch.knowledge_base.initialized if orch.knowledge_base else False
                ),
                "analytics": (
                    await orch.knowledge_base.get_knowledge_analytics()
                    if orch.knowledge_base and orch.knowledge_base.initialized
                    else {}
                ),
            },
            "sales_coach": {
                "initialized": (
                    orch.sales_coach.initialized if orch.sales_coach else False
                ),
                "analytics": (
                    orch.sales_coach.coaching_analytics
                    if orch.sales_coach and orch.sales_coach.initialized
                    else {}
                ),
            },
            "memory_preservation": {
                "initialized": (
                    orch.memory_preservation.initialized
                    if orch.memory_preservation
                    else False
                ),
                "analytics": (
                    await orch.memory_preservation.get_migration_analytics()
                    if orch.memory_preservation and orch.memory_preservation.initialized
                    else {}
                ),
            },
        }
...
async def _background_memory_preservation(
    orchestrator: SophiaAIOrchestrator, request: OrchestrationRequest
):
    ...
    try:
        ...
        if response.success:
            logger.info(
                f"Background memory preservation completed: {request.request_id}"
            )
        else:
            logger.error(
                f"Background memory preservation failed: {request.request_id} - {response.error}"
            )
