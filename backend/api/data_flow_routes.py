...
async def list_data_sources(
    ...
    try:
        ...
        return {
            ...
            "healthy_count": len([s for s in sources if s["health_status"] == "healthy"]),
            "timestamp": datetime.now().isoformat()
        }
...
async def get_processing_metrics(
    ...
    try:
        ...
        total_events = health_data["metrics"]["processed_events"] + health_data["metrics"]["failed_events"]
        success_rate = (health_data["metrics"]["processed_events"] / total_events * 100) if total_events > 0 else 100
        
        cache_total = health_data["metrics"]["cache_hits"] + health_data["metrics"]["cache_misses"]
        cache_hit_rate = (health_data["metrics"]["cache_hits"] / cache_total * 100) if cache_total > 0 else 0
        ...
        return {
            "processing_metrics": {
                ...
                "dead_letter_queue_size": health_data["queues"]["dead_letter_queue_size"],
    ...
    try:
        ...
        return {
            "processing_queue": {
                ...
                "status": "healthy" if queue_info["processing_queue_size"] < 100 else "busy"
            },
            "dead_letter_queue": {
                "size": queue_info["dead_letter_queue_size"],
                "status": "ok" if queue_info["dead_letter_queue_size"] == 0 else "attention_needed"
            },
            "workers": {
                "active_count": queue_info["active_workers"],
                "total_configured": data_manager.worker_count,
                "status": "healthy" if queue_info["active_workers"] == data_manager.worker_count else "degraded"
            },
...
async def clear_cache(
    ...
    try:
        if cache_type:
            ...
            keys_to_remove = [k for k in data_manager.cache.l1_cache.keys() if cache_type in k]
