...
class AsanaMCPClient:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Asana integration error: {str(e)}")
...
async def get_asana_health():
    ...
    try:
        ...
        if projects_result.get("projects"):
            ...
            tasks_result = await asana_client.call_tool("get_project_tasks", {
...
async def get_projects(
    ...
    try:
        ...
        for project in result.get("projects", []):
            ...
            budget = custom_fields.get("budget") if isinstance(custom_fields, dict) else None
            spent = custom_fields.get("spent") if isinstance(custom_fields, dict) else None
            risk_level = custom_fields.get("risk_level") if isinstance(custom_fields, dict) else None
            
            projects.append(AsanaProjectSummary(
                gid=project["gid"],
...
async def get_project_details(project_gid: str):
    ...
    try:
        result = await asana_client.call_tool("get_project_details", {"project_gid": project_gid})
        return result
    except Exception as e:
...
async def get_project_tasks(
    project_gid: str,
    completed_since: Optional[str] = Query(None, description="ISO date string for completed tasks filter"),
...
async def get_teams(workspace_gid: Optional[str] = Query(None, description="Workspace GID")):
    ...
    try:
        ...
        for team in result.get("teams", []):
            ...
            try:
                team_projects = await asana_client.call_tool("get_team_projects", {"team_gid": team["gid"]})
                project_count = team_projects.get("project_count", 0)
            except Exception:
                project_count = 0
            
            teams.append(AsanaTeamSummary(
                gid=team["gid"],
...
async def search_tasks(
    ...
    due_date_before: Optional[str] = Query(None, description="ISO date to filter tasks due before"),
    due_date_after: Optional[str] = Query(None, description="ISO date to filter tasks due after")
):
