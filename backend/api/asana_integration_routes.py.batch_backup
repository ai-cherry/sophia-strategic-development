...
class AsanaSyncRequest(BaseModel):
    ...
    sync_type: str = Field(
        default="incremental", description="Sync type: full, incremental"
    )
...
async def get_intelligence_agent() -> AsanaProjectIntelligenceAgent:
    ...
    if intelligence_agent is None:
        intelligence_agent = AsanaProjectIntelligenceAgent(
            {"agent_id": "asana_api_intelligence", "performance_target_ms": 200}
        )
...
async def get_estuary_manager() -> EnhancedEstuaryManager:
    ...
    if estuary_manager is None:
        estuary_manager = EnhancedEstuaryManager(
            "dev"
...
async def get_project_intelligence_report(
    ...
    try:
        logger.info(
            f"Generating intelligence report for project: {project_gid or 'all'}"
        )
...
async def get_projects(
    ...
    try:
        ...
        if sort_by == "health_score":
            ...
            projects.sort(
                key=lambda p: risk_order.get(p.risk_level.value, 0), reverse=True
            )
...
async def get_team_productivity(
    ...
    try:
        ...
        return JSONResponse(content={"teams": team_data, "total_teams": len(team_data)})
...
async def trigger_asana_sync(
    ...
    try:
        if request.force_sync or request.sync_type == "full":
            ...
            background_tasks.add_task(_run_full_sync_pipeline, estuary)
...
async def get_analytics_summary(
    ...
    try:
        ...
        if result.empty:
            raise HTTPException(
                status_code=404, detail="No data found for the specified time period"
            )
        ...
        return JSONResponse(
            content={
                ...
                "summary": {"projects": projects, "tasks": tasks, "teams": teams},
...
async def _run_full_sync_pipeline(estuary_manager: EnhancedEstuaryManager):
    ...
    try:
        ...
        success_count = sum(
            1 for result in results.values() if result.status.value == "success"
        )
        total_count = len(results)

        logger.info(
            f"Full sync pipeline completed: {success_count}/{total_count} successful"
        )
