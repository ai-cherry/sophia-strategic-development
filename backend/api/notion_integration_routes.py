...
class NotionMCPClient:
    ...
    async def call_tool(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Notion integration error: {str(e)}"
            )
...
async def search_pages(
    ...
    try:
        arguments = {"page_size": min(page_size, 100)}
...
async def get_page_content(
    page_id: str, page_size: int = Query(100, description="Number of blocks to return")
):
    """Get the content blocks of a page."""
    try:
        result = await notion_client.call_tool(
            "get_page_content", {"page_id": page_id, "page_size": min(page_size, 100)}
        )
        return result
    except Exception as e:
        logger.error(f"Error getting content for page {page_id}: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get page content: {str(e)}"
        )
...
async def get_page_analytics(page_id: str):
    ...
    try:
        result = await notion_client.call_tool(
            "get_page_analytics", {"page_id": page_id}
        )
        return result
    except Exception as e:
        logger.error(f"Error getting analytics for page {page_id}: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get page analytics: {str(e)}"
        )
...
async def get_database(database_id: str):
    ...
    try:
        result = await notion_client.call_tool(
            "get_database", {"database_id": database_id}
        )
        return result
    except Exception as e:
...
async def query_database(
    ...
    try:
        arguments = {"database_id": database_id, "page_size": min(page_size, 100)}
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to query database: {str(e)}"
        )
...
async def search_by_title(
    ...
    try:
        result = await notion_client.call_tool(
            "search_by_title", {"title": title, "exact_match": exact_match}
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to search by title: {str(e)}"
        )
...
async def get_recent_pages(
    ...
    try:
        result = await notion_client.call_tool(
            "get_recent_pages", {"days": days, "page_size": min(page_size, 100)}
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get recent pages: {str(e)}"
        )
...
async def get_strategic_content(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get strategic content: {str(e)}"
        )
...
async def get_users(
    ...
    try:
        result = await notion_client.call_tool(
            "get_users", {"page_size": min(page_size, 100)}
        )
        return result
    except Exception as e:
...
async def get_dashboard_summary():
    ...
    try:
        # Get recent pages
        recent_result = await notion_client.call_tool(
            "get_recent_pages", {"days": 30, "page_size": 100}
        )
        ...
        for page in recent_pages:
            ...
            if any(
                keyword in title
                for keyword in ["project", "initiative", "roadmap", "plan"]
            ):
...
def _convert_to_page_summary(page: Dict[str, Any]) -> NotionPageSummary:
    ...
    return NotionPageSummary(
        ...
        created_by=(
            page.get("created_by", {}).get("id") if page.get("created_by") else None
        ),
        last_edited_by=(
            page.get("last_edited_by", {}).get("id")
    ...
    if "properties" in page:
        for prop_name, prop_data in page["properties"].items():
            if prop_data.get("type") == "title":
                ...
                if title_blocks:
                    return "".join(
                        block.get("plain_text", "") for block in title_blocks
                    )
