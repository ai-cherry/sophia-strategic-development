...
async def unified_chat_endpoint(request: Request):
    ...
    try:
        ...
        messages = [
            {
                ...
                "content": system_prompts.get(
                    dashboard_type, system_prompts["general"]
                ),
            },
        ...
        if not task_type or task_type == "general":
            ...
            if any(
                word in message.lower() for word in ["summarize", "summary", "overview"]
            ):
                task_type = (
                    "executive_summary" if dashboard_type == "ceo" else "summarization"
                )
            elif any(
                word in message.lower()
                for word in ["competitor", "competition", "market"]
            ):
                task_type = "competitive_analysis"
            elif any(
                word in message.lower()
                for word in ["revenue", "financial", "cost", "profit"]
            ):
                task_type = "financial_analysis"
            elif any(
                word in message.lower() for word in ["code", "script", "function"]
            ):
