...
class EntityResolutionRequest(BaseModel):
    ...
    query: str = Field(..., description="Natural language query containing entity references")
...
class EntityRegistrationRequest(BaseModel):
    ...
    entity_type: str = Field(..., description="Type of entity (company, person, property, customer)")
...
class EntityResolutionResponse(BaseModel):
    ...
    type: str = Field(..., description="Response type: 'clarification_needed', 'query_result', or 'error'")
    message: Optional[str] = Field(None, description="Response message or clarification question")
    entity_matches: Optional[Dict[str, Any]] = Field(None, description="Entity match details")
    query_result: Optional[Dict[str, Any]] = Field(None, description="Query results if resolved")
...
async def search_entities(
    ...
    try:
        if not chat_service.entity_resolution_service:
            raise HTTPException(
                status_code=503, 
                detail="Entity resolution service not available"
            )
...
async def get_entity_details(entity_id: str):
    ...
    try:
        ...
        columns = [
            ...
            "primary_ids", "aliases", "confidence", "metadata", 
            "source_system_count", "created_at", "updated_at", "last_seen_at"
        ]
        
        entity_data = dict(zip(columns, result))
...
async def _populate_entities_background():
    ...
    try:
        if chat_service.entity_resolution_service:
            ...
            logger.info(f"Entity population completed: {result[0] if result else 'Success'}")
...
def generate_clarification_options(entity_matches: Dict[str, Any]) -> List[Dict[str, Any]]:
