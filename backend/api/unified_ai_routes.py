...
from backend.services.unified_ai_orchestration_service import get_unified_ai_service, UnifiedAIOrchestrationService
...
class ComplianceMonitoringRequest(BaseModel):
    ...
    time_range: str = Field("30d", description="Time range for analysis (e.g., '7d', '30d', '90d')")
...
class MultiSourceSearchRequest(BaseModel):
    ...
    sources: List[str] = Field(["gong", "slack", "hubspot", "intercom"], description="Data sources to search")
...
async def analyze_customer_intelligence(
    ...
    """
    try:
        logger.info(f"Processing customer intelligence query for customer {request.customer_id}")
...
async def _perform_multi_source_search(
    ...
    try:
        ...
        if search_queries:
            ...
            if customer_id:
                # Add customer filter
                combined_query = f"SELECT * FROM ({combined_query}) WHERE customer_id = %s"
