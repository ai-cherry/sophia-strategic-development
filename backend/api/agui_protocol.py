...
class WorkflowManager:
    ...
    def start_workflow(self, session_id: str, workflow_type: str, parameters: Dict[str, Any]) -> str:
        ...
        logger.info(f"Started workflow {workflow_id} of type {workflow_type} for session {session_id}")
        return workflow_id
    
    def update_workflow_progress(self, workflow_id: str, progress: float, step_info: Dict[str, Any] = None):
    ...
    def _notify_workflow_callbacks(self, workflow_id: str, event_type: str, workflow_data: Dict[str, Any]):
    ...
    def cleanup_completed_workflows(self, max_age_hours: int = 24):
        ...
        for workflow_id, workflow in self.active_workflows.items():
            if workflow["status"] in ["completed", "failed"]:
                age = current_time - workflow.get("completed_at", workflow.get("failed_at", workflow["updated_at"]))
...
class AGUIProtocol:
    ...
    def _register_default_handlers(self):
        ...
        self.register_event_handler(EventType.CONTEXT_UPDATE, self._handle_context_update)
    ...
    async def create_session(self, user_id: Optional[str] = None, context: Dict[str, Any] = None) -> str:
    ...
    async def connect_websocket(self, websocket: WebSocket, session_id: str):
        ...
        await self.emit_event(session_id, EventType.CONNECTION_START, {
            "session_id": session_id,
            "timestamp": time.time(),
        ...
        try:
            ...
            await self.emit_event(session_id, EventType.CONNECTION_END, {
                "session_id": session_id,
                "timestamp": time.time()
            })
            ...
            if event.type in self.event_handlers:
                for handler in self.event_handlers[event.type]:
                    try:
                        ...
                        await self.emit_event(event.session_id, EventType.ERROR, {
                            "error": str(e),
    ...
    async def emit_event(self, session_id: str, event_type: EventType, payload: Dict[str, Any]):
        ...
        if session_id in self.sessions:
            ...
            if len(session.context["pending_events"]) > 100:
                session.context["pending_events"] = session.context["pending_events"][-100:]
    ...
    async def stream_events(self, session_id: str) -> AsyncGenerator[str, None]:
        ...
        try:
            while True:
                ...
                new_events = [e for e in pending_events if e["timestamp"] > last_event_time]
    ...
    async def _handle_user_message(self, event: AGUIEvent):
        ...
        try:
            ...
            await self.emit_event(event.session_id, EventType.WORKFLOW_START, {
            ...
            self.workflow_manager.register_callback(workflow_id, 
            ...
            asyncio.create_task(self._process_user_message_async(event.session_id, workflow_id, message, context))
    ...
    async def _process_user_message_async(self, session_id: str, workflow_id: str, message: str, context: Dict[str, Any]):
        """Process user message asynchronously."""
        try:
            from backend.core.hybrid_rag_router import hybrid_rag_router
            
            # Update workflow progress
            self.workflow_manager.update_workflow_progress(workflow_id, 0.1, {
                "step": "routing_query",
            ...
            # Update workflow progress
            self.workflow_manager.update_workflow_progress(workflow_id, 0.8, {
                "step": "processing_complete",
                "description": "Query processing completed"
            })
            
            # Emit agent response
            await self.emit_event(session_id, EventType.AGENT_MESSAGE, {
                "message": result.get("data", {}).get("primary_result", "No response generated"),
    ...
    async def _workflow_callback(self, session_id: str, event_type: str, workflow_data: Dict[str, Any]):
        """Handle workflow callbacks."""
        if event_type == "progress":
            await self.emit_event(session_id, EventType.WORKFLOW_PROGRESS, {
            ...
            await self.emit_event(session_id, EventType.WORKFLOW_COMPLETE, {
                "workflow_id": workflow_data["id"],
                "result": workflow_data.get("result"),
                "execution_time": workflow_data.get("completed_at", 0) - workflow_data["started_at"]
            })
        elif event_type == "error":
            await self.emit_event(session_id, EventType.ERROR, {
                "workflow_id": workflow_data["id"],
    ...
    async def _handle_user_action(self, event: AGUIEvent):
        ...
        # Emit acknowledgment
        await self.emit_event(event.session_id, EventType.AGENT_ACTION, {
    ...
    async def _handle_state_update(self, event: AGUIEvent):
        ...
        if event.session_id in self.sessions:
            ...
            await self.emit_event(event.session_id, EventType.STATE_SYNC, {
                "updated_state": state_updates,
                "full_state": session.workflow_state
            })
    ...
    def _update_response_time_metrics(self, response_time_ms: float):
        ...
        self.metrics["avg_response_time_ms"] = (
            (current_avg * (total_events - 1) + response_time_ms) / total_events
        )
    ...
    def get_protocol_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "workflows": {
                ...
                "total_callbacks": sum(len(callbacks) for callbacks in self.workflow_manager.workflow_callbacks.values())
            },
            "event_handlers": {
