...
from ..core.data_transformation_pipeline import (
...
async def upload_file(
    ...
    try:
        ...
        await websocket_manager.broadcast(
            {"type": "file_upload_progress", "fileId": file_id, "progress": 100}
        )

        # Process file in background
        background_tasks.add_task(
            process_file_background, file_id, temp_file_path, file_metadata
        )
...
async def process_file_background(
    file_id: str, file_path: str, metadata: Dict[str, Any]
):
    ...
    try:
        ...
        await websocket_manager.broadcast(
            {"type": "file_processing_started", "fileId": file_id}
        )
        ...
        await websocket_manager.broadcast(
            {"type": "file_processing_complete", "fileId": file_id, "result": result}
        )

        # Update processing stats
        stats = await pipeline_instance.get_processing_stats()
        await websocket_manager.broadcast(
            {"type": "processing_stats_update", "stats": stats}
        )

    except Exception as e:
        logger.error(f"Error processing file {file_id}: {e}")
        await websocket_manager.broadcast(
            {"type": "file_processing_error", "fileId": file_id, "error": str(e)}
        )
    finally:
...
async def sync_hubspot_data(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return JSONResponse(
            {"status": "success", "job_id": job_id, "message": "HubSpot sync started"}
        )
...
async def process_hubspot_background(job: ProcessingJob):
    ...
    try:
        ...
        await websocket_manager.broadcast(
            {"type": "hubspot_sync_complete", "jobId": job.job_id, "result": result}
        )

    except Exception as e:
        logger.error(f"Error processing HubSpot data: {e}")
        await websocket_manager.broadcast(
            {"type": "hubspot_sync_error", "jobId": job.job_id, "error": str(e)}
        )
...
async def sync_gong_data(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return JSONResponse(
            {"status": "success", "job_id": job_id, "message": "Gong sync started"}
        )
...
async def process_gong_background(job: ProcessingJob):
    ...
    try:
        ...
        await websocket_manager.broadcast(
            {"type": "gong_sync_complete", "jobId": job.job_id, "result": result}
        )

    except Exception as e:
        logger.error(f"Error processing Gong data: {e}")
        await websocket_manager.broadcast(
            {"type": "gong_sync_error", "jobId": job.job_id, "error": str(e)}
        )
...
async def sync_slack_data(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return JSONResponse(
            {"status": "success", "job_id": job_id, "message": "Slack sync started"}
        )
...
async def process_slack_background(job: ProcessingJob):
    ...
    try:
        ...
        await websocket_manager.broadcast(
            {"type": "slack_sync_complete", "jobId": job.job_id, "result": result}
        )

    except Exception as e:
        logger.error(f"Error processing Slack data: {e}")
        await websocket_manager.broadcast(
            {"type": "slack_sync_error", "jobId": job.job_id, "error": str(e)}
        )
...
async def delete_file(file_id: str):
    ...
    try:
        ...
        # For now, return a placeholder response
        return JSONResponse(
            {"status": "success", "message": f"File {file_id} deleted successfully"}
        )
...
async def reprocess_file(file_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # For now, return a placeholder response
        return JSONResponse(
            {"status": "success", "message": f"File {file_id} reprocessing started"}
        )
