...
class CodacyMCPClient:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Codacy integration error: {str(e)}")
...
async def get_codacy_health():
    ...
    try:
        ...
        try:
            analysis_result = await codacy_client.call_tool("analyze_project", {
                "severity": "warning",
                "format": "json"
            })
            total_issues = analysis_result.get("categorized_issues", {}).get("total_issues", 0)
            critical_issues = analysis_result.get("categorized_issues", {}).get("by_severity", {}).get("critical", 0)
        except Exception:
            total_issues = 0
            critical_issues = 0
...
async def get_coverage_analysis(
    ...
    coverage_file: Optional[str] = Query(None, description="Path to coverage report file")
):
...
async def get_duplication_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to analyze duplication: {str(e)}")

@router.post("/custom-rules")
async def apply_custom_rules(
    rule_set: str = Query(..., description="Custom rule set to apply", enum=["sophia-security", "sophia-performance", "sophia-architecture"]),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to apply custom rules: {str(e)}")

@router.get("/report")
async def generate_report(
    format: str = Query("executive", description="Report format type", enum=["executive", "technical", "security"]),
...
async def get_dashboard_summary():
    ...
    try:
        ...
        # Get quality metrics
        quality_result = await codacy_client.call_tool("quality_metrics", {"include_history": False})
        ...
        return {
            "overall_health": "healthy" if health_checks.get("cli", {}).get("success") else "degraded",
...
async def cursor_integration_analysis():
    ...
    try:
        ...
        sophia_analysis = await codacy_client.call_tool("custom_rules", {
            "rule_set": "sophia-architecture"
        })
        
        # Run security scan with Sophia patterns
        security_scan = await codacy_client.call_tool("custom_rules", {
            "rule_set": "sophia-security"
        })
        
        # Run performance analysis
        performance_analysis = await codacy_client.call_tool("custom_rules", {
            "rule_set": "sophia-performance"
        })
        ...
        return {
            "cursor_optimization": {
                ...
                "performance_compliance": performance_analysis.get("analysis_results", {}),
        ...
        raise HTTPException(status_code=500, detail=f"Failed to run Cursor analysis: {str(e)}") 