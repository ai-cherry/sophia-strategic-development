...
async def get_import_job_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get import job status"
        ) from e
...
async def cancel_import_job(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Import job not found or cannot be cancelled"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to cancel import job"
        ) from e
