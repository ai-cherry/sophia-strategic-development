...
class ChatRequest(BaseModel):
    ...
    task_type: str = Field(
        default="chat_general", description="Task type for optimization"
    )
...
async def chat_completion(request: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Unknown task type '{request.task_type}', defaulting to chat_general"
            )
...
async def chat_stream(request: ChatRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Stream completion failed: {str(e)}"
        )
