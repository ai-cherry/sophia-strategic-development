...
class UnifiedIntelligenceRequest(BaseModel):
    ...
    optimization_mode: str = Field(
        default="balanced", 
        description="Optimization mode: balanced, performance, cost"
    )
...
class UnifiedIntelligenceResponse(BaseModel):
    ...
    unified_insights: str = Field(..., description="Synthesized insights from all sources")
    memory_context: Optional[List[Dict[str, Any]]] = Field(
        default=None, 
        description="Relevant memories from AI Memory"
    )
    business_data: Dict[str, Any] = Field(
        default_factory=dict, 
        description="Business data from integrations"
    )
    ...
    optimization_insights: Dict[str, Any] = Field(
        default_factory=dict,
        description="Insights for query optimization"
    )
...
async def unified_business_intelligence(
    ...
    """
    logger.info(f"📊 Unified intelligence query from user {current_user.get('id')}: {request.query[:100]}...")
    ...
    try:
        ...
        health_status = {
            ...
            "services": {
                ...
                "portkey_gateway": bool(getattr(unified_service, 'portkey_gateway', None))
            }
        }
