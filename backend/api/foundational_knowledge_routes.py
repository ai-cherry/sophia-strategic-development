...
async def sync_foundational_data(
    ...
    try:
        ...
        # Perform sync
        sync_result = (
            await foundational_service.sync_foundational_data_to_knowledge_base()
        )
        ...
        logger.info(
            f"Foundational knowledge sync completed by {user_id}: {sync_result['synced_records']} records"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to sync foundational knowledge"
        )
...
async def get_foundational_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch foundational statistics"
        )
    ...
    try:
        ...
        if data_types:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid data type: {str(e)}"
                )

        # Perform search
        results = await foundational_service.search_foundational_knowledge(
            query=query, data_types=filtered_types, limit=limit
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to search foundational knowledge"
        )
...
async def get_foundational_insights(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate foundational insights"
        )
...
async def get_available_data_types(
    ...
    return {"data_types": data_types, "total_types": len(data_types)}
...
async def update_foundational_record(
    ...
    try:
        ...
        success = await foundational_service.update_foundational_record(
            record_id=record_id, data_type=foundational_type, updates=updates
        )

        if not success:
            raise HTTPException(
                status_code=404, detail="Record not found or update failed"
            )
...
async def get_contextual_knowledge(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get contextual knowledge"
        )
