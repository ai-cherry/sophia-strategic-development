...
class GroupHealthMonitor:
    ...
    def __init__(
        ...
        self.metrics_collector = metrics_collector or MCPMetricsCollector("group_health")
    ...
    async def monitor_group_health(self, group_name: str) -> GroupHealthMetrics:
        ...
        health_percentage = (healthy_servers / len(servers_in_group) * 100) if servers_in_group else 0
        avg_response_time = total_response_time / len(servers_in_group) if servers_in_group else 0
        ...
        self.health_history[group_name] = [
            m for m in self.health_history[group_name]
    ...
    async def _predict_failure_risk(
        ...
        if group_name not in self.health_history or len(self.health_history[group_name]) < 5:
        ...
        health_trend = self._calculate_trend([m.health_percentage for m in recent_metrics])
        ...
        for group_name, metrics_history in self.health_history.items():
            ...
            if risk_score > 0.2:  # Threshold for creating risk assessment
                ...
                assessment = GroupRiskAssessment(
                    ...
                    confidence_score=min(len(recent_metrics) / 20, 1.0)  # More data = higher confidence
                )
    ...
    def _generate_mitigation_actions(
        ...
        if "Critical health level" in risk_factors:
            actions.append(f"Immediately investigate and restart failed servers in {group_name}")
    ...
    async def optimize_group_performance(self) -> List[OptimizationRecommendation]:
        ...
        for group_name in self.monitoring_intervals.keys():
            ...
            # Response time optimization
            if current_metrics.response_time_avg > thresholds.get("response_time_ms", 1000) * 0.8:
                improvement = (
                    (current_metrics.response_time_avg - thresholds["response_time_ms"]) /
                    current_metrics.response_time_avg * 100
                )
            ...
            if current_metrics.availability < thresholds.get("availability", 0.95):
                recommendations.append(OptimizationRecommendation(
                    ...
                    expected_improvement=(0.99 - current_metrics.availability) * 100,
                    implementation_effort="high",
                    priority=1 if current_metrics.availability < 0.9 else 2
                ))
            
            # Group-specific optimizations
            if group_name == "data_infrastructure" and len(current_metrics.server_metrics) > 3:
                recommendations.append(OptimizationRecommendation(
                    group_name=group_name,
        ...
        for group_name in self.monitoring_intervals.keys():
            ...
            if metrics.health_percentage < thresholds.get("health_percentage", 70):
                dashboard["active_alerts"].append({
                    "group": group_name,
                    "type": "health",
                    "severity": "critical" if metrics.health_percentage < 50 else "warning",
