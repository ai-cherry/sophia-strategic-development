    ...
    async def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        if not segments:
            ...
            if unattributed > len(segments) * 0.2:
                issues.append(f"High unattributed segments: {unattributed}/{len(segments)}")
...
class ParticipantEnrichmentRule(ValidationRule):
    ...
    async def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        if poorly_enriched > 0:
            issues.append(f"{poorly_enriched}/{len(participants)} participants poorly enriched")
    ...
    async def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if started and ended:
                ...
                if abs(calculated_duration - reported_duration) > 60:  # 1 minute tolerance
                    issues.append(
        ...
        if interactions:
            ...
            if duration_minutes > 0:
                ...
                if interactions_per_minute > 10:  # Unusually high
                    issues.append(f"Unusually high interaction rate: {interactions_per_minute:.1f}/min")
    ...
    async def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        if transcript:
            ...
            for i, segment in enumerate(segments):
                ...
                if len(phone_matches) > 3:  # Multiple phone numbers might indicate PII dump
...
class CustomRuleRegistry:
    ...
    async def validate_by_category(self, 
                                 data: Dict[str, Any], 
                                 category: RuleCategory) -> List[ValidationResult]:
