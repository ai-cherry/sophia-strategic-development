...
Provides real-time quality monitoring, metrics collection, and alerting
for the Sophia AI Gong webhook processing pipeline. Integrates with
existing infrastructure to enhance data quality visibility.
"""
...
data_freshness_score = Gauge(
    'gong_data_freshness_score',
    'Data recency assessment (0-1 scale)'
)
...
enrichment_success_rate = Gauge(
    'gong_enrichment_success_rate',
    'Data enhancement effectiveness percentage'
)
...
class DataQualityConfig(BaseSettings):
    ...
    REQUIRED_FIELDS: List[str] = [
        "call_id", "title", "started", "duration_seconds",
        "primary_user_id", "participants"
    ]
...
class GongDataQualityMonitor:
    ...
    async def monitor_webhook_quality(self, webhook_data: Dict[str, Any]) -> QualityReport:
        """
        ...
        report.processing_metrics.validation_duration_ms = (time.time() - validation_start) * 1000
        
        # Assess quality dimensions
        dimensions = await self._assess_quality_dimensions(webhook_data, validation_results)
        ...
        # Identify issues
        report.issues = self._identify_quality_issues(webhook_data, dimensions, validation_results)
        ...
        self.logger.info("Quality monitoring completed",
                        call_id=call_id,
                        webhook_id=webhook_id,
    ...
    async def track_api_enhancement(self, 
                                  call_id: str,
                                  api_response: Optional[Dict[str, Any]],
                                  api_calls_made: int,
                                  duration_ms: float,
                                  errors: List[Dict[str, Any]] = None) -> EnhancementReport:
        """
        ...
        # Track success rate
        success_rate = api_calls_successful / api_calls_made if api_calls_made > 0 else 0
        ...
        self.logger.info("API enhancement tracked",
                        call_id=call_id,
    ...
    async def validate_data_completeness(self, processed_data: Dict[str, Any]) -> CompletenessReport:
        """
        ...
        completeness_pct = (populated_fields / len(all_fields) * 100) if all_fields else 0
        ...
        report = CompletenessReport(
            ...
            optional_fields_missing=self._find_optional_missing(processed_data, all_fields),
        ...
        if len(self.transcript_completeness_window) > 0:
            transcript_completeness_rate.set(
                sum(self.transcript_completeness_window) / len(self.transcript_completeness_window) * 100
            )
        
        field_coverage_percentage.labels(field_category="required").set(
            (len(self.config.REQUIRED_FIELDS) - len(required_missing)) / len(self.config.REQUIRED_FIELDS) * 100
        )
        
        field_coverage_percentage.labels(field_category="all").set(completeness_pct)
        
        self.logger.info("Data completeness validated",
                        call_id=call_id,
                        completeness_percentage=completeness_pct,
    ...
    async def assess_participant_mapping(self, participants: List[Dict[str, Any]], 
                                       call_id: str) -> MappingReport:
        """
        ...
        for participant in participants:
            participant_id = participant.get("user_id", participant.get("email", "unknown"))
        ...
        accuracy_pct = (successfully_mapped / total_participants * 100) if total_participants > 0 else 0
        ...
        if len(self.participant_accuracy_window) > 0:
            participant_mapping_accuracy.set(
                sum(self.participant_accuracy_window) / len(self.participant_accuracy_window) * 100
            )
        
        self.logger.info("Participant mapping assessed",
                        call_id=call_id,
                        total_participants=total_participants,
                        accuracy_percentage=accuracy_pct)
    ...
    async def _assess_quality_dimensions(self, 
                                       webhook_data: Dict[str, Any],
                                       validation_results: List[ValidationResult]) -> Dict[QualityDimension, float]:
    ...
    def _calculate_completeness_score(self, data: Dict[str, Any]) -> float:
        ...
        required_present = sum(1 for field in self.config.REQUIRED_FIELDS 
    ...
    def _identify_quality_issues(self, 
                               data: Dict[str, Any],
                               dimensions: Dict[QualityDimension, float],
                               validation_results: List[ValidationResult]) -> List[QualityIssue]:
        ...
        for dimension, score in dimensions.items():
            if score < 0.8:  # Below threshold
                issue = self._create_quality_issue(dimension, score, data, validation_results)
                if issue:
                    issues.append(issue)
        
        return issues
        
    def _create_quality_issue(self,
                            dimension: QualityDimension,
                            score: float,
                            data: Dict[str, Any],
                            validation_results: List[ValidationResult]) -> Optional[QualityIssue]:
        """Create quality issue for a dimension."""
        if dimension == QualityDimension.COMPLETENESS:
            missing_fields = [f for f in self.config.REQUIRED_FIELDS 
    ...
    async def _check_quality_alerts(self, report: QualityReport):
        ...
        if report.processing_metrics.total_duration_ms > self.config.MAX_PROCESSING_LATENCY * 1000:
            await self._trigger_alert(
    ...
    async def _trigger_alert(self, 
        ...
        if alert_key in self.alert_history:
            ...
            if (datetime.now(timezone.utc) - last_alert["timestamp"]).total_seconds() < self.config.ALERT_COOLDOWN_MINUTES * 60:
                return
        ...
        # Log alert
        self.logger.warning("Quality alert triggered",
    ...
    def _count_populated_fields(self, data: Dict[str, Any], all_fields: Set[str]) -> int:
        """Count populated fields."""
        return sum(1 for field in all_fields if self._is_field_populated(data, field))
        
    def _find_optional_missing(self, data: Dict[str, Any], all_fields: Set[str]) -> List[str]:
        """Find optional fields that are missing."""
        optional_missing = []
        
        for field in all_fields:
            if field not in self.config.REQUIRED_FIELDS and not self._is_field_populated(data, field):
    ...
    def _has_complete_transcript(self, data: Dict[str, Any]) -> bool:
        ...
        transcript_duration = sum(s.get("end_time", 0) - s.get("start_time", 0) for s in segments)
    ...
    async def _assess_field_quality(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if "participants" in data:
            participant_score = self._calculate_participant_quality(data["participants"])
    ...
    def _calculate_participant_quality(self, participants: List[Dict[str, Any]]) -> float:
    ...
    def _count_enriched_fields(self, api_response: Dict[str, Any]) -> int:
        ...
        if api_response.get("participants"):
            enriched_count += len(api_response["participants"]) * 5  # Avg fields per participant
    ...
    def _assess_participant_mapping_quality(self, participant: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": "Missing required fields" if missing_fields else "Low data quality" if not success else "OK",
...
class QualityRuleEngine:
    ...
    async def validate_webhook_data(self, webhook_data: Dict[str, Any]) -> List[ValidationResult]:
    ...
    async def check_participant_data(self, participants: List[Dict[str, Any]]) -> ValidationResult:
    ...
    async def assess_metadata_completeness(self, metadata: Dict[str, Any]) -> ValidationResult:
...
class QualityMetricsCollector:
    ...
    def update_quality_metrics(self, quality_report: QualityReport):
        ...
        for dimension, score in quality_report.dimensions.items():
            self.logger.info("Quality dimension update",
