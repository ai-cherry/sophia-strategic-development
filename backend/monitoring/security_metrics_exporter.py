...
class SecurityMetricsSettings(BaseSettings):
    ...
    reports_dir: str = Field(default="security/reports", description="Directory containing security scan reports")
    allowlist_file: str = Field(default="security/vulnerability-allowlist.yaml", description="Path to vulnerability allowlist")
    scan_interval_seconds: int = Field(default=3600, description="Interval between report scans")
...
class SecurityMetricsExporter:
    ...
    def _calculate_metrics(self, vuln_data: Dict, allowlist_data: Dict) -> VulnerabilityMetrics:
        ...
        if comparison_file:
            try:
                with open(comparison_file, 'r') as f:
                    ...
                    metrics.resolved_count = comparison.get('resolved_vulnerabilities_count', 0)
            except Exception as e:
...
async def health():
    ...
    return {
        "status": "healthy",
        "last_scan": exporter._last_scan_time.isoformat() if exporter._last_scan_time else None,
        "vulnerabilities": {
