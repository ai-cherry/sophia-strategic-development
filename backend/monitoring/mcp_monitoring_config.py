        ...
        main_dashboard = DashboardConfig(
            ...
            panels=[
                ...
                },
                {
                    ...
                    "type": "graph",
                    "targets": [
                        {"expr": "rate(mcp_errors_total[5m]) by (server_name)"}
                    ],
    ...
    def _initialize_alert_handlers(self):
        ...
        async def github_issue_handler(alert: Alert) -> bool:
            ...
            try:
                ...
                logger.info(
                    f"GITHUB ISSUE ALERT: {alert.severity.value} - {alert.message}"
                )
                return True
            except Exception as e:
                ...
                if collector:
                    ...
                    if hasattr(health_status, "response_time_ms"):
                        collector.record_request(
                            "GET",
                            "/health",
                            "success" if health_status else "failed",
        ...
        try:
            ...
            if alert_triggered:
                ...
                should_alert = (
                    ...
                    >= rule.consecutive_failures_required
                    and not alert_state["currently_alerting"]
                    and self._check_cooldown(
                        alert_state["last_alert_time"], rule.cooldown_minutes
                    )
                )
