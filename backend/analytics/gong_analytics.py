...
def analyze_deal_signals(call_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    positive_signals = []
    if 'demo' in title: positive_signals.append('Product demo scheduled')
    if 'proposal' in title: positive_signals.append('Proposal discussion')
    if 'contract' in title: positive_signals.append('Contract negotiation')
    
    negative_signals = []
    if 'concern' in title: negative_signals.append('Customer concerns raised')
    if 'competitor' in title: negative_signals.append('Competitive pressure')
    
    deal_stage = 'discovery'
    if 'demo' in title: deal_stage = 'evaluation'
    elif 'proposal' in title: deal_stage = 'negotiation'
    elif 'contract' in title: deal_stage = 'closing'
    
    win_probability = 0.5 + (len(positive_signals) * 0.1) - (len(negative_signals) * 0.1)
