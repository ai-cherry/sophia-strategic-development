...
class Mem0RestClient:
    ...
    async def add_memories(self, messages: List[Dict[str, Any]], user_id: str, memory_type: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        payload = {
            "user_id": user_id,
            "messages": messages,
        ...
        response = await self._execute_with_retry("POST", f"{self.base_url}/add_memories", json=payload)
    ...
    async def search_memory(self, query: str, user_id: str, memory_types: List[str], limit: int = 10) -> Dict[str, Any]:
        payload = {
            "query": query,
            "user_id": user_id,
        ...
        start_time = time.time()
        response = await self._execute_with_retry("POST", f"{self.base_url}/search_memory", json=payload)
        execution_time = time.time() - start_time
    ...
    async def list_memories(self, user_id: str) -> Dict[str, Any]:
        start_time = time.time()
        response = await self._execute_with_retry("GET", f"{self.base_url}/list_memories/{user_id}")
        execution_time = time.time() - start_time
    ...
    async def delete_all_memories(self, user_id: str) -> Dict[str, Any]:
        start_time = time.time()
        response = await self._execute_with_retry("DELETE", f"{self.base_url}/delete_all_memories/{user_id}")
        execution_time = time.time() - start_time
...
class MemoryEntry(BaseModel):
    ...
    memory_type: str = Field(..., description="Memory type: working, episodic, semantic, factual")
...
async def add_memories(entry: MemoryEntry) -> Dict[str, Any]:
    try:
        ...
        logger.error(f"HTTP error in add_memories: {e.response.status_code} - {e.response.text}")
        raise HTTPException(
...
async def search_memory(query: MemoryQuery) -> Dict[str, Any]:
    try:
        result = await mem0_client.search_memory(
            ...
            memory_types=query.memory_types or ["working", "episodic", "semantic", "factual"],
