...
def _execute_cortex_complete_sync(query: str, model: str, temperature: float, max_tokens: int) -> Any:
    ...
    try:
        ...
        if isinstance(generated_sql, str):
            try:
                ...
                actual_sql = sql_response.get('choices', [{}])[0].get('text', '').strip()
            except:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "query": body.query[:100] + "..." if len(body.query) > 100 else body.query
            }
        )
...
async def generate_embeddings(body: EmbeddingRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        embedding = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: execute_with_retry(
                _execute_cortex_embedding_sync,
                body.text,
                body.model
            )
        )
