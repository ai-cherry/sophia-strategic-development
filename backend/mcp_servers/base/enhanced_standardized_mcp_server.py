...
class EnhancedStandardizedMCPServer(ABC):
    ...
    def _setup_error_handlers(self):
        ...
        async def global_exception_handler(request, exc):
            ...
            return {
                ...
                "message": (
                    str(exc) if logger.level <= logging.DEBUG else "An error occurred"
                ),
                "server": self.config.name,
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                if self.config.name in active_servers:
                    ...
                    logger.info(
                        f"Loaded port {self.config.port} for {self.config.name}"
                    )
        except Exception as e:
            logger.warning(
                f"Failed to load port config: {e}, using default port {self.config.port}"
            )
    ...
    async def get_health_status(
        self, level: HealthCheckLevel = HealthCheckLevel.BASIC
    ) -> HealthStatus:
    ...
    async def get_from_cache(self, key: str) -> Any | None:
        ...
        if key in self.cache:
            current_time = time.time()
            if (
                current_time - self.cache_timestamps[key]
