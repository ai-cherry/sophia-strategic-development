...
class DataCollectionService(ABC):
    ...
    async def collect_data(self, task: DataCollectionTask) -> Dict[str, Any]:
        """Collect data based on task configuration"""
        pass
...
class ApifyService(DataCollectionService):
    ...
    async def _wait_for_completion(self, session: aiohttp.ClientSession, run_id: str, timeout: int = 300) -> Dict[str, Any]:
...
class PhantomBusterService(DataCollectionService):
    ...
    def __init__(self):
        """Initialize PhantomBuster service"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        # Wait for completion
                        result = await self._wait_for_phantom_completion(session, container_id)
                        
                        logger.info(f"PhantomBuster task {task.task_id} completed successfully")
                        return {
                            "success": True,
                            "task_id": task.task_id,
                        ...
                        error_text = await response.text()
                        logger.error(f"PhantomBuster API error: {response.status} - {error_text}")
                        return {"success": False, "error": f"API error: {response.status}"}
    ...
    async def _wait_for_phantom_completion(self, session: aiohttp.ClientSession, container_id: str, timeout: int = 600) -> Dict[str, Any]:
...
class TwinglyService(DataCollectionService):
    ...
    def __init__(self):
        """Initialize Twingly service"""
        self.api_key = os.getenv("TWINGLY_API_KEY")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(f"Twingly search for '{query}' returned {len(results.get('posts', []))} results")
                        return {
                            "success": True,
                            "task_id": task.task_id,
                            "query": query,
                        ...
                        error_text = await response.text()
                        logger.error(f"Twingly API error: {response.status} - {error_text}")
                        return {"success": False, "error": f"API error: {response.status}"}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(f"Tavily search for '{query}' completed successfully")
                        return {
                            "success": True,
                            "task_id": task.task_id,
                            "query": query,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(f"ZenRows scraping of '{url}' completed successfully")
                        return {
                            "success": True,
                            "task_id": task.task_id,
                            "url": url,
                        ...
                        error_text = await response.text()
                        logger.error(f"ZenRows API error: {response.status} - {error_text}")
                        return {"success": False, "error": f"API error: {response.status}"}
...
def get_data_collector() -> SophiaDataCollectionOrchestrator:
    """Get or create global data collection orchestrator"""
    global sophia_data_collector
...
async def scrape_website(url: str, js_render: bool = False, service: str = "zenrows") -> Dict[str, Any]:
...
async def search_news(query: str, days_back: int = 7, service: str = "twingly") -> Dict[str, Any]:
...
async def ai_search(query: str, depth: str = "basic", service: str = "tavily") -> Dict[str, Any]:
