...
class InlineDocumentationSystem:
    ...
    def _initialize_agent_documentation(self) -> Dict[str, AgentDocumentation]:
        ...
        return {
            "docker_agent": AgentDocumentation(
                ...
                common_errors=[
                    ("Port already in use", "Check for running containers with: docker ps"),
                    ("Cannot connect to Docker daemon", "Ensure Docker Desktop is running"),
                    ("No space left on device", "Clean up unused images: docker system prune")
                ]
            ),
            ...
            "pulumi_agent": AgentDocumentation(
                ...
                common_errors=[
                    ...
                    ("Resource already exists", "Import existing resources or use different names")
                ]
            ),
            ...
            "claude_agent": AgentDocumentation(
                ...
                common_errors=[
                    ("Generated code doesn't compile", "Provide more specific requirements"),
                    ("Review too generic", "Specify particular aspects to review"),
                    ("Rate limit exceeded", "Implement caching or reduce request frequency")
                ]
            )
        }
    ...
    def get_help(self, query: str) -> Dict[str, Any]:
        ...
        if any(word in query_lower for word in ["docker", "container", "image"]):
            agent = "docker_agent"
        elif any(word in query_lower for word in ["pulumi", "infrastructure", "deploy", "stack"]):
            agent = "pulumi_agent"
        elif any(word in query_lower for word in ["claude", "review", "generate", "analyze"]):
            agent = "claude_agent"
        else:
            # Return general help
        ...
        relevant_examples = [
            ...
            if any(tag in query_lower for tag in ex.tags)
        ]
        
        return {
            "agent": doc.name,
            "description": doc.description,
            "relevant_commands": self._find_relevant_commands(query_lower, doc.command_patterns),
            "examples": [
                {
                    "command": ex.command,
            ...
            for error, solution in doc.common_errors:
                if error.lower() in error_lower:
                    return {
                        ...
                        "error": error,
                        "solution": solution,
                        "related_commands": self.quick_reference.get(agent_name.split("_")[0], [])
                    }
