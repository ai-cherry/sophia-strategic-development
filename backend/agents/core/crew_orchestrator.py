...
class CrewOrchestrator:
        ...
        try:
            ...
            if config.tools:
                for tool_name in config.tools:
                    tool = await self.resource_orchestrator.get_tool_by_function(
                        tool_name
                    )
                    if tool:
    ...
    async def create_crew(
        ...
        try:
            ...
            # Create crew
            crew = Crew(
                agents=crew_agents, tasks=crew_tasks, verbose=verbose, process=process
            )
    ...
    async def create_hierarchical_crew(
        self,
        name: str,
        manager_config: SophiaAgentConfig,
        worker_configs: List[SophiaAgentConfig],
        ...
        try:
            ...
            # Create crew
            all_agents = [manager_config.name] + [
                worker_config.name for worker_config in worker_configs
            ]
            ...
            return {
                "crew_name": name,
                "manager": {"name": manager_config.name, "task_id": manager_task_id},
                "workers": [
                    {"name": worker_configs[i].name, "task_id": worker_task_ids[i]}
