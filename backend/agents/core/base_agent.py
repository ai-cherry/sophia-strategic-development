...
from .orchestrator import AgentCapability, AgentStatus, Task, TaskStatus
...
class BaseAgent(ABC):
    ...
    async def _register_with_orchestrator(self):
        ...
        try:
            ...
            await self.redis_client.publish(
                "sophia:agents:registration", json.dumps(registration_data)
            )
    ...
    async def _execute_task(self, task: Task):
        ...
        try:
            ...
            logger.info(
                f"Agent {self.agent_id} completed task {task.task_id} in {duration:.2f}s"
            )
    ...
    async def _complete_task(self, task_id: str, result: Dict[str, Any], success: bool):
        ...
        try:
            ...
            await self.redis_client.publish(
                "sophia:agents:results", json.dumps(result_message)
            )
    ...
    async def _health_reporting_loop(self):
        ...
        while self.is_running:
            try:
                ...
                await self.redis_client.publish(
                    "sophia:agents:health", json.dumps(health_data)
                )
