...
class BrainAgent(BaseAgent):
    ...
    def __init__(self, config: AgentConfig, portkey_client: PortkeyClient):
        ...
        self.tools = {
            "snowflake": snowflake_integration
        }
    ...
    async def execute_task(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            plan_response = await self.portkey.llm_call(plan_request, system_prompt=system_prompt)
            ...
            try:
                ...
                if plan.get("tool") == "snowflake.query":
                    ...
                    query_results = await self.tools["snowflake"].execute_query(sql_query)
                    ...
                    final_answer = summary_response.get("choices", [{}])[0].get("message", {}).get("content", "")
                    return TaskResult(status="success", output=final_answer)

            except (json.JSONDecodeError, KeyError):
                logger.warning("Could not parse a structured plan. Falling back to direct answer.")
                # Fallback if the plan isn't structured as expected
                pass

            # Fallback for non-SQL tasks or failed plan parsing
            final_answer_prompt = f"""You are Sophia, an AI assistant for Pay Ready.
            ...
            final_answer = final_answer_response.get("choices", [{}])[0].get("message", {}).get("content", "")
            return TaskResult(status="success", output=final_answer)
