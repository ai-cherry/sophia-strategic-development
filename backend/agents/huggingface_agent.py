...
class HuggingFaceAgent(BaseAgent):
    ...
    async def execute_task(self, task: Task) -> TaskResult:
        ...
        try:
            if "search" in command and "model" in command:
                ...
                if not paper_id:
                    return TaskResult(status="error", output="Could not find a valid paper ID (e.g., '2404.19756') in the command.")
            ...
            # Add more command interpretations here (e.g., for running spaces)
                ...
                return TaskResult(status="error", output=f"Unknown Hugging Face command: {task.command}")

        except Exception as e:
            logger.error(f"Error executing Hugging Face task '{task.command}': {e}", exc_info=True)
    ...
    def _extract_query(self, command: str, keyword: str) -> str:
        ...
        try:
            ...
            return command.split(f"about")[-1].strip()
        except Exception:
    ...
    def _format_model_results(self, models: list) -> str:
        ...
        for model in models[:5]: # show top 5
            lines.append(f"- **{model.get('id')}**: {model.get('pipeline_tag', 'N/A')} | Downloads: {model.get('downloads', 0)}")
        return "\n".join(lines)
