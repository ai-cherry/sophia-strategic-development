...
class CodebaseAwarenessAgent(BaseAgent):
    ...
    async def execute_task(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not tool_name:
                return TaskResult(status="error", output=f"Unknown codebase awareness command: {command}")
                
            logger.info(f"CodebaseAwarenessAgent executing tool '{tool_name}' with params: {params}")
            
            result = await self.mcp_client.call_tool(
                server="codebase_awareness",
                tool=tool_name,
                **params
            )
            ...
            logger.error(f"Error executing codebase awareness task '{command}': {e}", exc_info=True)
