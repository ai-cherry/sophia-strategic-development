...
class CRMSyncAgent(BaseAgent):
    """Specialized agent for CRM data synchronization and maintenance. Integrated with AgnoPerformanceOptimizer."""
    ...
    @classmethod
    async def pooled(cls, config: AgentConfig) -> 'CRMSyncAgent':
        """Get a pooled or new instance using AgnoPerformanceOptimizer."""
        optimizer = AgnoPerformanceOptimizer()
        await optimizer.register_agent_class('crm_sync', cls)
        agent = await optimizer.get_or_create_agent('crm_sync', {'config': config})
        logger.info(f"[AgnoPerformanceOptimizer] Provided CRMSyncAgent instance (pooled or new)")
        return agent
    ...
    async def process_task(self, task: Task) -> Dict[str, Any]:
        """Process assigned task based on type."""try:.
    ...
    async def _sync_call_to_crm(self, task: Task) -> Dict[str, Any]:
        """Sync call data to CRM system."""try:.
            ...
            async with self.hubspot_integration:
                contact = await self.hubspot_integration.get_contact(
                    email=contact_email
                )
                ...
                if not contact:
                    ...
                    contact = await self.hubspot_integration.create_contact(
                        contact_info
                    )
                ...
                note_result = await self.hubspot_integration.create_note(
                    contact_id, note_content
                )
                ...
                update_result = None
                if contact_updates:
                    update_result = await self.hubspot_integration.update_contact(
                        contact_id, contact_updates
                    )
    ...
    async def _update_deal_stage(self, task: Task) -> Dict[str, Any]:
        """Update deal stage based on call insights."""try:.
            ...
            async with self.hubspot_integration:
                ...
                update_result = await self.hubspot_integration.update_deal(
                    deal_id, deal_updates
                )
    ...
    async def _create_follow_up_tasks(self, task: Task) -> Dict[str, Any]:
        """Create follow-up tasks based on call analysis."""try:.
    ...
    async def _perform_data_quality_check(self, task: Task) -> Dict[str, Any]:
        """Perform data quality check on CRM data."""try:.
    ...
    def _extract_contact_info(self, call_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract contact information from call data."""contact_info = {}.
    ...
    def _format_call_note(self, call_data: Dict[str, Any]) -> str:
        """Format call data as CRM note."""note_parts = [f"Call Summary - {datetime.now().strftime('%Y-%m-%d %H:%M')}"].
    ...
    def _prepare_contact_updates(self, call_data: Dict[str, Any]) -> Dict[str, Any]:
        """Prepare contact property updates based on call data."""updates = {}.
    ...
    async def _handle_deal_sync(
        self, contact_id: str, call_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """Handle deal creation or update based on call data."""try:.

            async with self.hubspot_integration:
                # Check if contact has existing deals
                existing_deals = await self.hubspot_integration.get_contact_deals(
                    contact_id
                )
                ...
                if existing_deals:
                    ...
                    deal_updates = {"last_activity_date": datetime.now().isoformat()}
                    ...
                    return await self.hubspot_integration.update_deal(
                        deal_id, deal_updates
                    )
                    ...
                    if deal:
                        ...
                        await self.hubspot_integration.associate_deal_contact(
                            deal["id"], contact_id
                        )
    ...
    def _determine_deal_stage(self, call_insights: Dict[str, Any]) -> str:
        """Determine appropriate deal stage based on call insights."""# Map call outcomes to deal stages.
    ...
    def _calculate_due_date(self, next_step: str) -> datetime:
        """Calculate appropriate due date for a next step."""next_step_lower = next_step.lower().
    ...
    def _determine_task_priority(self, next_step: str) -> str:
        """Determine task priority based on next step content."""next_step_lower = next_step.lower().
    ...
    def _analyze_contact_quality(self, contact: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Analyze contact data quality."""issues = [].
    ...
    def _generate_cleanup_suggestions(
        ...
        """Generate cleanup suggestions based on quality issues."""suggestions = [].
