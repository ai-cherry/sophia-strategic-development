...
class InsightExtractionAgent(BaseAgent):
    ...
    async def _analyze_single_transcript(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            if not transcript:
                return await create_agent_response(
                    False, error="No transcript available"
                )
            ...
            insights = await self._extract_insights_with_llm(
                transcript_text, call_details, call_id
            )
    ...
    async def _batch_analyze_calls(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            if not calls:
                return await create_agent_response(
                    True, data={"message": "No recent calls found", "total_insights": 0}
                )
                ...
                if call_id:
                    try:
                        ...
                        if result.get("success"):
                            insights_count = result.get("data", {}).get(
                                "insights_found", 0
                            )
    ...
    async def _update_insight_status(self, task: Task) -> Dict[str, Any]:
        ...
        if not insight_id or not new_status:
            return await create_agent_response(
                False, error="insight_id and status are required"
            )
        ...
        return await create_agent_response(
            False, error=f"Insight {insight_id} not found"
        )
