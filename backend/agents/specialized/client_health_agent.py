...
class ClientHealthAgent(BaseAgent):
    ...
    def _calculate_health_score(self, interaction_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        last_interaction_date = max(row['CONVERSATION_DATETIME'] for row in interaction_history)
        days_since_last_interaction = (datetime.now(last_interaction_date.tzinfo) - last_interaction_date).days
        ...
        has_negative_sentiment = any(
            "complaint" in (t.get("TRACKER_NAME") or "").lower() for row in interaction_history for t in row.get("trackers", [])
        )
        ...
        return {"score": max(0, min(100, score)), "positive_factors": positive_factors, "risk_factors": risk_factors}
    ...
    async def process_task(self, task: Task) -> Dict[str, Any]:
        ...
        if task.task_type == "calculate_health_score":
            ...
            if not client_id:
                return await create_agent_response(False, error="client_id is required.")

            conn = await self._get_snowflake_connection()
            if not conn:
                return await create_agent_response(False, error="Could not connect to database.")
            ...
            try:
                ...
                with conn.cursor() as cursor:
                    cursor.execute("""
                        ...
                        VALUES (%s, %s, %s, %s, %s)
                    """, (score_id, client_id, health_analysis['score'], json.dumps(health_analysis['positive_factors']), json.dumps(health_analysis['risk_factors'])))
