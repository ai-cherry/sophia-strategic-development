...
class CallAnalysisAgent(BaseAgent):
    ...
    async def _analyze_single_call(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.gong_integration:
                call_insights = await self.gong_integration.extract_call_insights(
                    call_id
                )
    ...
    async def _analyze_multiple_calls(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get calls from Gong
            async with self.gong_integration:
                calls = await self.gong_integration.get_calls(
                    from_date, to_date, limit=100
                )

            if not calls:
                return await create_agent_response(
                    True, {"message": "No calls found in date range"}
                )
            ...
            for call in calls[:20]:  # Limit to 20 calls for performance
                ...
                if call_id:
                    async with self.gong_integration:
                        insights = await self.gong_integration.extract_call_insights(
                            call_id
                        )
                    if insights:
    ...
    async def _sync_call_to_crm(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            if not await validate_task_data(task, ["call_insights", "contact_email"]):
                raise ValueError(
                    "Missing required fields: call_insights, contact_email"
                )
            ...
            async with self.hubspot_integration:
                ...
                if call_insights.get("deal_stage"):
                    updates["lifecyclestage"] = self._map_deal_stage_to_lifecycle(
                        call_insights["deal_stage"]
                    )
    ...
    async def _analyze_for_coaching(self, task: Task) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.gong_integration:
                ...
                sentiment_data = await self.gong_integration.analyze_call_sentiment(
                    call_id
                )
            ...
            coaching_analysis = await self._perform_coaching_analysis(
                call_insights, sentiment_data
            )

            # Generate improvement plan
            improvement_plan = await self._generate_improvement_plan(
                coaching_analysis, rep_email
            )
    ...
    async def _prepare_crm_updates(self, insights: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            updates["notes_to_add"].append({"content": note_content, "type": "CALL"})
    ...
    def _format_call_note(self, insights: Dict[str, Any]) -> str:
        ...
        if insights.get("success_probability"):
            note_parts.append(
                f"Success Probability: {insights['success_probability']}%"
            )
    ...
    def _calculate_urgency_score(self, insights: Dict[str, Any]) -> int:
        ...
        if any(
            word in next_steps
            for word in ["urgent", "asap", "immediately", "this week"]
        ):
            score += 20
    ...
    async def _analyze_call_trends(
        ...
        for analysis in call_analyses:
            ...
            for pain_point in analysis.get("pain_points", []):
                trends["common_pain_points"][pain_point] = (
                    trends["common_pain_points"].get(pain_point, 0) + 1
                )

            # Track objections
            for objection in analysis.get("objections", []):
                trends["common_objections"][objection] = (
                    trends["common_objections"].get(objection, 0) + 1
                )
            ...
            trends["deal_stage_distribution"][stage] = (
                trends["deal_stage_distribution"].get(stage, 0) + 1
            )

            # Track urgency
            urgency = analysis.get("urgency_level", "medium")
            trends["urgency_trends"][urgency] = (
                trends["urgency_trends"].get(urgency, 0) + 1
            )
    ...
    async def _generate_team_insights(
        ...
        insights = {
            ...
            "process_insights": {"effective_strategies": [], "process_gaps": []},
        }

        # This would be enhanced with actual team performance data
        insights["team_performance"]["average_call_quality"] = 7.5
        insights["team_performance"]["common_strengths"] = [
            "Discovery questions",
            "Product knowledge",
        ]
        insights["team_performance"]["improvement_areas"] = [
            "Objection handling",
            "Closing techniques",
        ]
    ...
    async def _perform_coaching_analysis(
        ...
        if talk_ratio > 0.7:
            ...
            analysis["specific_feedback"].append(
                "Rep talked too much - focus on asking questions and listening"
            )
        ...
        if questions_asked < 5:
            ...
            analysis["specific_feedback"].append(
                "Need to ask more discovery questions to understand prospect needs"
            )
        ...
        for area in coaching_analysis.get("improvement_areas", []):
            if area == "Discovery questioning":
                plan["action_items"].append(
                    "Practice 10 discovery questions before next call"
                )
                plan["training_recommendations"].append(
                    "Discovery methodology training"
                )
            elif area == "Listening skills":
                plan["action_items"].append(
                    "Aim for 60/40 prospect/rep talk ratio in next calls"
                )
