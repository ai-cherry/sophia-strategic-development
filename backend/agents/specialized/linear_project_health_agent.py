...
class LinearProjectHealthAgent(BaseAgent):
    ...
    async def detect_project_anomalies(
        ...
        try:
            ...
            if (
                ...
                < baseline_metrics.get("completion_rate", 0) * 0.7
            ):
                anomalies.append(
                    {
                        "type": "velocity_drop",
            ...
            if (
                baseline_cycle_time > 0
                and current_metrics.avg_cycle_time > baseline_cycle_time * 1.5
            ):
                anomalies.append(
                    {
    ...
    def _calculate_project_metrics(
        ...
        blocked_issues = len(
            [
                ...
                or "blocked" in [l.lower() for l in i.labels]
            ]
        )
        ...
        if completed_with_times:
            cycle_times = [
                (i.updated_at - i.created_at).days for i in completed_with_times
            ]
            avg_cycle_time = sum(cycle_times) / len(cycle_times)

        # Calculate team utilization (simplified)
        assignees = set(i.assignee for i in issues if i.assignee)
        team_utilization = min(
            in_progress_issues / len(assignees) if assignees else 0, 1.0
        )
            ...
            risks.append(
                ProjectRisk(
                    ...
                    affected_issues=[
                        i.issue_id
                        for i in issues
                        if i.due_date and i.due_date < datetime.now()
                    ],
        ...
        if metrics.quality_score < 0.6:
            risks.append(
                ProjectRisk(
                    ...
                    affected_issues=[
                        i.issue_id for i in issues if len(i.description) < 50
                    ],
            ...
            async with self.cortex_service as cortex:
                ...
                try:
                    ...
                    for risk_data in ai_risks_data.get("risks", []):
                        ...
                        risk_type = getattr(
                            RiskType, risk_type_str.upper(), RiskType.TECHNICAL_DEBT
                        )
                        ...
                        ai_risks.append(
                            ProjectRisk(
                                ...
                                affected_issues=[],
                                impact_assessment=risk_data.get(
    ...
    def _analyze_team_performance(self, issues: List[LinearIssue]) -> Dict[str, Any]:
        ...
        for issue in issues:
            ...
            if (
                issue.due_date
                and issue.due_date < datetime.now()
