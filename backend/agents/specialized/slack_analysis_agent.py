...
class SlackAnalysisAgent(BaseAgent):
    ...
    async def analyze_conversation(
        ...
        try:
            ...
            if SlackInsightType.SENTIMENT_ANALYSIS in analysis_types:
                sentiment_insight = await self._analyze_sentiment(conversation, conversation_text)
                if sentiment_insight:
                    insights.append(sentiment_insight)
            
            if SlackInsightType.TOPIC_EXTRACTION in analysis_types:
                topic_insight = await self._extract_topics(conversation, conversation_text)
                if topic_insight:
                    insights.append(topic_insight)
            
            # Calculate metrics
            overall_sentiment = await self._calculate_overall_sentiment(conversation_text)
            ...
            business_value_score = self._calculate_business_value_score(conversation_text, insights)
    ...
    async def _analyze_sentiment(self, conversation: SlackConversation, text: str) -> SlackInsight:
    ...
    async def _extract_topics(self, conversation: SlackConversation, text: str) -> SlackInsight:
    ...
    async def _calculate_overall_sentiment(self, text: str) -> float:
        ...
        try:
            async with self.cortex_service as cortex:
                sentiment_result = await cortex.analyze_sentiment_with_cortex(text[:500])
    ...
    async def _extract_key_topics(self, text: str) -> List[str]:
        ...
        try:
            async with self.cortex_service as cortex:
                ...
                topics_result = await cortex.complete_text_with_cortex(topics_prompt, max_tokens=50)
    ...
    async def _extract_action_items(self, text: str) -> List[str]:
        ...
        try:
            async with self.cortex_service as cortex:
                ...
                actions_result = await cortex.complete_text_with_cortex(actions_prompt, max_tokens=100)
                return [action.strip() for action in actions_result.split('\n') if action.strip()][:3]
        except:
            return []
    ...
    async def _extract_decisions(self, text: str) -> List[str]:
        ...
        try:
            async with self.cortex_service as cortex:
                ...
                decisions_result = await cortex.complete_text_with_cortex(decisions_prompt, max_tokens=100)
                return [decision.strip() for decision in decisions_result.split('\n') if decision.strip()][:3]
        except:
            return []
    
    def _calculate_business_value_score(self, text: str, insights: List[SlackInsight]) -> float:
