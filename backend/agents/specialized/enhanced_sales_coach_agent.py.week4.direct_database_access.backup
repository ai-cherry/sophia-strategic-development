...
class EnhancedSalesCoachAgent(BaseAgent):
    ...
    async def _analyze_sales_rep_performance(
        ...
        try:
            ...
            analysis = (
                await self.microsoft_gong_integration.analyze_sales_rep_performance(
                    sales_rep=sales_rep, days=days
                )
            )
            ...
            session = CoachingSession(
                ...
                action_items=analysis.get("improvement_recommendations", {}).get(
                    "action_items", []
                ),
            ...
            return {
                ...
                "competitive_intelligence": analysis.get(
    ...
    async def _generate_coaching_message(
        self, session: CoachingSession, analysis: dict[str, Any]
    ) -> str:
        """Generate friendly but stern coaching message"""

        sales_rep = session.sales_rep
        critical_insights = [
            i for i in session.insights if i.priority == CoachingPriority.CRITICAL
        ]
        high_insights = [
            i for i in session.insights if i.priority == CoachingPriority.HIGH
        ]
        ...
        message_parts.append(
            "You have **massive potential**. When you're in the zone, you're unstoppable. "
            "But right now, you're working harder, not smarter."
        )
        ...
        if avg_talk_ratio > 0.7:
            issues.append(
                "You're talking too much because you're nervous about silence"
            )
        ...
        message_parts.append(
            f"You've got this, {sales_rep}, but only if you're willing to change. Are you in?"
        )
    ...
    async def _store_coaching_insights(
        ...
        try:
            ...
            logger.info(
                f"Stored coaching insights for {session.sales_rep} in AI Memory"
            )
    ...
    async def _track_improvement_progress(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            historical_insights = await self.ai_memory.recall_coaching_insights(
                query=f"performance_analysis {sales_rep}", limit=10
            )
            ...
            return {
                ...
                "coaching_effectiveness": self._calculate_coaching_effectiveness(
                    improvement_trends
                ),
    ...
    def _analyze_improvement_trends(
        self, historical_insights: list[dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    def _calculate_coaching_effectiveness(
    ...
    def _get_effectiveness_recommendation(
        self, effectiveness: float, trend: str
    ) -> str:
