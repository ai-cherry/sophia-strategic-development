...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        f"📍 API documentation available at http://localhost:{settings.port}/docs"
    )
...
app.include_router(
    llm_strategy_router, prefix=settings.api_v3_prefix, tags=["LLM Strategy"]
)
...
# Integration routes
app.include_router(
    linear_router, prefix=f"{settings.api_v3_prefix}/linear", tags=["Linear"]
)
app.include_router(
    asana_router, prefix=f"{settings.api_v3_prefix}/asana", tags=["Asana"]
)
app.include_router(
    notion_router, prefix=f"{settings.api_v3_prefix}/notion", tags=["Notion"]
)
app.include_router(
    codacy_router, prefix=f"{settings.api_v3_prefix}/codacy", tags=["Codacy"]
)

# Admin routes
app.include_router(
    snowflake_admin_router, prefix=settings.api_admin_prefix, tags=["Admin"]
)
