...
from backend.integrations.agno_integration import agno_integration, AgnoTool
...
class AgentRequest(BaseModel):
    ...
    instructions: Optional[List[str]] = Field(None, description="The instructions for the agent")
...
async def process_request(
    ...
    bridge: MCPToAgnoBridge = Depends(get_agno_bridge)
):
    """
...
async def get_bridge_stats(bridge: MCPToAgnoBridge = Depends(get_agno_bridge)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get bridge stats: {str(e)}")
...
async def clear_bridge_cache(bridge: MCPToAgnoBridge = Depends(get_agno_bridge)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to clear bridge cache: {str(e)}")
