...
async def strategic_query(
    query: StrategicQuery, current_role: UserRole = Depends(get_current_user_role)
):
    ...
    if current_role != UserRole.CEO:
        raise HTTPException(
            status_code=403,
            detail="Forbidden: This endpoint is for executive use only.",
        )
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500,
            detail=result.get("error", "Failed to process strategic query."),
        )
