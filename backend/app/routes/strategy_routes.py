...
async def growth_opportunities():
    ...
    try:
        ...
        total_potential = sum(
            opp["potential_revenue"] for opp in opportunities.values()
        )
        total_investment = sum(
            opp["investment_required"] for opp in opportunities.values()
        )
            ...
            jsonify(
                {
                    ...
                    "summary": {
                        ...
                        "average_roi": (
                            (total_potential - total_investment)
                            / total_investment
                            * 100
                        )
...
async def strategic_goals():
    ...
    try:
        ...
        at_risk_goals = [
            name for name, goal in goals.items() if goal["status"] == "at_risk"
        ]
    ...
    try:
        ...
        for scenario in scenarios:
            result = {
                ...
                "projected_outcomes": {
                    "revenue_impact": scenario.get("assumptions", {}).get(
                        "growth_rate", 10
                    )
                    * 1500000,
                    "profit_margin": 15
                    + (scenario.get("assumptions", {}).get("efficiency_gain", 0) * 0.5),
                    "market_share": 0.42
                    + (
                        scenario.get("assumptions", {}).get("market_expansion", 0)
                        * 0.01
                    ),
                    "customer_count": 2847
                    + (scenario.get("assumptions", {}).get("customer_growth", 0) * 100),
                },
                "risks": [
                    "Market competition",
                ...
                if scenario.get("assumptions", {}).get("risk_level", "medium") == "high"
                else "Recommended",
                "confidence_score": 0.75
                if scenario.get("assumptions", {}).get("risk_level", "medium") == "high"
                else 0.85,
