...
async def get_sales_performance(
    timeframe: str = "30d", current_role: UserRole = Depends(get_current_user_role)
) -> Dict[str, Any]:
    ...
    if request:
        ...
        if request.search_query:
            ...
            filtered = [
                ...
                or any(query in cap for cap in m["capabilities"])
            ]
        if request.min_context_window:
            filtered = [
                m for m in filtered if m["context_window"] >= request.min_context_window
            ]
        if request.max_cost_per_token:
            filtered = [
                m
                for m in filtered
                if m["pricing"]["prompt"] <= request.max_cost_per_token
            ]
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            comparison_results.append(
                {"model_id": model_ids[i], "error": str(result), "success": False}
            )
        else:
...
async def get_infrastructure_health(
    ...
    return {
        "docker_containers": health_checks[0]
        if not isinstance(health_checks[0], Exception)
        else {"error": str(health_checks[0])},
        "snowflake": health_checks[1]
        if not isinstance(health_checks[1], Exception)
        else {"error": str(health_checks[1])},
        "pinecone": health_checks[2]
        if not isinstance(health_checks[2], Exception)
        else {"error": str(health_checks[2])},
        "timestamp": datetime.utcnow().isoformat(),
    }
...
async def _select_optimal_model(query: str, context_size: int = 0) -> str:
    ...
    query_type = "general"
    if any(
        word in query_lower for word in ["strategic", "plan", "long-term", "vision"]
    ):
        query_type = "strategic_analysis"
    elif any(word in query_lower for word in ["quick", "summary", "brief", "tldr"]):
        query_type = "quick_response"
    elif any(
        word in query_lower for word in ["analyze", "deep", "comprehensive", "detailed"]
    ):
        query_type = "deep_analysis"
    elif any(
        word in query_lower for word in ["code", "implement", "function", "algorithm"]
    ):
...
async def _get_latest_model() -> str:
    ...
    for model in models:
        ...
        if any(
            ...
            for indicator in ["preview", "latest", "3.5", "4", "o1"]
        ):
...
async def _run_model_comparison(
    client: OpenRouterClient, model_id: str, query: str
) -> Dict[str, Any]:
...
def _build_synthesis_prompt(
    query: str, internal_results: List, external_results: List, mode: str
) -> str:
