...
from backend.services.enhanced_mcp_orchestration_service import EnhancedMCPOrchestrationService
...
from backend.api import (
    ...
    hubspot_integration_routes,
    gong_integration_routes,
    snowflake_admin_routes,
    monitoring_routes,
    workflow_routes
)
...
ACTIVE_REQUESTS = Gauge(
    'sophia_http_active_requests',
    'Active HTTP requests'
)

ERROR_RATE = Gauge(
    'sophia_http_error_rate',
    'HTTP error rate'
)
...
class UnifiedFastAPIApp:
    ...
    async def _start_background_tasks(self):
        ...
        # Start MCP servers
        asyncio.create_task(
            self.services['mcp_orchestration'].start_all_servers()
        )
    ...
    def _configure_routes(self):
        ...
        async def health_check():
            ...
            return HealthResponse(
                status="healthy" if all(s == "healthy" for s in service_health.values()) else "degraded",
        ...
        routers = [
            ...
            (business_intelligence_routes.router, "/api/v3", ["Business Intelligence"]),
            (data_flow_routes.router, "/api/v3", ["Data Flow"]),
            (llm_strategy_routes.router, "/api/v3", ["LLM Strategy"]),
            ...
            (hubspot_integration_routes.router, "/api/v3", ["CRM"]),
            (gong_integration_routes.router, "/api/v3", ["Sales"]),
            (snowflake_admin_routes.router, "/api/v3", ["Data"]),
            (monitoring_routes.router, "/api/v3", ["Monitoring"]),
            (workflow_routes.router, "/api/v3", ["Workflows"])
        ]
