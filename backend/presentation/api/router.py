...
def create_application_router() -> APIRouter:
    ...
    router.include_router(
        enhanced_ceo_chat_routes.router,
        prefix="/api/v1/ceo",
        tags=["CEO Dashboard"]
    )
    ...
    # LLM and AI Routes
    router.include_router(
        simplified_llm_routes.router,
        prefix="/api/v1/llm",
        tags=["LLM Strategy"]
    )
    router.include_router(
        llm_strategy_routes.router,
        prefix="/api/v1/llm-strategy",
        tags=["LLM Strategy"]
    )
    router.include_router(
        smart_ai_routes.router,
        prefix="/api/v1/smart-ai",
        tags=["Smart AI"]
    )
    ...
    router.include_router(
        unified_ai_routes.router,
        prefix="/api/v1/unified-ai",
        tags=["Unified AI"]
    )
    ...
    router.include_router(
        costar_routes.router,
        prefix="/api/v1/costar",
        tags=["COSTAR"]
    )
    
    # Data and Knowledge Management Routes
    router.include_router(
        data_flow_routes.router,
        prefix="/api/v1/data-flow",
        tags=["Data Flow"]
    )
    router.include_router(
