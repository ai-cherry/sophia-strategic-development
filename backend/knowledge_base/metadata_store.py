...
class MetadataStore:
    ...
    def add_chunks(self, chunks: List[Chunk]):
        ...
        for i, chunk in enumerate(chunks):
            ...
            if not document_id:
                logger.warning("Chunk is missing a document_id in its metadata. Skipping.")
                continue
    ...
    def filter_documents(self, key: str, value: Any) -> List[DocumentMetadata]:
        ...
        logger.info(f"Filtered documents on '{key}={value}', found {len(results)} results.")
        return results
    ...
    def get_chunks_for_document(self, document_id: str) -> List[Chunk]:
        ...
        return [chunk for chunk in self.chunks.values() if chunk.metadata.get('document_id') == document_id]
...
async def main():
    ...
    chunks = [
        Chunk(content="The first key point about ROI.", metadata={"document_id": "doc_001", "chunk_index": 0}),
        Chunk(content="A detailed breakdown of financial benefits.", metadata={"document_id": "doc_001", "chunk_index": 1}),
    ]
