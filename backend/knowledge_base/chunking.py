    ...
    def create_chunks(
        ...
        for i, chunk_content in enumerate(text_chunks):
            ...
            chunk_metadata.update(
                {"chunk_index": i, "chunk_length_chars": len(chunk_content)}
            )
            chunks.append(Chunk(content=chunk_content, metadata=chunk_metadata))

        logger.info(
            f"Split text into {len(chunks)} chunks for document: {document_metadata.get('file_name')}"
        )
        return chunks
...
"""
    ...
    document_metadata = {"file_name": "test_document.txt", "source": "manual_test"}
