...
class QueryEngine:
    ...
    async def search(
        ...
        """
        logger.info(f"Performing search for query: '{query_text}' with filters: {filters}")
...
async def main():
    ...
    with open(file1_path, "w") as f:
        f.write("Product A increases team productivity by 20%. It is designed for enterprise clients.")
        
    file2_path = test_dir / "product_B_sheet.txt"
    with open(file2_path, "w") as f:
        f.write("Product B reduces operational costs. It is ideal for small businesses.")

    # Ingest documents
    await ingestion_pipeline.ingest_document(file1_path, "sales_deck", ["enterprise", "productivity"])
    ...
    results2 = await query_engine.search(
        ...
        filters={"document_type": "fact_sheet"}
    )
    for res in results2:
        print(f"Score: {res['score']:.4f} | Content: '{res['content']}' | Metadata: {res['metadata']}")
