    ...
    """try:
        ...
        updated_doc = KnowledgeDocument(
            ...
            content_type=ContentType(
                data.get("content_type", existing_doc.content_type.value)
            ),
        ...
        if success:
            return jsonify(
                {"message": "Document updated successfully", "document_id": document_id}
            )
        else:
...
def bulk_import():
    ...
    """try:
        ...
        return jsonify({"message": "Bulk import completed", "statistics": stats})
...
def health_check():
    """Knowledge base health check."""
        ...
        if not knowledge_base:
            return (
                jsonify(
                    {"status": "unhealthy", "message": "Knowledge base not initialized"}
                ),
                503,
            )
        ...
        return (
            jsonify({"status": "unhealthy", "message": f"Health check failed: {e}"}),
            503,
        )
