...
class SecretManager:
    ...
    def _get_or_create_encryption_key(self) -> bytes:
        ...
        if not encryption_key:
            ...
            self.logger.warning("Generated new encryption key - should be stored in ESC")
            return key
    ...
    async def validate_secrets(self) -> Dict[str, bool]:
        ...
        try:
            ...
            self.logger.info("Secret validation completed", 
    ...
    async def _validate_jwt_keys(self) -> bool:
        ...
        try:
            ...
            token = jwt.encode(
                test_payload,
                self.config.webhook_jwt_private_key,
                algorithm="RS256"
            )
            ...
            decoded = jwt.decode(
                token,
                self.config.webhook_jwt_public_key,
                algorithms=["RS256"]
            )
    ...
    async def _validate_anthropic_key(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                self.logger.info("Anthropic API validation", 
                               success=success, 
    ...
    async def setup_secret_rotation_monitoring(self) -> None:
        ...
        try:
            ...
            self.logger.info("Secret rotation monitoring setup completed",
    ...
    async def _initialize_secret_metadata(self) -> None:
        ...
        secret_configs = [
            ...
            ("snowflake_password", SecretType.DATABASE_PASSWORD, SecurityLevel.CRITICAL, 90),
            ("redis_password", SecretType.DATABASE_PASSWORD, SecurityLevel.HIGH, 90),
            ("webhook_jwt_private_key", SecretType.JWT_KEY, SecurityLevel.CRITICAL, 365),
            ("agent_orchestrator_auth_token", SecretType.API_KEY, SecurityLevel.HIGH, 90),
            ("agent_communication_secret", SecretType.ENCRYPTION_KEY, SecurityLevel.HIGH, 90),
        ]
    ...
    async def _notify_expiring_secrets(self, expiring_secrets: List[SecretMetadata]) -> None:
        ...
        for secret in expiring_secrets:
            ...
            self.logger.warning("Secret expiring soon",
                              secret_name=secret.name,
                              expires_at=secret.expires_at,
                              security_level=secret.security_level)
    
    async def audit_secret_access(self, secret_name: str, access_context: Dict[str, Any]) -> None:
    ...
    async def _audit_log_event(
        ...
        self.logger.info("Security audit event logged",
                        event_type=event_type,
                        resource=resource,
                        action=action,
                        status=status,
    ...
    async def get_security_status(self) -> Dict[str, Any]:
        ...
        for status in SecretStatus:
            status_counts[status.value] = sum(
                1 for metadata in self._secret_metadata.values()
