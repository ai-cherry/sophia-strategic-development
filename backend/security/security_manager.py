...
class SophiaSecurityManager:
    ...
    async def store_api_key(
        ...
        try:
            ...
            await self._log_security_event(
                "api_key_stored", {"service_name": service_name, "key_id": key_id}
            )
    ...
    async def get_api_key(self, service_name: str) -> Optional[str]:
        ...
        try:
            ...
            await self.redis_client.hset(
                key_id, "last_used", datetime.now().isoformat()
            )
    ...
    async def rotate_api_key(self, service_name: str, new_api_key: str) -> bool:
        ...
        try:
            ...
            if success:
                # Log the rotation
                await self._log_security_event(
                    "api_key_rotated", {"service_name": service_name}
                )
                    ...
                    keys.append(
                        {
                            ...
                            or None,
                            "needs_rotation": datetime.now()
                            > datetime.fromisoformat(
                                key_data[b"rotation_due"].decode()
                            ),
                        }
                    )
    ...
    async def create_session(
        ...
        try:
            ...
            session_data = {
                ...
                "created_at": datetime.now().isoformat(),
                "expires_at": (
                    datetime.now() + timedelta(hours=self.config.session_timeout_hours)
                ).isoformat(),
            ...
            await self.redis_client.expire(
                session_key, self.config.session_timeout_hours * 3600
            )
    ...
    async def invalidate_session(self, session_token: str) -> bool:
        ...
        try:
            ...
            await self._log_security_event(
                "session_invalidated", {"session_id": session_id}
            )
    ...
    async def record_failed_attempt(
        ...
        try:
            ...
            if new_attempts >= self.config.max_failed_attempts:
                await self._handle_security_breach(
                    identifier, attempt_type, new_attempts
                )
                ...
                async for key in self.redis_client.scan_iter(match=pattern):
                    ...
                    if session_data:
                        expires_at = datetime.fromisoformat(
                            session_data[b"expires_at"].decode()
                        )
                        if datetime.now() > expires_at:
                            await self.redis_client.delete(key)
                            expired_count += 1
    ...
    async def _schedule_key_rotation(self, service_name: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to schedule key rotation for {service_name}: {str(e)}"
            )
    ...
    async def _handle_security_breach(
        ...
        try:
            ...
            logger.warning(
                f"Security breach detected: {identifier} - {attempt_count} failed {attempt_type} attempts"
            )
        ...
        try:
            ...
            try:
                ...
                health["components"]["encryption"] = (
                    "healthy" if decrypted == test_data else "unhealthy"
                )
            except Exception:
