        ...
        try:
            ...
            async with self.cortex_service as cortex:
                ...
                for call in related_calls[:10]:  # Analyze top 10 calls
                    ...
                    if call.get('matching_content'):
                        ...
                        call_topics = [topic.strip() for topic in topics_result.split(',') if topic.strip()] if topics_result else []
                    ...
                    call_insights.append({
                        ...
                        "duration_minutes": (call.get('CALL_DURATION_SECONDS', 0) or 0) / 60,
            ...
            return {
                ...
                "metrics": {
                    ...
                    "talk_ratio_assessment": "optimal" if 0.3 <= avg_talk_ratio <= 0.6 else "needs_improvement"
                },
                "top_topics": dict(top_topics),
                "call_insights": call_insights,
                "recommendations": self._generate_call_recommendations(avg_sentiment, avg_talk_ratio, top_topics)
            }
...
class SupervisorAgent:
        ...
        try:
            ...
            async with self.hubspot_connector as connector:
                ...
                if not deals_data.empty:
                    ...
                    state["error_messages"].append(f"Deal {state['deal_id']} not found in HubSpot")
    ...
    async def consolidate_findings(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if state.get("sales_coach_insights", {}).get("recommendations"):
                all_recommendations.extend(state["sales_coach_insights"]["recommendations"])
            
            # Add call analysis recommendations
            if state.get("call_analysis_insights", {}).get("recommendations"):
                all_recommendations.extend(state["call_analysis_insights"]["recommendations"])
            ...
            state["consolidated_findings"] = {
                ...
                "key_metrics": {
                    "call_sentiment": state.get("call_analysis_insights", {}).get("metrics", {}).get("avg_sentiment", 0),
                    "call_count": state.get("call_analysis_insights", {}).get("call_count", 0),
                    "deal_value": state["hubspot_deal_data"]["amount"] if state["hubspot_deal_data"] else 0,
                    "deal_stage": state["hubspot_deal_data"]["deal_stage"] if state["hubspot_deal_data"] else "Unknown"
                },
                "analysis_timestamp": datetime.now().isoformat()
            }
            
            state["recommendations"] = all_recommendations[:10]  # Top 10 recommendations
    ...
    def _calculate_deal_health_score(self, state: WorkflowState) -> float:
        ...
        deal_stage = state["hubspot_deal_data"]["deal_stage"] if state["hubspot_deal_data"] else ""
...
class LangGraphWorkflowOrchestrator:
    ...
    async def _sales_coach_analysis_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if state["hubspot_deal_data"]:
                    ...
                    "recommendations": [
                        {
                            ...
                            "actions": ["Regular check-ins", "Process adherence", "Documentation"]
                        }
                    ]
                }
                ...
                state["error_messages"].append("No HubSpot deal data available for sales coach analysis")
    ...
    async def _call_analysis_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            if state["hubspot_deal_data"]:
                ...
                state["error_messages"].append("No deal data available for call analysis")
    ...
    def _should_continue_to_call_analysis(self, state: WorkflowState) -> str:
        ...
        if state["sales_coach_status"] == AgentStatus.COMPLETED:
            ...
            logger.warning("Sales coach analysis failed, proceeding with partial results")
    ...
    async def analyze_deal(
        ...
        try:
            ...
            return {
                ...
                "execution_time": (result["completed_at"] - result["started_at"]).total_seconds() if result.get("completed_at") else None,
