...
class LangGraphWorkflow:
    ...
    async def execute(self, initial_context: Dict[str, Any]) -> WorkflowState:
        ...
        logger.info(
            f"Workflow {self.workflow_id} completed with status: {self.state.status}"
        )
        ...
        for attempt in range(step.retry_count):
            try:
                ...
                if attempt < step.retry_count - 1:
                    logger.warning(
                        f"Step {step.name} failed, retrying... (attempt {attempt + 1})"
                    )
...
class WorkflowTemplates:
    ...
    def create_deployment_workflow(environment: str = "staging") -> LangGraphWorkflow:
        ...
        workflow.add_step(
            WorkflowStep(
                ...
                condition=lambda state: state.results.get(
    ...
    def create_infrastructure_update_workflow() -> LangGraphWorkflow:
        ...
        workflow.add_step(
            WorkflowStep(
                name="refresh_dev", agent="pulumi_agent", command="refresh stack dev"
            )
        )
        ...
        workflow.add_step(
            WorkflowStep(
                ...
                depends_on=["test_dev"],
                condition=lambda state: state.results.get("test_dev", {}).get("status")
                == "success",
            )
        )
...
class WorkflowManager:
    ...
    async def execute_workflow(
        self, workflow: LangGraphWorkflow, context: Dict[str, Any]
    ) -> WorkflowState:
