...
class MultiAgentWorkflow:
    ...
    async def execute_workflow(self) -> WorkflowExecution:
        ...
        try:
            ...
            logger.info(
                f"✅ Workflow {execution_id} completed in {total_duration:.2f}s with {success_rate:.1%} success rate"
            )
    ...
    async def _execute_single_task(self, task: WorkflowTask) -> WorkflowResult:
        ...
        while retry_count <= task.retry_attempts:
            try:
                logger.info(
                    f"🔄 Executing task {task.task_id} (attempt {retry_count + 1})"
                )
                ...
                if retry_count > task.retry_attempts:
                    ...
                    logger.warning(
                        f"⚠️ Task {task.task_id} failed, retrying in {backoff_time}s: {error_msg}"
                    )
