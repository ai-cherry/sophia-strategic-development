...
class MultiAgentWorkflow:
    ...
    def register_agent(self, agent_type: str, agent_instance: AgentWorkflowInterface) -> None:
    ...
    async def execute_workflow(self) -> WorkflowExecution:
        ...
        try:
            ...
            try:
                ...
                if all(result.status == TaskStatus.COMPLETED for result in self.task_results.values()):
            ...
            success_rate = sum(1 for r in self.task_results.values() if r.status == TaskStatus.COMPLETED) / len(self.task_results)
            
            logger.info(f"✅ Workflow {execution_id} completed in {total_duration:.2f}s with {success_rate:.1%} success rate")
    ...
    async def _execute_workflow_tasks(self) -> None:
        ...
        ready_tasks = [task for task in self.workflow_def.tasks if not task.dependencies]
        ...
        while self.execution_queue or self.running_tasks:
            ...
            tasks_to_start = []
            while self.execution_queue and len(self.running_tasks) < self._get_max_parallel_tasks():
                ...
                if task_id:
                    ...
                    for dependent_task_id in self.reverse_dependencies.get(task_id, []):
                        if self._are_dependencies_satisfied(dependent_task_id):
                            dependent_task = next(
                                task for task in self.workflow_def.tasks 
    ...
    async def _execute_single_task(self, task: WorkflowTask) -> WorkflowResult:
        ...
        while retry_count <= task.retry_attempts:
            try:
                logger.info(f"🔄 Executing task {task.task_id} (attempt {retry_count + 1})")
                ...
                if retry_count > task.retry_attempts:
                    ...
                    self.task_results[task.task_id] = result
                    logger.error(f"❌ Task {task.task_id} failed after {retry_count - 1} retries: {error_msg}")
                    return result
                else:
                    # Retry with exponential backoff
                    backoff_time = 2 ** (retry_count - 1)
                    logger.warning(f"⚠️ Task {task.task_id} failed, retrying in {backoff_time}s: {error_msg}")
    ...
    async def _prepare_task_input(self, task: WorkflowTask) -> Dict[str, Any]:
        ...
        for dependency in task.dependencies:
            if dependency in self.task_results:
                dependency_results[dependency] = self.task_results[dependency].output_data
    ...
    async def _validate_task_output(self, task: WorkflowTask, output: Dict[str, Any]) -> bool:
            ...
            if task.expected_output_schema:
                ...
                for field in required_fields:
                    if field not in output:
                        logger.warning(f"Missing required field '{field}' in task {task.task_id} output")
                        return False
    ...
    def _calculate_execution_metrics(self, execution: WorkflowExecution) -> Dict[str, Any]:
        ...
        completed_tasks = sum(1 for r in execution.task_results.values() if r.status == TaskStatus.COMPLETED)
        failed_tasks = sum(1 for r in execution.task_results.values() if r.status == TaskStatus.FAILED)
        total_retries = sum(r.retry_count for r in execution.task_results.values())
        avg_execution_time = sum(r.execution_time_seconds for r in execution.task_results.values()) / len(execution.task_results) if execution.task_results else 0
        ...
        return {
            ...
            "success_rate": completed_tasks / len(execution.task_results) if execution.task_results else 0,
            "total_retries": total_retries,
            "average_task_execution_time": avg_execution_time,
            "task_metrics": task_metrics,
            "workflow_efficiency": completed_tasks / (total_duration / 60) if total_duration > 0 else 0  # tasks per minute
        }
        ...
        workflow_def = WorkflowDefinition(
            ...
            tasks=[
                ...
                WorkflowTask(
                    ...
                    expected_output_schema={"required": ["project_health", "issues", "team_performance"]},
                ...
                WorkflowTask(
                    ...
                    agent_role=AgentRole.ANALYZER,
                    input_data={"project_gid": project_context.get("asana_project_gid")},
                    expected_output_schema={"required": ["project_status", "tasks", "team_metrics"]},
                    priority=WorkflowPriority.HIGH
                ),
                ...
                WorkflowTask(
                    ...
                    agent_role=AgentRole.ANALYZER,
                    input_data={"project_keywords": project_context.get("keywords", [])},
                    expected_output_schema={"required": ["call_insights", "sentiment_analysis"]},
                    priority=WorkflowPriority.HIGH
                ),
                ...
                WorkflowTask(
                    ...
                    expected_output_schema={"required": ["deal_analysis", "customer_insights"]},
                    priority=WorkflowPriority.HIGH
                ),
                ...
                WorkflowTask(
                    ...
                    dependencies=["collect_linear_data", "collect_asana_data", "collect_gong_data", "collect_hubspot_data"],
                    expected_output_schema={"required": ["cross_platform_insights", "correlation_analysis"]},
                ...
                WorkflowTask(
                    ...
                    expected_output_schema={"required": ["risk_score", "risk_factors", "mitigation_strategies"]},
                    priority=WorkflowPriority.CRITICAL
                ),
                ...
                WorkflowTask(
                    ...
                    expected_output_schema={"required": ["recommendations", "action_items", "success_metrics"]},
                    priority=WorkflowPriority.HIGH
                ),
                ...
                WorkflowTask(
                    ...
                    expected_output_schema={"required": ["executive_summary", "key_findings", "next_steps"]},
            ...
            success_criteria=["all_data_collected", "analysis_completed", "report_generated"]
        )
