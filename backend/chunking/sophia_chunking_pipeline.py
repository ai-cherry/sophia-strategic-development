...
class SophiaChunkingPipeline:
    ...
    async def chunk_content(
        self, content: str, content_type: str, **kwargs
    ) -> List[Dict[str, Any]]:
        ...
        if content_type != "text":
            logger.warning(
                f"Chunking for content type '{content_type}' is not fully supported. Treating as plain text."
            )
        ...
        if current_chunk:
            chunks.append(
                {"content": current_chunk, "metadata": {"source": "paragraph_split"}}
            )
    ...
    async def process_content(
        self, content: str, content_type: str, source_id: str, priority: str = "normal"
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Processed {len(ai_enhanced_chunks)} chunks in {processing_time:.2f}s"
            )
    ...
    async def _create_primary_chunks(
        ...
        emotional_chunks = await self.emotional_chunker.chunk_by_emotional_shifts(
            decision_chunks
        )
    ...
    async def _enhance_with_business_intelligence(
        self, chunks: List[Dict[str, Any]], content_type: str, source_id: str
    ) -> List[Dict[str, Any]]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            bi_data = await self.business_intelligence.extract_business_intelligence(
                [chunk]
            )
            ...
            decision_makers = (
                await self.decision_maker_extractor.extract_decision_makers(
                    chunk["text"]
                )
            )

            # Classify topics hierarchically
            topic_data = await self.topic_classifier.classify_topics_hierarchically(
                chunk["text"]
            )
            ...
            enhanced_chunk = {
                **chunk,
                ...
                "topic_confidence": topic_data.get("confidence_scores", {}).get(
                    "overall", 0.0
                ),
            }
    ...
    async def _extract_comprehensive_metadata(
        self, chunks: List[Dict[str, Any]], priority: str
    ) -> List[Dict[str, Any]]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            metadata = SophiaEnhancedMetadata(
                ...
                revenue_potential=chunk.get("business_intelligence", {})
                .get("financial", {})
                .get("revenue_potential", 0.0),
                technology_relevance=chunk.get("business_intelligence", {})
                .get("technology", {})
                .get("relevance_score", 0.0),
                performance_impact=chunk.get("business_intelligence", {})
                .get("performance", {})
                .get("impact_score", 0.0),
    ...
    async def _basic_processing(
        self, content: str, content_type: str, source_id: str
    ) -> List[Dict[str, Any]]:
