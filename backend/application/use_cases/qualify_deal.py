...
class QualifyDealUseCase:
    ...
    async def _gather_qualification_criteria(self, deal: Deal) -> QualificationCriteria:
        ...
        recent_calls = [
            c for c in calls 
            if (datetime.now() - c.scheduled_at).days <= 30
        ]
        
        # Check for decision maker engagement
        decision_makers = [c for c in contacts if c.is_decision_maker]
        decision_maker_engaged = any(
            c.engagement_level in [EngagementLevel.WARM, EngagementLevel.HOT, EngagementLevel.CHAMPION]
        ...
        if recent_calls:
            ...
            if calls_with_sentiment:
                avg_sentiment = sum(c.sentiment.score for c in calls_with_sentiment) / len(calls_with_sentiment)
    ...
    def _generate_next_steps(
        ...
        if is_qualified:
            ...
            if not criteria.recent_engagement:
                next_steps.append("Re-engage with the prospect through calls or meetings")
    ...
    def _identify_risk_factors(
        ...
        if deal.days_in_stage and deal.days_in_stage > 30:
            risks.append(f"Stuck in {deal.stage.value} stage for {deal.days_in_stage} days")
