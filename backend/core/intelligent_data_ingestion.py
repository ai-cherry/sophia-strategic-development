...
class IntelligentDataIngestion:
    ...
    async def _initialize_metadata_ai(self) -> None:
        ...
        self.metadata_patterns = {
            'business_terms': [
                'revenue', 'sales', 'customer', 'deal', 'pipeline', 'forecast',
                'kpi', 'metric', 'performance', 'growth', 'churn', 'retention'
            ],
            'temporal_patterns': [
                r'\d{4}-\d{2}-\d{2}', r'Q[1-4]\s*\d{4}', r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)',
                r'(weekly|monthly|quarterly|annual)', r'(daily|hourly)'
            ],
            'data_types': [
                'financial', 'operational', 'marketing', 'sales', 'hr', 'product',
                'customer_service', 'analytics', 'reporting'
            ]
        }
    ...
    async def ingest_data(self, 
        ...
        try:
            ...
            ai_suggestions = await self._generate_metadata_suggestions(source, content_data)
            result.suggested_tags = ai_suggestions
            
            # Step 4: Interactive metadata handling (if enabled)
            if interactive_mode:
                await self._update_ingestion_progress(source.id, 0.6, 'interactive_metadata')
                final_metadata = await self._handle_interactive_metadata(
                    source, ai_suggestions, user_metadata
                )
            else:
                final_metadata = await self._auto_apply_metadata(ai_suggestions, user_metadata)
            ...
            embeddings_result = await self._generate_and_store_embeddings(chunks, source)
            ...
            snowflake_result = await self._store_in_snowflake(content_data, final_metadata, source)
    ...
    async def _validate_source(self, source: DataSource) -> Dict[str, Any]:
        ...
        if source.size_bytes and source.size_bytes > max_size:
            errors.append(f"File too large: {source.size_bytes} bytes (max: {max_size})")
    ...
    async def _generate_metadata_suggestions(self, source: DataSource, content_data: Dict[str, Any]) -> List[MetadataTag]:
        ...
        for term in self.metadata_patterns['business_terms']:
            if term in filename_lower:
                suggestions.append(MetadataTag(
                    key='business_domain',
                    value=term,
                    confidence=0.8,
                    source='ai_suggested',
    ...
    async def _handle_interactive_metadata(self, 
                                         source: DataSource, 
                                         ai_suggestions: List[MetadataTag],
                                         user_metadata: Optional[Dict[str, Any]]) -> List[MetadataTag]:
        ...
        if user_metadata:
            for key, value in user_metadata.items():
                final_metadata.append(MetadataTag(
                    key=key,
                    value=str(value),
                    confidence=1.0,
                    source='user',
    ...
    async def _auto_apply_metadata(self, 
                                 ai_suggestions: List[MetadataTag],
                                 user_metadata: Optional[Dict[str, Any]]) -> List[MetadataTag]:
    ...
    async def _create_content_chunks(self, content_data: Dict[str, Any], metadata: List[MetadataTag]) -> List[Dict[str, Any]]:
    ...
    async def _generate_and_store_embeddings(self, chunks: List[Dict[str, Any]], source: DataSource) -> Dict[str, Any]:
    ...
    async def _store_in_snowflake(self, content_data: Dict[str, Any], metadata: List[MetadataTag], source: DataSource) -> Dict[str, Any]:
    ...
    async def _update_ingestion_progress(self, source_id: str, progress: float, stage: str) -> None:
    ...
    async def search_ingested_content(self, query: str, filters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
