...
class ConfigManager:
    ...
    async def _initialize_fallback_mode(self) -> None:
        ...
        logger.warning("Initializing configuration manager in fallback mode")
        self.esc_client = None
        await self._load_service_registry()
        self.initialized = True
    ...
    def _validate_service_registry(self, registry: Dict[str, Any]) -> bool:
        ...
        for service_name, config in registry.items():
            if not isinstance(config, dict):
                logger.error(f"Service {service_name} configuration must be a dictionary")
                return False
    ...
    def _validate_service_config(self, service_name: str, config: Dict[str, Any], secrets: Dict[str, str]) -> bool:
    ...
    async def get_connection_string(self, service_name: str) -> Optional[str]:
        ...
        try:
            if service_name == "snowflake":
                ...
                if not all(key in service_config.secrets or key in service_config.config for key in required_keys):
                    logger.error(f"Missing required keys for Snowflake connection")
                    return None
                ...
                if not all(key in service_config.secrets or key in service_config.config for key in required_keys):
                    logger.error(f"Missing required keys for PostgreSQL connection")
                    return None
                ...
                logger.warning(f"No connection string template for service {service_name}")
                return None
        ...
        try:
            if service_name == "pinecone":
                ...
                client = weaviate.Client(
                    ...
                    auth_client_secret=weaviate.AuthApiKey(api_key=service_config.secrets["api_key"])
                )
                return client
            
            else:
                logger.warning(f"No API client implementation for service {service_name}")
                return None
    ...
    async def health_check(self, service_name: str) -> bool:
        ...
        try:
            ...
            if service_name in ["snowflake"]:
                ...
                # API health check
                base_url = service_config.config.get("base_url") or service_config.config.get("url")
                if not base_url:
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
                    ...
                    if "api_key" in service_config.secrets:
                        headers["Authorization"] = f"Bearer {service_config.secrets['api_key']}"
                    
                    async with session.get(f"{base_url}{health_endpoint}", headers=headers) as response:
