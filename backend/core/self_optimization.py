...
class SophiaSelfOptimizer:
        ...
        while self.is_running:
            try:
                ...
                for opportunity in opportunities:
                    validation = await self.constitutional_ai.validate_optimization(opportunity)
                    if validation["approved"]:
                        safe_optimizations.append(opportunity)
                    else:
                        logger.warning(f"Optimization rejected by constitutional AI: {opportunity['type']}")
    ...
    async def learn_from_optimization_results(self):
        ...
        successful_count = sum(1 for opt in recent_optimizations if opt["status"] == "completed")
        success_rate = successful_count / len(recent_optimizations) if recent_optimizations else 0
