...
class SnowflakeAbstraction(ABC):
    ...
    async def execute_batch(
        self,
        queries: List[Tuple[str, Optional[Union[List[Any], Dict[str, Any]]]]],
...
class SecureSnowflakeExecutor(SnowflakeAbstraction):
    ...
    async def execute_query(
        ...
        if self.query_validator and not await self.query_validator.is_safe(query, query_type):
            return QueryResult(
                success=False,
        ...
        async with self.pool.get_connection() as conn:
            ...
            try:
                ...
                while True:
                    ...
                    chunk_data = []
                    for row in rows:
                        chunk_data.append(dict(zip(columns, row)))
