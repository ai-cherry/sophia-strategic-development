...
class DataTransformationPipeline:
    ...
    async def process_hubspot_data(self, job: ProcessingJob) -> Dict[str, Any]:
        ...
        try:
            ...
            job.data_quality_score = await self._calculate_data_quality(transformed_data)
    ...
    async def _fetch_hubspot_data(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            deals = await self._fetch_paginated_data(
                session,
                f"{hubspot_config['base_url']}/crm/v3/objects/deals",
                headers
            )
    ...
    async def _transform_hubspot_data(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for contact in data.get('contacts', []):
            transformed_record = {
                ...
                'created_date': self._parse_hubspot_date(contact['properties'].get('createdate')),
                'last_modified_date': self._parse_hubspot_date(contact['properties'].get('lastmodifieddate')),
    ...
    async def _fetch_gong_data(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            calls_params = {
                ...
                'contentSelector': [
                    'brief', 'outline', 'highlights', 'keyPoints', 
                    'trackers', 'topics', 'pointsOfInterest'
                ]
            }
            ...
            for call in calls[:10]:  # Limit for demo
                transcript_url = f"{gong_config['base_url']}/v2/calls/{call['id']}/transcript"
                try:
                    ...
                    logger.warning(f"Failed to fetch transcript for call {call['id']}: {e}")
    ...
    async def process_file_upload(self, file_path: str, file_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _load_to_snowflake(self, data: List[Dict[str, Any]], table_name: str, schema: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                logger.info(f"Successfully loaded {nrows} records to {schema}.{table_name}")
    ...
    async def _create_table_if_not_exists(self, df: pd.DataFrame, table_name: str, schema: str):
    ...
    async def _fetch_paginated_data(self, session: aiohttp.ClientSession, url: str, headers: Dict[str, str], params: Dict[str, Any] = None) -> List[Dict[str, Any]]:
