...
class MLQueryClassifier:
    ...
    def classify_query(
        ...
        primary_engine, secondary_engines = self._select_engines(
            primary_type, query, context
        )
        ...
        estimated_time = self._estimate_execution_time(
            primary_engine, secondary_engines, parallel_execution
        )

        # Generate reasoning
        reasoning = self._generate_reasoning(
            primary_type, primary_engine, secondary_engines, confidence
        )
    ...
    def _classify_by_context(self, query: str, context: Dict[str, Any]) -> QueryType:
        ...
        if context.get("business_context") or any(
            word in query.lower()
            for word in ["sales", "revenue", "customer", "business"]
        ):
            return QueryType.HYBRID_WORKFLOW

        # Check for automation context
        if context.get("automation_context") or any(
            word in query.lower() for word in ["automate", "workflow", "process"]
        ):
            return QueryType.AGENT_ORCHESTRATION

        # Check for data context
        if context.get("data_context") or any(
            word in query.lower() for word in ["data", "database", "records"]
        ):
    ...
    def _estimate_execution_time(
        ...
        secondary_time = max(
            base_times.get(engine, 500) for engine in secondary_engines
        )
...
class HybridRAGRouter:
    ...
    async def route_query(
        ...
        try:
            ...
            logger.info(
                f"Routing decision: {decision.reasoning} (took {routing_time:.2f}ms)"
            )
    ...
    async def _execute_hybrid_query(
        ...
        primary_result = await self._execute_engine_query(
            decision.primary_engine, query, context
        )
    ...
    async def _execute_engine_query(
        ...
        try:
            if engine == "vector_search":
                ...
                confidence = result.get("performance", {}).get(
                    "successful_servers", 0
                ) / max(
                    1, result.get("performance", {}).get("total_servers_queried", 1)
                )

            elif engine == "agno_orchestration":
                result = await enhanced_agno_integration.process_ultra_fast_request(
                    query, "general_assistant", False
                )
    ...
    def _combine_hybrid_results(
        ...
        all_confidences = [primary_result.confidence] + [
            r.confidence for r in successful_secondary
        ]
        overall_confidence = (
            statistics.mean(all_confidences)
            if all_confidences
            else primary_result.confidence
        )
        ...
        return {
            ...
            + [r.engine for r in successful_secondary],
            "execution_summary": {
                "primary_time_ms": primary_result.execution_time_ms,
                "secondary_times_ms": {
                    r.engine: r.execution_time_ms for r in successful_secondary
                },
            },
        }
    ...
    def _update_routing_metrics(
        ...
        self.metrics["avg_routing_time_ms"] = (
            current_avg * (total_queries - 1) + routing_time_ms
        ) / total_queries
    ...
    def _update_performance_metrics(
        ...
        self.metrics["success_rate"] = (
            current_success_rate * (total_queries - 1) + (1 if success else 0)
        ) / total_queries
        ...
        engine_metrics["avg_execution_time_ms"] = (
            current_avg * (total_engine_queries - 1) + execution_time_ms
        ) / total_engine_queries
        ...
        if self.learning_enabled:
            ...
            if len(self.performance_history[query_signature]) > 100:
                self.performance_history[query_signature] = self.performance_history[
                    query_signature
                ][-100:]
    ...
    async def optimize_routing(self):
        ...
        for signature, history in self.performance_history.items():
            ...
            # Log insights
            logger.info(
                f"Pattern {signature}: {success_rate:.1%} success rate, {avg_time:.0f}ms avg time"
            )
