...
class EnhancedPulumiESC:
    ...
    def __init__(self, access_token: Optional[str] = None, organization: str = "ai-cherry"):
    ...
    def _get_cache_key(self, secret_name: str) -> str:
        ...
        return hashlib.sha256(f"{self.organization}:{self.environment}:{secret_name}".encode()).hexdigest()
    ...
    async def get_secret(self, secret_name: str, use_cache: bool = True) -> Optional[str]:
        ...
        try:
            # Get from Pulumi ESC
            endpoint = f"/preview/environments/{self.organization}/{self.environment}/values"
    ...
    def _extract_secret_value(self, values: Dict[str, Any], secret_name: str) -> Optional[str]:
    ...
    async def set_secret(self, secret_name: str, secret_value: str, tags: List[str] = None) -> bool:
        ...
        try:
            ...
            updated_yaml = self._update_yaml_with_secret(yaml_content, secret_name, secret_value)
    ...
    def _update_yaml_with_secret(self, yaml_content: str, secret_name: str, secret_value: str) -> str:
    ...
    async def rotate_secret(self, secret_name: str, new_value: str) -> bool:
        ...
        try:
            ...
            if old_value:
                ...
                await self.set_secret(backup_name, old_value, tags=["backup", "rotated"])
    ...
    async def sync_from_github_secrets(self, github_secrets: Dict[str, str]) -> Dict[str, bool]:
        """Sync secrets from GitHub to Pulumi ESC"""
        results = {}
        
        for secret_name, secret_value in github_secrets.items():
            try:
                success = await self.set_secret(secret_name, secret_value, tags=["github-sync"])
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sophia_env_exists = any(env.get("name") == self.environment for env in environments)
