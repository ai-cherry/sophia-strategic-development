...
class ConcurrentAgentProcessor:
    ...
    async def process_agents_concurrently(
        ...
        try:
            ...
            logger.info(
                f"✅ Processed {len(tasks)} agent tasks concurrently in {time.time() - start_time:.2f}s"
            )
            return results
    ...
    async def _execute_task_batch(self, tasks: List[AgentTask]) -> List[AgentResult]:
        ...
        concurrent_tasks = [self._execute_single_task(task) for task in tasks]
    ...
    async def _execute_single_task(self, task: AgentTask) -> AgentResult:
        ...
        async with self.semaphore:  # Control concurrent resource usage
            ...
            try:
                ...
                logger.error(
                    f"Task execution failed for {task.agent_id}.{task.task_type}: {e}"
                )
                return AgentResult(
                    agent_id=task.agent_id,
                    task_type=task.task_type,
    ...
    async def _execute_with_retries(self, agent: Any, task: AgentTask) -> Any:
        ...
        for attempt in range(task.retry_count + 1):
            try:
                ...
                result = await asyncio.wait_for(
                    self._call_agent_method(agent, task), timeout=task.timeout
                )
                return result
                ...
                last_error = str(e)
                logger.warning(
                    f"Attempt {attempt + 1} failed for {task.agent_id}.{task.task_type}: {e}"
                )
        ...
        raise Exception(
            f"Task failed after {task.retry_count + 1} attempts: {last_error}"
        )
    ...
    async def _call_agent_method(self, agent: Any, task: AgentTask) -> Any:
        ...
        if not hasattr(agent, method_name):
            raise AttributeError(
                f"Agent {type(agent).__name__} does not have method {method_name}"
            )
    ...
    def _resolve_dependencies(self, tasks: List[AgentTask]) -> List[List[AgentTask]]:
        ...
        while remaining_tasks:
            ...
            for task_key in list(remaining_tasks):
                ...
                if not task.dependencies or task.dependencies.isdisjoint(
                    remaining_tasks
                ):
            ...
            if not ready_tasks:
                ...
                logger.warning(
                    f"Circular or missing dependencies detected for tasks: {remaining_tasks}"
                )
