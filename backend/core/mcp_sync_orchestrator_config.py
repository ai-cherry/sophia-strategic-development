...
class MCPSyncOrchestratorConfig:
    ...
    def _get_server_definitions(self) -> Dict[str, MCPServerDefinition]:
        ...
        return {
            ...
            "snowflake_admin": MCPServerDefinition(
                ...
                dependencies=["ai_memory"],  # Depends on AI Memory for intelligent analysis
                enable_monitoring=True
            ),
    ...
    async def _configure_server_sync(self, server_def: MCPServerDefinition):
        ...
        self.active_servers[server_def.name] = {
            ...
            "last_sync": None,
            "next_sync": datetime.now() + timedelta(minutes=server_def.sync_interval_minutes),
            "sync_count": 0,
            "error_count": 0
        }
    
    def _convert_sync_priority(self, orchestrator_priority: SyncPriority) -> ServerSyncPriority:
    ...
    async def _execute_priority_sync(self, servers_to_sync: List[tuple]):
        ...
        for server_name, server_info in servers_to_sync:
            try:
                ...
                if await self._check_dependencies(server_info["definition"].dependencies):
                    # Execute sync
                    sync_result = await self._execute_server_sync(server_name, server_info)
                    ...
                    logger.warning(f"Skipping {server_name} sync due to unmet dependencies")
    ...
    async def _execute_server_sync(self, server_name: str, server_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _metrics_collection_loop(self):
        ...
        while self.sync_scheduler_running:
            try:
                ...
                total_syncs = sum(server["sync_count"] for server in self.active_servers.values())
                total_errors = sum(server["error_count"] for server in self.active_servers.values())
                
                success_rate = (total_syncs - total_errors) / total_syncs if total_syncs > 0 else 1.0
                ...
                logger.info(f"📊 Orchestrator metrics: {total_syncs} syncs, {success_rate:.2%} success rate")
        ...
        return {
            ...
            "server_status": {
                name: {
                    ...
                    "success_rate": (info["sync_count"] - info["error_count"]) / info["sync_count"] if info["sync_count"] > 0 else 1.0
                }
