...
class ConfigValidator:
    ...
    async def validate_critical_services(self) -> Dict[str, bool]:
        ...
        try:
            ...
            if self.config_data.get('snowflake_account') and self.config_data.get('snowflake_password'):
                validation_results['snowflake'] = await self._validate_snowflake()
            
            # Validate Gong API access
            if self.config_data.get('gong_access_key') and self.config_data.get('gong_client_secret'):
...
class AutoESCConfig:
    ...
    def _load_config(self) -> None:
        ...
        logger.debug("Loading ESC config", command=" ".join(cmd), stack=stack)
    ...
    def as_settings(self) -> Settings:
        ...
        try:
            ...
            logger.error("Settings validation failed", errors=exc.errors())
    ...
    def as_enhanced_settings(self) -> EnhancedSettings:
        ...
        if self._enhanced_settings is None or self._should_refresh_cache():
            ...
            try:
                ...
                logger.error("Enhanced settings validation failed", 
                           errors=exc.errors())
                # Return minimal settings to prevent total failure
                self._enhanced_settings = self._get_minimal_enhanced_settings()
                
            except Exception as exc:
                logger.exception("Unexpected error creating enhanced settings", 
                               error=str(exc))
                self._enhanced_settings = self._get_minimal_enhanced_settings()
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        cache_age = (datetime.now(timezone.utc) - self._last_loaded).total_seconds() if self._last_loaded else None
