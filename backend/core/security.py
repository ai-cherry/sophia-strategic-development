...
class SQLInjectionPatterns:
    ...
    TAUTOLOGY = re.compile(r'(OR\s+1\s*=\s*1|AND\s+1\s*=\s*1|OR\s+\'1\'\s*=\s*\'1\')', re.IGNORECASE)
    
    # Advanced patterns
    STACKED_QUERIES = re.compile(r';\s*(DELETE|UPDATE|INSERT|DROP|CREATE|ALTER|EXEC)', re.IGNORECASE)
    TIME_BASED = re.compile(r'(WAITFOR\s+DELAY|SLEEP\s*\(|BENCHMARK\s*\()', re.IGNORECASE)
    OUT_OF_BAND = re.compile(r'(xp_cmdshell|sp_executesql|OPENROWSET|OPENDATASOURCE)', re.IGNORECASE)
    
    # Snowflake specific
    SYSTEM_FUNCTIONS = re.compile(r'(SYSTEM\$|ACCOUNT_USAGE\.|INFORMATION_SCHEMA\.)', re.IGNORECASE)
    ...
    def extract_tables(parsed_query: sqlparse.sql.Statement) -> List[str]:
        ...
        def extract_from_tokens(tokens):
            ...
            while idx < len(tokens):
                ...
                if token.ttype is Keyword and token.value.upper() in ['FROM', 'JOIN', 'INTO']:
                    idx += 1
                    while idx < len(tokens):
...
class ParameterizedQueryBuilder:
    ...
    def build_insert(
        ...
        table_name = ParameterizedQueryBuilder._build_table_name(table, database, schema)
