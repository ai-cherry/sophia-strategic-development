...
class AlignedSnowflakeConfig:
    ...
    def _validate_setup(self):
        ...
        try:
            ...
            connection_info_path = "/home/ubuntu/sophia-project/snowflake_connection_info.json"
            if os.path.exists(connection_info_path):
                ...
                logger.info("ℹ️ Connection info file not found, using default configuration")
    ...
    def get_analytics_views(self) -> Dict[str, str]:
        ...
        return {
            'contact_analytics': self.get_table_reference('CONTACT_ANALYTICS', 'analytics'),
            'deal_pipeline_analytics': self.get_table_reference('DEAL_PIPELINE_ANALYTICS', 'analytics'),
    ...
    def get_monitoring_tables(self) -> Dict[str, str]:
        ...
        return {
            'pipeline_metrics': self.get_table_reference('PIPELINE_METRICS', 'monitoring'),
            'data_quality_metrics': self.get_table_reference('DATA_QUALITY_METRICS', 'monitoring')
        }
    ...
    def get_estuary_materialization_config(self) -> Dict:
        ...
        return {
            ...
            'bindings': [
                ...
                {
                    'source': 'sophia_ai/gong_calls',
                    'resource': {'table': 'GONG_CALLS'}
                },
                {
...
def get_snowflake_connection():
    ...
    import snowflake.connector
    return snowflake.connector.connect(**aligned_snowflake_config.get_connection_params())


def get_snowflake_connection_for_schema(schema_name: str):
    """Get Snowflake connection for specific schema"""
    import snowflake.connector
    return snowflake.connector.connect(**aligned_snowflake_config.get_connection_params_for_schema(schema_name))
