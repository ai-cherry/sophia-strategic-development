...
class SecureEnvironmentValidator:
    ...
    def validate_environment(self, env_vars: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        for rule_name, rule in self.validation_rules.items():
            ...
            if not validation_result["valid"]:
                ...
                if validation_result["invalid_format"]:
                    ...
                    results["errors"].append(f"Invalid format for {rule_name}: {validation_result['error']}")
                
                if validation_result["security_issue"]:
                    results["security_issues"].append(rule_name)
                    results["warnings"].append(f"Security issue with {rule_name}: {validation_result['error']}")
    ...
    def _validate_single_var(self, rule: ValidationRule, value: Optional[str]) -> Dict[str, Any]:
        ...
        if value:
            if rule.min_length and len(value) < rule.min_length:
                result["valid"] = False
                result["invalid_format"] = True
                result["error"] = f"Value too short (minimum {rule.min_length} characters)"
                return result
            
            if rule.max_length and len(value) > rule.max_length:
                result["valid"] = False
                result["invalid_format"] = True
                result["error"] = f"Value too long (maximum {rule.max_length} characters)"
                return result
        ...
        if rule.allowed_values and value:
            if value not in rule.allowed_values:
                result["valid"] = False
                result["invalid_format"] = True
                result["error"] = f"Value must be one of: {', '.join(rule.allowed_values)}"
                return result
        
        # Security checks for secrets
        if rule.secret_type and value:
            security_check = self._check_secret_security(rule.name, value, rule.secret_type)
    ...
    def _check_secret_security(self, name: str, value: str, secret_type: SecretType) -> Dict[str, Any]:
    ...
    def generate_secure_env_template(self) -> str:
        ...
        categories = {
            ...
            "Business Integrations": ["GONG_API_KEY", "GONG_API_SECRET", "HUBSPOT_API_KEY"],
            "Slack": ["SLACK_BOT_TOKEN", "SLACK_APP_TOKEN", "SLACK_SIGNING_SECRET"],
            "Cloud Services": ["LAMBDA_LABS_API_KEY", "VERCEL_ACCESS_TOKEN", "PULUMI_ACCESS_TOKEN"],
        ...
        for category, var_names in categories.items():
            ...
            for var_name in var_names:
                ...
                if rule:
                    ...
                    if rule.secret_type:
                        template_lines.append(f"{var_name}=your_secure_{var_name.lower()}_here")
                    else:
    ...
    def get_validation_summary(self) -> str:
        ...
        if summary["overall_status"] == "PASS":
            ...
            lines.append("❌ Environment validation failed. Please fix the issues above.")
