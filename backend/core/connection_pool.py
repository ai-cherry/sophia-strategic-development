...
class PoolConfig:
    ...
    role: str = field(default_factory=lambda: esc_config.get("snowflake_role", "ROLE_SOPHIA_AI_AGENT_SERVICE"))
    warehouse: str = field(default_factory=lambda: esc_config.get("snowflake_warehouse", "WH_SOPHIA_AGENT_QUERY"))
    database: str = field(default_factory=lambda: esc_config.get("snowflake_database", "SOPHIA_AI_DEV"))
    schema: str = field(default_factory=lambda: esc_config.get("snowflake_schema", "CORE"))
...
class ConnectionWrapper:
    ...
    def __init__(self, connection_info: ConnectionInfo, pool: 'SnowflakeConnectionPool'):
...
class SnowflakeConnectionPool:
    ...
    async def initialize(self):
        ...
        logger.info(f"Initializing Snowflake connection pool with min_size={self.config.min_size}")
        ...
        logger.info(f"Connection pool initialized with {self.metrics.current_size} connections")
    ...
    async def _validate_connection(self, connection_info: ConnectionInfo) -> bool:
        ...
        try:
            ...
            if age > timedelta(seconds=self.config.idle_timeout):
                logger.debug(f"Connection {connection_info.connection_id} exceeded idle timeout")
                return False
