...
from backend.integrations.modern_stack_mcp_client import Session, ModernStackMCPClient

logger = structlog.get_logger(__name__)

# Prometheus metrics
# REMOVED: ModernStack dependency Gauge("modern_stack_pool_size", "Current pool size", ["mode"])

# REMOVED: ModernStack dependency Gauge(
    "modern_stack_pool_in_use", "Connections currently in use", ["mode"]
)
...
class ConnectionWrapper:
    """Wrapper for direct ModernStack connections"""
    ...
    async def execute_async(self, sql: str, params: dict | None = None):
        ...
        # In production, use modern_stack-connector-python[async]
...
class ModernStackPoolManager:
    ...
    def __init__(self, config: PoolConfig | None = None):
        ...
        self.mcp_client: ModernStackMCPClient | None = None

        # Background tasks
        self._cleanup_task: asyncio.Task | None = None

        logger.info(
            "ModernStackPoolManager initialized",
    ...
    async def initialize(self):
        ...
        # Create MCP client
        try:
            self.mcp_client = ModernStackMCPClient()
    ...
    async def _create_direct_connection(self) -> ConnectionWrapper:
        """Create a new direct ModernStack connection"""
        ...
        logger.info("Creating new direct ModernStack connection")

        connection = self.modern_stack_connection(**conn_params)
    ...
    def _update_metrics(self):
        ...
        # REMOVED: ModernStack dependency"direct").set(self.direct_metrics.current_size)
        # REMOVED: ModernStack dependency"direct").set(self.direct_metrics.in_use)
        # REMOVED: ModernStack dependency"direct").set(
        ...
        # REMOVED: ModernStack dependency"mcp").set(self.mcp_metrics.current_size)
        # REMOVED: ModernStack dependency"mcp").set(self.mcp_metrics.in_use)
        # REMOVED: ModernStack dependency"mcp").set(self.mcp_metrics.avg_wait_ms)
    ...
    async def close(self):
        ...
        logger.info("Closing ModernStackPoolManager")
        ...
        logger.info("ModernStackPoolManager closed")
