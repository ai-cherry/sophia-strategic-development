...
class PerformanceMonitoringIntegration:
    ...
    async def track_service_health(self, 
                                 service_name: str, 
                                 response_time: float, 
                                 error_rate: float, 
                                 details: Dict[str, Any] = None):
        ...
        try:
            ...
            await self.track_performance(service_name, 'health_score', self._health_to_score(status))
    ...
    def _calculate_health_status(self, response_time: float, error_rate: float) -> str:
        ...
        if (response_time <= rt_thresholds['excellent'] and 
            error_rate <= er_thresholds['excellent']):
            return 'healthy'
        elif (response_time <= rt_thresholds['good'] and 
              error_rate <= er_thresholds['good']):
            return 'healthy'
        elif (response_time <= rt_thresholds['degraded'] and 
              error_rate <= er_thresholds['degraded']):
            return 'degraded'
        else:
            return 'unhealthy'
    ...
    async def _metrics_collector_task(self):
        ...
        while self.monitoring_active:
            try:
                ...
                await self.track_performance('system', 'memory_usage', memory.percent / 100.0)
                await self.track_performance('system', 'disk_usage', disk.percent / 100.0)
                ...
                if self.redis_client:
                    try:
                        ...
                        await self.track_performance('redis', 'memory_usage', redis_memory)
                        ...
                        await self.track_performance('redis', 'cache_hit_ratio', hit_ratio)
            ...
            try:
                ...
                for service in services_to_check:
                    try:
                        ...
                        await self.track_service_health(service, response_time, error_rate)
                        
                    except Exception as e:
                        # Service is unhealthy
                        await self.track_service_health(service, 5.0, 1.0, {'error': str(e)})
    ...
    async def _flush_metrics(self):
        ...
        try:
            ...
            for metric in self.metrics_buffer:
                ...
                pipe.set(key, json.dumps(metric.to_dict()), ex=86400 * self.config['metrics_retention_days'])
                
                # Update latest value
                latest_key = f"sophia:metrics:latest:{metric.service_name}:{metric.metric_name}"
    ...
    async def _send_alert(self, severity: str, metric: PerformanceMetric):
        ...
        try:
            ...
            if self.redis_client:
                ...
                await self.redis_client.set(alert_key, json.dumps(alert_data), ex=86400 * 7)  # Keep for 7 days
    ...
    async def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get service health
            service_health_data = await self.redis_client.hgetall('sophia:monitoring:service_health')
...
def track_performance(metric_name: str = None, service_name: str = None):
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            start_time = time.time()
            service = service_name or getattr(args[0], '__class__', {}).get('__name__', 'unknown') if args else 'unknown'
            metric = metric_name or func.__name__
            
            try:
                result = await func(*args, **kwargs)
                execution_time = time.time() - start_time
                await performance_monitoring.track_performance(service, f"{metric}_time", execution_time)
                await performance_monitoring.track_performance(service, f"{metric}_success", 1)
                return result
            except Exception as e:
                execution_time = time.time() - start_time
                await performance_monitoring.track_performance(service, f"{metric}_time", execution_time)
                await performance_monitoring.track_performance(service, f"{metric}_error", 1)
                raise
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                asyncio.create_task(performance_monitoring.track_performance(service, f"{metric}_time", execution_time))
                asyncio.create_task(performance_monitoring.track_performance(service, f"{metric}_success", 1))
                return result
            except Exception as e:
                execution_time = time.time() - start_time
                asyncio.create_task(performance_monitoring.track_performance(service, f"{metric}_time", execution_time))
                asyncio.create_task(performance_monitoring.track_performance(service, f"{metric}_error", 1))
                raise
...
async def track_metric(service_name: str, metric_name: str, value: float, tags: Dict[str, str] = None):
    """Track a performance metric"""
    await performance_monitoring.track_performance(service_name, metric_name, value, tags)
