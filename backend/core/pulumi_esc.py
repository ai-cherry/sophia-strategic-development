...
class ESCClient:
    ...
    async def get_configuration(self, key: str) -> Optional[Any]:
        ...
        try:
            endpoint = f"/api/environments/{self.organization}/{self.environment}/values"
    ...
    def _get_nested_value(self, data: Dict[str, Any], key: str) -> Optional[Any]:
        ...
        if len(parts) == 2:
            ...
            if "secrets" in data and isinstance(data["secrets"], dict):
                ...
                if service in data["secrets"] and isinstance(data["secrets"][service], dict):
            ...
            if "config" in data and isinstance(data["config"], dict):
                ...
                if service in data["config"] and isinstance(data["config"][service], dict):
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            if self.environment in environments:
                logger.info(f"Successfully validated connection to Pulumi ESC environment: {self.environment}")
                return True
            else:
                logger.warning(f"Environment {self.environment} not found in organization {self.organization}")
                return False
        except Exception as e:
