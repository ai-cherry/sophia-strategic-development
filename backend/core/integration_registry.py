...
class IntegrationRegistry:
    ...
    async def initialize(self) -> bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.info(f"Integration registry initialized with {len(self.integrations)} integrations")
                return True
    ...
    def register(self, 
        ...
        if not issubclass(integration_class, Integration):
            raise IntegrationError(f"Integration class must inherit from Integration base class")
    ...
    async def get_integration(self, service_name: str) -> Optional[Integration]:
        ...
        try:
            ...
            if await instance.initialize():
                ...
                logger.info(f"Created and initialized integration instance for {service_name}")
                return instance
            else:
                logger.error(f"Failed to initialize integration instance for {service_name}")
                return None
        except Exception as e:
            logger.error(f"Failed to create integration instance for {service_name}: {e}")
            return None
            ...
            if metadata:
                for dependency in metadata.dependencies:
                    try:
                        ...
                        logger.error(f"Missing dependency {dependency} for {service_name}")
                        return False
            
            # Try to create instance
            integration = await self.get_integration(service_name)
            if not integration:
                logger.error(f"Failed to create integration instance for {service_name}")
                return False
        ...
        try:
            ...
            for service_name in services:
                ...
                try:
                    ...
                    logger.warning(f"Error auto-discovering integration for {service_name}: {e}")
        except Exception as e:
    ...
    async def _start_health_monitoring(self) -> None:
        ...
        for service_name in self.integrations:
            ...
            task = asyncio.create_task(self._health_monitor_loop(service_name, interval))
    ...
    async def _create_client(self, config: ServiceConfig) -> Optional[Any]:
        ...
        try:
            ...
            client = weaviate.Client(
                ...
                auth_client_secret=weaviate.AuthApiKey(api_key=config.get_secret("api_key"))
            )
...
class EstuaryIntegration(Integration):
    ...
    async def _perform_health_check(self) -> bool:
        ...
        try:
            ...
            return self.client.initialized if hasattr(self.client, 'initialized') else True
        except Exception as e:
...
def register_builtin_integrations():
    ...
    integrations = [
        ("snowflake", SnowflakeIntegration, IntegrationMetadata(
            name="snowflake",
            version="1.0.0",
        ...
        ("pinecone", PineconeIntegration, IntegrationMetadata(
            name="pinecone",
            version="1.0.0",
        ...
        ("weaviate", WeaviateIntegration, IntegrationMetadata(
            name="weaviate",
            version="1.0.0",
        ...
        ("openai", OpenAIIntegration, IntegrationMetadata(
            name="openai",
            version="1.0.0",
        ...
        ("anthropic", AnthropicIntegration, IntegrationMetadata(
            name="anthropic",
            version="1.0.0",
        ...
        ("estuary", EstuaryIntegration, IntegrationMetadata(
            name="estuary",
            version="1.0.0",
        ...
        ("airbyte", AirbyteIntegration, IntegrationMetadata(
            name="airbyte",
            version="1.0.0",
