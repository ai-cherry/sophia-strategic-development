...
class IntegrationConfig:
    ...
    async def _initialize_fallback_mode(self) -> None:
        ...
        logger.warning("Initializing integration configuration manager in fallback mode")
        self.esc_client = None
        await self._load_service_registry()
        self.initialized = True
    ...
    async def get_connection_string(self, service_name: str) -> Optional[str]:
        ...
        try:
            if service_name == "snowflake":
                ...
                if not all(service_config.has_secret(key) or service_config.has_config(key) for key in required_keys):
            ...
            logger.error(f"Failed to generate connection string for {service_name}: {e}")
            return None
    ...
    async def validate_service(self, service_name: str) -> bool:
        ...
        for package in required_packages:
            try:
                ...
                logger.error(f"Required package {package} not installed for {service_name}")
                return False
...
class Integration(ABC, Generic[ConfigType, ClientType]):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Get service configuration
            self.service_config = await self.config_manager.get_service_config(self.service_name)
