    ...
    async def _validate_anthropic(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                )  # 400 means auth passed but method wrong
...
class EnhancedESCConfig:
    ...
    def _fallback_to_env_vars(self) -> None:
        ...
        logger.info("Fallback configuration loaded", key_count=len(fallback_config))
    ...
    def get_typed_config(self) -> SophiaConfig:
        ...
        if self._settings is None or self._should_refresh_cache():
            ...
            try:
                ...
                logger.exception(
                    "Unexpected error creating typed config", error=str(exc)
                )
                self._settings = self._get_minimal_config()
    ...
    def _get_minimal_config(self) -> SophiaConfig:
        ...
        minimal_data = {
            ...
            "snowflake": {"account": "dummy", "password": "dummy"},
            "redis": {"password": "dummy"},
            "agents": {"auth_token": "dummy", "communication_secret": "dummy"},
            "gong": {
                "access_key": "dummy",
                "secret_key": "dummy",
