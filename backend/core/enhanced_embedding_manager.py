...
class EnhancedEmbeddingManager:
    ...
    async def generate_multimodal_embedding(
        ...
        try:
            # Text embedding
            if text:
                text_emb, _ = await self.generate_text_embedding(
            ...
            metadata = EmbeddingMetadata(
                embedding_id=self._generate_embedding_id(
                    f"{text or ''}{image_path or ''}{audio_path or ''}", "multimodal"
                ),
    ...
    def _combine_embeddings(
        self, embeddings: List[List[float]], content_types: List[str]
    ) -> List[float]:
    ...
    def _cache_embedding(
        self, cache_key: str, result: Tuple[List[float], EmbeddingMetadata]
    ):
