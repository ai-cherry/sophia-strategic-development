...
class MemoryCache(CacheBackend):
    ...
    async def get(self, key: str) -> Optional[Any]:
        async with self._lock:
            ...
            # Check TTL
            if entry.ttl_seconds and datetime.now() > entry.created_at + timedelta(
...
class RedisCache(CacheBackend):
    ...
    def __init__(
        self, redis_url: str = "redis://localhost:6379", prefix: str = "sophia:"
    ):
...
class HierarchicalCache:
    ...
    def _calculate_adaptive_ttl(self, key: str) -> int:
        ...
        if l1_entry and hasattr(l1_entry, "access_count"):
            ...
            adaptive_ttl = int(
                base_ttl * (1 + access_factor * self.ttl_adjustment_factor)
            )
            return adaptive_ttl
    ...
    def _update_access_time(self, start_time: float):
        ...
        if self.metrics.total_requests > 1:
            self.metrics.avg_access_time_ms = (
                self.metrics.avg_access_time_ms * (self.metrics.total_requests - 1)
                + access_time
        ...
        while True:
            try:
                ...
                self.metrics.total_size_bytes = sum(
                    entry.size_bytes for entry in self.l1_cache.cache.values()
                )
