...
class VectorIntegration:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if pinecone_config:
                ...
                self.pinecone_environment = pinecone_config.config.get(
                    "environment", self.pinecone_environment
                )
                self.pinecone_index_name = pinecone_config.config.get(
            ...
            if weaviate_config:
                ...
                self.weaviate_api_key = weaviate_config.secrets.get(
            ...
            if vector_config:
                self.embedding_model_name = vector_config.config.get(
                    "embedding_model", self.embedding_model_name
                )
                self.vector_dimension = vector_config.config.get(
                    "vector_dimension", self.vector_dimension
                )
                self.batch_size = vector_config.config.get(
                    "batch_size", self.batch_size
                )
                self.similarity_threshold = vector_config.config.get(
    ...
    async def setup_weaviate(self):
        ...
        try:
            if not self.weaviate_url or not self.weaviate_api_key:
                self.logger.warning(
                    "Weaviate URL or API key not found, skipping Weaviate initialization"
                )
                return
    ...
    async def index_content_pinecone(
        ...
        try:
            ...
            self.pinecone_index.upsert(
                vectors=[(content_id, embedding, safe_metadata)], namespace=namespace
            )

            self.logger.info(
                f"Indexed content {content_id} in Pinecone namespace '{namespace}'"
            )
            return True
    ...
    async def index_content_weaviate(
        ...
        try:
            ...
            self.weaviate_client.data_object.create(
                data_object=data_object, class_name=class_name, uuid=content_id
            )

            self.logger.info(
                f"Indexed content {content_id} in Weaviate class '{class_name}'"
            )
            return True
    ...
    async def index_content(
        ...
        results = {
            "pinecone": await self.index_content_pinecone(
                content_id, text, metadata, namespace
            ),
            "weaviate": await self.index_content_weaviate(
                content_id, text, metadata, namespace
            ),
        }
    ...
    async def batch_index_content(
        self, content_batch: List[Dict[str, Any]], namespace: str = "default"
    ) -> Dict[str, Any]:
        ...
        for item in content_batch:
            try:
                ...
                index_results = await self.index_content(
                    content_id, text, metadata, namespace
                )
        ...
        self.logger.info(
            f"Batch indexing completed: {results['successful']}/{results['total']} successful"
        )
        return results
        ...
        try:
            ...
            search_results = (
                self.weaviate_client.query.get(
                    class_name,
                    ["content", "title", "category", "source", "timestamp", "metadata"],
                )
            ...
            results = []
            if (
                "data" in results_data
                and "Get" in results_data["data"]
                and class_name in results_data["data"]["Get"]
            ):
    ...
    async def hybrid_search(
        ...
        weaviate_results = await self.search_weaviate(
            query, top_k, category_filter, namespace=namespace
        )
        ...
        self.logger.info(
            f"Hybrid search completed in {response_time:.3f}s, returned {len(unique_results)} results"
        )
    ...
    async def delete_content(self, content_id: str, namespace: str = "default"):
        ...
        if self.pinecone_index:
            try:
                ...
                self.logger.info(
                    f"Deleted {content_id} from Pinecone namespace '{namespace}'"
                )
            except Exception as e:
