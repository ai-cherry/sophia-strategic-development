...
class ArchitectureParser:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.decorator_list:
                    for decorator in node.decorator_list:
                        if isinstance(decorator, ast.Call) and hasattr(decorator.func, 'value') and hasattr(decorator.func.value, 'id') and decorator.func.value.id in ['router', 'app']:
                            http_method = decorator.func.attr.upper() # GET, POST, etc.
                            path = decorator.args[0].s if decorator.args and isinstance(decorator.args[0], ast.Constant) else "unknown"
    ...
    def parse_db_schema(file_content: str, file_path: str) -> List[Dict[str, Any]]:
        ...
        for match in table_pattern.finditer(file_content):
            ...
            columns = [f"{col_match.group(1)} ({col_match.group(2)})" for col_match in column_pattern.finditer(columns_str)]
