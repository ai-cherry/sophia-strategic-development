...
from backend.knowledge_base.metadata_store import MetadataStore # Can be adapted for code
...
class CodebaseIngestionPipeline:
    ...
    def __init__(self, vector_store: VectorStore, metadata_store: MetadataStore, project_root: Path):
        self.vector_store = vector_store
        self.metadata_store = metadata_store # We can use the same metadata store for now
                ...
                try:
                    ...
                    if p.suffix == '.py':
                        ...
                        if mcp_dir in p.parents:
                            all_items.extend(self.parser.parse_mcp_tools(content, str(p)))
                        if routes_dir in p.parents:
                            all_items.extend(self.parser.parse_fastapi_routes(content, str(p)))
                            
                    elif p.suffix == '.sql':
                        if db_dir in p.parents:
                            all_items.extend(self.parser.parse_db_schema(content, str(p)))

                except Exception as e:
                    logger.warning(f"Could not read or parse file {p}: {e}")
                    
        logger.info(f"Codebase scan complete. Found {len(all_items)} architectural items.")
        return all_items
    ...
    def _create_embedding_content(self, item: Dict[str, Any]) -> str:
        ...
        if item_type == "python_function":
            ...
            return f"MCP Tool: {item.get('name')}\nDescription: {item.get('description')}"
...
async def main():
    ...
    pipeline = CodebaseIngestionPipeline(codebase_vector_store, codebase_metadata_store, project_root)
