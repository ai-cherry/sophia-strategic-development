...
class AgentSettings(BaseSettings):
    ...
    enable_slack_notifications: bool = Field(
        default=True, env="SOPHIA_ENABLE_SLACK_NOTIFICATIONS"
    )
...
class MonitoringSettings(BaseSettings):
    ...
    metrics_retention_hours: int = Field(
        default=168, env="SOPHIA_METRICS_RETENTION_HOURS"
    )
...
class Settings(BaseSettings):
    ...
    temp_dir: Path = Field(
        default_factory=lambda: Path(__file__).parent.parent.parent / "temp"
    )
    ...
    def validate_production_settings(self):
        ...
        if not self.api_keys.has_ai_provider:
            warnings.append(
                "No AI provider configured (OpenAI or Anthropic). AI features will be disabled."
            )
    ...
    def get_enabled_features(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "call_analysis": self.agents.enable_call_analysis
            and self.api_keys.has_gong,
            "crm_sync": self.agents.enable_crm_sync and self.api_keys.has_hubspot,
            "notifications": self.agents.enable_slack_notifications
            and self.api_keys.has_slack,
