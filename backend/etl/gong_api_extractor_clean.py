...
class GongAPIExtractor:
    ...
    async def extract_calls(self, days_back: int = 7) -> List[Dict]:
        ...
        params = {
            ...
            "contentSelector": [
                "parties", "content", "context", "interaction",
                "pointsOfInterest", "questions", "trackers", "topics"
            ]
        }
...
class PostgreSQLStaging:
    ...
    async def store_crm_entities(self, entities: List[Dict]) -> int:
        ...
        try:
            records = [
                (entity.get("id", ""), json.dumps(entity))
    ...
    async def update_pipeline_status(self, pipeline_id: str, status: str, records_processed: int = 0, error_message: str = None):
        ...
        try:
            if status == "completed":
                ...
                """, pipeline_id, status, records_processed)
            else:
                await conn.execute("""
                ...
                """, pipeline_id, status, records_processed, error_message)
