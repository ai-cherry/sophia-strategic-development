...
- gong_data_quality_module_core.py - Core functionality
- gong_data_quality_module_utils.py - Utility functions
- gong_data_quality_module_models.py - Data models
- gong_data_quality_module_handlers.py - Request handlers
...
class DataQualityRepository(Protocol):
    ...
    async def store_quality_metrics(
        self, record_id: str, metrics: QualityMetrics
    ) -> None:
...
class SnowflakeDataQualityRepository:
    ...
    async def store_quality_metrics(
        ...
        try:
            ...
            logger.info(
                f"📊 Storing quality metrics for {record_id}: {metrics.overall_score:.2f}"
            )
...
class GongDataQualityModule:
    ...
    def __init__(
        self, event_publisher: EventPublisher, repository: DataQualityRepository
    ):
    ...
    async def process_gong_data(
        self, raw_data: dict[str, Any], record_type: str
    ) -> GongDataRecord:
    ...
    async def _perform_quality_checks(
        self, record: GongDataRecord, correlation_id: str
    ) -> QualityMetrics:
    ...
    def _check_completeness(self, record: GongDataRecord) -> float:
        ...
        required_fields = self.quality_rules["required_fields"].get(
            record.record_type, []
        )
        ...
        present_fields = sum(
            ...
            if field in record.raw_data and record.raw_data[field] is not None
        )
