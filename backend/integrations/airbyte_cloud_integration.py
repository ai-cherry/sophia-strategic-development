...
class AirbyteCloudManager:
    ...
    async def create_gong_source(
        ...
        try:
            response = self.session.post(
                f"{self.credentials.base_url}/sources", json=source_config
            )
            response.raise_for_status()
    ...
    async def create_postgres_destination(
        ...
        try:
            response = self.session.post(
                f"{self.credentials.base_url}/destinations", json=destination_config
            )
            response.raise_for_status()
            dest_data = response.json()
            logger.info(
                f"Created PostgreSQL destination: {dest_data.get('destinationId')}"
            )
    ...
    async def create_connection(
        ...
        try:
            response = self.session.post(
                f"{self.credentials.base_url}/connections", json=connection_config
            )
            response.raise_for_status()
...
class DataDictionaryManager:
    ...
    def load_data_dictionary(
        ...
        try:.
            ...
            logger.info(
                f"Loaded data dictionary with {len(self.data_dictionary)} fields"
            )
    ...
    def add_field_definition(self, field_name: str, definition: Dict[str, Any]):
        """Add or update field definition."""self.data_dictionary[field_name] = {.
            ...
            "apartment_industry_context": definition.get(
                "apartment_industry_context", ""
            ),
...
async def setup_pay_ready_data_pipeline():
    ...
    if not gong_source_id:
        gong_source_id = await airbyte.create_gong_source(
            workspace_id, gong_credentials
        )
    ...
    for dest in destinations:
        if (
            "sophia" in dest.get("name", "").lower()
            or "postgres" in dest.get("name", "").lower()
        ):
            postgres_dest_id = dest["destinationId"]
            break

    if not postgres_dest_id:
        postgres_dest_id = await airbyte.create_postgres_destination(
            workspace_id, db_config
        )
