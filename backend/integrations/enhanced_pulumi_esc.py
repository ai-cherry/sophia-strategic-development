...
class EnhancedSophiaESCManager:
    ...
    def create_environment(
        ...
        """Create or update the Pulumi ESC environment."""try:.
            ...
            logger.info(
                f"Successfully created/updated ESC environment: {self.environment_path}"
            )
    ...
    def generate_env_file(self, output_path: str = ".env") -> bool:
        """Generate a .env file from ESC environment values."""try:.
            ...
            with open(output_path, "w") as f:
                ...
                self._write_env_section(
                    f,
                    "Application Configuration",
                    {
                        "SOPHIA_ENV": values.get("app", {}).get(
                ...
                self._write_env_section(
                    ...
                    "LLM Gateway Configuration",
                    {
                        ...
                        "OPENROUTER_API_KEY": llm_config.get("openrouter", {}).get(
                            "api_key"
                        ),
                    },
                )
                ...
                vector_env_vars = {
                    "PINECONE_API_KEY": vector_config.get("pinecone", {}).get(
                        "api_key"
                    ),
                    "WEAVIATE_URL": vector_config.get("weaviate", {}).get("url"),
                    "WEAVIATE_GRPC_URL": vector_config.get("weaviate", {}).get(
                        "grpc_url"
                    ),
                    "WEAVIATE_API_KEY": vector_config.get("weaviate", {}).get(
                        "api_key"
                    ),
                }
    ...
    def _write_env_section(
        self, file_handle, section_name: str, env_vars: Dict[str, Any]
    ):
        """Write a section of environment variables to the file."""file_handle.write(f"\n# {section_name}\n").
    ...
    def _write_remaining_sections(self, file_handle, values: Dict[str, Any]):
        ...
        self._write_env_section(
            file_handle, "Monitoring & Analytics", monitoring_env_vars
        )
        ...
        self._write_env_section(
            file_handle, "Infrastructure & Deployment", infra_env_vars
        )
            ...
            for key, value in section_config.items():
                ...
                if isinstance(value, dict):
                    ...
                    for nested_key, nested_value in value.items():
                        nested_env_key = (
                            f"{env_key}_{nested_key.upper()}"
                            if section_key != "additional_tools"
                            else env_key
                        )
    ...
    def validate_environment(self) -> Dict[str, bool]:
        ...
        try:
            ...
            for section in required_sections:
                validation_results[f"section_{section}"] = section in values and bool(
                    values[section]
                )
            ...
            for secret_path in critical_secrets:
                validation_results[f"secret_{secret_path}"] = (
                    self.get_secret(secret_path) is not None
                )
    ...
    def run_with_environment(
        ...
        """Run a command with ESC environment variables injected."""try:.
            ...
            result = subprocess.run(
                esc_command, capture_output=True, text=True, check=True, cwd=working_dir
            )

            logger.info("Command executed successfully with ESC environment")
            return result
