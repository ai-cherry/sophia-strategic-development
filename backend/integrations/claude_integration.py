...
class ClaudeIntegration:
    ...
    async def _get_claude_config(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not get Claude configuration from Pulumi ESC: {e}")
            ...
            env_config = {
                ...
                "api_base_url": os.getenv("CLAUDE_API_BASE_URL", "https://api.anthropic.com/v1"),
            ...
            if env_config["api_key"]:
                ...
                logger.error("Claude API key not found in Pulumi ESC or environment variables")
                return None
    ...
    async def send_message(self, message: str, 
                          ...
                          system_prompt: Optional[str] = None) -> Optional[ClaudeMessage]:
    ...
    async def generate_code(self, prompt: str, 
                           language: str = "python",
                           context: Optional[str] = None) -> Optional[ClaudeCodeGeneration]:
...
[brief explanation of the code]"""
            ...
            if response:
                ...
                if code_start != -1 and code_end != -1:
                    generated_code = content[code_start + len(f"```{language}"):code_end].strip()
    ...
    async def analyze_code(self, code: str, 
                          language: str = "python",
                          analysis_type: str = "review") -> Optional[ClaudeMessage]:
    ...
    async def refactor_code(self, code: str, 
                           language: str = "python",
                           refactor_goal: str = "improve readability") -> Optional[ClaudeCodeGeneration]:
...
[explanation of changes made and improvements]"""
            ...
            if response:
                ...
                if code_start != -1 and code_end != -1:
                    refactored_code = content[code_start + len(f"```{language}"):code_end].strip()
    ...
    async def generate_documentation(self, code: str, 
                                   language: str = "python",
                                   doc_type: str = "api") -> Optional[ClaudeMessage]:
    ...
    async def generate_tests(self, code: str, 
                           language: str = "python",
                           test_framework: str = "pytest") -> Optional[ClaudeCodeGeneration]:
...
[explanation of test strategy and coverage]"""
            ...
            if response:
                ...
                if code_start != -1 and code_end != -1:
                    test_code = content[code_start + len(f"```{language}"):code_end].strip()
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._config:
                ...
                health_status["max_tokens"] = self._config.get("max_tokens", self.max_tokens)
...
async def generate_code(prompt: str, language: str = "python", **kwargs) -> Optional[ClaudeCodeGeneration]:
    """Generate code using Claude"""
    return await claude_integration.generate_code(prompt, language, **kwargs)

async def analyze_code(code: str, language: str = "python", analysis_type: str = "review") -> Optional[ClaudeMessage]:
    """Analyze code using Claude"""
    return await claude_integration.analyze_code(code, language, analysis_type)

async def refactor_code(code: str, language: str = "python", refactor_goal: str = "improve readability") -> Optional[ClaudeCodeGeneration]:
    """Refactor code using Claude"""
    return await claude_integration.refactor_code(code, language, refactor_goal)

async def generate_documentation(code: str, language: str = "python", doc_type: str = "api") -> Optional[ClaudeMessage]:
    """Generate documentation for code"""
    return await claude_integration.generate_documentation(code, language, doc_type)

async def generate_tests(code: str, language: str = "python", test_framework: str = "pytest") -> Optional[ClaudeCodeGeneration]:
