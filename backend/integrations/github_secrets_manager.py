...
class GitHubSecretsManager:
    ...
    def create_organization_secret(
        ...
        """Create or update an organization secret."""try:.
            ...
            logger.info(
                f"Successfully created/updated organization secret: {secret_name}"
            )
            return True
    ...
    def audit_secrets_coverage(self) -> Dict[str, Any]:
        ...
        matched_secrets = len(expected_secrets) - len(
            coverage_report["missing_secrets"]
        )
        coverage_report["coverage_percentage"] = (
            matched_secrets / len(expected_secrets)
        ) * 100
