...
class EnhancedGongIntegration:
    ...
    def __init__(self):
        try:
            ...
            if gong_api_key and gong_api_key != "None" and len(gong_api_key) > 10:
                ...
                try:
                    from backend.integrations.gong_api_client_enhanced import (
                        EnhancedGongAPIClient,
                    )
                    ...
                    logger.warning(
                        "⚠️ EnhancedGongAPIClient not found - running in mock mode"
                    )
                    self.gong_client = None
                except Exception as e:
                    logger.warning(
                        f"⚠️ Gong client initialization failed: {e} - running in mock mode"
                    )
                    self.gong_client = None
            else:
    ...
    async def get_customer_interaction_timeline(
        self, customer_id: str, days_back: int = 90
    ) -> CustomerTimeline:
    ...
    async def analyze_customer_expansion_potential(
        ...
        try:
            ...
            timeline = await self.get_customer_interaction_timeline(
                customer_id, days_back=180
            )
            ...
            readiness_score = await self._calculate_expansion_readiness(
            ...
            return {
                ...
                "timeline_summary": {
                    ...
                    "sentiment_trend": timeline.overall_sentiment_trend[
                        -30:
                    ],  # Last 30 data points
    ...
    async def assess_churn_risk(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommended_actions": self._prioritize_retention_actions(
                    retention_strategy
                ),
                "timeline_insights": {
                    "interaction_frequency_trend": await self._analyze_interaction_frequency(
                        timeline
                    ),
                    "sentiment_decline": await self._analyze_sentiment_decline(
                        timeline
                    ),
    ...
    async def analyze_relationship_health(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            timeline = await self.get_customer_interaction_timeline(
                customer_id, days_back=365
            )

            # Health score components
            health_components = {
                "communication_frequency": await self._score_communication_frequency(
                    timeline
                ),
                ...
                "stakeholder_involvement": await self._score_stakeholder_involvement(
                    timeline
                ),
            }
    ...
    async def _get_customer_meetings(
        ...
        try:
            ...
            for meeting in meetings_data:
                interaction = await self._analyze_meeting_interaction(
                    meeting, customer_id
                )
                interactions.append(interaction)
    ...
    async def _get_customer_calendar_events(
        ...
        try:
            ...
            for event in calendar_data:
                interaction = await self._analyze_calendar_interaction(
                    event, customer_id
                )
                interactions.append(interaction)
    ...
    async def _analyze_call_interaction(
        ...
        try:
            ...
            return CustomerInteraction(
                ...
                sentiment_label=self._score_to_sentiment_label(
                    cortex_analysis.get("sentiment_score", 0.0)
                ),
            ...
            return self._get_fallback_interaction(
                call_data, customer_id, InteractionType.CALL
            )
    ...
    async def _analyze_email_interaction(
        ...
        try:
            ...
            return CustomerInteraction(
                ...
                timestamp=datetime.fromisoformat(
                    email_data.get("sent_at", datetime.now().isoformat())
                ),
            ...
            return self._get_fallback_interaction(
                email_data, customer_id, InteractionType.EMAIL
            )
    ...
    async def _analyze_meeting_interaction(
        ...
        try:
            ...
            return CustomerInteraction(
                ...
                timestamp=datetime.fromisoformat(
                    meeting_data.get("start_time", datetime.now().isoformat())
                ),
            ...
            return self._get_fallback_interaction(
                meeting_data, customer_id, InteractionType.MEETING
            )
    ...
    async def _analyze_customer_timeline(
        ...
        try:
            ...
            relationship_health = await self._calculate_relationship_health_score(
                interactions
            )

            # Calculate expansion readiness
            expansion_readiness = await self._calculate_expansion_readiness_score(
                interactions
            )
    ...
    async def _calculate_churn_risk_score(
        ...
        recent_sentiment = [
            i.sentiment_score for i in interactions[:5]
        ]  # Last 5 interactions
        avg_recent_sentiment = (
            sum(recent_sentiment) / len(recent_sentiment) if recent_sentiment else 0
        )
