...
from ..mcp.sophia_mcp_server import server as mcp_server
...
class SophiaSlackBot:
    ...
    async def _process_sophia_command(self, command: Dict[str, Any], client: AsyncWebClient):
    ...
    async def _handle_dashboard_command(self, command: Dict[str, Any], client: AsyncWebClient):
        """Handle dedicated dashboard command"""
        await self._handle_dashboard_request(command["channel_id"], client, command["user_id"])
    
    async def _handle_query_command(self, command: Dict[str, Any], client: AsyncWebClient):
        """Handle dedicated query command"""
        query = command.get("text", "").strip()
        await self._handle_natural_language_query(command["channel_id"], client, query, command["user_id"])
    
    async def _handle_search_command(self, command: Dict[str, Any], client: AsyncWebClient):
        """Handle dedicated search command"""
        search_params = command.get("text", "").strip()
        await self._handle_conversation_search(command["channel_id"], client, search_params, command["user_id"])
    
    async def _handle_health_command(self, command: Dict[str, Any], client: AsyncWebClient):
        """Handle dedicated health command"""
        await self._handle_system_health(command["channel_id"], client, command["user_id"])
    
    async def _handle_deploy_command(self, command: Dict[str, Any], client: AsyncWebClient):
        """Handle dedicated deploy command"""
        deploy_params = command.get("text", "").strip()
        await self._handle_deployment_request(command["channel_id"], client, deploy_params, command["user_id"])
    
    async def _handle_dashboard_request(self, channel_id: str, client: AsyncWebClient, user_id: str):
        """Handle dashboard statistics request"""
        try:
            # Fetch dashboard stats from admin API
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.admin_api_base}/dashboard/stats") as response:
    ...
    async def _handle_natural_language_query(self, channel_id: str, client: AsyncWebClient, query: str, user_id: str):
    ...
    async def _handle_conversation_search(self, channel_id: str, client: AsyncWebClient, search_params: str, user_id: str):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.admin_api_base}/conversations/search", params=params) as response:
                    if response.status == 200:
                        results = await response.json()
                        blocks = self._create_search_result_blocks(results, search_params)
    ...
    async def _handle_system_health(self, channel_id: str, client: AsyncWebClient, user_id: str):
    ...
    async def _handle_deployment_request(self, channel_id: str, client: AsyncWebClient, deploy_params: str, user_id: str):
    ...
    async def _handle_integration_stats(self, channel_id: str, client: AsyncWebClient, user_id: str):
    ...
    async def _handle_direct_message(self, event: Dict[str, Any], client: AsyncWebClient):
    ...
    async def _process_query_via_api(self, query: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.admin_api_base}/natural-query",
                json={"query": query}
            ) as response:
                if response.status == 200:
    ...
    def _create_query_result_blocks(self, query: str, result: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _create_search_result_blocks(self, results: Dict[str, Any], search_params: str) -> List[Dict[str, Any]]:
    ...
    def _create_health_status_blocks(self, health_status: Dict[str, bool], stats: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _create_deployment_confirmation_blocks(self, deploy_params: str) -> List[Dict[str, Any]]:
    ...
    def _create_integration_stats_blocks(self, stats: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def start(self):
        ...
        try:
            ...
            await self.socket_client.socket_mode_request_listeners.append(handler.handle)
