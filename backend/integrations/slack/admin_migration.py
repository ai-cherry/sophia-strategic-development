...
class AdminMigrationManager:
    ...
    async def run_full_migration(self) -> Dict[str, Any]:
        ...
        migration_results["natural_queries"] = await self.migrate_natural_language_queries()
        migration_results["conversation_search"] = await self.migrate_conversation_search()
        migration_results["health_monitoring"] = await self.migrate_health_monitoring()
        migration_results["deployment_commands"] = await self.migrate_deployment_commands()
        migration_results["automated_workflows"] = await self.setup_automated_workflows()
