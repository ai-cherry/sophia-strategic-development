...
class SlackIntegration:
    ...
    def __init__(self, config: SlackConfig = None):.
        ...
        self.webhook_client = (
            AsyncWebhookClient(self.config.notification_webhook_url)
    ...
    async def initialize(self):
        ...
        try:.
            ...
            logger.error(
                f"Failed to initialize Slack integration: {e.response['error']}"
            )
            return False
    ...
    async def send_notification(self, notification: SlackNotification) -> bool:
        ...
        try:.
            ...
            if notification.mentions:
                mentions = " "join(
                    [
                        f"<@{await self._resolve_user(user)}>"
                        for user in notification.mentions
                    ]
                )
    ...
    async def _format_call_notification(
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "📞 Call Analysis Complete"},
            },
            {
                "type": "section",
                "fields": [
        ...
        if data.get("insights"):
            ...
            if insights.get("success_probability"):
                emoji = (
                    "🟢"
                    if insights["success_probability"] > 70
                    else "🟡"
                    if insights["success_probability"] > 40
                    else "🔴"
                )
                blocks.append(
                    {
    ...
    async def _format_insight_notification(
        ...
        return {
            ...
            "blocks": [
                {
                    "type": "header",
                    "text": {"type": "plain_text", "text": "💡 Business Insight Alert"},
                },
                {
                    "type": "section",
                    "text": {
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": notification.data.get(
                            "message", "Notification from Sophia AI"
                        ),
                    },
                }
            ],
                ...
                "blocks": [
                    {
                        ...
                        "text": {
                            "type": "mrkdwn",
                            "text": "How long would you like to snooze this task?",
                        },
                        "accessory": {
                            "type": "static_select",
    ...
    async def _handle_mention_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Reply in thread
        await self.reply_to_thread(channel=channel, thread_ts=thread_ts, text=response)
