...
class EnhancedNaturalLanguageProcessor:
    ...
    async def _extract_intent(
        ...
        if confidence < 0.3:
            ai_intent, ai_confidence = await self._ai_intent_classification(
                query, context
            )
        ...
        suggested_actions = self._generate_suggested_actions(
            best_intent, entities, parameters
        )
    ...
    async def _ai_intent_classification(
        ...
        try:
            response = await self.kong_gateway.process_request(
                prompt=prompt, model="gpt-4", max_tokens=50, temperature=0.1
            )
    ...
    def _extract_parameters(
        ...
        # Extract amounts
        amount_entities = [
            e for e in entities if e.entity_type == EntityType.DOLLAR_AMOUNT
        ]
        if amount_entities:
        ...
        if intent == IntentType.CLIENT_MANAGEMENT:
            client_entities = [
                e for e in entities if e.entity_type == EntityType.CLIENT_NAME
            ]
            if client_entities:
            ...
            metric_entities = [
                e for e in entities if e.entity_type == EntityType.METRIC_NAME
            ]
            if metric_entities:
            ...
            infra_entities = [
                e
                for e in entities
                if e.entity_type == EntityType.INFRASTRUCTURE_COMPONENT
            ]
            if infra_entities:
    ...
    async def _generate_response(
        ...
        try:
            response = await self.kong_gateway.process_request(
                prompt=prompt, model="gpt-4", max_tokens=300, temperature=0.7
            )
