...
class PortkeyGatewayService:
                    ...
                    # Technical tasks -> DeepSeek (cost effective)
                    {
                        "query": {
                            "$or": [
                                ...
                                {"metadata.task_type": {"$eq": "system_administration"}}
                            ]
                        },
    ...
    async def stream_completion(
        ...
        **kwargs
        ...
        try:
            ...
            if self.api_key:
                # Stream from Portkey
                async for chunk in self._stream_portkey_request(messages, target_model, **kwargs):
                    yield chunk
            else:
                # Mock streaming response
                mock_response = self._generate_mock_response(task_type, messages, target_model)
    ...
    def _select_optimal_model(
        ...
        elif task_type in ["code_generation", "technical_implementation", "system_administration"]:
