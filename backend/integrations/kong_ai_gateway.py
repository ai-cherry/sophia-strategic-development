...
class KongAIGateway:
    ...
    def register_agent(self, agent_type: AgentType, config: Dict[str, Any]):
        """Register an AI agent with the gateway."""self.agent_registry[agent_type] = {.
        ...
        self.redis_client.hset("agent_registry", agent_type.value, json.dumps(config))
    ...
    async def route_request(self, request: AgentRequest) -> AgentResponse:
        ...
        try:
            ...
            await self._update_agent_metrics(
                request.agent_type, execution_time, response.success
            )
    ...
    async def _handle_gong_request(self, request: AgentRequest) -> AgentResponse:
        ...
        try:.
            ...
            if request.action == "analyze_conversation":
                ...
                return AgentResponse(
                    success=True, data=response_data, agent_type=request.agent_type
                )
                ...
                response_data = {
                    ...
                    "recommended_actions": [
                        "follow_up_on_pricing",
                        "address_technical_concerns",
                    ],
                }
                    ...
                    json.dumps(
                        {
                            ...
                            "average_response_time": agent_info[
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for agent_type, agent_info in self.agent_registry.items():
            try:
                ...
                if (
                    last_check and (datetime.now() - last_check).seconds < 300
                ):  # 5 minutes
                    status = "healthy"
                else:
                    status = "stale"
                ...
                health_status["agents"][agent_type.value] = {
                    ...
                    "error_rate": agent_info.get("error_count", 0)
                    / max(agent_info.get("request_count", 1), 1),
                }
...
def get_kong_gateway():
    """Get Kong AI Gateway instance (lazy initialization)."""global kong_gateway.
