...
class EmailThreadAnalysis:
    ...
    source: EmailSource
    gong_conversation_id: Optional[str] = None
...
class EnhancedMicrosoftGongIntegration:
    ...
    async def initialize(self):
        ...
        if not self.initialized:
            ...
            logger.info("✅ Enhanced Microsoft+Gong integration initialized")
    ...
    async def _analyze_email_threads(
        ...
        if not emails_data:
            return {"thread_count": 0, "threads": [], "summary": "No email data available"}
    ...
    async def _analyze_single_thread(
        ...
        if len(sorted_emails) > 1:
            ...
            for i in range(1, len(sorted_emails)):
                try:
                    prev_time = datetime.fromisoformat(sorted_emails[i-1].get("sent_time", ""))
                    curr_time = datetime.fromisoformat(sorted_emails[i].get("sent_time", ""))
        ...
        email_source = EmailSource.OUTLOOK  # Default since emails originate from Microsoft
    ...
    async def _calculate_personalization_score(
        ...
        if any(industry in content_lower for industry in [
            "fintech", "payments", "financial services", "banking"
        ]):
            score += personalization_indicators["specific_industry"]
        
        if any(news in content_lower for news in [
            "expansion", "funding", "partnership", "acquisition", "launch"
        ]):
    ...
    async def _analyze_cta_effectiveness(
        self, 
        ...
        if any(time in content_lower for time in [
            "tuesday", "wednesday", "thursday", "friday",
            "2 pm", "10 am", "morning", "afternoon"
        ]):
            score += 0.2
        
        # Check for value proposition in CTA
        if any(value in content_lower for value in [
            "5-minute insight", "quick case study", "relevant example"
        ]):
            score += 0.2
        ...
        return {
            ...
            "summary": {
                ...
                "calls_needing_coaching": len([
                    c for c in call_analyses 
                    if c.get("sentiment_score", 0) < 0.5 or c.get("talk_ratio", 0) > 0.7
                ])
            }
        }
    ...
    async def _analyze_single_call(self, call: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "coaching_needed": (
                call.get("sentiment_score", 0) < 0.5 or 
                call.get("talk_ratio", 0) > 0.7
            )
        }
    ...
    async def _generate_coaching_insights(
        ...
        if response_rate < 35:
            insights.append(RealTimeCoachingInsight(
        ...
        if personalization < 0.4:
            insights.append(RealTimeCoachingInsight(
                ...
                context={
                    ...
                    "target_score": 0.7,
                    "improvement_areas": ["company_research", "industry_insights", "custom_subject_lines"]
                }
            ))
    ...
    async def _analyze_competitive_mentions(
        ...
        for competitor, data in competitors.items():
            if data["mentions"] > 0:
                battle_cards.append({
                    ...
                    "recommended_response": self._get_battle_card_response(competitor),
                    "calls": data["context"]
                })
        ...
        return {
            ...
            "total_competitive_calls": len([
                ...
                if any(comp in call.get("transcript", "").lower() for comp in competitors.keys())
            ])
        }
    ...
    def _get_battle_card_response(self, competitor: str) -> str:
        ...
        return battle_cards.get(competitor, "Focus on our unique value proposition and customer success stories")
    ...
    def _calculate_overall_score(
        ...
        talk_ratio_score = max(0, 1 - abs(call_summary.get("avg_talk_ratio", 0.7) - 0.6))
    ...
    async def _generate_improvement_plan(
        ...
        # Priority insights
        critical_insights = [i for i in coaching_insights if i.priority == CoachingPriority.CRITICAL]
        high_insights = [i for i in coaching_insights if i.priority == CoachingPriority.HIGH]
