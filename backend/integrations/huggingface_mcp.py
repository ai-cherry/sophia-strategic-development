...
class HuggingFaceMCPServer:
    ...
    async def list_spaces(
        ...
        """List Hugging Face Spaces (Gradio apps)."""try:.
            ...
            response = requests.get(
                f"{self.base_url}/spaces", headers=self.headers, params=params
            )
            response.raise_for_status()
        ...
        """Recommend models based on business task and context."""try:.
            ...
            for tag in pipeline_tags:
                models = await self.search_models(
                    query=f"{task} {business_context}", limit=5, filter_tags=[tag]
                )
            ...
            sorted_models = sorted(
                unique_models.values(), key=lambda x: x.downloads, reverse=True
            )
    ...
    async def enhance_knowledge_base_with_models(
        ...
        """Enhance Sophia's knowledge base with relevant HF models."""try:.
            ...
            models = await self.hf_server.recommend_models_for_task(
                task=task, business_context="business intelligence financial analysis"
            )

            # Get relevant datasets
            datasets = await self.hf_server.search_datasets(
                query=f"business {business_domain} financial", limit=5
            )

            # Get research papers
            papers = await self.hf_server.semantic_search_papers(
                query=f"{business_domain} business intelligence AI", limit=5
            )
...
async def search_models():
    ...
    models = await sophia_hf.hf_server.search_models(
        query=query, limit=limit, filter_tags=tags if tags else None
    )
...
async def search_datasets():
    ...
    datasets = await sophia_hf.hf_server.search_datasets(query=query, limit=limit)
...
async def main():
    ...
    print(
        f"Found {len(enhancement.get('recommended_models', []))} models for revenue analysis"
    )

    # Get all recommendations
    recommendations = await sophia_hf.get_model_recommendations_for_sophia()
    print(
        f"Total recommendations: {sum(len(models) for models in recommendations.values())}"
    )
