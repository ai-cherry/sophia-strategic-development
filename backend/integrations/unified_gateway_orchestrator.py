...
class UnifiedGatewayOrchestrator:
    ...
    def _initialize_routes(self):
        ...
        self.logger.info(
            f"Initialized {sum(len(routes) for routes in self.routes.values())} routes across {len(self.routes)} services"
        )
    ...
    def _add_vector_db_routes(self):
        ...
        if (
            self.config.vector_databases
            and self.config.vector_databases.weaviate_api_key
        ):
            vector_routes.append(
                GatewayRoute(
                    ...
                    category=ServiceCategory.VECTOR_DB,
                    gateway_type=GatewayType.DIRECT,
                    endpoint=self.config.vector_databases.weaviate_url
                    or "https://weaviate.io",
    ...
    def _add_business_intelligence_routes(self):
        ...
        # HubSpot
        if (
            self.config.business_intelligence
            and self.config.business_intelligence.hubspot_api_key
        ):
            bi_routes.append(
                GatewayRoute(
        ...
        if (
            self.config.business_intelligence
            and self.config.business_intelligence.gong_api_key
        ):
            bi_routes.append(
                GatewayRoute(
                    service_name="gong",
        ...
        if (
            self.config.business_intelligence
            and self.config.business_intelligence.salesforce_api_key
        ):
            bi_routes.append(
                GatewayRoute(
    ...
    def _add_property_management_routes(self):
        ...
        # Yardi
        if (
            self.config.property_management
            and self.config.property_management.yardi_api_key
        ):
            pm_routes.append(
                GatewayRoute(
                    service_name="yardi",
        ...
        if (
            self.config.property_management
            and self.config.property_management.realpage_api_key
        ):
            pm_routes.append(
                GatewayRoute(
        ...
        if (
            self.config.property_management
            and self.config.property_management.appfolio_api_key
        ):
            pm_routes.append(
                GatewayRoute(
        ...
        if (
            self.config.property_management
            and self.config.property_management.costar_api_key
        ):
            pm_routes.append(
                GatewayRoute(
                    service_name="costar",
        ...
        if (
            ...
            analytics_routes.append(
                GatewayRoute(
                    ...
                    headers={
                        "X-Arize-Api-Key": self.config.observability.arize_api_key
                    },
                    rate_limit=100,
                    priority=1,
                )
        ...
        for route in sorted_routes:
            try:
                ...
                await self._update_metrics(
                    route, success=True, latency=response.get("latency", 0)
                )
                ...
                self.logger.warning(
                    f"Route failed for {route.service_name} via {route.gateway_type}: {str(e)}"
                )
                continue

        # All routes failed
        raise Exception(
            f"All routes failed for {service_name}. Last error: {last_error}"
        )
                        ...
                        if route.health_check_endpoint:
                            try:
                                response = await self.http_client.get(
                                    route.health_check_endpoint, timeout=10.0
                                )
    ...
    async def _metrics_aggregation_loop(self):
        ...
        while True:.
            ...
            try:
                ...
                # Log summary
                self.logger.info(
                    f"Gateway metrics - Requests: {total_requests}, Cost: ${total_cost:.2f}, Success Rate: {average_success_rate:.2%}"
                )
    ...
    async def get_client_for_route(self, route: GatewayRoute) -> Optional[Any]:
        ...
        if not api_key:
            self.logger.warning(
                f"API key '{route.api_key_name}' not found for route '{route.service_name}'."
            )
            return None
