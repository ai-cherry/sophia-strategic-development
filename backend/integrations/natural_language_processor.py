...
from .kong_ai_gateway import AgentType, AgentRequest, kong_gateway
...
class NaturalLanguageProcessor:
    ...
    async def process_natural_language(self, text: str, context: Optional[Dict[str, Any]] = None) -> ParsedIntent:
        ...
        try:
            ...
            confidence = await self._calculate_confidence(cleaned_text, intent, entities)
    ...
    async def _calculate_confidence(self, text: str, intent: IntentType, entities: Dict[str, Any]) -> float:
    ...
    async def _generate_suggested_actions(self, intent: IntentType, entities: Dict[str, Any]) -> List[str]:
        """Generate suggested actions based on intent and entities"""
        
        if intent == IntentType.DEPLOY_INFRASTRUCTURE:
            actions = ["generate_infrastructure_code", "estimate_costs", "create_deployment_plan"]
            ...
            actions = ["analyze_current_usage", "calculate_scaling_requirements", "update_auto_scaling_policies"]
            if entities.get('target_count'):
                actions.append("scale_to_target_count")
        
        elif intent == IntentType.MONITOR_PERFORMANCE:
            actions = ["fetch_system_metrics", "generate_performance_report", "check_alert_status"]
        
        elif intent == IntentType.ANALYZE_CONVERSATION:
            actions = ["extract_conversation_insights", "analyze_sentiment", "identify_action_items"]
    ...
    async def convert_to_agent_requests(self, parsed_intent: ParsedIntent) -> List[AgentRequest]:
            ...
            if parsed_intent.intent == IntentType.DEPLOY_INFRASTRUCTURE:
                ...
                requests.append(AgentRequest(
                    ...
                    parameters={
                        ...
                        "service_type": parsed_intent.entities.get("service_type", "web_application"),
                        "environment": parsed_intent.entities.get("environment", "development"),
                ...
                requests.append(AgentRequest(
                    ...
                    parameters={
                        "conversation_id": parsed_intent.entities.get("conversation_id"),
                        "analysis_type": "full"
                    },
                    context=parsed_intent.context
                ))
                ...
                requests.append(AgentRequest(
                    ...
                    parameters={
                        "time_range": parsed_intent.entities.get("time_range", "1h"),
                ...
                requests.append(AgentRequest(
                    ...
                    parameters={
                        "resource_type": parsed_intent.entities.get("resource_type", "compute"),
                        "scale_direction": "up" if "up" in parsed_intent.original_text else "down",
    ...
    async def process_and_execute(self, text: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _generate_summary(self, parsed_intent: ParsedIntent, results: List[Dict[str, Any]]) -> str:
