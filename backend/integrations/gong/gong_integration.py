...
class GongIntegration:
            ...
            try:
                async with self.session.request(
                    ...
                    if response.status == 429:  # Rate limited
                        ...
                        logger.warning(
                            f"Rate limited by Gong API, waiting {retry_after}s"
                        )
    ...
    async def analyze_call_sentiment(self, call_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            sentiment_score = (sentiments["positive"] - sentiments["negative"]) / max(
                total_segments, 1
            )
    ...
    async def _calculate_success_probability(
        self, pain_points: List[str], next_steps: List[str], objections: List[str]
    ) -> int:
    ...
    async def get_rep_performance(
        self, rep_email: str, from_date: datetime, to_date: datetime
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate averages
            avg_success_prob = (
                sum(m["success_probability"] for m in recent_metrics)
                / len(recent_metrics)
                if recent_metrics
                else 0
            )
            ...
            avg_sentiment = (
                sum(m["sentiment_score"] for m in recent_metrics) / len(recent_metrics)
                if recent_metrics
                else 0
            )
