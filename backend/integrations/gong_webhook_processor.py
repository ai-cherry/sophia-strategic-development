...
class WebhookProcessor:
        ...
        try:
            ...
            try:
                ...
                await self.snowflake.log_processing_stage(
                    webhook_id, "call", "raw_storage", "success",
                    duration_ms=int((time.time() - stage_start) * 1000)
                )
            ...
            try:
                ...
                await self.snowflake.log_processing_stage(
                    webhook_id, "call", "api_enhancement", "success",
                    duration_ms=int((time.time() - stage_start) * 1000),
                    metadata={"quality_score": self._calculate_data_quality(enhanced_data)}
                )
            except Exception as e:
                self.logger.error("Failed to enhance call data",
                                webhook_id=webhook_id,
                                call_id=call_id,
                                error=str(e))
                processing_result["stages"]["api_enhancement"] = {
            ...
            if enhanced_data:
                ...
                try:
                    ...
                    await self.snowflake.log_processing_stage(
                        webhook_id, "call", "enhanced_storage", "success",
                        duration_ms=int((time.time() - stage_start) * 1000)
                    )
                except Exception as e:
                    self.logger.error("Failed to store enhanced data",
                                    webhook_id=webhook_id,
                                    error=str(e))
                    processing_result["stages"]["enhanced_storage"] = {
                        "status": "failed",
            ...
            try:
                ...
                for insight in insights:
                    if insight.get("type") in ["competitor_mention", "churn_risk", "upsell_opportunity"]:
                ...
                await self.snowflake.log_processing_stage(
                    webhook_id, "call", "notification", "success",
                    duration_ms=int((time.time() - stage_start) * 1000),
            ...
            processing_result["total_duration_ms"] = int((time.time() - start_time) * 1000)
            
            await self.snowflake.update_webhook_status(webhook_id, "completed")
            
            self.logger.info("Webhook processing completed",
                           webhook_id=webhook_id,
    ...
    async def process_email_webhook(
        ...
        try:
            ...
            await self.redis.notify_email_processed(
                processed_email,
                NotificationPriority.HIGH if processed_email.requires_response else NotificationPriority.LOW
            )
    ...
    def _extract_insights(self, enhanced_data: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if transcript:
            competitors = ["Competitor A", "Competitor B"]  # Configure based on business
        ...
        for topic in topics:
            if any(keyword in topic.get("name", "").lower() for keyword in upsell_keywords):
                insights.append({
    ...
    def _determine_priority(self, processed_call: ProcessedCallData) -> NotificationPriority:
        ...
        if any(insight["type"] in ["churn_risk", "competitor_mention"] 
    ...
    def _determine_next_steps(self, enhanced_data: Optional[Dict[str, Any]]) -> List[str]:
