...
class GongIntegration(Integration):
    ...
    async def get_calls(
        self, from_date: datetime = None, to_date: datetime = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """Get calls from Gong"""
        try:
            if not from_date:
            ...
            async with self.session.get(
                f"{self.base_url}/v2/calls", params=params
            ) as response:
    ...
    async def get_deals(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.session.get(
                f"{self.base_url}/v2/deals", params=params
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def search_calls(
        self, query: str, from_date: datetime = None, to_date: datetime = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            payload = {
                "filter": {
                    ...
                    "contentSelector": {
                        "exposedFields": {
                            "content": {"transcriptSelector": {"query": query}}
                        }
                    },
                }
            }
            ...
            async with self.session.post(
                f"{self.base_url}/v2/calls/search", json=payload
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    return data.get("calls", [])
    ...
    async def store_call_analytics(
        ...
        print(
            f"  Deal Stage: {analytics_data.get('deal_signals', {}).get('deal_progression_stage')}"
        )
        print(
            f"  Competitors: {analytics_data.get('competitive_intelligence', {}).get('competitors_mentioned')}"
        )
        print("-----------------------------------------")
