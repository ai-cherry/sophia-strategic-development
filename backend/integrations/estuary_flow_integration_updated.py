...
from ..core.config_manager import get_config, get_secret
...
class EstuaryFlowClient:
    ...
    async def setup(self):
        ...
        try:
            # Get API key from centralized config manager
            self.api_key = await get_secret("estuary", "api_key")
            if not self.api_key:
                raise ValueError("Estuary API key not found")
            
            # Get base URL from centralized config manager
            estuary_config = await get_config("estuary")
            self.base_url = estuary_config.config.get("api_url", "https://api.estuary.tech")
