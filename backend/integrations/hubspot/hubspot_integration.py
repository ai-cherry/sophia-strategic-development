        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    logger.error(
                        f"HubSpot API request failed after {self.config.max_retries} attempts: {str(e)}"
                    )
                    raise
    ...
    async def get_contact_analytics(self, contact_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            open_deals = [
                deal
                for deal in deals
                if deal.get("properties", {}).get("dealstage")
                not in ["closedwon", "closedlost"]
            ]
            closed_won_deals = [
                deal
                for deal in deals
    ...
    async def get_pipeline_analytics(self, pipeline_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            closed_lost = [
                deal
                for deal in deals
                if deal.get("properties", {}).get("dealstage") == "closedlost"
            ]
            open_deals = [
                deal
                for deal in deals
            ...
            analytics = {
                ...
                "win_rate": len(closed_won) / (len(closed_won) + len(closed_lost))
