...
class LlamaIndexProcessor:
    ...
    def _setup_pinecone(self) -> PineconeVectorStore:
        ...
        try:
            ...
            api_key = self.secrets.get(
                "LLAMAINDEX_PINECONE_API_KEY", settings.vector_db.pinecone_api_key
            )
            pinecone.init(
                api_key=api_key, environment=settings.vector_db.pinecone_environment
            )
    ...
    def _setup_weaviate(self) -> WeaviateVectorStore:
        ...
        try:
            ...
            api_key = self.secrets.get(
                "LLAMAINDEX_WEAVIATE_API_KEY", settings.vector_db.weaviate_api_key
            )
            ...
            return WeaviateVectorStore(
                weaviate_client=client, index_name="SophiaDocuments"
            )
    ...
    async def process_document(
        self, document: Union[str, Document, Dict], context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(document, str):
                ...
                doc = Document(
                    text=document.get("text", ""), metadata=document.get("metadata", {})
                )
            else:
                doc = document
            ...
            return {
                ...
                "metadata": {
                    ...
                    "questions_answered": nodes[0].metadata.get(
                        "questions_answered", []
                    ),
    ...
    async def _rank_across_sources(
        self, pinecone_results: List[Dict], weaviate_results: List[Dict]
    ) -> List[Dict]:
