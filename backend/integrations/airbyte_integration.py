...
class AirbyteIntegration:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(f"Failed to initialize Airbyte integration: {e}", exc_info=True)
            raise
    ...
    async def trigger_sync(self, connection_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with self.session.post(url, json=payload) as response:
                ...
                logger.info(f"Successfully triggered Airbyte job: {job_info.get('job', {}).get('id')}")
                return job_info
        except aiohttp.ClientError as e:
            logger.error(f"Error triggering Airbyte sync for connection {connection_id}: {e}")
            return None
