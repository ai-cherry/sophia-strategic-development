...
class RedisNotificationClient:
    ...
    async def notify_call_processed(
        ...
        if priority in [NotificationPriority.HIGH, NotificationPriority.URGENT]:
            await self.redis.publish(
                self.priority_channels[priority], 
                json.dumps(message)
            )
        ...
        self.logger.info("Call notification sent", 
    ...
    async def notify_meeting_processed(
        ...
        self.logger.info("Meeting notification sent", 
    ...
    async def notify_insight_detected(
        ...
        self.logger.info("Insight notification sent",
                        webhook_id=webhook_id,
                        insight_type=insight_type)
    ...
    async def notify_action_required(
        ...
        await self.redis.publish(
            self.priority_channels[NotificationPriority.HIGH],
            json.dumps(message)
        )
        ...
        self.logger.info("Action notification sent",
                        action_type=action_type,
                        assigned_to=assigned_to)
    ...
    async def notify_processing_error(
        ...
        self.logger.error("Error notification sent",
                         webhook_id=webhook_id,
                         error_type=error_type)
    ...
    async def _store_notification(
        ...
        try:
            ...
            self.logger.warning("Failed to store notification",
                              key=key,
                              error=str(e))
            ...
            async for message in pubsub.listen():
                if message["type"] == "message":
                    try:
                        ...
                        self.logger.error("Error in subscription callback",
                                        channel=channel,
                                        error=str(e))
        finally:
