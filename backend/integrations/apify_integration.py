...
class ApifyIntegration:
    ...
    async def list_actors(self) -> List[Dict[str, Any]]:
        ...
        return [{"id": actor.get("id"), "name": actor.get("name")} for actor in actor_list.items]

    async def run_actor_and_get_results(self, actor_id: str, run_input: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        while run_details['status'] not in ('SUCCEEDED', 'FAILED', 'ABORTED', 'TIMED_OUT'):
