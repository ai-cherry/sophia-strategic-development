...
class HuggingFaceIntegration:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"Successfully connected to Hugging Face MCP server. Found {len(self.tools)} tools.")

        except Exception as e:
            logger.error(f"Failed to initialize Hugging Face integration: {e}", exc_info=True)
            self.initialized = False

    async def search_models(self, query: str, top_k: int = 5, tags: List[str] = None) -> List[Dict[str, Any]]:
        """
