...
class UnifiedAIAssistant:
    ...
    def __init__(self):
        ...
        self.patterns = {
            "coding": ["write code", "generate", "debug", "refactor", "function", "class", "programming", "python", "javascript", "react", "implement", "develop"],
            "infrastructure": ["deploy", "scale", "infrastructure", "server", "performance", "health", "docker", "kubernetes", "pulumi", "devops"],
            "data": ["query", "database", "snowflake", "sql", "analytics", "data", "analysis", "insights", "business intelligence"],
            "design": ["design", "ui", "ux", "component", "figma", "dashboard", "interface", "styling", "layout"],
            "business": ["sales", "marketing", "revenue", "customer", "deals", "hubspot", "gong", "business", "analyze"],
            "chat": ["chat", "ask", "question", "help", "explain", "understand", "tell me about"],
            "mcp": ["mcp", "server", "integration", "memory", "recall", "store", "context"]
        }
        ...
        self.model_priority = {
            "coding": ["claude-3-5-sonnet-20241119", "claude-3-5-sonnet-20241022", "gemini-2.5-pro"],
            "analysis": ["claude-3-5-sonnet-20241119", "claude-3-5-sonnet-20241022", "gemini-2.5-pro"],
            "design": ["claude-3-5-sonnet-20241119", "gemini-2.5-pro"],
            "business": ["claude-3-5-sonnet-20241119", "gemini-2.5-pro"],
            "general": ["claude-3-5-sonnet-20241119", "claude-3-5-sonnet-20241022", "gemini-2.5-pro"]
        }
    ...
    async def _handle_infrastructure(self, query: str) -> str:
        ...
        try:
            ...
            result = subprocess.run(
                ["python", "backend/services/infrastructure_chat/sophia_infrastructure_chat.py", query],
    ...
    async def _fallback_to_chat(self, query: str, intent: str = "general", model: str = None) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                chat_response = requests.post(
                    f"{self.base_url}/api/chat/enhanced",
                    json={"message": query, "context": intent, "preferred_model": model},
                    timeout=30
                )
