...
class SecurityMiddleware:
    ...
    def __init__(self, app: Flask, redis_url: str | None = None):
        ...
        self.config = {
            ...
            'cors': {
                'origins': ['https://sophia-ai-frontend-dev.vercel.app', 'https://localhost:3000'],
    ...
    def _setup_middleware(self):
        ...
        def before_request():
            ...
            if request.content_length and request.content_length > self.config['input_validation']['max_content_length']:
                logger.warning(f"Request {g.request_id}: Content length too large: {request.content_length}")
                raise BadRequest("Request entity too large")

            # Validate content type
            if request.content_type and request.content_type.split(';')[0] not in self.config['input_validation']['allowed_content_types']:
                logger.warning(f"Request {g.request_id}: Invalid content type: {request.content_type}")
        ...
        def after_request(response):
            ...
            if origin in self.config['cors']['origins']:
                ...
                response.headers['Access-Control-Allow-Methods'] = ', '.join(self.config['cors']['methods'])
                response.headers['Access-Control-Allow-Headers'] = ', '.join(self.config['cors']['headers'])
            ...
            logger.info(f"Response {g.get('request_id', 'unknown')}: {response.status_code}")
