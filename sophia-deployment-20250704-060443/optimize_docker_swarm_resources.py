...
RESOURCE_LIMITS = {
    ...
    "monitoring": {"cpus": "0.5", "memory": "1G", "replicas": 1, "placement": []},
    "ai-service": {
        "cpus": "4.0",
        "memory": "8G",
        "replicas": 2,
    ...
    "default": {"cpus": "1.0", "memory": "2G", "replicas": 2, "placement": []},
}
...
def get_service_port(service_config: dict[str, Any]) -> int | None:
...
def generate_health_check(
    ...
    detect_service_type(service_name)
...
def optimize_service(
    ...
    if limits["placement"] and "placement" not in deploy:
        deploy["placement"] = {"constraints": limits["placement"]}
...
def optimize_networks(config: dict[str, Any]) -> dict[str, Any]:
    ...
    optimized_networks = {
        "frontend": {
            "driver": "overlay",
            "attachable": True,
            "driver_opts": {"encrypted": "false"},  # Encryption adds latency
        },
        "backend": {
            "driver": "overlay",
            "internal": True,
            "driver_opts": {"encrypted": "false"},
        },
        "data": {
            "driver": "overlay",
            "internal": True,
            "driver_opts": {"encrypted": "true"},  # Encrypt data layer
        },
    }
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", help="Output file (default: adds .optimized suffix)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate the configuration"
    )
    ...
    for service_name, service_config in config.get("services", {}).items():
        config["services"][service_name] = optimize_service(
            service_name, service_config
        )
        config["services"][service_name] = assign_service_networks(
            service_name, service_config
        )
