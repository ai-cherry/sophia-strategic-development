...
async def run_integration_tests():
    ...
    for service, configured in auth_status.items():
        ...
        logger.info(
            f"   {status_emoji} {service}: {'Configured' if configured else 'Needs Configuration'}"
        )
    ...
    for service_health in system_health.services:
        ...
        logger.info(
            f"   {status_emoji} {service_health.name}: {service_health.status} ({service_health.response_time_ms:.1f}ms)"
        )
    ...
    logger.info(
        f"   Health Score: {health_score:.1f}% ({healthy_services}/{total_services} services)"
    )
