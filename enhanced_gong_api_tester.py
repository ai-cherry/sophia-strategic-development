...
class EnhancedGongAPITester:
    ...
    def __init__(self):
        ...
        credentials_string = (
            f"{self.credentials['access_key']}:{self.credentials['client_secret']}"
        )
        ...
        if isinstance(data, list):
            ...
            for i, (k, v) in enumerate(data.items()):
                ...
                if isinstance(v, (str, int, float, bool)):
                    ...
                    sample[k] = {
                        kk: vv for i, (kk, vv) in enumerate(v.items()) if i < 2
                    }
            return sample
        return data
    ...
    async def extract_and_populate_data(
        ...
        try:
            ...
            if endpoint_results.get("users", {}).get("success"):
                ...
                if users_data:
                    population_results["data_extracted"][
                        "users"
                    ] = await self._process_users_data(conn, users_data)
            ...
            for call_type in ["calls_basic", "calls_extensive"]:
                if endpoint_results.get(call_type, {}).get("success"):
                    ...
                    if calls_data:
                        population_results["data_extracted"][
                            call_type
                        ] = await self._process_calls_data(conn, calls_data, call_type)
            ...
            for stats_type in ["stats_activity", "stats_interaction"]:
                if endpoint_results.get(stats_type, {}).get("success"):
                    ...
                    if stats_data:
                        population_results["data_extracted"][
                            stats_type
                        ] = await self._process_stats_data(conn, stats_data, stats_type)
    ...
    async def _process_calls_data(
        self, conn, calls_data: List[Dict], call_type: str
    ) -> Dict[str, Any]:
    ...
    async def _process_stats_data(
        self, conn, stats_data: List[Dict], stats_type: str
    ) -> Dict[str, Any]:
    ...
    def analyze_apartment_industry_relevance(
        self, endpoint_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for endpoint_name, result in endpoint_results.items():
            if result.get("success") and result.get("sample_data"):
                relevance_analysis["analysis_results"][
                    endpoint_name
                ] = self._analyze_content_relevance(
                    result["sample_data"],
    ...
    def _analyze_content_relevance(
        self, data: Any, apartment_keywords: List[str], competitor_keywords: List[str]
    ) -> Dict[str, Any]:
        """Analyze content for apartment industry and competitor relevance"""
        # Convert data to searchable text
        text_content = json.dumps(data).lower()

        apartment_matches = sum(
            1 for keyword in apartment_keywords if keyword in text_content
        )
        competitor_matches = sum(
            1 for keyword in competitor_keywords if keyword in text_content
        )
        ...
        return {
            ...
            "total_relevance_score": (apartment_matches + competitor_matches)
            / (len(apartment_keywords) + len(competitor_keywords)),
        }
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        try:
            population_results = asyncio.run(
                self.extract_and_populate_data(endpoint_results)
            )
        except Exception as e:
        ...
        report = {
            "test_summary": {
                ...
                "successful_endpoints": sum(
                    1 for r in endpoint_results.values() if r.get("success")
                ),
                "failed_endpoints": sum(
                    1 for r in endpoint_results.values() if not r.get("success")
                ),
            },
...
def main():
    ...
    print(
        f"   Success rate: {(summary['successful_endpoints']/summary['total_endpoints_tested']*100):.1f}%"
    )

    # Print apartment relevance
    if report["apartment_relevance"]["analysis_results"]:
        print("\n🏢 APARTMENT INDUSTRY RELEVANCE:")
        for endpoint, analysis in report["apartment_relevance"][
            "analysis_results"
        ].items():
            if analysis["total_relevance_score"] > 0:
                print(
                    f"   {endpoint}: {analysis['total_relevance_score']:.2f} relevance score"
                )
