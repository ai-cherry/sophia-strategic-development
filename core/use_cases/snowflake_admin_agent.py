...
"""
ModernStack Admin Agent
LangChain-powered natural language interface for ModernStack administration tasks

File Decomposition Plan (auto-generated by Phase 3)
Current size: 811 lines

Recommended decomposition:
- modern_stack_admin_agent_core.py - Core functionality
- modern_stack_admin_agent_utils.py - Utility functions
- modern_stack_admin_agent_models.py - Data models
- modern_stack_admin_agent_handlers.py - Request handlers
...
class ModernStackEnvironment(Enum):
    """ModernStack environment targets"""
...
class AdminTaskRequest:
    ...
    target_environment: ModernStackEnvironment
...
class AdminTaskResponse:
    ...
    task_type: AdminTaskType | None = None
    environment: ModernStackEnvironment | None = None
    execution_time: float = 0.0


class ModernStackAdminCallbackHandler(BaseCallbackHandler):
    """Custom callback handler for ModernStack admin operations"""
...
class ModernStackAdminAgent:
    """
    LangChain-powered ModernStack administration agent

    Provides natural language interface for ModernStack DDL/DCL operations
    ...
    async def initialize(self):
        """Initialize the ModernStack admin agent"""
        if self.initialized:
            return
        ...
        try:
            ...
            for env in ModernStackEnvironment:
                await self._initialize_environment(env)

            self.initialized = True
            logger.info("✅ ModernStack Admin Agent initialized successfully")

        except Exception as e:
            logger.error(f"Failed to initialize ModernStack Admin Agent: {e}")
            raise
    ...
    async def _initialize_llm(self):
        ...
        try:
            ...
            if openai_key and OPENAI_AVAILABLE:
                ...
                logger.info("✅ Initialized ChatOpenAI for ModernStack Admin Agent")
            else:
    ...
    async def _initialize_environment(self, environment: ModernStackEnvironment):
        """Initialize ModernStack connection for specific environment"""
        try:
            # Get environment-specific configuration
            config_prefix = f"modern_stack_{environment.value}"
            ...
            logger.info(f"✅ Initialized ModernStack connection for {environment.value}")
    ...
    def _build_connection_string(
        self, params: dict[str, str], environment: ModernStackEnvironment
    ) -> str:
        """Build ModernStack connection string for SQLDatabase"""
        ...
        return f"modern_stack://{user}@{account}/{database}/{schema}?warehouse={warehouse}"

    def _get_agent_prefix(self, environment: ModernStackEnvironment) -> str:
        """Get environment-specific agent prefix with safety rules"""
        base_prefix = """
You are a ModernStack database administrator assistant. You help users manage ModernStack objects through SQL commands.
...
"""
        ...
        if environment == ModernStackEnvironment.PROD:
            env_rules = """
...
"""
        elif environment == ModernStackEnvironment.STG:
            env_rules = """
...
"""
    ...
    async def execute_admin_task(self, request: AdminTaskRequest) -> AdminTaskResponse:
        """
        Execute a ModernStack admin task based on natural language request
        ...
        try:
            ...
            callback_handler = ModernStackAdminCallbackHandler()
            ...
            logger.error(f"ModernStack admin task failed: {e}")
            return AdminTaskResponse(
                success=False,
    ...
    async def get_environment_info(
        self, environment: ModernStackEnvironment
    ) -> dict[str, Any]:
        """Get information about a ModernStack environment"""
        try:
            connection = self.connections.get(environment)
            if not connection:
    ...
    async def close(self):
        """Close all ModernStack connections"""
...
# Global instance
# REMOVED: ModernStack dependency ModernStackAdminAgent()


# Convenience functions
async def execute_modern_stack_admin_task(
    natural_language_request: str,
    target_environment: str = "dev",
    user_id: str = "system",
) -> AdminTaskResponse:
    """
    Convenience function for executing ModernStack admin tasks
    ...
    """
    try:
        env = ModernStackEnvironment(target_environment.lower())
    except ValueError:
        return AdminTaskResponse(
            success=False,
    ...
    return await modern_stack_admin_agent.execute_admin_task(request)


async def confirm_modern_stack_admin_task(
    natural_language_request: str,
    confirmed_sql: str,
    target_environment: str = "dev",
    user_id: str = "system",
    ...
    return await modern_stack_admin_agent.confirm_and_execute(request, confirmed_sql)


# Example usage and testing
if __name__ == "__main__":

    async def test_modern_stack_admin_agent():
        """Test the ModernStack admin agent"""

        # Test basic functionality
        response = await execute_modern_stack_admin_task(
        ...
        # Test health check
        health = await modern_stack_admin_agent.health_check()
        print(f"Health check: {json.dumps(health, indent=2)}")

    # Run test
    asyncio.run(test_modern_stack_admin_agent())
