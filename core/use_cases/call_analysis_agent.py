...
class CallAnalysisAgent(BaseAgent):
    ...
    async def _analyze_individual_call(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            key_insights = await self._generate_ai_insights(
                call_details, sentiment_analysis, call_summary
            )

            # Calculate overall call score
            overall_score = await self._calculate_call_score(
                call_details, sentiment_analysis
            )
            ...
            business_impact = await self._analyze_call_business_impact(
                call_details, sentiment_analysis
            )
        ...
        try:
            ...
            if call_ids:
                ...
                coaching_calls = await connector.get_calls_for_coaching(
                    sales_rep=sales_rep, date_range_days=7, limit=limit
                )
            ...
            for i in range(0, len(calls_to_analyze), 10):  # Process 10 at a time
                ...
                batch_tasks = [
                    self._analyze_individual_call(
                        {"call_id": call_id, "include_similar": False}
                    )
            ...
            if pattern_query:
                ...
                if sales_rep:
                    recent_calls = await connector.get_calls_for_coaching(
                        sales_rep=sales_rep, date_range_days=date_range_days, limit=20
                    )
                    similar_calls = [
                        {"call_id": call["CALL_ID"]} for call in recent_calls
                    ]
                else:
                    return {
                        "success": False,
    ...
    async def _track_sentiment_trends(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            sentiment_trends = {
                ...
                "sentiment_category": self._classify_sentiment(
                    performance_data.get("avg_sentiment", 0.5)
                ),
                ...
                "improvement_recommendations": self._generate_sentiment_recommendations(
                    performance_data
                ),
            }
    ...
    async def _generate_ai_insights(
            ...
            insights.append(
                CallInsight(
                    ...
                    supporting_data={
                        ...
                        "negative_segments": sentiment_analysis.get(
                            "negative_segments"
                        ),
                        "transcript_segments": sentiment_analysis.get(
                            "transcript_segments"
                        ),
                    },
                )
        ...
        if (
            ...
            and call_details["INTERACTIVITY_SCORE"] < 0.3
        ):
            insights.append(
                CallInsight(
    ...
    async def _calculate_call_score(
        ...
        talk_ratio_component = max(
            0, min(100, 100 - abs(talk_ratio - ideal_ratio) * 200)
        )
    ...
    async def _analyze_call_business_impact(
        ...
        if deal_stage:
            if "closed" in deal_stage.lower():
                ...
                "proposal" in deal_stage.lower() or "negotiation" in deal_stage.lower()
            ):
    ...
    async def _generate_batch_insights(
        ...
        sentiments = [
            result["call_analysis"]["sentiment_analysis"].get(
                "call_sentiment_score", 0.5
            )
            for result in batch_results
        ]
        ...
        return {
            ...
            "recommendations": self._generate_batch_recommendations(
                avg_score, avg_sentiment, priority_counts
            ),
        }
    ...
    async def _score_call_performance(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        analysis_result = await self._analyze_individual_call(
            {"call_id": call_id, "include_similar": False}
        )
    ...
    async def _analyze_call_patterns(
        ...
        patterns = {
            ...
            "average_sentiment": (
                sum(sentiments) / len(sentiments) if sentiments else 0.5
            ),
    ...
    def _identify_success_patterns(self, calls: list[dict[str, Any]]) -> list[str]:
        ...
        high_sentiment_calls = [
            call for call in calls if call.get("sentiment_score", 0) > 0.6
        ]
        ...
        # Deal progression
        closed_won_calls = [
            call for call in calls if "won" in str(call.get("deal_stage", "")).lower()
        ]
        if closed_won_calls:
