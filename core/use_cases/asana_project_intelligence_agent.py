...
class AsanaProjectIntelligenceAgent(LangGraphAgentBase):
    ...
    async def get_project_metrics(self, project_gid: str | None=None) -> list[AsanaProjectMetrics]:
        ...
        try:
            ...
            for _, row in result.iterrows():
                ...
                metrics.append(AsanaProjectMetrics(project_gid=row['PROJECT_GID'], project_name=row['PROJECT_NAME'], completion_percentage=row.get('COMPLETION_PERCENTAGE', 0.0), task_count=row.get('TOTAL_TASKS', 0), completed_task_count=row.get('COMPLETED_TASKS', 0), overdue_task_count=row.get('OVERDUE_TASKS', 0), team_name=row.get('TEAM_NAME'), owner_name=row.get('OWNER_NAME'), due_date=row.get('DUE_DATE'), created_at=row['CREATED_AT'], modified_at=row['MODIFIED_AT'], health_score=health_score, risk_level=risk_level, ai_insights=ai_metadata))
            return metrics
        except Exception as e:
    ...
    async def analyze_team_productivity(self, team_name: str | None=None) -> list[TeamProductivityMetrics]:
        ...
        try:
            ...
            for _, row in result.iterrows():
                ...
                team_metrics.append(TeamProductivityMetrics(team_name=row['TEAM_NAME'], total_projects=row.get('TOTAL_PROJECTS', 0), active_projects=row.get('ACTIVE_PROJECTS', 0), completed_projects=row.get('COMPLETED_PROJECTS', 0), average_completion_rate=row.get('AVG_COMPLETION_RATE', 0.0), overdue_tasks_ratio=overdue_ratio, team_velocity=velocity, member_count=member_count, productivity_score=productivity_score))
            return team_metrics
        except Exception as e:
    ...
    async def perform_risk_assessment(self, project_gid: str | None=None) -> list[ProjectRiskAssessment]:
        ...
        try:
            ...
            for project in projects:
                ...
                risk_factors, mitigation_suggestions = await self._generate_risk_insights(project, task_data, schedule_risk, resource_risk, scope_risk, quality_risk)
                predicted_completion = self._predict_completion_date(project, task_data)
                risk_assessments.append(ProjectRiskAssessment(project_gid=project.project_gid, project_name=project.project_name, overall_risk=overall_risk, schedule_risk=self._score_to_risk_level(schedule_risk), resource_risk=self._score_to_risk_level(resource_risk), scope_risk=self._score_to_risk_level(scope_risk), quality_risk=self._score_to_risk_level(quality_risk), risk_factors=risk_factors, mitigation_suggestions=mitigation_suggestions, predicted_completion_date=predicted_completion))
    ...
    async def _generate_risk_insights(self, project: AsanaProjectMetrics, task_data: dict[str, Any], schedule_risk: float, resource_risk: float, scope_risk: float, quality_risk: float) -> tuple[list[str], list[str]]:
        ...
        try:
            ...
            try:
                ...
                risk_factors, mitigation_suggestions = self._generate_fallback_insights(project, task_data, schedule_risk, resource_risk, scope_risk, quality_risk)
            return (risk_factors, mitigation_suggestions)
        except Exception as e:
            logger.exception(f'❌ Error generating risk insights: {e}')
            return self._generate_fallback_insights(project, task_data, schedule_risk, resource_risk, scope_risk, quality_risk)

    def _generate_fallback_insights(self, project: AsanaProjectMetrics, task_data: dict[str, Any], schedule_risk: float, resource_risk: float, scope_risk: float, quality_risk: float) -> tuple[list[str], list[str]]:
    ...
    def _predict_completion_date(self, project: AsanaProjectMetrics, task_data: dict[str, Any]) -> datetime | None:
    ...
    async def generate_project_intelligence_report(self, project_gid: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            report = {'generated_at': datetime.now().isoformat(), 'scope': 'single_project' if project_gid else 'all_projects', 'project_metrics': [{'project_gid': p.project_gid, 'project_name': p.project_name, 'health_score': p.health_score, 'completion_percentage': p.completion_percentage, 'risk_level': p.risk_level.value, 'task_count': p.task_count, 'overdue_tasks': p.overdue_task_count, 'team_name': p.team_name, 'owner_name': p.owner_name} for p in projects], 'team_productivity': [{'team_name': t.team_name, 'productivity_score': t.productivity_score, 'active_projects': t.active_projects, 'completion_rate': t.average_completion_rate, 'team_velocity': t.team_velocity, 'member_count': t.member_count} for t in teams], 'risk_assessments': [{'project_gid': r.project_gid, 'project_name': r.project_name, 'overall_risk': r.overall_risk.value, 'risk_factors': r.risk_factors, 'mitigation_suggestions': r.mitigation_suggestions, 'predicted_completion': r.predicted_completion_date.isoformat() if r.predicted_completion_date else None} for r in risks], 'summary': summary}
    ...
    async def _store_intelligence_insights(self, projects: list[AsanaProjectMetrics], teams: list[TeamProductivityMetrics], risks: list[ProjectRiskAssessment]) -> None:
        ...
        try:
            for project in projects:
                ...
                await self.ai_memory_service.store_memory(content=insight_content, category='asana_project_intelligence', tags=['project_analysis', 'health_score', project.project_gid], metadata={'project_gid': project.project_gid, 'health_score': project.health_score, 'risk_level': project.risk_level.value, 'analysis_date': datetime.now().isoformat()})
            for team in teams:
                ...
                await self.ai_memory_service.store_memory(content=team_insight, category='asana_team_productivity', tags=['team_analysis', 'productivity', team.team_name.lower().replace(' ', '_')], metadata={'team_name': team.team_name, 'productivity_score': team.productivity_score, 'analysis_date': datetime.now().isoformat()})
        except Exception as e:
            logger.exception(f'❌ Failed to store intelligence insights: {e}')

    async def _generate_summary_insights(self, projects: list[AsanaProjectMetrics], teams: list[TeamProductivityMetrics], risks: list[ProjectRiskAssessment]) -> dict[str, Any]:
        ...
        try:
            ...
            summary = {'overall_health_score': round(avg_health_score, 2), 'average_completion': round(avg_completion, 1), 'total_projects': len(projects), 'total_overdue_tasks': total_overdue_tasks, 'risk_distribution': risk_distribution, 'top_performing_teams': [{'name': t.team_name, 'score': round(t.productivity_score, 2)} for t in top_teams], 'projects_needing_attention': [{'name': p.project_name, 'health_score': round(p.health_score, 2), 'risk_level': p.risk_level.value} for p in at_risk_projects[:5]], 'key_insights': [f'Portfolio health score: {avg_health_score:.1%}', f'Average project completion: {avg_completion:.1f}%', f'{len(at_risk_projects)} projects need immediate attention', f'Top team: {top_teams[0].team_name}' if top_teams else 'No team data available']}
            return summary
        except Exception as e:
...
async def main():
    ...
    config = {'agent_id': 'asana_project_intelligence', 'performance_target_ms': 200, 'cache_ttl_seconds': 300}
