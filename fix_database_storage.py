            ...
            if workspaces_data.get("success", False):
                ...
                for workspace in workspaces:
                    try:
                        ...
                        storage_results["errors"].append(
                            f"Error storing workspace {workspace.get('id', 'unknown')}: {str(e)}"
                        )
    ...
    async def verify_data_storage(self) -> dict:
        ...
        try:
            ...
            workspaces_count = await conn.fetchval(
                "SELECT COUNT(*) FROM gong_workspaces"
            )
...
async def main():
    ...
    if storage_result["success"]:
        print("✅ Database storage fixed successfully!")
    ...
    if verification_result["success"]:
        ...
        print(
            f"   Apartment-relevant users: {intelligence['apartment_relevant_users']}"
        )
        print(
            f"   Apartment-relevant calls: {intelligence['apartment_relevant_calls']}"
        )
        print(
            f"   High-value opportunities: {intelligence['high_value_opportunities']}"
        )

        print("\n👥 TOP APARTMENT-RELEVANT USERS:")
        for user in verification_result["sample_data"]["top_users"]:
            print(
                f"   {user['first_name']} {user['last_name']} ({user['company']}) - Relevance: {user['apartment_relevance']:.2f}"
            )
        ...
        print(
            f"❌ Data verification failed: {verification_result.get('error', 'Unknown error')}"
        )
