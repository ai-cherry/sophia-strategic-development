...
class SecretManager:
    ...
    def import_env_to_pulumi(self, env_vars: Dict[str, str], stack: str) -> bool:
        ...
        try:
            ...
            # Set the stack
            subprocess.run(
                ["pulumi", "stack", "select", stack], check=True, capture_output=True
            )
            ...
            logger.info(
                f"Imported {len(env_vars)} environment variables to Pulumi ESC stack {stack}"
            )
            return True
    ...
    def export_pulumi_to_env(self, stack: str) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Exported {len(env_vars)} environment variables from Pulumi ESC stack {stack}"
            )
            return env_vars
        except subprocess.CalledProcessError as e:
    ...
    def sync_to_github_repo(self, env_vars: Dict[str, str], repo: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Synced {len(env_vars)} environment variables to GitHub repository {repo}"
            )
            return True
        except subprocess.CalledProcessError as e:
            logger.error(
                f"Failed to sync environment variables to GitHub repository: {e}"
            )
    ...
    def sync_to_github_org(self, env_vars: Dict[str, str], org: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Synced {len(env_vars)} environment variables to GitHub organization {org}"
            )
            return True
        except subprocess.CalledProcessError as e:
            logger.error(
                f"Failed to sync environment variables to GitHub organization: {e}"
            )
            logger.error(f"GitHub CLI output: {e.stdout.decode() if e.stdout else ''}")
            logger.error(f"GitHub CLI error: {e.stderr.decode() if e.stderr else ''}")
            return False
    ...
    def rotate_secrets(self, service: str) -> Dict[str, str]:
        ...
        if service.lower() == "snowflake":
            ...
            logger.warning(
                "Please update your Snowflake password manually and then update the SNOWFLAKE_PASSWORD environment variable"
            )
            ...
            logger.warning(
                "Please generate new API keys in the Gong dashboard and then update the GONG_API_KEY and GONG_API_SECRET environment variables"
            )
            ...
            logger.warning(
                "Please generate a new access token in the Vercel dashboard and then update the VERCEL_ACCESS_TOKEN environment variable"
            )
            ...
            logger.warning(
                "Please generate a new API key in the Estuary dashboard and then update the ESTUARY_API_KEY environment variable"
            )
    ...
    def audit_secrets(self) -> Dict[str, Any]:
        ...
        # Snowflake
        snowflake_vars = {
            k: v for k, v in env_vars.items() if k.startswith("SNOWFLAKE_")
        }
        ...
        if not audit_report["services"]["gong"]["configured"]:
            audit_report["recommendations"].append(
                "Configure Gong environment variables"
            )
        ...
        if not audit_report["services"]["estuary"]["configured"]:
            audit_report["recommendations"].append(
                "Configure Estuary environment variables"
            )
    ...
    def print_audit_report(self, audit_report: Dict[str, Any]) -> None:
        ...
        for service, config in audit_report["services"].items():
            ...
            print(
                f"{status_icon} {service.upper()}: {'Configured' if config['configured'] else 'Not Configured'}"
            )
...
def main():
    ...
    import_parser = subparsers.add_parser(
        "import-env", help="Import environment variables from .env file to Pulumi ESC"
    )
    ...
    export_parser = subparsers.add_parser(
        "export-env", help="Export environment variables from Pulumi ESC to .env file"
    )
    ...
    sync_github_parser = subparsers.add_parser(
        "sync-github", help="Sync environment variables to GitHub repository"
    )
    sync_github_parser.add_argument(
        "--repo", required=True, help="GitHub repository name (e.g., owner/repo)"
    )
    ...
    sync_github_org_parser = subparsers.add_parser(
        "sync-github-org", help="Sync environment variables to GitHub organization"
    )
    sync_github_org_parser.add_argument(
        "--org", required=True, help="GitHub organization name"
    )
    sync_github_org_parser.add_argument(
        "--env-file", help="Path to .env file (optional)"
    )
    sync_github_org_parser.add_argument("--stack", help="Pulumi stack name (optional)")

    # Rotate secrets
    rotate_parser = subparsers.add_parser(
        "rotate", help="Rotate secrets for a specific service"
    )
    rotate_parser.add_argument(
        "--service",
        required=True,
        choices=["all", "snowflake", "gong", "vercel", "estuary"],
        help="Service to rotate secrets for",
    )

    # Audit secrets
    audit_parser = subparsers.add_parser(
        "audit", help="Audit secret usage and rotation status"
    )
