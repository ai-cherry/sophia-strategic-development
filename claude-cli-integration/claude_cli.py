...
class ClaudeMCPIntegration:
    ...
    def __init__(self, config_path: str = "claude-cli-integration/claude_mcp_config.json"):
    ...
    def _setup_anthropic(self) -> None:
        ...
        api_key_env = self.config.get("globalSettings", {}).get("claudeApiSettings", {}).get("apiKeyEnv", "ANTHROPIC_API_KEY")
        self.anthropic_api_key = os.getenv(api_key_env)
        
        if not self.anthropic_api_key:
            logger.warning(f"⚠️ Anthropic API key not found in environment variable: {api_key_env}")
    ...
    def _select_model(self, message: str) -> str:
        ...
        routing_config = self.config.get("globalSettings", {}).get("intelligentRouting", {})
    ...
    async def query_mcp_server(self, server_name: str, endpoint: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            if data:
                async with self.session.post(f"{url}/{endpoint}", json=data) as response:
    ...
    async def call_claude_api(self, message: str, mcp_context: str = "") -> str:
        ...
        base_url = self.config.get("globalSettings", {}).get("claudeApiSettings", {}).get("baseUrl")
...
Always provide practical, actionable responses that leverage the available MCP capabilities."""
    ...
    def _fallback_response(self, message: str, mcp_context: str) -> str:
        ...
        if any(keyword in message.lower() for keyword in ['code', 'function', 'python', 'javascript', 'implement']):
        ...
        if any(keyword in message.lower() for keyword in ['design', 'ui', 'ux', 'component', 'figma']):
        ...
        if any(keyword in message.lower() for keyword in ['analyze', 'data', 'business', 'insights']):
    ...
    async def _gather_mcp_context(self, message: str) -> str:
        ...
        for server_name in self.config.get("mcpServers", {}):
            ...
            if health.get("status") == "healthy":
                context_parts.append(f"✅ {server_name}: {health.get('server', 'Available')}")
                context_parts.append(f"   Capabilities: {', '.join(server_config.get('capabilities', []))}")
            else:
                context_parts.append(f"❌ {server_name}: {health.get('status', 'Unknown')}")
    ...
    try:
        if args.command == "chat":
            ...
            for name, info in servers.items():
                ...
                print(f"      🔧 Capabilities: {', '.join(info['config'].get('capabilities', []))}")
            ...
            model_config = claude_mcp.config.get("globalSettings", {}).get("modelConfiguration", {})
            print("\n🧠 **Model Configuration:**")
            print(json.dumps(model_config, indent=2))
            
            routing_config = claude_mcp.config.get("globalSettings", {}).get("intelligentRouting", {})
            print(f"\n🎯 **Intelligent Routing:** {'✅ Enabled' if routing_config.get('enabled') else '❌ Disabled'}")
            
            api_status = "✅ Connected" if claude_mcp.anthropic_api_key else "❌ API Key Missing"
