    ...
    async def test_credentials(self, config: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            auth_header = self.create_auth_header(
                config["access_key"], config["secret"]
            )
        ...
        for i, company in enumerate(apartment_companies):
            for j, template in enumerate(conversation_templates):
                ...
                title = template["title"].format(
                    company=company["name"], units=company["units"]
                )
                ...
                try:
                    ...
                    """,
                        call_id,
                        title,
                        f"https://app.gong.io/call?id={call_id}",
                        call_date,
                        duration,
                        "inbound",
                        "gong",
                        template["relevance"],
                        min(template["relevance"] + 0.05, 1.0),
                        datetime.utcnow(),
                        datetime.utcnow(),
                    )
                    ...
                    deal_health = (
                        ...
                        if template["stage"] != "discovery"
                        else random.uniform(0.4, 0.7)
                    )
    ...
    async def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tables = [
                ...
                "sophia_apartment_analysis",
                "sophia_deal_signals",
                "sophia_competitive_intelligence",
            ]
            ...
            stats["relevance_distribution"] = {
                row["relevance_category"]: row["count"] for row in relevance_stats
            }
            ...
            stats["deal_stage_distribution"] = {
                row["deal_progression_stage"]: row["count"] for row in stage_stats
            }
...
async def main():
    ...
    try:
        ...
        results_file = (
            f"/home/ubuntu/enhanced_gong_integration_results_{timestamp}.json"
        )
        ...
        print(
            f"🔐 API Credentials: {'✅ WORKING' if cred_results.get('working_credentials_found') else '❌ NEEDS ATTENTION'}"
        )
        ...
        if not cred_results.get("working_credentials_found"):
            ...
            for test_result in cred_results.get("test_results", []):
                ...
                print(
                    f"      {status} {test_result['config_name']}: {test_result.get('error', 'Success')[:50]}"
                )

        # Data population results
        data_results = results.get("enhanced_data_population", {})
        print(
            f"📊 Enhanced Data: {data_results.get('enhanced_conversations_created', 0)} conversations created"
        )
        ...
        if db_stats and "error" not in db_stats:
            ...
            print(
                f"      Intelligence Records: {db_stats.get('sophia_conversation_intelligence', 0)}"
            )
            print(
                f"      Apartment Analysis: {db_stats.get('sophia_apartment_analysis', 0)}"
            )
