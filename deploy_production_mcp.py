...
class ProductionDeployer:
    ...
    def verify_environment_variables(self) -> bool:
        ...
        if os.environ.get("DEPLOYMENT_ENV") != "production":
            ...
            user_input = input("Do you want to continue with deployment? (y/n) ")
    ...
    async def deploy(self) -> Dict[str, Any]:
        ...
        logger.info("==================================================")
        logger.info(f"Starting deployment at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info("")
        ...
        try:
            ...
            if not result.get("success", False) or "status.*ok" not in result.get("output", ""):
                return {
                    "success": False,
